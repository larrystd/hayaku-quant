...
/**
 * Strategy context, it defines the security / K-line level information
 * contained when the strategy is executed
 * @ingroup Strategy
 *
 */
class HAYAKU_API StrategyContext {
 public:
  ...
  /**
   * Constructor
   * @param stockCodeList the given security code list, e.g. {"sz000001",
   * "sz000002"}
   */
   ...
   *       1. The given K-line type list also determines the priority loading
   * order of the K-line data, the earlier ones are loaded first.
   *       2. When ktypelist or preloadNum is not given, the global config file
   * parameters are used
   * @param stockCodeList the given security code list, e.g. {"sh000001",
   * "sz000001"}
   * @param ktypeList the given K-line data list, e.g. {"day", "min"}
   * @param preloadNum the given preload number, e.g. {{"min_max", 100},
   * {"day_max", 200}}
   */
  StrategyContext(const vector<string>& stockCodeList,
                  const vector<KQuery::KType>& ktypeList,
                  const unordered_map<string, int64_t>& preloadNum = {});

  // A user-defined move constructor and assignment cause python to fail to exit
  // normally StrategyContext(const StrategyContext&) = default;
  ...
  /**
   * Whether all the securities are loaded; as long as stockCodeList contains
   * "ALL"(case insensitive) it is regarded as loading everything
   * @return true
   * @return false
   */
   ...
   * @note It affects the trading calendar judgment and some securities that are
   * often used as the default comparison benchmark, and is usually used as the
  ...
  /**
   * Return all the security code lists that need to be loaded (including the
   * given security list and the default must-load security list)
