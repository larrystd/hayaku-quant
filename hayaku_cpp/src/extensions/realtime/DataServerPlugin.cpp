...
#include "application/plugins/DataServerPluginInterface.h"
#include "application/plugins/PluginIds.h"
#include "data/DataRuntime.h"
#include "extensions/realtime/NodeClient.h"
#include "extensions/realtime/ShmClientHook.h"

namespace hayaku {

void startDataServer(const std::string& addr, size_t work_num, bool save_tick,
...
void stopDataServer() {
  auto* plugin = getPlugin<DataServerPluginInterface>(HAYAKU_PLUGIN_DATASERVER);
  HAYAKU_ERROR_IF_RETURN(!plugin, void(), "Can't find {} plugin!",
                         HAYAKU_PLUGIN_DATASERVER);
  plugin->stop();
}

void getDataFromBufferServer(const std::string& addr, const StockList& stklist,
                             const KQuery::KType& ktype) {
  // The client mode: there is no local preload buffer (the preload check of
  // pullFromBufferServerLocal returns directly) The main process pulls, applies
  // to the buffer and mirrors to the shared memory, and all the clients then
  // read the update through shm. When the main process is unreachable the
  // forwarding warns internally and returns false; here it returns silently
  // without interrupting the caller flow.
...
void pullFromBufferServerLocal(const std::string& addr,
                               const StockList& stklist,
                               const KQuery::KType& ktype) {
  // SPEND_TIME(getDataFromBufferServer);
  const auto& preload = getDataRuntime().getPreloadParameter();
  string low_ktype = ktype;
  to_lower(low_ktype);
  HAYAKU_ERROR_IF_RETURN(!preload.tryGet<bool>(low_ktype, false), void(),
                         "The {} kdata is not preload! Can't update!",
                         low_ktype);

  NodeClient client(addr);
  try {
    HAYAKU_CHECK(client.dial(), "Failed dial server!");
    json req;
    req["cmd"] = "market";
    req["ktype"] = ktype;
    json code_list;
    ...
    json res;
    client.post(req, res);
    HAYAKU_ERROR_IF_RETURN(res["ret"] != NodeErrorCode::SUCCESS, void(),
                           "Recieved error: {}, msg: {}", res["ret"].get<int>(),
                           res["msg"].get<string>());

    const auto& jdata = res["data"];
    // HAYAKU_INFO("{}", to_string(jdata));
    for (auto iter = jdata.cbegin(); iter != jdata.cend(); ++iter) {
      ...
      try {
        ...
        for (auto kiter = jklist.cbegin(); kiter != jklist.cend(); ++kiter) {
          const auto& k = *kiter;
          KRecord kr(Datetime(k[0].get<string>()), k[1], k[2], k[3], k[4], k[5],
                     k[6]);
          stk.realtimeUpdate(kr, ktype);
        }

      } catch (const std::exception& e) {
        HAYAKU_ERROR("Failed decode json: {}! {}", to_string(r), e.what());
      }
    }

  } catch (const std::exception& e) {
    HAYAKU_ERROR("Failed get data from buffer server! {}", e.what());
  } catch (...) {
    HAYAKU_ERROR_UNKNOWN;
  }
}

vector<SpotRecord> getSpotFromBufferServer(const std::string& addr,
  ...
  try {
    ...
    json res;
    client.post(req, res);
    HAYAKU_ERROR_IF_RETURN(res["ret"] != NodeErrorCode::SUCCESS, ret,
                           "Recieved error: {}, msg: {}", res["ret"].get<int>(),
                           res["msg"].get<string>());
    ...
    for (auto iter = jdata.cbegin(); iter != jdata.cend(); ++iter) {
      ...
      try {
        ...
        for (auto siter = bid_amount.cbegin(); siter != bid_amount.cend();
             ++siter) {
        ...
        for (auto siter = ask_amount.cbegin(); siter != ask_amount.cend();
             ++siter) {
