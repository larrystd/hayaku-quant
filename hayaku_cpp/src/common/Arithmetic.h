 ...
 * only; use HAYAKU_PATH if it is needed for a path),
 ...
 * HAYAKU_PATH is for std::string
 * HAYAKU_CPATH is for char *
 */
#if defined(_MSC_VER)
...
bool HAYAKU_UTILS_API utf8_fold_equal(const std::string &s1, const std::string &s2) noexcept;

/** Whether a UTF-8 string contains a substring */
bool HAYAKU_UTILS_API utf8_contains(const std::string &s, const std::string &sub) noexcept;
...
extern template double HAYAKU_UTILS_API get_quantile(const std::vector<double> &vec, double quantile);
extern template float HAYAKU_UTILS_API get_quantile(const std::vector<float> &vec, double quantile);

HAYAKU_UTILS_API std::ostream &operator<<(std::ostream &os, const std::vector<double> &p);
HAYAKU_UTILS_API std::ostream &operator<<(std::ostream &os, const std::vector<float> &p);
