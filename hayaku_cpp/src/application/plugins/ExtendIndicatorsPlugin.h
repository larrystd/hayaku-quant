 ...
 * calculation
 * @param start_index the start index position; when it is negative, the
 * calculation starts from the index time points before the current latest one
 * @return Indicator
 * @note For some indicators that change with the time start point (such as AD),
 * the fixed start index ensures that the calculation starts from the first data
 ...
 * @return Indicator
 * @note For some indicators that change with the time start point (such as AD),
 * the fixed start date ensures that the calculation starts from the given date.
 ...
 * @param fill_null fill the null data when extending, otherwise the latest
 ...
 * @param mode rank mode: 0-descending rank (the highest indicator value has the
 * rank 1), 1-ascending rank (the larger the indicator value, the larger the
 * rank value), 2-descending rank percentage, 3-ascending rank percentage
 ...
 * mean of the data of the given K-line type into the K-line period of the
 * context
 * @param ktype the given K-line period
 * @param fill_null whether to fill the null data
 * @param unit aggregation period unit, 1 by default. It is calculated by unit
 ...
 *
 * Sample the indicator data at the given time point. If an exactly matching
 * time cannot be found, the valid data closest to and before the target time is
 * selected.
 *
 * @param ind the input indicator
 * @param ktype the given K-line period
 ...
 *
 * Count the maximum of the indicator data within the given time range
 * [start_time, last_time], including start_time and last_time themselves.
 *
 * @param ind the input indicator
 * @param start_time the start time of the time range in the format HH:MM,
 * "9:30" by default
 * @param last_time the end time of the time range in the format HH:MM, "10:00"
 * by default
 * @param ktype the given K-line period
 * @param fill_null whether to fill the null data
 * @param unit aggregation period unit
 ...
 *
 * Count the minimum of the indicator data within the given time range
 * [start_time, last_time], including start_time and last_time themselves.
 *
 * @param ind the input indicator
 * @param start_time the start time of the time range in the format HH:MM,
 ...
 *
 * Count the average of the indicator data within the given time range
 * [start_time, last_time], including start_time and last_time themselves.
 *
 * @param ind the input indicator
 * @param start_time the start time of the time range in the format HH:MM,
 ...
 * @param ktype the given K-line period
 * @param unit grouping period unit, 1 by default. It is calculated by unit
