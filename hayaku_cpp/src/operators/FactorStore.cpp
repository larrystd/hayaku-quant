...
bool hasFactorStore() noexcept { return getFactorStore() != nullptr; }
...
IndicatorList getValues(const Factor& factor, const StockList& stocks,
                        const KQuery& query, bool align, bool fillNull,
                        bool toValue, const DatetimeList& alignDates) {
  auto* store = getFactorStore();
  return store ? store->getValues(factor, stocks, query, align, fillNull,
                                  toValue, alignDates)
               : IndicatorList();
}

vector<IndicatorList> getValues(const FactorSet& factorSet,
                                const StockList& stocks, const KQuery& query,
                                bool align, bool fillNull, bool toValue,
                                const DatetimeList& alignDates) {
  auto* store = getFactorStore();
  return store ? store->getValues(factorSet, stocks, query, align, fillNull,
                                  toValue, alignDates)
