...
std::ostream& operator<<(std::ostream& os, const TimeLineRecord& record) {
  string strip(", ");
  os << std::fixed;
  (void)os.precision(4);
  os << "TimeLineRecord(Datetime(" << record.datetime.number() << ")" << strip
     << record.price << strip << record.vol << ")";
  os.unsetf(std::ostream::floatfield);
  (void)os.precision();
  return os;
}

std::ostream& operator<<(std::ostream& os, const TimeLineList& data) {
  if (data.size() > 0) {
    os << "TimeLineList{\n  size : " << data.size()
...
TimeLineRecord::TimeLineRecord(const Datetime& datetime, price_t price,
                               price_t vol)
