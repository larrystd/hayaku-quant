...
TEST_CASE("test_MF_Weight") {
  ...
  CHECK_THROWS_AS(MF_Weight(src_inds, weights, {Null<Stock>()}, query, ref_stk),
                  std::exception);

  /** @arg The input original factor list is empty */
  CHECK_THROWS_AS(MF_Weight(IndicatorList{}, weights, stks, query, ref_stk),
                  std::exception);

  /** @arg The input reference security is empty */
  CHECK_THROWS_AS(
      MF_Weight(IndicatorList{}, weights, stks, query, Null<Stock>()),
      std::exception);

  /** @arg The data length is insufficient */
  CHECK_THROWS_AS(MF_Weight(src_inds, weights, stks, KQuery(-1), ref_stk),
                  std::exception);

  /** @arg The security list count is insufficient */
  CHECK_THROWS_AS(
      MF_Weight(src_inds, weights, {sm["sh600004"]}, KQuery(-2), ref_stk),
      std::exception);

  /** @arg An invalid ic_n is passed */
  CHECK_THROWS_AS(MF_Weight(src_inds, weights, stks, KQuery(-2), ref_stk, 0),
                  std::exception);

  /** @arg The factor list and the weight list have different lengths */
  CHECK_THROWS_AS(
      MF_Weight(src_inds, PriceList{0.1}, stks, KQuery(-2), ref_stk, 0),
      std::exception);
  ...
  /** @arg 3 factors, 4 securities, the data length 20 and the comparison return
   * of 3 days */
  int ndays = 3;
  src_inds = {MA(ROCR(CLOSE(), ndays)), AMA(ROCR(CLOSE(), ndays)),
  ...
  for (size_t i = std::max(ind4.discard(), ind1.discard()),
              len = ref_dates.size();
       i < len; i++) {
    CHECK_EQ(ind4[i],
             doctest::Approx(ind1[i] * weights[0] + ind2[i] * weights[1] +
                             ind3[i] * weights[2])
                 .epsilon(0.0001));
  }
}
...
TEST_CASE("test_MF_Weight_benchmark") {
...

  {
    ...
    for (int i = 0; i < cycle; i++) {
      auto mf =
          MF_Weight(src_inds, PriceList{0.2, 0.3, 0.5}, stks, query, ref_stk);
      mf->setParam<bool>("save_all_factors", true);
      auto ic = mf->getIC();
    }
  }
...
TEST_CASE("test_MF_Weight_export") {
  ...
  auto mf1 =
      MF_Weight(src_inds, PriceList{0.1, 0.3, 0.6}, stks, query, ref_stk);
