...
> 状态：执行中；在独立的 `hikyuu-auto` 工作树实施，执行记录见本文末尾
>
> 前置条件：无第 6 步依赖；以本工作树的现行构建和测试基线为准
>
> 决策日期：2026-09-26
>
...
本步基于当前工作树建立可重复的代码质量基线，不重新设计业务对象：
...
以下是本步按**精确路径**处理的文件，不是 `find`/glob 的批量删除规则。
已核对内容和消费者后执行；没有扩大到同目录其他文件。
...
上述数据文件经场景核对没有独有断言；分钟线样本的缩减结果见执行记录。
...
1. 冻结本工作树基线，记录文件清单、LLVM 版本、现有格式差异、测试结果和待删候选。
...
- [x] 固定 LLVM 版本下，手写 C/C++ 文件的全量 `fmt-check` 通过；生成和第三方文件未被改写。
- [ ] 默认构建、`small-test`、`unit-test`、相关可选模块与完整 Python 测试无新增失败，公共 API 清单和回测结果无非预期变化（测试已通过；尚缺独立的公共 API 清单与回测结果差异核对）。
- [ ] tidy 规则、现存告警和新增代码门禁均有可复现记录；未用批量改名掩盖 API 变化。
- [x] ASan 覆盖的 target、平台、测试和抑制项有清单；项目代码无未处理的可复现内存错误。
- [x] 每个删除、缩减或迁移的对象都有内容价值结论、替代方式和消费者清单；Doxygen 文档入口不再指向不存在的配置。
- [x] 中文 `.mo` 与当前译文源一致；测试夹具能解释对应断言且默认测试不依赖公网；中英文用户手册不再把旧 GUI、旧 Python API 或外部 VIP 细节作为核心功能发布。
- [x] `git diff --check`、架构边界检查和适用的中英文文档检查通过；普通非 sanitizer 构建已恢复。
...
本次直接在独立的 `hikyuu-auto` 工作树执行，起点为 `poc` 分支
`d83838ee0`；不以第 6 步收敛为前置条件。尚未创建提交。

### 代码质量

- Homebrew LLVM 20.1.8 的 `clang-format` 已格式化 968 个手写 C/C++ 文件；
  删除失效示例后原工作树 967 个文件的全量 `./op.sh fmt-check` 通过。
  Rebase 到远程 `poc` 的 Python 目录重组后，当前 902 个文件检查通过。
  生成文件、第三方包和模板未加入清单。
- 包含顺序变更暴露了 `Lang.h` 的直接 `fmt`/`utility` 包含需求、
  `SystemInfo.h` 的导出宏依赖，以及 `test_TimeDelta.cpp` 的 Boost archive
  与序列化注册先后关系；这些文件已单独修正并经构建验证。
- Xmake 编译数据库初始包含 623 条、移除失效示例后重生为 622 条记录。
  Rebase 后的远程绑定目录重组使当前数据库变为 558 条记录。
  首轮抽样 tidy 发现 52 条项目告警，
  已修复 `SpotAgent.cpp` 的 4 处并改进 `FuncWrapper.h` 的所有权构造。
  扩展扫描还发现 `demo2.cpp` 使用已删除的策略 API，已移除该示例及其
  Xmake target；`demo3` 依赖的行情符号来自独立的 `hayaku-realtime`，
  已补齐 target 依赖。`IniParser.cpp` 的三处默认值验证结果被覆盖，
  已消除无用赋值；`SystemInfo.cpp` 缺失的直接包含也已补齐。
  规则、抽样结果及待审告警见 `step-7-quality-baseline.md`；
  `Lang.cpp` 和 `SystemInfo.cpp` 的 `tidy-strict` 通过。
- ASan 使用 `build/asan` 独立目录，并只对项目 target 启用 sanitizer，
  避免将第三方包构建成需要 ASan 注入的库。macOS 不执行 LeakSanitizer；
  旧 `asan.sup` 的宽泛 OpenSSL 抑制项没有消费者，已删除。已用 `otool -L`
  核对 core、两个测试程序、插件 ABI 测试库、ingest 和 realtime 的 ASan
  运行时链接。两套 ASan 测试分别通过 51/3356 和 805/208437；本机没有执行
  Linux LeakSanitizer。首次 `asan-test` 使用了 Xmake 已废弃的 `--buildir`
  参数，测试成功后返回 2；改为 `--builddir` 后完整复跑以 0 退出，
  Xmake 配置恢复至普通 `build` 目录。两次测试均未报告 ASan 错误。

### 内容清理和消费者

- 删除两个失效 Doxygen 配置及开发手册中的旧命令；删除旧 `.pot`，由
  `i18n/refresh_translation.py` 从当前源码和业绩报告键重建 `.po`，再编译
  `.mo`。gettext 格式检查和运行时中文翻译抽样通过。
- 删除旧交易库、2011 年下载样本及五份未配置的板块文件；清空本地生成的
  `tmp/*.plk`。分钟 CSV 从 24000 行缩至 578 行，并调整对应断言；四份分钟线
  HDF5 从 47.3 MB 缩至 23.7 MB，保留测试实际使用的记录和索引。
  `test_data/README.md` 记录场景、字段和文件映射。
- 中英文手册同步退出 VIP、共享内存插件和旧版本历史页面及相关旧截图；
  更新导航、安装、入门、开发者、API 说明、发布说明和两个 README。
  已核对 `toctree`、图片引用和删除对象的残留引用。

### 已运行的回归

| 检查 | 结果 |
|---|---|
| 普通 core、ingest、realtime 构建与导入 | 通过 |
| 剩余 C++ 示例 `demo1`、`demo3` 独立构建 | 通过 |
| C++ `small-test` | 普通及 ASan 测试均为 51 用例、3356 断言通过 |
| C++ `unit-test` | 普通及 ASan 测试均为 805 用例、208437 断言通过；NNG 监听用例在当前沙箱中报告受限警告 |
| Python 测试 | 原工作树 66 项通过；rebase 后 67 项通过，`test` 命令会先构建两个可选扩展 |
| `docs/tools/check_docs_parity.py` 与 `check_readme_parity.py` | 通过 |
| 手册导航、图片路径及旧页面残留引用检查 | 通过 |

本机没有 Sphinx，因此 HTML 构建仍需在已配置 Sphinx 的 CI 中执行。
全仓 tidy 告警清单和 Linux 泄漏检查仍待补齐；并发全量 tidy 在本机造成
明显内存换页，已停止，保存了已完成编译单元的诊断并优先处理真实错误。
本机没有 Sphinx，HTML 构建
需由配置了依赖的 CI 执行。
ASan 复跑后已再次构建普通 core、ingest 和 realtime；`import hayaku`
及 66 项 Python 测试再次通过。`git diff --check`、架构依赖检查和双语
一致性检查也再次通过。

与远程 `poc` 的 Python 包目录重组对齐后，core、ingest 和 realtime 重新
构建通过，C++ `small-test` 与 `unit-test` 保持 51/3356、805/208437，
Python 回归扩展到 67 项并通过。旧 Python 包目录遗留的忽略 `.pyc` 缓存
已从本地工作树清理，否则 Python 会把空目录误识别成命名空间包。
