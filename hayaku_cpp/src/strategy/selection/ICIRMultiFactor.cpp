...
BOOST_CLASS_EXPORT(hayaku::ICIRMultiFactor)
#endif
...
ICIRMultiFactor::ICIRMultiFactor(const StockList& stks, const KQuery& query,
                                 const Stock& ref_stk, int ic_n,
                                 int ic_rolling_n, bool spearman, int mode,
                                 bool save_all_factors)
    : MultiFactorBase(stks, query, ref_stk, "MF_ICIRWeight", ic_n, spearman,
...
IndicatorList ICIRMultiFactor::_calculate(
  ...
  IndicatorList icir = global_parallel_for_index(
      0, ind_count, [&, ic_n, ir_n, spearman](size_t ii) {
