 ...
 * Cache implementation (not a strictly defined LRU, in order to improve the
 * concurrent reading performance)
 * @tparam KeyType the key type, it must support hashing and equality comparison
 * @tparam ValueType the value type, it must support the copy and move
 * operations
 */
...
class LruCache final {
 public:
  ...
  // Storage structure: the value + an atomic dirty flag (marking whether it has
  // been accessed by get and the LRU order needs to be updated)
  using CacheValue = std::pair<value_type, std::atomic<bool>>;
  using LruList = std::list<key_type>;
  using CacheMap =
      std::unordered_map<key_type,
                         std::pair<typename LruList::iterator, CacheValue>>;

  /**
   * @brief Constructor
   * @param capacity cache capacity, 64 by default; 0 means an unlimited
   * capacity
   * @param overflow overflow capacity, 8 by default; it allows the cache to
   * temporarily exceed the given capacity The eviction mechanism is triggered
   ...
   * @param key key
   * @return the value is returned if it exists, otherwise the default
   ...
   * @return true is returned on a successful deletion, false when it does not
   * exist
   */
 ...
 private:
  ...
  // Update all the dirty nodes in a batch: move them to the head of the list
  // and clear the dirty flags
  void _batch_update_dirty_nodes() {
    // Fix 1: avoid the default construction and use a pointer/reference to
    // track the latest dirty node
    ...
    for (auto it = lru_list_.rbegin(); it != lru_list_.rend(); ++it) {
      ...
      if (dirty_flag.load(std::memory_order_relaxed)) {
        latest_dirty_it = it;
        has_dirty = true;
        break;  // Only the most recently accessed dirty node is handled, one is
                // kept at the tail
      }
    }

    // Only the most recently accessed dirty node is moved to the head (fix 3:
    // the reverse iterator is converted to a forward iterator)
    if (has_dirty) {
      // C++17: convert the reverse iterator to a forward iterator (the base()
      // method)
      auto forward_it = latest_dirty_it.base();
      --forward_it;  // base() of the reverse iterator returns the next forward
