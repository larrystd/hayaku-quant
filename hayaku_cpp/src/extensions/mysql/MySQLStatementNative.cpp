...
void MySQLStatement::_prepare() {
  ...
  HAYAKU_CHECK(m_impl->stmt, "Failed mysql_stmt_init! SQL: {}", m_sql_string);

  int ret = mysql_stmt_prepare(m_impl->stmt, m_sql_string.c_str(),
                               m_sql_string.size());
  ...
  if (1 == ret || CR_SERVER_LOST == ret || CR_SERVER_GONE_ERROR == ret) {
    ...
    if (connect && connect->ping()) {
      ...
      HAYAKU_THROW("Failed reconnect mysql! SQL: {}", m_sql_string);
    }
  } else if (CR_OUT_OF_MEMORY == ret) {
    HAYAKU_THROW("Out of memory! SQL: {}", m_sql_string);
  }

  m_impl->stmt = mysql_stmt_init(m_impl->db);
  ret = mysql_stmt_prepare(m_impl->stmt, m_sql_string.c_str(),
                           m_sql_string.size());
  HAYAKU_IF_RETURN(0 == ret, void());
  ...
  HAYAKU_THROW("Failed prepare statement: {}! ret: {}, error msg: {}!",
               m_sql_string, ret, stmt_errorstr);
}
...
void MySQLStatement::_bindResult() {
  HAYAKU_IF_RETURN(!m_impl->meta_result, void());
  MYSQL_FIELD* field;
  int idx = 0;
...
void MySQLStatement::sub_getColumnAsDatetime(int idx, Datetime& item) {
  ...
  try {
    const MYSQL_TIME* tm =
        boost::any_cast<MYSQL_TIME>(&(m_impl->result_buffer[idx]));
