 ...
 *      Author: woleigegg
 *
 *  The residual regression of the style factor neutralization, a serial kernel
 * extracted from MultiFactorBase.
 *
 *  Design points:
 *    - The Eigen ColPivHouseholderQR solver is kept, which is not in the Eigen
 * parallel algorithm set itself;
 *    - The fitted values use an explicit row-by-row dot product, avoiding the
 * parallel path of the general matrix-matrix multiplication;
 *    - The process level Eigen::setNbThreads is not called, avoiding the
 * concurrent MFs polluting the global config mutually;
 *    - The valid sample / rank deficiency / NaN semantics of the original
 * calculate_residuals are kept.
 */
...
PriceList calculate_style_residuals(const PriceList& y,
  ...
  HAYAKU_ASSERT(!x.empty());
  size_t n = y.size();
  for (const auto& xi : x) {
    HAYAKU_ASSERT(xi.size() == n);
  }
  ...
  // Calculate the residuals: the fitted value is a row-by-row dot product,
  // avoiding the general matrix-matrix parallel path
  for (size_t i = 0; i < n; ++i) {
    if (!valid[i]) {
      continue;
    }
