...
TEST_CASE("test_STDEV") {
  ...
  // BREAKING CHANGE: discard changed from 1 to data.discard()+n-1=9 (NaN while
  // the window is not full)
  CHECK_EQ(dev.discard(), 9);
  CHECK_EQ(dev.size(), 15);

  // While the window is not full (i<9) NaN is output; from i>=9 the steady
  // value matches the old implementation (within the Welford precision)
  vector<price_t> expected{0,       0,       0,       0,     0,
                           0,       0,       0,       0,     2.92309,
...
//----------------------------------------------------------------------------
// The NaN semantics test (the Welford rolling variance + the conditional
// recalculation when an outlier leaves)
//----------------------------------------------------------------------------
...
/** @par Test point: the window has a single valid value (count=1, std=NaN
 * instead of Inf) */
TEST_CASE("test_STDEV_nan_single_valid") {
  ...
  Indicator dev = STDEV(ind, 4);
  CHECK_EQ(dev.discard(), 3);
  // vc is always 1 and std always NaN (the sample variance divides by 0, NaN is
  // returned safely)
  for (size_t i = dev.discard(); i < dev.size(); ++i) {
    CHECK_UNARY(std::isnan(dev[i]));
  }
...
/** @par Test point: a large base with a small variance (the Welford precision
 * check) */
TEST_CASE("test_STDEV_large_base") {
  ...
  Indicator dev = STDEV(ind, 3);
  CHECK_EQ(dev.discard(), 2);
  // The window [1e8+1,1e8+2,1e8+3] -> std=1.0 (exact, Welford removes the large
  // base)
...
TEST_CASE("test_STDEV_outlier_leaving_rescan") {
  ...
  // The true std = sqrt(1.2) ~ 1.095445, the recalculation must restore it
  // exactly
...
/** @par Test point: the _dyn dynamic path is equivalent to the static n (with
 * NaN) */
TEST_CASE("test_STDEV_dyn_nan_equivalence") {
  ...
  CHECK_EQ(expect.size(), result.size());
  // The _dyn path has the window-not-full guard, so its discard matches the
  // static one
  CHECK_EQ(expect.discard(), result.discard());
...
/** @par Test point: the n=1 parameter is intercepted (the sample standard
 * deviation is undefined, an exception is thrown instead of Inf/a crash) */
TEST_CASE("test_STDEV_n1_rejected") {
  ...
  Indicator ind = PRICELIST(d);
  // n=1: the sample variance divides by N-1=0 and _checkParam intercepts it
  // (HAYAKU_ASSERT(n==0 || n>=2))
  CHECK_THROWS_AS(STDEV(ind, 1), std::exception);
}

/** @par Test point: the zero variance of a constant sequence (the floating
 * point noise defense) */
TEST_CASE("test_STDEV_zero_variance") {
  ...
  Indicator dev = STDEV(ind, 3);
  CHECK_EQ(dev.discard(), 2);
  // The variance of a constant sequence is 0; the floating point noise may make
  // M2 slightly negative, the max(0,...) defense ensures 0.0 instead of NaN
  for (size_t i = dev.discard(); i < dev.size(); ++i) {
