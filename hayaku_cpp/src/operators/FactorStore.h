 ...
 *
 * Data-owned port for optional factor persistence and backend-side factor evaluation.
 */
...
public:
    ...
    [[nodiscard]] virtual bool hasFactor(const string& name, const KQuery::KType& ktype) = 0;
    [[nodiscard]] virtual Factor getFactor(const string& name, const KQuery::KType& ktype) = 0;
    ...
    [[nodiscard]] virtual IndicatorList getValues(
      const Factor& factor, const StockList& stocks, const KQuery& query, bool align,
      bool fillNull, bool toValue, const DatetimeList& alignDates) = 0;
    [[nodiscard]] virtual vector<IndicatorList> getValues(
      const FactorSet& factorSet, const StockList& stocks, const KQuery& query, bool align,
      bool fillNull, bool toValue, const DatetimeList& alignDates) = 0;
};
...
HAYAKU_API bool hasFactor(const string& name, const KQuery::KType& ktype = KQuery::DAY);
HAYAKU_API Factor getFactor(const string& name, const KQuery::KType& ktype = KQuery::DAY);
HAYAKU_API void saveFactor(const Factor& factor, bool updateBefore = true);
HAYAKU_API void saveSpecialFactorValues(const Factor& factor, const Stock& stock,
                                     const DatetimeList& dates, const PriceList& values,
                                     bool replace);
HAYAKU_API void removeFactor(const string& name, const KQuery::KType& ktype = KQuery::DAY);
HAYAKU_API FactorList getAllFactors();
HAYAKU_API FactorSetList getAllFactorSets();
HAYAKU_API void updateAllFactorsValues(const KQuery::KType& ktype = KQuery::DAY);
...
[[nodiscard]] IndicatorList getValues(const Factor& factor, const StockList& stocks,
                                      const KQuery& query, bool align, bool fillNull,
                                      bool toValue, const DatetimeList& alignDates);
[[nodiscard]] vector<IndicatorList> getValues(const FactorSet& factorSet,
                                              const StockList& stocks, const KQuery& query,
