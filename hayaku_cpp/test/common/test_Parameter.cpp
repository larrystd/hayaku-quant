...
/**
 * @defgroup test_hayaku_Parameter test_hayaku_Parameter
...
TEST_CASE("test_Parameter") {
  ...
  /** @arg When modifying a parameter, the given type does not match the
   * existing one */
  CHECK_THROWS_AS(param.set<float>("n", 10.0), std::logic_error);
...
TEST_CASE("test_Parameter_KData") {
  ...
  /** @arg Verify that KData can be read several times, avoiding the move
   * semantics */
...
TEST_CASE("test_Parameter_serialize") {
  ...
  Stock stk = getStock("sh600000");
  KQuery q = KQueryByDate(Datetime(200001041025), Datetime(200001041100),
                          KQuery::MIN5);
