 ...
 * @ingroup DBConnect
 */
class HAYAKU_UTILS_API DBConnectBase
    : public std::enable_shared_from_this<DBConnectBase> {
 PARAMETER_SUPPORT  // NOSONAR
   ...
   *           SQLStatementPtr st = driver->getStatement("insert into ttt (name,
   * age) values
  ...
  void batchSave(InputIterator first, InputIterator last,
                 bool autotrans = true);
  ...
  /**
   * Batch load the model data into a container (a container supporting
   * push_back, such as vector and list)
   ...
   */
  template <class InputIterator>
  void batchUpdate(InputIterator first, InputIterator last,
                   bool autotrans = true);
   ...
   */
  template <class InputIterator>
  void batchSaveOrUpdate(InputIterator first, InputIterator last,
                         bool autotrans = true);

  /**
   * Delete the data satisfying the condition from the given table, i.e. delete
   * by the given condition
   * @param tablename the table name of the data to be deleted
   * @param where the deletion condition
   * @param autotrans start a transaction
   */
  void remove(const std::string &tablename, const std::string &where,
              bool autotrans = true);
   ...
   * @param autotrans start a transaction
   */
  void remove(const std::string &tablename, const DBCondition &cond,
              bool autotrans = true);
   ...
   */
  template <class InputIterator>
  void batchRemove(InputIterator first, InputIterator last,
                   bool autotrans = true);
  ...
  NumberType queryNumber(const std::string &query,
...
template <typename NumberType>
NumberType DBConnectBase::queryNumber(const std::string &query,
...
void DBConnectBase::save(T &item, bool autotrans) {
  SQLStatementPtr st = item.valid() ? getStatement(T::getUpdateSQL())
  ...
  } catch (::hayaku::SQLException &e) {
    if (autotrans) {
      rollback();
    }
    SQL_THROW(e.errcode(), "failed save! sql: {}! {}", st->getSqlString(),
              e.what());
  } catch (std::exception &e) {
    if (autotrans) {
      rollback();
    }
    HAYAKU_THROW("failed save! sql: {}! {}", st->getSqlString(), e.what());
  } catch (...) {
    if (autotrans) {
      rollback();
    }
    HAYAKU_THROW("failed save! sql: {}! Unknown error!", st->getSqlString());
  }
}
...
template <class InputIterator>
void DBConnectBase::batchSave(InputIterator first, InputIterator last,
                              bool autotrans) {
  size_t count = std::distance(first, last);
  HAYAKU_IF_RETURN(count == 0, void());
  ...
  try {
    ...
    SQL_THROW(e.errcode(), "failed batch save! sql: {}! {}", st->getSqlString(),
              e.what());
  } catch (std::exception &e) {
    if (autotrans) {
      rollback();
    }
    HAYAKU_THROW("failed batch save! sql: {}! {}", st->getSqlString(),
                 e.what());
  } catch (...) {
    if (autotrans) {
      rollback();
    }
    HAYAKU_THROW("failed batch save! sql: {}! Unknown error!",
                 st->getSqlString());
  }
}
...
template <typename Container>
void DBConnectBase::batchLoadView(Container &container,
...
template <class InputIterator>
void DBConnectBase::batchUpdate(InputIterator first, InputIterator last,
                                bool autotrans) {
  size_t count = std::distance(first, last);
  HAYAKU_IF_RETURN(count == 0, void());
  ...
  try {
    ...
    HAYAKU_THROW("failed batch update! sql: {}! {}", st->getSqlString(),
                 e.what());
  } catch (...) {
    if (autotrans) {
      rollback();
...
template <class InputIterator>
void DBConnectBase::batchSaveOrUpdate(InputIterator first, InputIterator last,
                                      bool autotrans) {
...
template <class Container>
inline void DBConnectBase::batchSaveOrUpdate(Container &container,
                                             bool autotrans) {
...
void DBConnectBase::remove(T &item, bool autotrans) {
  ...
  SQLStatementPtr st = getStatement(fmt::format(
      "delete from {} where id={}", T::getTableName(), item.rowid()));
  if (autotrans) {
    transaction();
  }
  ...
  try {
    ...
    SQL_THROW(e.errcode(), "failed delete! sql: {}! {}", st->getSqlString(),
              e.what());
  } catch (std::exception &e) {
    if (autotrans) {
      rollback();
    }
    HAYAKU_THROW("failed delete! sql: {}! {}", st->getSqlString(), e.what());
  } catch (...) {
    if (autotrans) {
      rollback();
    }
    HAYAKU_THROW("failed delete! sql: {}! Unknown error!", st->getSqlString());
  }
}
...
template <class InputIterator>
void DBConnectBase::batchRemove(InputIterator first, InputIterator last,
                                bool autotrans) {
  size_t count = std::distance(first, last);
  HAYAKU_IF_RETURN(count == 0, void());
  ...
  try {
    ...
    HAYAKU_THROW("failed batch delete! {}", e.what());
  } catch (...) {
    if (autotrans) {
      rollback();
    }
    HAYAKU_THROW("failed batch delete! Unknown error!");
  }
}

inline void DBConnectBase::remove(const std::string &tablename,
  ...
  try {
    ...
    HAYAKU_THROW("Failed exec sql: {}! {}", sql, e.what());
  } catch (...) {
    if (autotrans) {
      rollback();
    }
    HAYAKU_THROW(R"(Failed exec sql: {}! Unknown error!)", sql);
  }
}
