...
KRecordList MySQLKDataDriver::_getKRecordList(const string& market, const string& code,
                                              ...
                                              Datetime end_date) {
    KRecordList result;
    HAYAKU_IF_RETURN(start_date.isNull() || (!end_date.isNull() && start_date >= end_date), result);
...
bool MySQLKDataDriver::getIndexRangeByDate(const string& market, const string& code,
    ...
    out_start = 0;
    out_end = 0;
    HAYAKU_ERROR_IF_RETURN(query.queryType() != KQuery::DATE, false, "queryType must be KQuery::DATE");
    HAYAKU_IF_RETURN(
...
TimeLineList MySQLKDataDriver::getTimeLineList(const string& market, const string& code,
                                               const KQuery& query) {
    TimeLineList result;
    HAYAKU_ERROR_IF_RETURN(query.queryType() >= KQuery::INVALID, result, "Invalid queryType! {}{} {}",
                        market, code, query);
