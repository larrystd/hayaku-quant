...
class PyBaseInfoDriver : public BaseInfoDriver {
 public:
  ...
  vector<StockInfo> getAllStockInfo() override {
    PYBIND11_OVERLOAD_PURE(vector<StockInfo>, BaseInfoDriver,
                           getAllStockInfo, );
  }

  StockInfo getStockInfo(string market, const string& code) override {
    PYBIND11_OVERLOAD_PURE(StockInfo, BaseInfoDriver, getStockInfo, market,
                           code);
  }

  StockWeightList getStockWeightList(const string& market, const string& code,
                                     Datetime start, Datetime end) override {
    PYBIND11_OVERLOAD(StockWeightList, BaseInfoDriver, getStockWeightList,
  ...
  vector<HistoryFinanceInfo> getHistoryFinance(const string& market,
                                               const string& code,
                                               Datetime start,
                                               Datetime end) override {
    PYBIND11_OVERLOAD(vector<HistoryFinanceInfo>, BaseInfoDriver,
  ...
  vector<MarketInfo> getAllMarketInfo() override {
    PYBIND11_OVERLOAD_PURE(vector<MarketInfo>, BaseInfoDriver,
                           getAllMarketInfo, );
  }

  vector<StockTypeInfo> getAllStockTypeInfo() override {
    PYBIND11_OVERLOAD_PURE(vector<StockTypeInfo>, BaseInfoDriver,
                           getAllStockTypeInfo, );
  }

  StockTypeInfo getStockTypeInfo(uint32_t type) override {
    PYBIND11_OVERLOAD_PURE(StockTypeInfo, BaseInfoDriver, getStockTypeInfo,
                           type);
  }

  std::unordered_set<Datetime> getAllHolidays() override {
    PYBIND11_OVERLOAD_PURE(std::unordered_set<Datetime>, BaseInfoDriver,
                           getAllHolidays, );
  }
