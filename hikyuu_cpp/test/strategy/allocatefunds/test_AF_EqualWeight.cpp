...
public:
    ...
    StrategyWeightList _allocateWeight(const Datetime&, const StrategyWeightList& se_list) override {
        return se_list;
    }
};
...
TEST_CASE("test_AF_without_running_uses_sub_tm_funds") {
    ...
    auto cash_tm = create_test_account(AccountConfig(Datetime(200101010000L), 100000));

    // running: cash + position, so total_assets goes through cash+market_value, not the cash
    // shortcut
    auto sub_tm_running = create_test_account(AccountConfig(Datetime(200101010000L), 30000));
    price_t buy_price = 10.0;
    double buy_num = 1000;
    auto tr = sub_tm_running
                ->submit(OrderRequest(OrderSide::BUY, buy_date, stock, buy_price, buy_num, 0.0,
                                      buy_price, buy_price, OrderOrigin::SIGNAL))
                .trade();
    ...
    auto sub_tm_new = create_test_account(AccountConfig(Datetime(200101010000L), 0));
