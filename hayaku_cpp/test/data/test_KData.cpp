...
/**
 * @defgroup test_hayaku_KData test_hayaku_KData
...
TEST_CASE("test_getKData_by_index") {
  ...
  record = kdata[0];
  expect =
      KRecord(Datetime(199012200000), 104.3, 104.39, 99.98, 104.39, 8.4, 197);
  CHECK_EQ(record, expect);
  ...
  record = kdata[kdata.size() - 1];
  expect =
      KRecord(Datetime(199101020000), 127.61, 128.84, 127.61, 128.84, 5.9, 91);
  CHECK_EQ(record, expect);
  ...
  record = kdata[0];
  expect = KRecord(Datetime(201112050000), 2363.111, 2363.127, 2327.61,
                   2333.229, 4864121.6, 52214970);
  CHECK_EQ(record, expect);
  ...
  record = kdata[0];
  expect = KRecord(Datetime(201111230000), 2415.197, 2418.56, 2390.654,
                   2395.065, 5004336, 50547851);
  CHECK_EQ(record, expect);

  record = kdata[kdata.size() - 1];
  expect = KRecord(Datetime(201112020000), 2374.899, 2378.299, 2344.846,
                   2360.664, 5732015.7, 59868846);
  CHECK_EQ(record, expect);
  ...
  record = kdata[0];
  expect =
      KRecord(Datetime(199012190000), 96.05, 99.98, 95.79, 99.980, 49.4, 1260);
  CHECK_EQ(record, expect);
  ...
  record = kdata[0];
  expect = KRecord(Datetime(200001040931), 1366.58, 1368.692, 1366.579,
                   1368.692, 4124880, 191158);
  CHECK_EQ(record, expect);

  record = kdata[kdata.size() - 1];
  expect = KRecord(Datetime(201112061500), 2325.38, 2326.1, 2324.6, 2325.90,
                   5215150, 566312);
  CHECK_EQ(record, expect);
  ...
  CHECK_EQ(kdata[0], KRecord(Datetime(199012280000), 113.57, 126.45, 109.13,
                             126.45, 28.2, 321));
  CHECK_EQ(kdata[1], KRecord(Datetime(199101040000), 126.56, 131.44, 126.48,
                             131.44, 47.3, 730));
  ...
  CHECK_EQ(kdata[0], KRecord(Datetime(199101310000), 127.61, 135.19, 127.61,
                             129.97, 3637.1, 67197));
  CHECK_EQ(kdata[1], KRecord(Datetime(199102280000), 129.5, 134.87, 128.06,
                             133.01, 3027.3, 50982));
  CHECK_EQ(kdata[2], KRecord(Datetime(199103310000), 132.53, 132.53, 120.11,
                             120.19, 1725.3, 24528));
  ...
  CHECK_EQ(kdata.endPos(), 4);
  CHECK_EQ(kdata.lastPos(), 3);
  CHECK_EQ(kdata[0], KRecord(Datetime(199103310000), 127.61, 135.19, 120.11,
                             120.19, 8389.7, 142707));
  CHECK_EQ(kdata[1], KRecord(Datetime(199106300000), 120.69, 137.56, 104.96,
                             137.56, 12095.6, 222753));
  CHECK_EQ(kdata[2], KRecord(Datetime(199109300000), 136.64, 191.18, 131.87,
                             180.92, 32436.9, 527079));
  ...
  CHECK_EQ(kdata.endPos(), 4);
  CHECK_EQ(kdata.lastPos(), 3);
  CHECK_EQ(kdata[0], KRecord(Datetime(200001040940), 1369.187, 1369.371,
                             1367.389, 1367.389, 70687.3, 251473));
  CHECK_EQ(kdata[1], KRecord(Datetime(200001040945), 1367.389, 1367.389,
                             1364.645, 1364.763, 73577.1, 223346));
  CHECK_EQ(kdata[2], KRecord(Datetime(200001040950), 1364.763, 1364.763,
                             1363.128, 1363.192, 65066.3, 167971));
  ...
  CHECK_EQ(kdata.endPos(), 4);
  CHECK_EQ(kdata.lastPos(), 3);
  CHECK_EQ(kdata[0], KRecord(Datetime(200001041000), 1364.7630, 1364.7630,
                             1361.4590, 1361.4590, 204277.1000, 457452.0000));
  CHECK_EQ(kdata[1], KRecord(Datetime(200001041015), 1361.5580, 1366.0930,
                             1361.2950, 1365.9270, 307531.8000, 548313.0000));
  CHECK_EQ(kdata[2], KRecord(Datetime(200001041030), 1366.0430, 1367.6830,
                             1365.9460, 1367.6830, 279102.7000, 451981.0000));
}
...
TEST_CASE("test_getKData_by_date") {
  ...
  /** @arg The SH000001 daily line data; the start time is the day before the
   * first record date */
  stock = sm.getStock("sh000001");
  ...
  /** @arg The SH000001 daily line data; the start time equals the first record
   * date */
  stock = sm.getStock("sh000001");
  ...
  /** @arg The SH000001 daily line data; the start time is the day after the
   * first record date */
  stock = sm.getStock("sh000001");
  ...
  /** @arg The SH000001 daily line data; the start is the first record and the
   * end date is the day before the last record */
  stock = sm.getStock("sh000001");
  ...
  record = kdata[kdata.size() - 1];
  CHECK_EQ(record, KRecord(Datetime(201112020000), 2374.8990, 2378.2990,
                           2344.8460, 2360.6640, 5732015.7000, 59868846.0000));

  /** @arg The SH000001 daily line data; the start is the first record and the
   * end date is the last record */
  stock = sm.getStock("sh000001");
  ...
  record = kdata[kdata.size() - 1];
  CHECK_EQ(record, KRecord(Datetime(201112050000), 2363.1110, 2363.1270,
                           2327.6100, 2333.2290, 4864121.6000, 52214970.0000));

  /** @arg The SH000001 daily line data; the start is the first record and the
   * end date is the day after the last record */
  stock = sm.getStock("sh000001");
  ...
  record = kdata[0];
  CHECK_EQ(record, KRecord(Datetime(199510180000), 705.7200, 716.8300, 705.7200,
                           716.8300, 232685.5000, 4273054.0000));

  record = kdata[kdata.size() - 1];
  CHECK_EQ(record, KRecord(Datetime(199510230000), 729.7600, 729.9700, 718.4400,
                           723.0900, 235482.7000, 3877994.0000));
  ...
  /** @arg The SH000001 daily line data; the end date is the day after the start
   * date and the data exists */
  stock = sm.getStock("sh000001");
  ...
  record = kdata[0];
  CHECK_EQ(record, KRecord(Datetime(201111140000), 2498.6720, 2529.6320,
                           2496.3350, 2528.7140, 8252378.1000, 81229813.0000));
  ...
  CHECK_EQ(kdata.endPos(), total);
  CHECK_EQ(kdata.lastPos(), total - 1);
  CHECK_EQ(kdata[0], KRecord(Datetime(199012210000), 96.0500, 109.1300, 95.7900,
                             109.1300, 59.4000, 1485.0000));
  CHECK_EQ(kdata[1], KRecord(Datetime(199012280000), 113.5700, 126.4500,
                             109.1300, 126.4500, 28.2000, 321.0000));
  CHECK_EQ(kdata[kdata.size() - 1],
           KRecord(Datetime(201112090000), 2363.1110, 2363.1270, 2310.1550,
                   2325.9050, 9126681.1000, 98132048.0000));
  ...
  CHECK_EQ(kdata[0], KRecord(Datetime(199012210000), 96.0500, 109.1300, 95.7900,
                             109.1300, 59.4000, 1485.0000));
  CHECK_EQ(kdata[10], KRecord(Datetime(199103010000), 134.3700, 134.8700,
                              132.4700, 132.5300, 827.1000, 11500.0000));
  CHECK_EQ(kdata[kdata.size() - 1],
           KRecord(Datetime(201112090000), 2363.1110, 2363.1270, 2310.1550,
                   2325.9050, 9126681.1000, 98132048.0000));
  ...
  CHECK_EQ(kdata.endPos(), total);
  CHECK_EQ(kdata.lastPos(), total - 1);
  CHECK_EQ(kdata[0], KRecord(Datetime(199012280000), 113.5700, 126.4500,
                             109.1300, 126.4500, 28.2000, 321.0000));
  CHECK_EQ(kdata[1], KRecord(Datetime(199101040000), 126.5600, 131.4400,
                             126.4800, 131.4400, 47.3000, 730.0000));
  CHECK_EQ(kdata[kdata.size() - 1],
           KRecord(Datetime(201112090000), 2363.1110, 2363.1270, 2310.1550,
                   2325.9050, 9126681.1000, 98132048.0000));

  /** @arg the start is the first record and the end date is the day before the
   * last record, not equal to the previous record date */
  stock = sm.getStock("sh000001");
  query = KQueryByDate(Datetime(199012210000), Datetime(201112080000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), total - 1);
  CHECK_EQ(kdata.empty(), false);
  ...
  CHECK_EQ(kdata[0], KRecord(Datetime(199012210000), 96.0500, 109.1300, 95.7900,
                             109.1300, 59.4000, 1485.0000));
  CHECK_EQ(kdata[kdata.size() - 1],
           KRecord(Datetime(201112020000), 2383.8930, 2423.5590, 2319.4400,
                   2360.6640, 32821965.5000, 336317856.0000));

  /** @arg the start is the first record and the end date is the last record */
  query = KQueryByDate(Datetime(199012210000), Datetime(201112090000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), total - 1);
  CHECK_EQ(kdata.empty(), false);
  ...
  /** @arg the start is the first record and the end date is the day after the
   * last record */
  query = KQueryByDate(Datetime(199012210000), Datetime(201112100000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), total);
  CHECK_EQ(kdata.empty(), false);
  ...
  stock = sm.getStock("sh000001");
  query = KQueryByDate(Datetime(201108150000), Datetime(201111070000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  ...
  CHECK_EQ(kdata[0],
           KRecord(Datetime(201108190000), 2598.1380, 2636.3600, 2513.7540,
                   2534.3580, 40330572.2000, 385608932.0000));
  CHECK_EQ(kdata[kdata.size() - 1],
           KRecord(Datetime(201111040000), 2470.2540, 2536.7790, 2433.7150,
                   2528.2940, 50520578.9000, 516983184.0000));

  /** @arg the start date equals the end date */
  stock = sm.getStock("sh000001");
  query = KQueryByDate(Datetime(199012170000), Datetime(199012170000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 0);
  CHECK_EQ(kdata.empty(), true);
  ...
  query = KQueryByDate(Datetime(199104220000), Datetime(199104220000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 0);
  CHECK_EQ(kdata.empty(), true);
  ...
  query = KQueryByDate(Datetime(199507170000), Datetime(199507170000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 0);
  CHECK_EQ(kdata.empty(), true);
  ...
  query = KQueryByDate(Datetime(201112050000), Datetime(201112050000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 0);
  CHECK_EQ(kdata.empty(), true);
  ...
  stock = sm.getStock("sh000001");
  query = KQueryByDate(Datetime(199012210000), Datetime(199012280000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 1);
  CHECK_EQ(kdata.empty(), false);
  ...
  query = KQueryByDate(Datetime(199707250000), Datetime(199708010000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 1);
  CHECK_EQ(kdata.empty(), false);
  ...
  CHECK_EQ(kdata[0],
           KRecord(Datetime(199707250000), 1215.8390, 1223.8230, 1158.1360,
                   1170.8620, 2433586.7000, 20739544.0000));

  query = KQueryByDate(Datetime(201112090000), Datetime(201112100000),
                       KQuery::WEEK);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 1);
  CHECK_EQ(kdata.empty(), false);
  ...
  record = kdata[1];
  expect = KRecord(Datetime(200001040932), 1368.3, 1368.491, 1368.3, 1368.491,
                   2783200, 106916);
  CHECK_EQ(record, expect);
  ...
  /** @arg The SH000001 minute line data; the start time is one minute before
  ...
  /** @arg The SH000001 minute line data; the start time equals the first record
   * date */
  stock = sm.getStock("sh000001");
  ...
  /** @arg The SH000001 minute line data; the start time is one moment after the
   * first record date
   */
  stock = sm.getStock("sh000001");
  ...
  /** @arg The SH000001 minute line data; the start is the first record and the
   * end date is one moment before the last record */
  stock = sm.getStock("sh000001");
  query =
      KQueryByDate(Datetime(200001040931), Datetime(201112061459), KQuery::MIN);
  kdata = stock.getKData(query);
  ...
  stock = sm.getStock("sh000001");
  query =
      KQueryByDate(Datetime(200001040931), Datetime(201112061500), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 682822L);
  record = kdata[0];
  CHECK_EQ(record.datetime, Datetime(200001040931));
  ...
  /** @arg The SH000001 minute line data; the start is the first record and the
   * end date is one moment after the last record */
  stock = sm.getStock("sh000001");
  query =
      KQueryByDate(Datetime(200001040931), Datetime(201112061501), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 682823L);
  record = kdata[0];
  CHECK_EQ(record.datetime, Datetime(200001040931));
  ...
  stock = sm.getStock("sh000001");
  query =
      KQueryByDate(Datetime(200303051451), Datetime(200303051455), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 4);
  record = kdata[0];
  ...
  stock = sm.getStock("sh000001");
  query =
      KQueryByDate(Datetime(200001040931), Datetime(200001040931), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 0);
  query =
      KQueryByDate(Datetime(200601041446), Datetime(200601041446), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 0);
  query =
      KQueryByDate(Datetime(200801300955), Datetime(200601041446), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 0);
  query =
      KQueryByDate(Datetime(201112061500), Datetime(201112061500), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 0);

  /** @arg The SH000001 minute line data; the end date is one moment after the
   * start date and the data exists */
  stock = sm.getStock("sh000001");
  query =
      KQueryByDate(Datetime(200001040931), Datetime(200001040932), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 1);
  record = kdata[0];
  CHECK_EQ(record.datetime, Datetime(200001040931));
  ...
  query =
      KQueryByDate(Datetime(200601041446), Datetime(200601041447), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 1);
  record = kdata[0];
  ...
  query =
      KQueryByDate(Datetime(201112061500), Datetime(201112061501), KQuery::MIN);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata.size(), 1);
  record = kdata[0];
  CHECK_EQ(record.datetime, Datetime(201112061500));
...
TEST_CASE("test_getKData_recover") {
  ...
  kdata = stock.getKData(query);
  CHECK_EQ(kdata[2710], KRecord(Datetime(201106030000), 10.02, 10.14, 10.0,
                                10.09, 38726.1, 384820));
  CHECK_EQ(kdata[2709], KRecord(Datetime(20110602000000), 10.3385, 10.3769,
                                9.9308, 10.0385, 6027269.9112, 600417.6923));
  CHECK_EQ(kdata[2554], KRecord(Datetime(20101014000000), 11.0385, 11.4154,
                                10.9077, 10.9462, 18482210.2840, 1688466.1538));
  CHECK_EQ(kdata[2548], KRecord(Datetime(20100929000000), 9.2615, 9.6385,
                                9.2000, 9.4846, 5830594.3491, 614742.3077));
  CHECK_EQ(kdata[2547], KRecord(Datetime(20100928000000), 9.8154, 9.8154,
                                9.5462, 9.5538, 4702564.7574, 492216.9231));

  /** @arg The backward adjustment */
  query = KQuery(0, Null<int64_t>(), KQuery::DAY, KQuery::BACKWARD);
  kdata = stock.getKData(query);
  CHECK_EQ(kdata[0], KRecord(Datetime(199911100000), 29.5, 29.8, 27.0, 27.75,
                             485910.2, 1740850));
  CHECK_EQ(kdata[151], KRecord(Datetime(200007050000), 23.25, 23.47, 23.15,
                               23.22, 3298.8, 14218));
  CHECK_EQ(kdata[152], KRecord(Datetime(200007060000), 23.30, 23.42, 23.16,
                               23.23, 306636., 13200.0));
  CHECK_EQ(kdata[657], KRecord(Datetime(200208210000), 18.35, 18.75, 18.18,
                               18.55, 3666222., 197640.0));
  CHECK_EQ(kdata[658], KRecord(Datetime(20020822000000), 18.7700, 18.8900,
                               18.6200, 18.8150, 1340531.1200, 71248.0000));
  ...
  CHECK_EQ(kdata[2710], KRecord(Datetime(201106030000), 10.02, 10.14, 10.0,
                                10.09, 38726.1, 384820));
  CHECK_EQ(kdata[2709], KRecord(Datetime(20110602000000), 10.3348, 10.3728,
                                9.9321, 10.0385, 6027269.9112, 600417.6923));
  CHECK_EQ(kdata[2554], KRecord(Datetime(20101014000000), 11.0263, 11.3987,
                                10.8972, 10.9352, 18463647.2834, 1688466.1538));
  CHECK_EQ(kdata[2548], KRecord(Datetime(20100929000000), 9.2709, 9.6433,
                                9.2102, 9.4913, 5834718.1664, 614742.3077));
  CHECK_EQ(kdata[2547], KRecord(Datetime(20100928000000), 9.8181, 9.8181,
                                9.5521, 9.5597, 4705453.9128, 492216.9231));
  ...
  CHECK_EQ(kdata[151], KRecord(Datetime(200007050000), 23.25, 23.47, 23.15,
                               23.22, 3298.8, 14218));
  CHECK_EQ(kdata[152], KRecord(Datetime(20000706000000), 23.3005, 23.4213,
                               23.1596, 23.2301, 306636.8583, 13200.0000));
  CHECK_EQ(kdata[657], KRecord(Datetime(20020821000000), 18.3183, 18.7209,
                               18.1472, 18.5196, 3660220.8375, 197640.0000));
  CHECK_EQ(kdata[658], KRecord(Datetime(20020822000000), 18.7435, 18.8656,
                               18.5909, 18.7893, 3012073.4901, 160308.0000));
}
...
TEST_CASE("test_KData_getOtherFromSelf") {
  ...
  // Instead of the same query, a different but similar query is tested,
  // avoiding the possible shared_from_this issue
  KQuery query_same(
      0, 10, KQuery::MIN);  // The same type of query but not the same object
  ...
  CHECK_NE(kdata3.getQuery().kType(),
           kdata1.getQuery().kType());  // A different type
  CHECK_UNARY(kdata3 != kdata1);

  /** @arg Test adjusted data with a different adjustment type, a new instance
   * should be created
   */
  ...
  CHECK_NE(kdata4.getQuery().recoverType(),
           kdata1.getQuery().recoverType());  // Different types
  ...
  KQuery query7(Datetime(201001010000), Datetime(201002010000),
                KQuery::DAY);              // The same type
  ...
  KQuery query9(Datetime(201112050930), Datetime(201112061030),
                KQuery::MIN);  // The minute level
  ...
  query10 =
      KQueryByDate(Datetime(201112060930), Datetime(201112061530), KQuery::MIN);
  ...
  query10 =
      KQueryByDate(Datetime(201112010930), Datetime(201112050930), KQuery::MIN);
  kdata10 = kdata9.getKData(query10);
  expect = stock.getKData(query10);
  CHECK_UNARY(!kdata10.empty());
  CHECK_KDATA_EQUAL(kdata10, expect);
  ...
  query10 =
      KQueryByDate(Datetime(201112061131), Datetime(201112061530), KQuery::MIN);
  kdata10 = kdata9.getKData(query10);
  expect = stock.getKData(query10);
  CHECK_UNARY(!kdata10.empty());
  CHECK_KDATA_EQUAL(kdata10, expect);

  query10 = KQueryByDate(Datetime(201112060930), Datetime(201112061530),
                         KQuery::MIN, KQuery::BACKWARD);
  kdata10 = kdata9.getKData(query10);
  expect = stock.getKData(query10);
  CHECK_UNARY(!kdata10.empty());
  CHECK_KDATA_EQUAL(kdata10, expect);
...
/** @par Test point - cover the branches of _getOtherFromSelfByIndex and
 * _getOtherFromSelfByDate */
TEST_CASE("test_KData_getOtherFromSelf_subfunctions") {
  DataRuntime& sm = getDataRuntime();
  Stock stock = sm.getStock("sh600000");

  /** @arg Test _getOtherFromSelfByIndex: the new start is before the old data,
   * a new instance */
  ...
  CHECK_NE(kdata2.getQuery(),
           kdata1.getQuery());  // A new instance should be created

  /** @arg Test _getOtherFromSelfByIndex: the new start is inside the old data
   * but the end is beyond it */
  KQuery query_extend_end(kdata1.startPos() + 2, kdata1.endPos() + 5,
                          KQuery::DAY);
  KData kdata3 = stock.getKData(
      query_extend_end);  // Fetch again to make the data complete
  ...
  /** @arg Test _getOtherFromSelfByIndex: the new query is entirely inside the
   * old data */
  ...
  /** @arg Test _getOtherFromSelfByDate: the new start date is earlier than the
   * old one */
  KQuery date_query1(Datetime(201001010000), Datetime(201012010000),
                     KQuery::DAY);
  ...
  CHECK_NE(kdata_date2.getQuery(),
           kdata_date1.getQuery());  // A new instance should be created

  /** @arg Test _getOtherFromSelfByDate: the new start date is inside the old
   * data */
  ...
  /** @arg Test _getOtherFromSelfByDate: the date query crosses the existing
   * data range */
  Datetime end_date = kdata_date1.back().datetime;
  Datetime future_date =
      end_date + TimeDelta(30 * 24 * 3600);  // 30 days later via TimeDelta
  ...
  CHECK_GE(kdata_date4.size(),
           kdata_date1.size());  // The new data should not be smaller
  ...
  KQuery date_from_idx(Datetime(200001010000), Datetime(200005010000),
                       KQuery::DAY);
  ...
  KQuery date_query_orig(Datetime(200001010000), Datetime(200005010000),
                         KQuery::DAY);
...
TEST_CASE("test_KData_getKData") {
  ...
  k1 = getKData("sh000001",
                KQueryByIndex(10000, 100010, KQuery::DAY, KQuery::FORWARD));
  REQUIRE(k1.size() == 0);
  k2 = k1.getKData(KQuery::MIN);
  ...
  k1 = getKData("sh000001",
                KQuery(-10, Null<int64_t>(), KQuery::MONTH, KQuery::FORWARD));
  ...
  k1 = getKData("sh000001",
                KQuery(Datetime(201101010000), Datetime(201112010000)));
  REQUIRE(k1.size() > 0);
