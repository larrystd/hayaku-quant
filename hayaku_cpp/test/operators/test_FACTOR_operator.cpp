...
TEST_CASE("test_FACTOR") {
  ...
  // Check whether the directly applied result matches creating it and then
  // setting the context
...
TEST_CASE("test_FACTOR_different_factors") {
  ...
  Factor factor_high_ma3("HIGH_MA3_FACTOR", ma3, KQuery::DAY,
                         "3-day MA of high factor",
...
TEST_CASE("test_FACTOR_alike_uses_factor_identity") {
  ...
  Factor same_identity_different_formula("CLOSE_MA5", MA(HIGH(), 3),
                                         KQuery::DAY);
