...
TEST_CASE("test_IniParser_read") {
  ...
  CHECK_THROWS_AS(ini_parser.read("sdjfljoiEI487sdbc.txt"),
                  std::invalid_argument);

  /** @arg A wrong file format: the section is missing and a logic_error is
   * thrown */
  ...
  /** @arg A wrong file format: the section line is wrong, "]" is missing, e.g.
   * "[section "*/
  testini.open(test_filename, ofstream::trunc);
  ...
  /** @arg A wrong file format: the key=value line is wrong, the key is missing,
   * e.g. "= value "*/
  testini.open(test_filename, ofstream::trunc);
  testini << "[section]\n"
          << "= value";
  ...
  /** @arg A wrong file format: the key=value line is wrong, the value is
   * missing, e.g. "key="*/
  testini.open(test_filename, ofstream::trunc);
  testini << "[section]\n"
          << "key=";
  ...
  /** @arg A wrong file format: a non-comment line with no section and no
   * key=value, e.g. "key"*/
  testini.open(test_filename, ofstream::trunc);
  testini << "[section]\n"
          << "key";
  ...
  /** @arg Besides the comments and the empty lines, the only valid line is a
   * section line */
  testini.open(test_filename, ofstream::trunc);
  testini << "[section1]";
  testini.close();
...
TEST_CASE("test_IniParser_get") {
  ...
  /** @arg The given section exists but the option does not, and there is no
   * default value */
  CHECK_THROWS_AS(ini_parser.get("section1", "key2"), std::invalid_argument);

  /** @arg The section exists but the option does not, and a default value is
   * given */
  ...
  CHECK_THROWS_AS(ini_parser.get("section3", "key1", "value1"),
                  std::invalid_argument);
...
TEST_CASE("test_IniParser_getInt") {
  ...
  /** @arg The given section and option exist but the value cannot be converted
   * into int */
  CHECK_THROWS_AS(ini_parser.getInt("section2", "key2"), std::invalid_argument);

  /** @arg The section and option exist but an invalid default value is given
   * (it cannot be converted into int), an exception should be thrown */
  CHECK_THROWS_AS(ini_parser.getInt("section1", "key1", "tow"),
                  std::invalid_argument);
  CHECK_THROWS_AS(ini_parser.getInt("section2", "key1", "twenty"),
                  std::invalid_argument);
  ...
  /** @arg The section exists but the option does not, and a valid default value
   * is given */
  CHECK_EQ(ini_parser.getInt("section1", "key2", "10"), 10);

  /** @arg The section exists but the option does not, and an invalid default
   * value is given (it cannot be converted into int) */
  CHECK_THROWS_AS(ini_parser.getInt("section1", "key2", "10.0"),
                  std::invalid_argument);
   ...
   */
  CHECK_THROWS_AS(ini_parser.getInt("section3", "key1", "1"),
                  std::invalid_argument);
...
TEST_CASE("test_IniParser_getFloat") {
  ...
  /** @arg The given section and option exist and there is no default value */
  CHECK_EQ(ini_parser.getFloat("section1", "key1"),
           doctest::Approx(1.123456).epsilon(0.00001));
  CHECK_EQ(ini_parser.getFloat("section1", "key3"),
           doctest::Approx(-3.145789).epsilon(0.00001));
  CHECK_EQ(ini_parser.getFloat("section2", "key1"),
  ...
  /** @arg The given section and option exist but the value cannot be converted
   * into int */
  CHECK_THROWS_AS(ini_parser.getFloat("section2", "key2"),
                  std::invalid_argument);

  /** @arg The section and option exist but an invalid default value is given
   * (it cannot be converted into int), an exception should be thrown */
  CHECK_THROWS_AS(ini_parser.getFloat("section1", "key1", "tow"),
                  std::invalid_argument);
  CHECK_THROWS_AS(ini_parser.getFloat("section2", "key1", "twenty"),
                  std::invalid_argument);

  /** @arg The given section exists but the option does not, and there is no
   * default value */
  CHECK_THROWS_AS(ini_parser.getFloat("section1", "key2"),
                  std::invalid_argument);

  /** @arg The section exists but the option does not, and a valid default value
   * is given */
  CHECK_EQ(ini_parser.getFloat("section1", "key2", "10"),
           doctest::Approx(10.0));

  /** @arg The section exists but the option does not, and an invalid default
   * value is given (it cannot be converted into int) */
  CHECK_THROWS_AS(ini_parser.getFloat("section1", "key2", "1A0"),
                  std::invalid_argument);

  /** @arg The given section does not exist and there is no default value */
  CHECK_THROWS_AS(ini_parser.getFloat("section3", "key1"),
                  std::invalid_argument);

  /** @arg The given section does not exist but a valid default value is given
   */
  CHECK_THROWS_AS(ini_parser.getFloat("section3", "key1", "1"),
                  std::invalid_argument);
  ...
  /** @arg
   * Test the data beyond the float range: 3.41+38. Note: with MSVC such data
   * does not throw but becomes INF
   */
#ifdef __MSVC__
  CHECK_THROWS_AS(ini_parser.getFloat("section1", "key2", "3.41e+38"),
                  std::invalid_argument);
#endif
...
TEST_CASE("test_IniParser_getDouble") {
  ...
  /** @arg The given section and option exist and there is no default value */
  CHECK_EQ(ini_parser.getDouble("section1", "key1"),
           doctest::Approx(1.123456).epsilon(0.00001));
  CHECK_EQ(ini_parser.getDouble("section1", "key3"),
           doctest::Approx(-3.145789).epsilon(0.00001));
  CHECK_EQ(ini_parser.getDouble("section2", "key1"),
           doctest::Approx(10).epsilon(0.00001));

  /** @arg The given section and option exist and a valid default value is given
   */
  CHECK_EQ(ini_parser.getDouble("section1", "key1", "2.01"),
           doctest::Approx(1.123456));
  CHECK_EQ(ini_parser.getDouble("section2", "key1", "20.20"),
           doctest::Approx(10));

  /** @arg The given section and option exist but the value cannot be converted
   * into int */
  CHECK_THROWS_AS(ini_parser.getDouble("section2", "key2"),
                  std::invalid_argument);

  /** @arg The section and option exist but an invalid default value is given
   * (it cannot be converted into int), an exception should be thrown */
  CHECK_THROWS_AS(ini_parser.getDouble("section1", "key1", "tow"),
                  std::invalid_argument);
  CHECK_THROWS_AS(ini_parser.getDouble("section2", "key1", "twenty"),
                  std::invalid_argument);

  /** @arg The given section exists but the option does not, and there is no
   * default value */
  CHECK_THROWS_AS(ini_parser.getDouble("section1", "key2"),
                  std::invalid_argument);
  ...
  /** @arg The section exists but the option does not, and an invalid default
   * value is given (it cannot be converted into int) */
  CHECK_THROWS_AS(ini_parser.getDouble("section1", "key2", "1A0"),
                  std::invalid_argument);

  /** @arg The given section does not exist and there is no default value */
  CHECK_THROWS_AS(ini_parser.getDouble("section3", "key1"),
                  std::invalid_argument);

  /** @arg The given section does not exist but a valid default value is given
   */
  CHECK_THROWS_AS(ini_parser.getDouble("section3", "key1", "1"),
                  std::invalid_argument);
...
TEST_CASE("test_IniParser_getBool") {
  ...
  /** @arg The given section and option exist,
   * no default value is given and the values are 1|0|true|yes|on|false|no|off
   * (mixed cases) */
  ...
  /** @arg The section and option exist but an invalid default value is given
   * (not convertible to bool), an exception should be thrown */
  CHECK_THROWS_AS(ini_parser.getBool("section1", "key1", "tow"),
                  std::invalid_argument);
  CHECK_THROWS_AS(ini_parser.getBool("section1", "key1", "twenty"),
                  std::invalid_argument);

  /** @arg The given section exists but the option does not, and there is no
   * default value */
  CHECK_THROWS_AS(ini_parser.getBool("section1", "key22"),
                  std::invalid_argument);

  /** @arg
   * The section exists but the option does not, and a valid default value is
   * given (0|1|true|false|yes|no|on|off) (mixed cases)
   */
  ...
  /** @arg The section exists but the option does not, and an invalid default
   * value is given (not convertible to bool) */
  CHECK_THROWS_AS(ini_parser.getBool("section1", "key22", "10"),
                  std::invalid_argument);

  /** @arg The given section does not exist and there is no default value */
  CHECK_THROWS_AS(ini_parser.getBool("section3", "key1"),
                  std::invalid_argument);

  /** @arg The given section does not exist but a valid default value is given
   */
  CHECK_THROWS_AS(ini_parser.getBool("section3", "key1", "1"),
                  std::invalid_argument);
