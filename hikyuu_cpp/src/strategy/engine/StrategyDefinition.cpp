...
StrategyDefinition::StrategyDefinition(MoneyManagerPtr moneyManager, SignalPtr signal, string name,
...
: m_moneyManager(std::move(moneyManager)),
  ...
  m_parameters(std::move(parameters)) {
    HKU_CHECK(m_moneyManager, "StrategyDefinition requires a MoneyManager");
    HKU_CHECK(m_signal, "StrategyDefinition requires a Signal");
}
...
bool StrategyDefinition::operator==(const StrategyDefinition& other) const {
    return m_moneyManager == other.m_moneyManager && m_signal == other.m_signal &&
           m_name == other.m_name &&
           m_environment == other.m_environment && m_condition == other.m_condition &&
           m_stoploss == other.m_stoploss && m_takeProfit == other.m_takeProfit &&
