...
def main(datadir, publish_shm, recv_spot, config_file):
    ...
    # 同时确保按 --config（或默认）配置完成数据初始化（import hayaku 本身不加载数据）。
    load_kwargs = {"use_shm_server": False}
    if config_file:
        load_kwargs["config_file"] = config_file
    hayaku.load_hayaku(**load_kwargs)

    try:
        if not hayaku.start_shm_server(datadir, publish_shm=publish_shm, recv_spot=recv_spot):
