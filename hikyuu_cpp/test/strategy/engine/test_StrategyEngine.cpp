...
#include "strategy/engine/internal/PendingOrderState.h"
...
StrategyDefinition makeTestDefinition() {
    ...
    return StrategyDefinition(MM_FixedCount(100), sg, "StrategyEngineTest", EnvironmentPtr(),
                              ConditionPtr(), StoplossPtr(), StoplossPtr(), ProfitGoalPtr(),
                              SlippagePtr(), std::move(parameters));
}
...
TEST_CASE("test_StrategyDefinition") {
    ...
    /** @arg The definition owns only strategy components and no account/runtime object. */
...
TEST_CASE("test_StrategyEngine_constructor") {
    ...
    ExecutionEngine execution(
      AccountConfig(Datetime(199001010000LL), 100000.0, TC_Zero(), "StrategyEngineTest"));
...
TEST_CASE("test_StrategyEngine_run") {
    ...
    /** @arg The result is the immutable snapshot of the bound execution account's strategy trades. */
    ...
    /** @arg A completed result remains stable after a later run mutates account state. */
