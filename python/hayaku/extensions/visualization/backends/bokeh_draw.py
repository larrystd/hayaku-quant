...
def kplot(kdata, new=True, axes=None, colorup='r', colordown='g'):
    ...
    axes.add_tools(
        HoverTool(
            tooltips=[
                ("index", "$index"), ('Date', get_date_format(k)), ('Open', "@open{0.0000}"),
                ('High', "@high{0.0000}"), ('Low', "@low{0.0000}"), ('Close', "@close{0.0000}"),
                ('Amount', "@amount{0.0000}"), ('Volume', "@volume{0.0000}")
            ],
...
def mkplot(kdata, new=True, axes=None, colorup='r', colordown='g', ticksize=3):
    ...
    """
    print("Bokeh does not support the American K-line chart yet, please use matplotlib")
    return None
    ...
    **kwargs
    ...
    if kref:
        ...
        axes.add_tools(
            HoverTool(
                tooltips=[
                    ("index", "$index"), ('Indicator', indicator.name), ('Date', get_date_format(kref)),
                    ('Value', "@value{0.0000}")
                ],
                formatters={"@datetime": "datetime"}
            )
