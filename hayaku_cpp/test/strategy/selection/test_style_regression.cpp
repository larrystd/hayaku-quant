 ...
 *
 *   - golden values: checked against the analytical solution of the unary
 * linear regression;
 *   - boundaries: all NaN with few samples, NaN rows with a NaN variable, no
 * crash when collinear;
 *   - the global state: Eigen::nbThreads() is unchanged around the concurrent
 * calls (anti-regression: if a runtime Eigen::setNbThreads is reintroduced this
 * test fails).
 */

#include <strategy/selection/StyleRegression.h>
 ...
 *  y = [1,2,3,4] and x = [1,2,3,4]: the analytical solution is slope=1,
 * intercept=0, residuals 0
 */
...
/** @par golden: a non-perfect fit unary regression checked against the
 * analytical solution y = [2,1,4,3] and x = [0,1,2,3]: x_mean=1.5, y_mean=2.5
...
/** @par Boundary: with few samples (valid_count <= k+1) all NaN is returned
 * without a crash */
...
/** @par Boundary: the residual of a row with a NaN variable is NaN and the
...
/** @par Boundary: collinear variables (two fully collinear columns) neither
 * crash nor give Inf */
...
/** @par The global state: the Eigen thread configuration is unchanged around
 * the concurrent calls Eigen::nbThreads() exists unconditionally in
 * <Eigen/Core> (it returns 1 without OpenMP), so it is asserted
 * unconditionally: whether it starts as 1 or N, it must stay afterwards.
 *  Anti-regression: if a runtime global Eigen::setNbThreads switch is
...
TEST_CASE("test_style_regression_eigen_threads_unchanged") {
  ...
  for (size_t i = 0; i < 8; i++) {
    threads.emplace_back([&, i]() {
      // Every thread writes its own slot and the main thread verifies after the
      // join
  ...
  // The Eigen global thread configuration must not change around the concurrent
  // calls
