/*
 * test_INDEXX.cpp
 * Test the correctness of INDEXC in the TDX unusual price formula with real
 * data
 *
 ...
 * @ingroup test_hayaku_real_data
 * @{
 */

/** @par Test point: the correctness of INDEXC in the unusual price formula
 * (suspended stocks) */
TEST_CASE("test_INDEXC") {
  ...
  auto testOne = [&](const char* code, const char* name) {
    ...
    std::cout << "  unusual price 30 days=" << v30 << " 10 days=" << v10
              << std::endl;
    ...
    ic.setContext(kdata);
    // Compare bar by bar: INDEXC[i] should equal the close of sh000001 on
    // kdata[i].datetime
    ...
    KQuery idxQuery =
        KQueryByDate(kdata[0].datetime, kdata[n - 1].datetime + Seconds(secs),
                     query.kType(), query.recoverType());
    auto idx_kdata = idx_stock.getKData(idxQuery);

    // Build the index date -> close price mapping
    std::unordered_map<hayaku::Datetime, double> idx_close_map;
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (std::abs(ic_val - expected) > 0.0001) {
        diff_count++;
        if (diff_count <= 5) {
          std::cout << "  difference bar[" << i << "] date=" << dt
                    << " INDEXC=" << ic_val << " expected=" << expected
                    << " diff=" << std::abs(ic_val - expected) << std::endl;
        }
      }
    }
    std::cout << "  INDEXC bar by bar: the difference count=" << diff_count
              << "/" << n << " the missing index day count=" << null_count
              << std::endl;
  ...
  // 603045: suspended (2021-09-27~2021-10-15, 2022-11-30), the two K-line
  // counts differ
