...
TEST_CASE("test_BETA") {
  ...
  PriceList stock_returns{0.01,  0.02,  -0.01, 0.015,  0.005,
                          -0.02, 0.012, 0.018, -0.008, 0.02};
  ...
  // Per the IBeta implementation, kx = first_stock_value and ky =
  // first_market_value The accumulation starts from the second data point
  // stock: [0.01, 0.02, -0.01, 0.015, 0.005, -0.02, 0.012, 0.018, -0.008, 0.02]
  // market: [0.008, 0.015, -0.005, 0.012, 0.003, -0.015, 0.008, 0.012, -0.003,
  // 0.015]
...
TEST_CASE("test_BETA_benchmark") {
  ...
  {
    BENCHMARK_TIME_MSG(test_BETA_benchmark, cycle,
