...
SQLiteConnect::SQLiteConnect(const Parameter &param) : DBConnectBase(param), m_db(nullptr) {
    try {
        ...
        HAYAKU_FATAL("Failed open database: {})! SQLite3 errcode: {}, errmsg: {}", m_dbname,
                  e.errcode(), e.what());
        close();
        throw;
    } catch (std::exception &e) {
        HAYAKU_FATAL("Failed initialize data driver({})! exception: {}", m_dbname, e.what());
        close();
        throw;
    } catch (...) {
        HAYAKU_FATAL("Failed open database({})! Unkown error!", m_dbname);
        close();
        throw;
    }
}
...
void SQLiteConnect::resetAutoIncrement(const std::string &tablename) {
    ...
    HAYAKU_CHECK(count == 0, "The ID cannot be reset when data is present in table({})", tablename);
