...
class MarketInfoTable {
 public:
  ...
  uint64_t id() const { return m_marketid; }
  ...
  Datetime lastDate() const {
    HAYAKU_CHECK(m_lastDate <= 99999999, "Invalid lastDate: {}!", m_lastDate);
    return Datetime(m_lastDate * 10000LL);
  }

  TimeDelta openTime1() const { return _transTimeDelta(m_openTime1); }

  TimeDelta closeTime1() const { return _transTimeDelta(m_closeTime1); }

  TimeDelta openTime2() const { return _transTimeDelta(m_openTime2); }

  TimeDelta closeTime2() const { return _transTimeDelta(m_closeTime2); }
 ...
 public:
  ...
  void update(const SQLStatementPtr& st) const {
    st->bind(0, m_market, m_name, m_description, m_code, m_lastDate,
             m_openTime1, m_closeTime1, m_openTime2, m_closeTime2, m_marketid);
  }

  void load(const SQLStatementPtr& st) {
    st->getColumn(0, m_marketid, m_market, m_name, m_description, m_code,
                  m_lastDate, m_openTime1, m_closeTime1, m_openTime2,
                  m_closeTime2);
  }
