...
internal::ExecutionAccountPortPtr makeBrokerAccount(
  ...
  const std::vector<OrderBrokerPtr>& other_brokers) {
    ...
    HKU_CHECK(broker_port, "Execution account does not support live broker synchronization");
...
void Strategy::_init() {
    ...
    if (sm.thread_id() == std::thread::id()) {
        ...
        // Initialization
        m_session = std::make_unique<HikyuuSession>(HikyuuSession::open(
...
TradeRecord Strategy::orderValue(const Stock& stk, price_t value, const string& remark) {
    ...
    if (value > 0.0) {
        ...
        HKU_CHECK(m_account, "Strategy execution account is not configured");
...
void HKU_API runInStrategy(const internal::StrategyRuntimePtr& strategy, const Stock& stk,
                           const KQuery& query,
    ...
    HKU_CHECK(!strategy->getParam<bool>("buy_delay") &&
    ...
    HKU_CHECK(portfolio_account, "Execution account does not support strategy capital management");
...
void HKU_API runInStrategy(const PFPtr& pf, const KQuery& query, const OrderBrokerPtr& broker,
    ...
    auto portfolio_account = std::dynamic_pointer_cast<internal::PortfolioAccountPort>(account);
    HKU_CHECK(portfolio_account, "Execution account does not support portfolio capital management");
