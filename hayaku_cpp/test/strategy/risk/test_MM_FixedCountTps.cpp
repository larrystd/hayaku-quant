...
TEST_CASE("test_MM_FixedCountTpsTps") {
  ...
  CHECK_THROWS_AS(MM_FixedCountTps({100., 100.}, {100., -100.}),
                  std::exception);
  ...
  /** @arg n = 100; a trade account with the initial capital 0 auto-fills and
   * can buy */
  ...
  double buy_num = mm->getBuyNumber(Datetime(200001200000), stock, 24.11, 24.11,
                                    OrderOrigin::SIGNAL);
  CHECK_EQ(buy_num, 100);
  CHECK_EQ(tm->cash(Datetime(200001200000)), 2417.02);
  auto tr =
      tm->submit(OrderRequest(OrderSide::BUY, Datetime(200001200000), stock,
                              24.11, 100, 0, 0, 24.11, OrderOrigin::SIGNAL))
          .trade();
  mm->buyNotify(tr);
  CHECK_EQ(mm->currentBuyCount(stock), 1);

  buy_num = mm->getBuyNumber(Datetime(200001200000), stock, 24.11, 24.11,
                             OrderOrigin::SIGNAL);
  CHECK_EQ(buy_num, 200);
  tr = tm->submit(OrderRequest(OrderSide::BUY, Datetime(200001200000), stock,
                               24.11, 100, 0, 0, 24.11, OrderOrigin::SIGNAL))
           .trade();
  mm->buyNotify(tr);
  ...
  double sell_num = mm->getSellNumber(Datetime(200001210000), stock, 24.11,
                                      24.11, OrderOrigin::SIGNAL);
  CHECK_EQ(sell_num, 200);
  tr = tm->submit(OrderRequest(OrderSide::SELL, Datetime(200001200000), stock,
                               24.11, 100, 0, 0, 24.11, OrderOrigin::SIGNAL))
           .trade();
  mm->sellNotify(tr);
  CHECK_EQ(mm->currentBuyCount(stock), 0);
  ...
  sell_num = mm->getSellNumber(Datetime(200001210000), stock, 24.11, 24.11,
                               OrderOrigin::SIGNAL);
  ...
  buy_num = mm->getBuyNumber(Datetime(200001202000), stock, 24.11, 24.11,
                             OrderOrigin::SIGNAL);
  CHECK_EQ(buy_num, 100);
  tr = tm->submit(OrderRequest(OrderSide::BUY, Datetime(200001202000), stock,
                               24.11, 100, 0, 0, 24.11, OrderOrigin::SIGNAL))
           .trade();
  mm->buyNotify(tr);
  CHECK_EQ(mm->currentBuyCount(stock), 1);
