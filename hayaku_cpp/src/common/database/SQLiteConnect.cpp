...
SQLiteConnect::SQLiteConnect(const Parameter &param)
  ...
  try {
    ...
    HAYAKU_FATAL("Failed open database: {})! SQLite3 errcode: {}, errmsg: {}",
                 m_dbname, e.errcode(), e.what());
    close();
    throw;
  } catch (std::exception &e) {
    HAYAKU_FATAL("Failed initialize data driver({})! exception: {}", m_dbname,
                 e.what());
    close();
    throw;
  } catch (...) {
...
int64_t SQLiteConnect::exec(const std::string &sql_string) {
  ...
  SQL_CHECK(rc == SQLITE_OK, rc, "SQL error: {}! ({})", sqlite3_errmsg(m_db),
            sql_string);
...
bool SQLiteConnect::check(bool quick) noexcept {
  ...
  if (sqlite3_prepare_v2(m_db, check_pragma.c_str(), -1, &integrity, NULL) ==
      SQLITE_OK) {
