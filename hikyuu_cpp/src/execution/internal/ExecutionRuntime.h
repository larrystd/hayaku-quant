...
#pragma once
#ifndef HIKYUU_TRADE_INTERNAL_EXECUTIONRUNTIME_H_
#define HIKYUU_TRADE_INTERNAL_EXECUTIONRUNTIME_H_

#include "common/Parameter.h"
#include "../broker/OrderBrokerBase.h"
#include "../cost/TC_Zero.h"
...
public:
    ...
    [[nodiscard]] CostRecord getSellCost(const Datetime& datetime, const Stock& stock,
                                         price_t price, double number) const {
        return m_costfunc ? m_costfunc->getSellCost(datetime, stock, price, number) : CostRecord();
    }

    [[nodiscard]] CostRecord getBorrowCashCost(const Datetime& datetime, price_t cash) const {
    ...
    [[nodiscard]] CostRecord getReturnCashCost(const Datetime& borrowDatetime,
                                               ...
                                               price_t cash) const {
        return m_costfunc
                 ? m_costfunc->getReturnCashCost(borrowDatetime, returnDatetime, cash)
                 : CostRecord();
    }

    [[nodiscard]] CostRecord getBorrowStockCost(const Datetime& datetime, const Stock& stock,
                                                price_t price, double number) const {
        return m_costfunc ? m_costfunc->getBorrowStockCost(datetime, stock, price, number)
                          : CostRecord();
    }

    [[nodiscard]] CostRecord getReturnStockCost(const Datetime& borrowDatetime,
                                                const Datetime& returnDatetime, const Stock& stock,
                                                price_t price, double number) const {
        return m_costfunc ? m_costfunc->getReturnStockCost(borrowDatetime, returnDatetime, stock,
                                                           price, number)
                          : CostRecord();
    }
    ...
    [[nodiscard]] CostRecord getBuyCost(const Datetime& datetime, const Stock& stock,
                                        price_t price, double number) const override {
        return m_costfunc ? m_costfunc->getBuyCost(datetime, stock, price, number) : CostRecord();
    }
    ...
    price_t initCash() const override {
        return m_ledger.m_initCash;
    }

    /** Account creation date */
    Datetime initDatetime() const override {
        return m_ledger.m_initDatetime;
    }
    ...
    Datetime lastDatetime() const override {
        return m_ledger.m_tradeList.empty() ? m_ledger.m_initDatetime : m_ledger.m_tradeList.back().datetime;
    }
     ...
     *       information
     */
    price_t cash(const Datetime& datetime, KQuery::KType ktype = KQuery::DAY) override;
    ...
    size_t getStockNumber() const override {
        return m_ledger.m_position.size();
    }
    ...
    virtual TradeRecord buy(const Datetime& datetime, const Stock& stock, price_t realPrice,
                            ...
                            price_t planPrice = 0.0, OrderOrigin from = OrderOrigin::UNSPECIFIED,
                            const string& remark = "");
    ...
    virtual TradeRecord sell(const Datetime& datetime, const Stock& stock, price_t realPrice,
                             ...
                             OrderOrigin from = OrderOrigin::UNSPECIFIED, const string& remark = "");
    ...
    [[nodiscard]] PriceList getProfitCurve(const DatetimeList& dates,
                                           KQuery::KType ktype = KQuery::DAY) override;
...
inline void ExecutionRuntime::baseCheckParam(const string& name) const {
    if (name == "precision") {
        HKU_ASSERT(getParam<int>("precision") > 0);
    }
}
...
#endif /* HIKYUU_TRADE_INTERNAL_EXECUTIONRUNTIME_H_ */
