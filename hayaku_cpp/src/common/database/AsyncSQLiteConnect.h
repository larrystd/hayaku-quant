 ...
 * Every connection holds an independent ThreadPool(1) single thread pool
 * internally, and the synchronous operations are converted into the
 * asynchronous interfaces through co_run.
 */
class HAYAKU_UTILS_API AsyncSQLiteConnect : public AsyncDBConnectBase {
 public:
   ...
   * @note This function cannot distinguish whether the file is not a sqlite
   * file or the sqlite itself is corrupted If the file itself needs to be
   * judged as a valid sqlite file, please use the isValidSqliteFile method
   ...
   * other database connections (the asynchronous version)
   ...
   */
  net::awaitable<bool> backup(const char *zFilename, int n_page = -1,
                              int step_sleep = 250);

 private:
  friend class AsyncSQLiteStatement;

  // The method provided to AsyncSQLiteStatement to access the original
  // connection
  void *getRawConnection() const noexcept;

  // The method provided to AsyncSQLiteStatement to access the thread pool
  // executor
  ...
  // Internal helper methods - the synchronous version (used to ensure the
  // connection has been initialized when the Statement is constructed)
  void _connect();
