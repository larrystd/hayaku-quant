...
#pragma once
#ifndef HIKYUU_TRADE_BROKER_ORDERBROKERBASE_H
#define HIKYUU_TRADE_BROKER_ORDERBROKERBASE_H
 ...
 * orders.
 * @details Multiple order broker instances can be registered with an execution account. These
 *          brokers can perform extra buy / sell actions; for example an e-mail order broker can
 *          send an e-mail when the execution runtime issues a buy / sell
 *          instruction.
 *
 *          By default the execution runtime calls the order broker for buy / sell actions when it
 ...
 *          it calculates the signal indication, so the execution runtime would execute the order
 ...
 *          of the execution account is used to specify that moment.
 * @ingroup OrderBroker
 */
...
public:
    ...
    void buy(Datetime datetime, const string& market, const string& code, price_t price, double num,
             price_t stoploss, price_t goalPrice, OrderOrigin from, const string& remark) noexcept;
    ...
    void sell(Datetime datetime, const string& market, const string& code, price_t price,
              double num, price_t stoploss, price_t goalPrice, OrderOrigin from,
...
#endif /* HIKYUU_TRADE_BROKER_ORDERBROKERBASE_H */
