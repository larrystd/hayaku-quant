...
static bool _pluginValid() {
    auto* plugin = getPlugin<HayakuExtraPluginInterface>(HAYAKU_PLUGIN_HAYAKU_EXTRA);
...
TEST_CASE("test_Stock_extra_ktype") {
    ...
    // Extra K types are now an explicit optional extension, not registered by core Session startup.
    registerExtraKType(KQuery::DAY3, KQuery::DAY, 3);
    registerExtraKType(KQuery::MIN3, KQuery::MIN, 3, [](const Datetime& datetime) {
        auto minute = datetime.minute();
        if (minute % 3 == 0) {
            return datetime;
        }
        minute = (minute / 3 + 1) * 3;
        return Datetime(datetime.year(), datetime.month(), datetime.day(), datetime.hour(), minute);
    });
