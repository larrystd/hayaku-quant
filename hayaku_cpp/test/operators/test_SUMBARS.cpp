...
TEST_CASE("test_SUMBARS_with_cval_dyn_param") {
  // The original trigger scenario: a nested CVAL used as the SUMBARS dynamic
  // parameter. Before the fix: one=CVAL(10) inside CVAL(one, 10) -> goes
  // through Indicator::operator() -> alike true
  //   -> it short-circuits and returns an uncalculated empty shell (size==0) ->
  //   SUMBARS throws the HAYAKU_CHECK(ind_param.size()==ind.size()) exception.
  // After the fix: ind is reused, CVAL(one,10).size()==1 and SUMBARS calculates
  // normally.
  ...
  // Reachable: ind[0]=10 >= a[0]=10, satisfied at the first bar, the distance
  // is 0
...
//-----------------------------------------------------------------------------
// The sign regression of the dynamic/static paths (fixing the negative sign bug
// of _dyn_calculate)
//-----------------------------------------------------------------------------
/** @par Test points */
TEST_CASE("test_SUMBARS_dyn_sign_regression") {
  // A counter example: the last bar must go back 2 bars to accumulate to a, the
  // distance is > 0 (a non-degenerate solution)
  //   before the fix the dynamic version returned -2 (a wrong sign) and after
  //   the fix it returns 2 (matching the static one)
  PriceList a;
  a.push_back(5);
  a.push_back(3);
  a.push_back(2);  // [5, 3, 2]
  ...
  // The per-bar value symmetry of the dynamic/static paths (the discard need
  // not be equal, see the case below)
...
TEST_CASE("test_SUMBARS_dyn_static_equivalence") {
  // Several positions have a distance > 0, covering the non-degenerate solution
  // equivalence class
  PriceList a;
  for (int i = 0; i < 10; i++) {
...
TEST_CASE("test_SUMBARS_dyn_unreachable_discard") {
  // The whole-range accumulation is unreachable: the dynamic version writes NaN
  // per bar without advancing the discard, while the static version discards
  // everything
  PriceList a;
  for (int i = 0; i < 10; i++) {
    a.push_back(1);  // All 1
  ...
  CHECK_EQ(d.discard(),
           data.discard());  // The dynamic version keeps the input discard
...
TEST_CASE("test_SUMBARS_dyn_edge") {
  // 1. a <= 0: sum>=a holds at the first bar so the distance is always 0
  // (verifying the i==j degenerate solution, no infinite loop)
  {
    PriceList a;
  ...
  // 2. The input sequence has a discard > 0: the lower bound of the inner j
  // loop is ind.discard(), verifying the termination without an out of range
  // access
  {
    PriceList a;
    a.push_back(10);
    ...
    Indicator ma =
        MA(PRICELIST(a), 2);  // discard=1, [1]=(10+1)/2=5.5, and then always 1
  ...
  // 3. The extreme lengths
  {
    ...
    // A single reachable bar (the static path can use CVAL; the dynamic path
    // builds an a sequence of size=1 with PRICELIST, avoiding the defect that a
    // nested CVAL propagates a size of 0 on a single element)
    Indicator one = CVAL(10);
