...
/**
 * @defgroup test_hayaku_TransList test_hayaku_TransList
...
TEST_CASE("test_TransList_query_by_index") {
  ...
  CHECK_EQ(result[0],
           TransRecord(Datetime(2019, 2, 1, 9, 30, 2), 11.21, 1339, 0));
  CHECK_EQ(result[1],
           TransRecord(Datetime(2019, 2, 1, 9, 30, 5), 11.21, 5263, 0));
  ...
  CHECK_EQ(result.size(), 2);
  CHECK_EQ(result[0],
           TransRecord(Datetime(2019, 2, 11, 14, 56, 59), 11.20, 210, 1));
  CHECK_EQ(result[1],
           TransRecord(Datetime(2019, 2, 11, 14, 57, 2), 11.20, 31, 1));
  ...
  CHECK_EQ(result[0],
           TransRecord(Datetime(2019, 2, 1, 9, 25, 2), 11.20, 15714, 2));
  CHECK_EQ(result[563],
           TransRecord(Datetime(2019, 2, 1, 9, 58, 47), 11.04, 91, 0));
  CHECK_EQ(result[8883],
           TransRecord(Datetime(2019, 2, 11, 15, 0, 0), 11.21, 5794, 2));
...
TEST_CASE("test_TransList_query_by_date") {
  ...
  query = KQueryByDate(Datetime(2019, 2, 11, 14, 56, 59),
                       Datetime(2019, 2, 11, 14, 56, 59));
  result = stock.getTransList(query);
  CHECK_EQ(result.size(), 0);

  /** @arg Both start and end are inside the data range and end is the last
   * record date */
  query = KQueryByDate(Datetime(2019, 2, 11, 14, 56, 59),
  ...
  /** @arg start is inside the data range and end is later than the last record
   * date */
  query =
      KQueryByDate(Datetime(2019, 2, 11, 14, 56, 59), Datetime(201902120000));
  result = stock.getTransList(query);
  CHECK_EQ(result.size(), 3);
  CHECK_EQ(result[0],
           TransRecord(Datetime(2019, 2, 11, 14, 56, 59), 11.20, 210, 1));
