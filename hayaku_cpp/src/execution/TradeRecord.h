...
class HAYAKU_API TradeRecord {
 public:
  ...
  TradeRecord(const Stock& stock, const Datetime& datetime, BUSINESS business,
              price_t planPrice, price_t realPrice, price_t goalPrice,
              double number, const CostRecord& cost, price_t stoploss,
              price_t cash, OrderOrigin from, const string& remark = "");
  ...
  OrderOrigin from;   ///< Domain origin of the execution request
 ...
 private:
  ...
  void save(Archive& ar, const unsigned int version) const {
    ...
    string part_name(getOrderOriginName(from));
  ...
  void load(Archive& ar, const unsigned int version) {
    ...
    string part_name;
    ar& bs::make_nvp<string>("from", part_name);
    from = getOrderOriginEnum(part_name);
  }
...
template <>
struct fmt::formatter<hayaku::TradeRecord> : ostream_formatter {};
#endif
