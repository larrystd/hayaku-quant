...
TEST_CASE("test_MF_EqualWeight") {
  ...
  CHECK_THROWS_AS(MF_EqualWeight(IndicatorList{}, stks, query, ref_stk),
                  std::exception);

  /** @arg The input reference security is empty */
  CHECK_THROWS_AS(MF_EqualWeight(IndicatorList{}, stks, query, Null<Stock>()),
                  std::exception);

  /** @arg An invalid ic_n is passed */
  CHECK_THROWS_AS(MF_EqualWeight(src_inds, stks, KQuery(-2), ref_stk, 0),
                  std::exception);
