...
std::vector<T> python_list_to_vector(const py::sequence& obj) {
  // If len(obj) is zero it may succeed even when the type is not the expected
  // one, but there is no risk
  auto total = len(obj);
...
py::list vector_to_python_list(const std::vector<T>& vect) {
  py::list obj;
  for (unsigned long i = 0; i < vect.size(); ++i) obj.append(vect[i]);
  return obj;
}

template <typename T>
void extend_vector_with_python_list(std::vector<T>& v, const py::sequence& l) {
  for (const auto& item : l) v.push_back(item.cast<T>());
}
...
// Using the pybind11 overload of _clone directly would lose the python type in
// C++ Refer to https://github.com/pybind/pybind11/issues/1049 for the
// modification PYBIND11_OVERLOAD(IndicatorImpPtr, IndicatorImp, _clone, );
...
// Used to check whether the number of the function arguments passed as
// py::object is as expected
...
inline StockList get_stock_list_from_python(const py::object& stks) {
  ...
  } else if (py::isinstance<DataEngine>(stks)) {
    const auto& data = stks.cast<DataEngine&>();
    ret = data.getStockList();
  } else if (py::isinstance<py::sequence>(stks)) {
    ret = python_list_to_vector<Stock>(stks);
  } else {
    HAYAKU_THROW(
        "Failed get StockList! Input stks must be Block, DataEngine or "
        "sequence(Stock)!");
  }
  return ret;
}
...
inline Block get_block_from_python(const py::object& blk) {
  Block ret;
  HAYAKU_IF_RETURN(blk.is_none(), ret);

  if (py::isinstance<StockList>(blk)) {
    ret = Block(blk.cast<StockList>());
  } else if (py::isinstance<Block>(blk)) {
    ret = blk.cast<Block>();
  } else if (py::isinstance<DataEngine>(blk)) {
    const auto& data = blk.cast<DataEngine&>();
    ret = Block(data.getStockList());
  } else if (py::isinstance<py::sequence>(blk)) {
    ret = Block(python_list_to_vector<Stock>(blk));
  } else {
    HAYAKU_THROW(
        "Failed get Block! Input blk must be Block, DataEngine or "
        "sequence(Stock)!");
  }
  return ret;
}
