...
KRecordList df_to_krecords(const py::object& df, const StringList& cols) {
  HAYAKU_CHECK(cols.size() == 7, "cols size must equal to 7!");

  KRecordList ret;
  py::module_ pandas = py::module_::import("pandas");
  py::object datetime_col =
      pandas.attr("to_datetime")(df.attr("__getitem__")(cols[0]));
  py::array np_dates = datetime_col.attr("to_numpy")().attr("view")("int64");

  py::array_t<double> open_array =
      df.attr("__getitem__")(cols[1]).attr("to_numpy")().attr("astype")(
          py::dtype("float64"));
  py::array_t<double> high_array =
      df.attr("__getitem__")(cols[2]).attr("to_numpy")().attr("astype")(
          py::dtype("float64"));
  py::array_t<double> low_array =
      df.attr("__getitem__")(cols[3]).attr("to_numpy")().attr("astype")(
          py::dtype("float64"));
  py::array_t<double> close_array =
      df.attr("__getitem__")(cols[4]).attr("to_numpy")().attr("astype")(
          py::dtype("float64"));
  py::array_t<double> amount_array =
      df.attr("__getitem__")(cols[5]).attr("to_numpy")().attr("astype")(
          py::dtype("float64"));
  py::array_t<double> volume_array =
      df.attr("__getitem__")(cols[6]).attr("to_numpy")().attr("astype")(
          py::dtype("float64"));
