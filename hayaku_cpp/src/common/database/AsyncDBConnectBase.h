 ...
 * @ingroup DBConnect
 *
 * It provides an asynchronous database operation interface based on the
 * boost::asio coroutine. All the I/O intensive operations (such as the query
 * and the execution) return an awaitable, and co_await can be used in a
 * coroutine.
 *
 * @note Not all the interfaces need to be changed into await coroutines, only
 * the I/O intensive operations need to be made asynchronous
 * @note The synchronous operations such as the transaction control and the
 * configuration access keep the original synchronous interfaces
 */
class HAYAKU_UTILS_API AsyncDBConnectBase
    : public std::enable_shared_from_this<AsyncDBConnectBase> {
 PARAMETER_SUPPORT  // NOSONAR
  ...
  virtual net::awaitable<AsyncSQLStatementPtr> getStatement(
      const std::string &sql_statement) = 0;
  ...
  /**
   * Batch saving; the data in the iterators must be the table model bound
   * through TABLE_BIND
  ...
  net::awaitable<void> batchSave(InputIterator first, InputIterator last,
                                 bool autotrans = true);

  /**
   * Load the model data into the given model instance
   * @note The query condition should return one record only; if there are
   * multiple query results, only one is taken
  ...
  net::awaitable<void> batchLoad(Container &container,
   ...
   */
  template <typename Container>
  net::awaitable<void> batchLoadView(Container &container,
   ...
   */
  template <class Container>
  net::awaitable<void> batchSaveOrUpdate(Container &container,
                                         bool autotrans = true);
   ...
   */
  template <class InputIterator>
  net::awaitable<void> batchSaveOrUpdate(InputIterator first,
                                         InputIterator last,
                                         bool autotrans = true);
   ...
   * @param autotrans start a transaction
   */
  net::awaitable<void> remove(const std::string &tablename,
                              const std::string &where, bool autotrans = true);
   ...
   */
  net::awaitable<void> remove(const std::string &tablename,
                              const DBCondition &cond, bool autotrans = true);

  /**
   * Delete
   * @param item the data to be deleted; it is deleted through item.rowid(), and
   * afterwards the rowid is set to invalid
   * @param autotrans start a transaction
   */
  template <typename T>
  ...
  /**
   * Batch deleting; the data in the iterators must be the table model bound
   * through TABLE_BIND
   * @param first the iterator start point
   * @param last the iterator end point
   * @param autotrans start a transaction
   ...
   * @note The sql statement should return a single element only, otherwise an
   * exception is thrown, such as for multiple records or multiple columns
   * @param query query statement
   * @param default_val the default value returned when the query fails. An
   * exception is thrown if it is Null<int>().
   */
   ...
   * @param query query statement
   * @param default_val the default value returned when the query fails. An
   * exception is thrown if it is Null<NumberType>().
   */
  template <typename NumberType>
  net::awaitable<NumberType> queryNumber(
      const std::string &query, NumberType default_val = Null<NumberType>());
...
template <typename NumberType>
net::awaitable<NumberType> AsyncDBConnectBase::queryNumber(
    const std::string &query, NumberType default_val) {
  auto st = co_await getStatement(query);
  co_await st->exec();

  if (!(co_await st->moveNext() && st->getNumColumns() == 1)) {
    HAYAKU_CHECK(default_val != Null<NumberType>(),
...
net::awaitable<void> AsyncDBConnectBase::save(T &item, bool autotrans) {
  auto st = co_await (item.valid() ? getStatement(T::getUpdateSQL())
                                   : getStatement(T::getInsertSQL()));
...
template <class Container>
inline net::awaitable<void> AsyncDBConnectBase::batchSave(Container &container,
                                                          bool autotrans) {
  co_await batchSave(container.begin(), container.end(), autotrans);
}

template <class InputIterator>
net::awaitable<void> AsyncDBConnectBase::batchSave(InputIterator first,
                                                   InputIterator last,
                                                   bool autotrans) {
...
template <typename T>
net::awaitable<void> AsyncDBConnectBase::load(T &item,
                                              const std::string &where) {
...
template <typename T>
net::awaitable<void> AsyncDBConnectBase::load(T &item,
                                              const DBCondition &cond) {
  co_await load(item, cond.str());
  co_return;
}

template <typename Container>
net::awaitable<void> AsyncDBConnectBase::batchLoad(Container &container,
                                                   const std::string &where) {
  std::ostringstream sql;
  if (where != "") {
...
template <typename T>
net::awaitable<void> AsyncDBConnectBase::loadView(T &item,
                                                  const std::string &sql) {
...
template <class Container>
inline net::awaitable<void> AsyncDBConnectBase::batchUpdate(
    Container &container, bool autotrans) {
  co_await batchUpdate(container.begin(), container.end(), autotrans);
}

template <class InputIterator>
net::awaitable<void> AsyncDBConnectBase::batchUpdate(InputIterator first,
                                                     InputIterator last,
                                                     bool autotrans) {
  size_t count = std::distance(first, last);
  if (count == 0) {
...
template <class InputIterator>
net::awaitable<void> AsyncDBConnectBase::batchSaveOrUpdate(InputIterator first,
                                                           InputIterator last,
                                                           bool autotrans) {
...
template <class Container>
inline net::awaitable<void> AsyncDBConnectBase::batchSaveOrUpdate(
    Container &container, bool autotrans) {
...
net::awaitable<void> AsyncDBConnectBase::remove(T &item, bool autotrans) {
  HAYAKU_CHECK(item.valid(), "Invalid item, id is 0!");

  auto st = co_await getStatement(fmt::format("delete from {} where id={}",
                                              T::getTableName(), item.rowid()));
...
template <class Container>
inline net::awaitable<void> AsyncDBConnectBase::batchRemove(
    Container &container, bool autotrans) {
  co_await batchRemove(container.begin(), container.end(), autotrans);
}

template <class InputIterator>
net::awaitable<void> AsyncDBConnectBase::batchRemove(InputIterator first,
                                                     InputIterator last,
                                                     bool autotrans) {
  size_t count = std::distance(first, last);
  if (count == 0) {
  ...
  try {
    for (InputIterator iter = first; iter != last; ++iter) {
      co_await remove(*iter,
                      false);  // The outer layer has handled the transaction
    }
...
inline net::awaitable<void> AsyncDBConnectBase::remove(
    const std::string &tablename, const std::string &where, bool autotrans) {
  if (autotrans) {
    co_await transaction();
  }
...
inline net::awaitable<void> AsyncDBConnectBase::remove(
    const std::string &tablename, const DBCondition &cond, bool autotrans) {
