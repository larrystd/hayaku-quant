...
Indicator HAYAKU_API ADVANCE(const KQuery& query, const string& market, int stk_type,
...
Indicator HAYAKU_API DECLINE(const KQuery& query, const string& market, int stk_type,
                          bool ignore_context, bool fill_null) {
...
void IBlockSetNum::_calculate(const Indicator& ind) {
    ...
    if (!ignore_context && !k.empty()) {
        ...
        if (q != KQuery(0, 0)) {
            dates = getDataRuntime().getTradingCalendar(q, getParam<string>("market"));
        }
    }
...
Indicator HAYAKU_API INBLOCK(const KData& k, const string& category, const string& name) {
    auto p = make_shared<IInBlock>();
    p->setParam<string>("category", category);
    p->setParam<string>("name", name);
    p->setContext(k);
...
void IInSum::_checkParam(const string& name) const {
    if ("market" == name) {
        ...
        HAYAKU_ASSERT(mode == 0 || mode == 1 || mode == 2 || mode == 3 || mode == 4 || mode == 5);
    }
}
...
void IInSum::_calculate(const Indicator& ind) {
    ...
    if (0 == mode) {
        ...
        if (ind.size() != total) {
            ...
            HAYAKU_CHECK(nind.size() == total, "ind size: {}  != total: {}", ind.size(), total);
        }
...
Indicator HAYAKU_API INSUM(const Block& block, const Indicator& ind, int mode, bool fill_null) {
