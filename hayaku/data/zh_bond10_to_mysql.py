...
import datetime
from hayaku.data.common import get_china_bond10_rate
from hayaku.util import *


@hayaku_catch(trace=True)
def import_zh_bond10_to_mysql(connect):
    sql = "select max(date) from hayaku_base.zh_bond10"
    cur = connect.cursor()
    cur.execute(sql)
    a = cur.fetchall()
    if a[0][0] is None:
    ...
    if rates:
        cur.executemany("INSERT INTO hayaku_base.zh_bond10(date, value) VALUES (%s, %s)", rates)
        connect.commit()
        cur.close()
