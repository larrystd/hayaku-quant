...
Hayaku is a Python quantitative framework for researching the system trading, and you can build your own client programs on top of it. The Hayaku library itself comes with an interactive tool for the Python shell; load it with ``from hayaku.application.interactive import *``. This tool mainly provides the plotting functions based on matplotlib, making it easy to draw the K-lines, the indicators, the signals, etc. in the interactive environment.
...
`<https://nbviewer.jupyter.org/github/larrystd/hayaku-quant/blob/poc/examples/python/notebook/en/000-Index.ipynb?flush_cache=True>`_
