...
BOOST_CLASS_EXPORT(hayaku::GroupSCFilter)
#endif
...
void GroupSCFilter::_checkParam(const string& name) const {
    if (name == "group") {
        HAYAKU_ASSERT(getParam<int>("group") > 0);
    } else if (name == "group_index") {
        int group_index = getParam<int>(name);
        HAYAKU_ASSERT(group_index >= 0 && group_index < getParam<int>("group"));
    }
}
...
ScoresFilterPtr HAYAKU_API SCFilter_Group(int group, int group_index) {
    ...
    HAYAKU_CHECK(group_index >= 0 && group_index < group, "group_index must >= 0 and < group!");
