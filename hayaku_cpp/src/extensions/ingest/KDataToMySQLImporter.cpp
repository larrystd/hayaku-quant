...
KDataToMySQLImporter::KDataToMySQLImporter() {
    m_plugin = getPlugin<ImportKDataToMySQLPluginInterface>(HAYAKU_PLUGIN_IMPORTKDATATOMYSQL);
    if (!m_plugin) {
        HAYAKU_ERROR(htr("Can't find {} plugin!", HAYAKU_PLUGIN_IMPORTKDATATOMYSQL));
    }
}
