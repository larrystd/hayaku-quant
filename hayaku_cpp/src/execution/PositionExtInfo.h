...
struct PositionExtInfo final {
  ...
  price_t maxPullBack1{
      0.};  // Maximum drawdown ratio 1 (calculated from the maximum and
            // the minimum close prices only) (negative)
  price_t maxPullBack2{
      0.};  // Maximum drawdown ratio 2 (calculated from the maximum high
            // price and the minimum low price in the period) (negative)
  price_t currentProfit{0.};  // Current floating profit and loss (excluding the
                              // estimated sell cost)

  /** Current drawdown ratio 1 (calculated from the maximum close price and the
   * current close price only) */
  price_t currentPullBack1() const {
    price_t ret = (maxClosePrice - currentClosePrice) / maxClosePrice;
    return ret > 0. ? 0. : ret;
  }

  /** Current drawdown percentage 2 (calculated from the maximum high price in
   * the period and the current close price) */
  price_t currentPullBack2() const {
    price_t ret = (maxHighPrice - currentClosePrice) / maxHighPrice;
    return ret > 0. ? 0. : ret;
  }

  /** Maximum floating profit 1 in the period (positive, calculated from the
   * close prices only, excluding the estimated sell cost; inaccurate when there
   * are multiple trades) */
  price_t maxFloatingProfit1() const {
    price_t ret = maxClosePrice * position.number + position.sellMoney -
                  position.buyMoney;
    return ret < 0. ? 0. : ret;
  }

  /** Maximum floating profit 2 in the period (positive, calculated from the
   * maximum of the high prices, excluding the estimated sell cost; inaccurate
   * when there are multiple trades) */
  price_t maxFloatingProfit2() const {
    price_t ret =
        maxHighPrice * position.number + position.sellMoney - position.buyMoney;
    return ret < 0. ? 0. : ret;
  }

  /** Maximum floating loss 1 in the period (negative, calculated from the close
   * prices only, excluding the estimated sell cost; inaccurate when there are
   * multiple trades) */
  price_t minLossProfit1() const {
    price_t ret = minClosePrice * position.number + position.sellMoney -
                  position.buyMoney;
    return ret > 0. ? 0. : ret;
  }

  /** Maximum floating loss 2 in the period (negative, calculated from the low
   * prices in the period only, excluding the estimated sell cost; inaccurate
   * when there are multiple trades)
   */
  price_t minLossProfit2() const {
    price_t ret =
        minLowPrice * position.number + position.sellMoney - position.buyMoney;
    return ret > 0. ? 0. : ret;
  }
