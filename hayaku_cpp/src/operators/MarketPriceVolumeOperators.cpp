...
void IAd::_calculate(const Indicator& data) {
  ...
  for (size_t i = m_discard + 1; i < total; i++) {
    ...
    if (tmp != 0.0) {
      ...
      ad += ((r.closePrice + r.closePrice - r.highPrice - r.lowPrice) / tmp) *
            r.transAmount;
    }
    dst[i] = ad;
  }
...
void ICost::_checkParam(const string& name) const {
  if (name == "percent") {
    ...
    HAYAKU_CHECK(percent >= 0.0 && percent <= 100.0,
                 "Invalid param percent: {}", percent);
  }
}
  ...
  for (; sw_iter != sw_list.end(); ++sw_iter) {
    ...
    while (pos < total && kdata[pos].datetime < cur_sw_date) {
      ...
      if (krecord.datetime >= pre_sw_date) {
        x = krecord.closePrice +
            (krecord.highPrice - krecord.lowPrice) * percent;
        // transCount is in lots and the outstanding shares are in units of 10
        // thousand shares
...
void IIndex::_checkParam(const string& name) const {
  if ("kpart" == name) {
    ...
    HAYAKU_ASSERT("OPEN" == part || "HIGH" == part || "LOW" == part ||
...
void IIndex::_calculate(const Indicator& ind) {
  ...
  if (stk.type() == STOCKTYPE_A) {
    ...
    HAYAKU_WARN("Not known the index code, will use {} as default.",
                market_code);
  }
  ...
  query = KQueryByDate(k[0].datetime, k[total - 1].datetime + Seconds(secs),
                       query.kType(), query.recoverType());

  KData index_k = getKData(market_code, query);
  Indicator index = ALIGN(KDATA_PART(index_k, getParam<string>("kpart")), k,
...
void ITime::_checkParam(const string& name) const {
  if ("type" == name) {
    ...
    HAYAKU_CHECK("TIME" == param_type || "DATE" == param_type ||
                     "YEAR" == param_type || "MONTH" == param_type ||
                     "WEEK" == param_type || "DAY" == param_type ||
...
void IWinner::_calculate(const Indicator& data) {
  ...
  global_parallel_for_index_void(1, 101,
                                 [&cost_data, &cost_list, &context](size_t i) {
