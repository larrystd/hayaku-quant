...
TEST_CASE("test_MF_ICIRWeight") {
  ...
  CHECK_THROWS_AS(MF_ICIRWeight(src_inds, stks, query, ref_stk, 0),
                  std::exception);
  CHECK_THROWS_AS(MF_ICIRWeight(src_inds, stks, query, ref_stk, 1, -1),
                  std::exception);
  ...
  for (size_t i = ind4.discard(), len = ref_dates.size(); i < len; i++) {
    ...
    // HAYAKU_INFO("{}: {}, {}, {}, {}, {}", i, w, ind4[i], ind1[i], ma_ic1[i],
    // stdev_ic1[i]);
