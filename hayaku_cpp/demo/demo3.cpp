 ...
 * It receives the market data sent by the HayakuTDX collector and offers an
 *interface for the other programs to get the latest data
 *
 * Purpose:
 * In the programmatic trading the latest data is often wanted when placing a
 *real order, or in daily trading the automatic receiving of hayaku is off and
 *the latest data is fetched manually during a scheduled run before the close
 *
 * hayaku provides get_data_from_buffer_server (python) and
 *getDataFromBufferServer (C++) to get the latest data from this service
 *(filling the data of the day), e.g.:
...
int main(int argc, char* argv[]) {
  ...
  // Method 1: before the initialization, set the plugin path to "." or "" and
  // it is taken automatically from the plugindir of the hayaku.ini config:
  // setPluginPath("."); Method 2: before initialization, set the
  // plugin path yourself (if needed) otherwise it defaults to the
  // .hayaku/plugin directory under the user home, where the plugins can be
  // copied setPluginPath("./plugin");

  try {
    // Get the basic configuration parameters
    Parameter baseParam, blockParam, kdataParam, preloadParam, hayakuParam;
    getConfigFromIni(fmt::format("{}/.hayaku/hayaku.ini", getUserDir()),
                     baseParam, blockParam, kdataParam, preloadParam,
                     hayakuParam);
    ...
    // Do not load the historical financial information nor the
    // ex-rights/ex-dividend data
    hayakuParam.set<bool>("load_history_finance", false);
    hayakuParam.set<bool>("load_stock_weight", false);
    getDataRuntime().init(baseParam, blockParam, kdataParam, new_preloadParam,
                          hayakuParam);
    ...
    server.regHandle("market", [](json&& req) {
      ...
      to_lower(low_ktype);
      HAYAKU_CHECK(param.tryGet<bool>(low_ktype, false),
                   "The ktype: {} is not be preloaded!", ktype);

      const auto& jcodes = req["codes"];
      const auto& jdates = req["dates"];
      HAYAKU_CHECK(jcodes.size() == jdates.size(),
                   "The leght of codes and dates is not equal!");
      ...
      for (size_t i = 0, len = jcodes.size(); i < len; i++) {
        try {
          ...
          KData kdata =
              stk.getKData(KQueryByDate(start_date, Null<Datetime>(), ktype));
          if (kdata.empty()) {
            continue;
          }
