...
string Stock::Data::marketCode() const {
  if (m_type == STOCKTYPE_CRYPTO) return m_market + "/" + m_code;
...
Stock::Stock(const Stock& x) noexcept
    : m_data(x.m_data), m_kdataDriver(x.m_kdataDriver) {}
...
Stock::Stock(const string& market, const string& code, const string& name) {
  m_data = make_shared<Data>(market, code, name, default_type, default_valid,
...
Stock::Stock(const string& market, const string& code, const string& name,
    ...
    : m_data(make_shared<Data>(market, code, name, type, valid, startDate,
...
void Stock::releaseKDataBuffer(KQuery::KType ktype) const {
  HAYAKU_IF_RETURN(!m_data, void());

  to_upper(ktype);
  HAYAKU_IF_RETURN(m_data->pMutex.find(ktype) == m_data->pMutex.end(), void());
...
void Stock::loadKDataToBuffer(KQuery::KType kType) const {
  HAYAKU_IF_RETURN(!m_data || !m_kdataDriver, void());
...

  {
    ...
    if (total != 0) {
      (*ptr_klist) =
          driver->getKRecordList(m_data->m_market, m_data->m_code, query);
...
KRecordList Stock::getKRecordListFromBuffer(KQuery::KType inkType) const {
  ...
  HAYAKU_IF_RETURN(mutex_iter == m_data->pMutex.end(), result);
  std::shared_lock<std::shared_mutex> lock(*(mutex_iter->second));
  auto iter = m_data->pKData.find(kType);
  HAYAKU_IF_RETURN(iter == m_data->pKData.end() || !iter->second, result);
  result = *(iter->second);
  return result;
}
...
StockWeightList Stock::getWeight(const Datetime& start,
  ...
  HAYAKU_IF_RETURN(!m_data || start >= end, result);
  ...
  start_iter =
      lower_bound(m_data->m_weightList.begin(), m_data->m_weightList.end(),
  ...
  end_iter = lower_bound(
      start_iter, (StockWeightList::const_iterator)m_data->m_weightList.end(),
...
bool Stock::getIndexRange(const KQuery& query, size_t& out_start,
  ...
  out_start = 0;
  out_end = 0;
  HAYAKU_IF_RETURN(!m_data || !m_kdataDriver, false);
  ...
  if (KQuery::isBaseKType(query.kType())) {
    ...
    if (!m_kdataDriver->getConnect()->getIndexRangeByDate(
            m_data->m_market, m_data->m_code, query, out_start, out_end)) {
      out_start = 0;
      out_end = 0;
      return false;
    }
...
KRecord Stock::getKRecord(size_t pos, const KQuery::KType& kType) const {
  HAYAKU_IF_RETURN(!m_data, KRecord::NullKRecord);
  ...
  if (KQuery::isBaseKType(kType)) {
    ...
    HAYAKU_IF_RETURN(!m_kdataDriver || pos >= size_t(Null<int64_t>()),
                     KRecord::NullKRecord);
    auto klist = m_kdataDriver->getConnect()->getKRecordList(
  ...
  if (isBuffer(query.kType())) {
    ...
    if (query.queryType() == KQuery::DATE) {
      result = m_kdataDriver->getConnect()->getKRecordList(
          m_data->m_market, m_data->m_code, query);
    } else {
      size_t start_ix = 0, end_ix = 0;
...
TimeLineList Stock::getTimeLineList(const KQuery& query) const {
  TimeLineList result;
  HAYAKU_IF_RETURN(!m_kdataDriver, result);
  result =
      m_kdataDriver->getConnect()->getTimeLineList(market(), code(), query);
...
void Stock::setKRecordList(const KRecordList& ks, const KQuery::KType& ktype) {
  ...
  // Write lock
  std::unique_lock<std::shared_mutex> lock(*(m_data->pMutex[ktype]));
  HAYAKU_CHECK(m_data->pKData.find(nktype) != m_data->pKData.end(),
               "Invalid ktype: {}", ktype);
...
vector<HistoryFinanceInfo> Stock::getHistoryFinance() const {
  ...
  if (m_data->m_history_finance.empty() && !m_data->m_history_finance_ready) {
    ...
    std::unique_lock<std::shared_mutex> unique_lock(
        m_data->m_history_finance_mutex);
    if (!m_data->m_history_finance_ready) {
      lazy_load_finance();
    }
    unique_lock.unlock();
