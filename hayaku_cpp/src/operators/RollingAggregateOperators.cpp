 ...
 * condition. COUNT(X,N) counts the number of the periods satisfying the X
 * condition within N periods; if N=0 it starts from the first valid value.
...
ICount::ICount() : IndicatorImp("COUNT", 1) { setParam<int>("n", 20); }
...
void ICount::_dyn_run_one_step(const Indicator& ind, size_t curPos,
                               size_t step) {
  size_t start = 0;
  if (0 == step) {
    start = ind.discard();
...
ISum::ISum() : IndicatorImp("SUM", 1) { setParam<int>("n", 20); }
...
bool ISum::supportIncrementCalculate() const { return getParam<int>("n") > 1; }

size_t ISum::min_increment_start() const { return getParam<int>("n") - 1; }
...
IDevsq::IDevsq() : IndicatorImp("DEVSQ", 1) { setParam<int>("n", 10); }
...
void IDevsq::_dyn_run_one_step(const Indicator& ind, size_t curPos,
                               size_t step) {
  size_t start = _get_step_start(curPos, step, ind.discard());
  if (curPos + 1 < ind.discard() + step) {
    return;
