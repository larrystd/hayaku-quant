...
class ImportKDataToClickHousePluginInterface : public PluginBase {
 public:
   ...
   * @param pwd password
   * @param baseinfo_db the baseinfo database name (hayaku_base by default);
   * addMarket / addStockType write into this database
   */
  virtual bool setConfig(const string& host, int port, const string& user,
                         const string& pwd, const string& baseinfo_db) = 0;
