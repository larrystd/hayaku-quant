...
TEST_CASE("test_ADJ_FACTOR") {
  ...
  /** @arg Test the version with a KData parameter - a stock with
  ...
  // All the adjustment factors should be >= 1.0 (a backward adjustment factor
  // only grows or stays)
  ...
  // An index has no ex-rights/ex-dividend data, so all the factors should
  // be 1.0
  for (size_t i = 0; i < adj_factor.size(); ++i) {
  ...
  // Verify the consistency of the overlapping part (the correctness of the
