...
void MySQLStatement::_prepare() {
    ...
    mysql_stmt_close(m_impl->stmt);
    m_impl->stmt = nullptr;
    HAYAKU_THROW("Failed prepare statement: {}! ret: {}, error msg: {}!", m_sql_string, ret,
              stmt_errorstr);
}
...
void MySQLStatement::_bindResult() {
    ...
    while ((field = mysql_fetch_field(m_impl->meta_result))) {
        ...
        if (field->type == MYSQL_TYPE_LONGLONG) {
            ...
            HAYAKU_THROW("Unsupport field type: {}, field name: {}", int(field->type), field->name);
        }
...
void MySQLStatement::sub_getColumnAsDouble(int idx, double& item) {
    ...
    try {
        if (m_impl->result_bind[idx].buffer_type == MYSQL_TYPE_DOUBLE) {
            ...
            HAYAKU_THROW("Field type({}) mismatch! idx: {}", int(m_impl->result_bind[idx].buffer_type),
                      idx);
        }
    } catch (const hayaku::exception&) {
        throw;
