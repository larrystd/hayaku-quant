...
protected:
    virtual void bindRealToProto(const internal::StrategyRuntimePtr& real, const internal::StrategyRuntimePtr& proto) override {
        m_real_to_proto[real] = proto;
    }

    StrategyWeightList getUnionSelected(Datetime date,
                                      const std::function<double(double, double)>&& func);

    StrategyWeightList getIntersectionSelected(Datetime date,
                                             const std::function<double(double, double)>&& func);
...
protected:
    SelectorPtr m_se1;
    SelectorPtr m_se2;
    std::unordered_set<internal::StrategyRuntimePtr> m_se1_set;  // The prototype system instance set of se1
    std::unordered_set<internal::StrategyRuntimePtr> m_se2_set;  // The prototype system instance set of se2
    std::unordered_map<internal::StrategyRuntimePtr, internal::StrategyRuntimePtr> m_real_to_proto;

private:
    static std::unordered_set<internal::StrategyRuntime*> findIntersection(const SelectorPtr& se1,
