...
void AsioHttpClient::_parseUrl() noexcept {
    ...
    if (proto == "http") {
        ...
        m_is_https = true;
#else
        HAYAKU_ERROR("Not support https protocol, please enable http_client_ssl feature!");
#endif
    } else {
        m_is_valid_url = false;
...
net::awaitable<std::vector<tcp::endpoint>> AsioHttpClient::_resolveDNS() {
    ...
    freeaddrinfo(res);
    HAYAKU_CHECK(!dns_endpoints.empty(), "DNS resolve failed! {}:{}", m_host, m_port);
...
net::awaitable<std::pair<std::shared_ptr<HttpConnection>, bool>> AsioHttpClient::_getConnection() {
    ...
    if (!conn_result) {
        HAYAKU_THROW_EXCEPTION(HttpTimeoutException, "Failed to get connection from pool: {}",
                            conn_result.error());
    }
    ...
    if (!conn_ptr->is_open()) {
        ...
        if (m_is_https) {
            ...
            if (!connected) {
                HAYAKU_THROW_EXCEPTION(HttpTimeoutException, "Connect timeout to {}:{}", m_host,
                                    m_port);
            }
...
net::awaitable<void> AsioHttpClient::_connect(SocketVariant& socket_variant,
    ...
    if (!connected) {
        HAYAKU_THROW_EXCEPTION(HttpTimeoutException, "Connect timeout to {}:{}", m_host, m_port);
    }
