...
struct TdxDayData {
  ...
  Datetime getDatetime() { return Datetime(uint64_t(date) * 10000); }
  ...
  void toKRecord(KRecord& record) {
    ...
    // Note: the volume unit of the indices is shares, unlike the HDF5 data
    // which is in lots
  }
};
...
KRecordList TdxKDataDriver::getKRecordList(const string& market,
                                           const string& code,
                                           const KQuery& query) {
  KRecordList result;
  auto ktype = query.kType();
  HAYAKU_WARN_IF_RETURN(
      !(ktype == KQuery::MIN || ktype == KQuery::MIN5 || ktype == KQuery::DAY),
      result, "The driver({}) don't support the ktype: {}", name(), ktype);

  if (query.queryType() == KQuery::INDEX) {
    if (ktype == KQuery::DAY) {
      result =
          _getDayKRecordList(market, code, ktype, query.start(), query.end());
    } else {
      result =
          _getMinKRecordList(market, code, ktype, query.start(), query.end());
    }
  } else {
    HAYAKU_INFO("Query by date are not supported!");
  }

  return result;
}

KRecordList TdxKDataDriver::_getDayKRecordList(const string& market,
                                               const string& code,
                                               const KQuery::KType& ktype,
                                               size_t start_ix, size_t end_ix) {
  KRecordList result;
  size_t total = getCount(market, code, ktype);
  HAYAKU_IF_RETURN(0 == total || start_ix >= total, result);

  string filename = _getFileName(market, code, ktype);
  std::ifstream file(filename.c_str(), std::ios::binary | std::ios::in);
  HAYAKU_IF_RETURN(!file, result);
...
KRecordList TdxKDataDriver::_getMinKRecordList(const string& market,
                                               const string& code,
                                               const KQuery::KType& ktype,
                                               size_t start_ix, size_t end_ix) {
...
bool TdxKDataDriver::getIndexRangeByDate(const string& market,
                                         const string& code,
...
bool TdxKDataDriver::_getDayIndexRangeByDate(const string& market,
                                             const string& code,
                                             const KQuery& query,
                                             size_t& out_start,
                                             size_t& out_end) {
  ...
  HAYAKU_IF_RETURN(query.kType() != KQuery::DAY, false);
...
bool TdxKDataDriver::_getMinIndexRangeByDate(const string& market,
                                             const string& code,
                                             const KQuery& query,
                                             size_t& out_start,
                                             size_t& out_end) {
  ...
  HAYAKU_IF_RETURN(
      query.kType() != KQuery::MIN && query.kType() != KQuery::MIN5, false);
  HAYAKU_IF_RETURN(query.startDatetime() >= query.endDatetime() ||
                       query.startDatetime() > Datetime::max(),
                   false);
...
size_t TdxKDataDriver::_getBaseCount(const string& market, const string& code,
  ...
  string filename = _getFileName(market, code, ktype);
  HAYAKU_IF_RETURN(filename.empty(), 0);
