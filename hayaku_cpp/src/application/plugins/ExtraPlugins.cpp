...
void installHayakuExtraPluginBridge() {
    setKDataExtensionResolver([]() -> KDataExtension* {
        return getPlugin<HayakuExtraPluginInterface>(HAYAKU_PLUGIN_HAYAKU_EXTRA, false);
    });
}
...
void HAYAKU_API registerExtraKType(const string& ktype, const string& basetype, int32_t minutes,
    ...
    HAYAKU_ERROR_IF_RETURN(!plugin, void(), htr("Can't find {} plugin!", HAYAKU_PLUGIN_HAYAKU_EXTRA));
