...
/**
 * @defgroup test_hikyuusession_suite test_hikyuusession_suite
 * @ingroup test_hikyuu_application_suite
 * @{
 */
...
SessionOptions makeCurrentOptions() {
    ...
    return SessionOptions(data.getBaseInfoDriverParameter(), data.getBlockDriverParameter(),
                          ...
                          data.getHikyuuParameter(), data.getStrategyContext());
}
...
TEST_CASE("test_HikyuuSession_open_close") {
    auto* dataRuntime = &getDataRuntime();
    auto options = makeCurrentOptions();
    auto session = HikyuuSession::open(options);
    ...
    auto reopened = HikyuuSession::open(options);
    CHECK_EQ(reopened.data().getStock("sh000001"), getDataRuntime().getStock("sh000001"));
}

TEST_CASE("test_HikyuuSession_multiple_handles") {
    auto first = HikyuuSession::open(makeCurrentOptions());
    auto second = HikyuuSession::open(makeCurrentOptions());
...
TEST_CASE("test_HikyuuSession_native_execution") {
    ...
    auto session = HikyuuSession::open(makeCurrentOptions(), accountConfig);
...
TEST_CASE("test_HikyuuSession_three_engine_lifecycle") {
