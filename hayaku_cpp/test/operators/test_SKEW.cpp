...
TEST_CASE("test_SKEW") {
  ...
  // The test data: a perfectly symmetric distribution (an even number of
  // points), the skewness should be 0
  ...
  // The test data: a right skewed distribution (positive skew) - a long tail on
  // the right
  ...
  // The test data: a left skewed distribution (negative skew) - a short tail on
  // the left and the large values on the right
  PriceList left_skewed{1.0,  92.0, 93.0, 94.0, 95.0,
...
TEST_CASE("test_SKEW_benchmark") {
  ...
  {
    BENCHMARK_TIME_MSG(test_SKEW_benchmark, cycle,
