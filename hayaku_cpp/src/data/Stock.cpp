...
void Stock::loadKDataToBuffer(KQuery::KType kType) const {
    ...
    if (driver->name() != "TMPCSV") {
        ...
        HAYAKU_ERROR_IF_RETURN(max_num < 0, void(), "Invalid preload {} param: {}", preload_type,
                            max_num);
...
StockWeightList Stock::getWeight(const Datetime& start, const Datetime& end) const {
    ...
    //   DataRuntime::loadAllStockWeights); here only the not materialized securities are handled
    ...
    // DataRuntime::releaseShmServerBaseInfoCache()
...
KRecord Stock::getKRecord(size_t pos, const KQuery::KType& kType) const {
    ...
    if (KQuery::isBaseKType(kType)) {
        ...
        HAYAKU_IF_RETURN(!m_kdataDriver || pos >= size_t(Null<int64_t>()), KRecord::NullKRecord);
...
KRecordList Stock::_getKRecordListFromBuffer(size_t start_ix, size_t end_ix,
    ...
    HAYAKU_WARN_IF_RETURN(start_ix >= end_ix || start_ix >= total, result,
...
Parameter Stock::getFinanceInfo() const {
    Parameter result;
    HAYAKU_IF_RETURN(type() != STOCKTYPE_A && type() != STOCKTYPE_GEM && type() != STOCKTYPE_START &&
                    type() != STOCKTYPE_A_BJ,
                  result);
...
void Stock::realtimeUpdate(KRecord record, const KQuery::KType& inktype) {
    ...
    if (getDataRuntime().isIpcClientMode() && !isBuffer(inktype)) {
        HAYAKU_IF_RETURN(
          record.datetime.isNull() || getDataRuntime().isHoliday(record.datetime), void());
    ...
    if (tmp.datetime == record.datetime) {
        ...
        HAYAKU_DEBUG("Ignore record, datetime({}) < last record.datetime({})! {} {}", record.datetime,
...
void Stock::setKRecordList(const KRecordList& ks, const KQuery::KType& ktype) {
    ...
    // Write lock
    std::unique_lock<std::shared_mutex> lock(*(m_data->pMutex[ktype]));
    HAYAKU_CHECK(m_data->pKData.find(nktype) != m_data->pKData.end(), "Invalid ktype: {}", ktype);
...
vector<HistoryFinanceInfo> Stock::getHistoryFinance() const {
    ...
    // of the client mode in DataRuntime::loadAllKData) and it is read on demand
