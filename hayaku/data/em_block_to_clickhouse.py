...
from hayaku.data.download_block import *
...
def em_import_block_to_clickhouse(connect):
    ...
    hayaku_info("更新数据库")
    sql = f"delete from hayaku_base.block where category in {tuple(blks.keys())}"
    connect.command(sql)
    sql = f"delete from hayaku_base.blockindex where category in {tuple(blks.keys())}"
    connect.command(sql)
    ...
    if insert_records:
        ic = connect.create_insert_context(table='block', database='hayaku_base',
    ...
    if index_records:
        ic = connect.create_insert_context(table='blockindex', database='hayaku_base',
