...
ComponentContext::ComponentContext(StrategyRuntime& runtime) noexcept : m_runtime(runtime) {}
...
void ComponentContext::prepare() {
    HKU_CHECK(m_runtime.m_account, "Strategy has no execution account: {}", m_runtime.m_name);
    HKU_CHECK(m_runtime.m_mm, "Strategy has no MoneyManager: {}", m_runtime.m_name);
    HKU_CHECK(m_runtime.m_sg, "Strategy has no Signal: {}", m_runtime.m_name);
...
void ComponentContext::bind(const KData& kdata) {
    ...
    HKU_TRACE_IF_RETURN(m_runtime.m_calculated, void(), "No need to calculate!");
    ...
    HKU_ASSERT(m_runtime.m_kdata.size() == m_runtime.m_rawKData.size());
