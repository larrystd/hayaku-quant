...
KDataToClickHouseImporter::KDataToClickHouseImporter() {
    m_plugin =
      getPlugin<ImportKDataToClickHousePluginInterface>(HAYAKU_PLUGIN_IMPORTKDATATOCLICKHOUSE);
    if (!m_plugin) {
        HAYAKU_ERROR(htr("Can't find {} plugin!", HAYAKU_PLUGIN_IMPORTKDATATOCLICKHOUSE));
    }
}
