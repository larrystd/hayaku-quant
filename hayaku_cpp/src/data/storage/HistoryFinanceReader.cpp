...
PriceList HistoryFinanceReader ::getHistoryFinanceInfo(Datetime date,
                                                       const string& market,
                                                       const string& code) {
  PriceList result;
  ...
  if (!fread(header_buf, 1, 20, fp)) {
    HAYAKU_ERROR("read data failed! {}", filename);
    fclose(fp);
    return result;
  }
  ...
  for (int i = 0; i < max_count; i++) {
    if (!fread(stock_code, 1, 7, fp)) {
      HAYAKU_ERROR("read stock_code failed! {}", filename);
      fclose(fp);
      return result;
    }

    if (!fread(&address, 4, 1, fp)) {
      HAYAKU_ERROR("read stock_item address failed! {}", filename);
      fclose(fp);
      return result;
    }
  ...
  if (address != 0) {
    ...
    if (report_fields_count >= MAX_COL_NUM) {
      HAYAKU_WARN("Over MAX_COL_NUM! {}", filename);
    ...
    if (!fread(result_buffer, 4, report_fields_count, fp)) {
      HAYAKU_ERROR("read col data failed! {}", filename);
      fclose(fp);
      return result;
    }
    ...
    HAYAKU_ERROR("Invalid address(0)! {}", filename);
  }
