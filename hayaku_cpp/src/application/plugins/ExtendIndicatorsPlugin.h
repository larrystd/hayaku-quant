 ...
 */
Indicator HAYAKU_API FIXED_START_INDEX(const Indicator& ind, int start_index = 0);
 ...
 */
Indicator HAYAKU_API FIXED_START_DATE(const Datetime& start_date = Datetime::min());
 ...
 *       Example: FIXED_START_DATE(AD(), Datetime("2020-01-01"))(getKData("sz000001", Query(-500)))
 */
Indicator HAYAKU_API FIXED_START_DATE(const Indicator& ind,
 ...
 * @return Indicator
 */
Indicator HAYAKU_API WITHKTYPE(const KQuery::KType& ktype, bool fill_null = false);
 ...
 * @return Indicator
 */
Indicator HAYAKU_API RANK(const Block& block, const Indicator& ref_ind, int mode = 0,
                       bool fill_null = true, const string& market = "SH");
Indicator HAYAKU_API RANK(const Block& block, int mode = 0, bool fill_null = true,
...
#define AGG_FUNC_DEFINE(agg_name)                                                              \
    Indicator HAYAKU_API agg_name(const Indicator& ind, const KQuery::KType& ktype = KQuery::MIN, \
                               bool fill_null = false, int unit = 1);

#define AGG_FUNC_IMP(agg_name)                                                                   \
    Indicator HAYAKU_API agg_name(const Indicator& ind, const KQuery::KType& ktype, bool fill_null, \
                               int unit) {                                                       \
        Parameter params;                                                                        \
 ...
 * @return Indicator
 */
Indicator HAYAKU_API AGG_SAMPLE(const Indicator& ind, const string& time = "9:35",
 ...
 * @return Indicator
 */
Indicator HAYAKU_API AGG_SAMPLE_MAX(const Indicator& ind, const string& start_time = "9:30",
 ...
 * @return Indicator
 */
Indicator HAYAKU_API AGG_SAMPLE_MIN(const Indicator& ind, const string& start_time = "9:30",
                                 const string& last_time = "10:00",
                                 const KQuery::KType& ktype = KQuery::MIN, bool fill_null = false,
                                 int unit = 1);
 ...
 * @return Indicator
 */
Indicator HAYAKU_API AGG_SAMPLE_MEAN(const Indicator& ind, const string& start_time = "9:30",
                                  const string& last_time = "10:00",
                                  const KQuery::KType& ktype = KQuery::MIN, bool fill_null = false,
                                  int unit = 1);

Indicator HAYAKU_API AGG_STD(const Indicator& ind, const KQuery::KType& ktype = KQuery::MIN,
                          bool fill_null = false, int unit = 1, int ddof = 1);
Indicator HAYAKU_API AGG_VAR(const Indicator& ind, const KQuery::KType& ktype = KQuery::MIN,
                          bool fill_null = false, int unit = 1, int ddof = 1);
Indicator HAYAKU_API AGG_QUANTILE(const Indicator& ind, const KQuery::KType& ktype = KQuery::MIN,
                               bool fill_null = false, int unit = 1, double quantile = 0.75);

Indicator HAYAKU_API AGG_VWAP(const KQuery::KType& ktype = KQuery::MIN, bool fill_null = false,
                           int unit = 1);
...
#define GROUP_FUNC_DEFINE(group_name)                                                            \
    Indicator HAYAKU_API group_name(const Indicator& ind, const KQuery::KType& ktype = KQuery::DAY, \
                                 int unit = 1);

#define GROUP_FUNC_IMP(group_name)                                                             \
    Indicator HAYAKU_API group_name(const Indicator& ind, const KQuery::KType& ktype, int unit) { \
        Parameter params;                                                                      \
        params.set<string>("ktype", ktype);                                                    \
        params.set<int>("unit", unit);                                                         \
