...
string MySQLKDataDriver ::_getTableName(const string& market,
                                        const string& code,
  ...
  if (ktype == KQuery::TIMELINE) {
    ...
    table =
        fmt::format("`{}_{}`.`{}`", market, KQuery::getKTypeName(ktype), code);
  }
  to_lower(table);
  return table;
}

KRecordList MySQLKDataDriver::getKRecordList(const string& market,
                                             const string& code,
                                             const KQuery& query) {
  KRecordList result;
  ...
  if (kType == KQuery::TIMELINE) {
    ...
    result = _getKRecordList(market, code, query.kType(), query.start(),
                             query.end());
  } else {
    result = _getKRecordList(market, code, query.kType(), query.startDatetime(),
                             query.endDatetime());
  }
  return result;
}

KRecordList MySQLKDataDriver::_getKRecordList(const string& market,
                                              const string& code,
                                              const KQuery::KType& kType,
  ...
  try {
    ...
    if (start_ix < mid_total) {
      sql = fmt::format(
          "select date, open, high, low, close, amount, `count` from {} order "
          "by date limit "
          "{}, {}",
          _getTableName(market, code, kType), start_ix, end_ix - start_ix);
    } else {
      sql = fmt::format(
          "select * from (select date as date, open, high, low, close, amount, "
          "`count` from {} "
          "order by date desc limit {}) a order by date limit 0, {}",
          _getTableName(market, code, kType), total - start_ix,
          end_ix - start_ix);
    }
    ...
    while (st->moveNext()) {
      try {
        ...
        result.emplace_back(Datetime(date), open, high, low, close,
                            amount * 0.1, count);
      } catch (...) {
        HAYAKU_ERROR("Failed get krecord: {}{} {}", market, code, kType);
      }
    }
...
KRecordList MySQLKDataDriver::_getKRecordList(const string& market,
  ...
  KRecordList result;
  HAYAKU_IF_RETURN(
      start_date.isNull() || (!end_date.isNull() && start_date >= end_date),
      result);
  ...
  try {
    ...
    while (st->moveNext()) {
      ...
      try {
        ...
        HAYAKU_ERROR("Failed get record: {}", record.str());
      }
    }
  } catch (...) {
    // The table may not exist
...
bool MySQLKDataDriver::getIndexRangeByDate(const string& market,
                                           const string& code,
                                           const KQuery& query,
                                           size_t& out_start, size_t& out_end) {
  out_start = 0;
  out_end = 0;
  HAYAKU_ERROR_IF_RETURN(query.queryType() != KQuery::DATE, false,
                         "queryType must be KQuery::DATE");
  HAYAKU_IF_RETURN(query.startDatetime() >= query.endDatetime() ||
  ...
  try {
    ...
    string sql = fmt::format(
        "select COUNT(CASE WHEN date<{} THEN 1 END) AS startix, COUNT(CASE "
        "WHEN date<{} THEN 1 "
        "END) AS endix from {}",
        start, end, tablename);
    SQLStatementPtr st = m_connect->getStatement(sql);
...
TimeLineList MySQLKDataDriver::getTimeLineList(const string& market,
                                               const string& code,
                                               const KQuery& query) {
  TimeLineList result;
  HAYAKU_ERROR_IF_RETURN(query.queryType() >= KQuery::INVALID, result,
                         "Invalid queryType! {}{} {}", market, code, query);
...
TimeLineList MySQLKDataDriver::_getTimeLineListByDate(const string& market,
                                                      const string& code,
                                                      const KQuery& query) {
  ...
  try {
    ...
    while (st->moveNext()) {
      try {
        ...
        HAYAKU_ERROR("Failed get {} timeline! {}", table, e.what());
      } catch (...) {
        HAYAKU_ERROR("Failed get {} timeline!", table, code);
      }
    }
...
TimeLineList MySQLKDataDriver::_getTimeLineListByIndex(const string& market,
                                                       const string& code,
                                                       const KQuery& query) {
  string table = fmt::format("`{}_time`.`{}`", market, code);
  to_lower(table);
  ...
  try {
    ...
    if (startix < endix) {
      ...
      if (startix < mid_total) {
        sql = fmt::format(
            "select `date`, `price`, `vol` from {} order by date limit {}, {}",
            table, startix, endix - startix);
      } else {
        sql = fmt::format(
            "select * from (select `date`, `price`, `vol` from {} order by "
            "date desc limit "
...
TransList MySQLKDataDriver::getTransList(const string& market,
                                         const string& code,
                                         const KQuery& query) {
  TransList result;
  HAYAKU_ERROR_IF_RETURN(query.queryType() >= KQuery::INVALID, result,
...
TransList MySQLKDataDriver::_getTransListByDate(const string& market,
                                                const string& code,
                                                const KQuery& query) {
  ...
  try {
    ...
    while (st->moveNext()) {
      try {
        ...
        HAYAKU_ERROR("Failed get {} trans! {}", table, e.what());
      } catch (...) {
        HAYAKU_ERROR("Failed get {} trans!", table, code);
      }
    }
...
TransList MySQLKDataDriver::_getTransListByIndex(const string& market,
                                                 const string& code,
                                                 const KQuery& query) {
  string table = fmt::format("`{}_trans`.`{}`", market, code);
  to_lower(table);
  ...
  try {
    ...
    if (startix < endix) {
      ...
      if (startix < mid_total) {
        sql = fmt::format(
            "select `date`, `price`, `vol`, `buyorsell` from {} order by date "
            "limit {}, {}",
            table, startix, endix - startix);
      } else {
        sql = fmt::format(
            "select * from (select `date`, `price`, `vol`, `buyorsell` from {} "
            "order by date "
