...
TEST_CASE("test_SE_AddValue") {
  ...
  CHECK_THROWS_AS(se->addStock(sm["sh600000"], internal::StrategyRuntimePtr()),
                  std::exception);
...
TEST_CASE("test_SE_Add") {
  ...
  /** A heterogeneous union: the real system instance must be added with
   * addSystem */
  sys->reset();
  auto sys1 = sys->clone();
  ...
  result = se->getSelected(Datetime(200001010000L));
  CHECK_EQ(result.size(), 3);
  CHECK_EQ(sm["sz000001"], result[0].strategy->getStock());
  CHECK_EQ(sm["sz000002"], result[1].strategy->getStock());
  CHECK_EQ(sm["sh600000"], result[2].strategy->getStock());
  CHECK_EQ(result[0].weight, 0.5);
