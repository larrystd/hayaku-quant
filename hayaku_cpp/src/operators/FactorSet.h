...
class HAYAKU_API FactorSet {
 public:
  ...
  /**
   * Constructor, create a factor set with the given indicator list, the factor
   * name is the indicator name by default; it is mainly used to create a
   * temporary factor set
   * @note An indicator with the same name is overwritten, the last one with the
   * same name is kept
   * @param inds indicator list
   * @param ktype K-line type of the factor set, the daily line by default
   */
  explicit FactorSet(const IndicatorList& inds,
                     const KQuery::KType& ktype = KQuery::DAY);
   ...
   * @note An indicator with the same name is overwritten, the last one with the
   * same name is kept
   * @param inds indicator map, the key is the factor name and the value is the
   ...
   */
  vector<IndicatorList> getAllValues(
      const KQuery& query, bool align = false, bool fill_null = false,
      bool tovalue = false, const DatetimeList& align_dates = {}) const;
  ...
  /** Convenience method: add an indicator and use the given name as the factor
   * name */
  void add(const string& name, const Indicator& ind);

  /** Convenience method: add an indicator and use the indicator name as the
   * factor name. Note: the later indicator with the same name overwrites the
   * one added before */
