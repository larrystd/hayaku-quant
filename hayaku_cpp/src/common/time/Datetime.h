 ...
 * @ingroup DataType
 */
class HAYAKU_UTILS_API Datetime {
 public:
   ...
   * datetime format: except the most significant byte, every byte represents
   * the century, the year in the century, the month, the day, the hour, the
   * minute and the second in turn
   */
  static Datetime fromHex(uint64_t time);

  /**
   * @brief Create a Datetime object from a timestamp (microseconds)
   * @param timestamp the timestamp in microseconds (starting from 00:00:00 on
   * January 1, 1970)
   * @return Datetime
   */
  ...
  /**
   * @brief Create a Datetime object from a timestamp (microseconds) and add the
   * local UTC time offset
   * @param timestamp the timestamp in microseconds (starting from 00:00:00 on
   * January 1, 1970)
   * @return Datetime
   */
  ...
  /**
   * @brief Create a Datetime object from std::chrono::time_point and add the
   * local UTC time offset
  ...
  static Datetime fromTimePointUTC(
 ...
 public:
   ...
   */
  Datetime(long year, long month, long day, long hh = 0, long mm = 0,
           long sec = 0, long millisec = 0, long microsec = 0);
  ...
  /**
   * Return a number in the YYYYMMDDhhmm format, convenient for the comparison
   * operations The number corresponding to Null<Datetime>() is Null<unsigned
   * long long>
  ...
  /**
   * Convert to a number in the oracle datetime way; the last 7 bytes represent
   * the century, the year in the century, the month, the day, the hour, the
  ...
  /** Timestamp, the number of the microseconds since 1970-01-01 00:00:00, with
   * the local UTC offset deducted */
  ...
  /**
   * Return the date of the given day of this week; Sunday is day 0 and Saturday
   * is day 6
   * @param day the day of this week; Null is returned if it is not between 0
   * and 6
   */
...
/**
 * Get the calendar date list within the given range [start,end), it contains
 * the days only, without the hour, minute and second.
 * @note A large amount of memory may be consumed if the given end date is too
 * large
 * @param start start date
 * @param end end date; Datetime::max is used when it is empty
 * @return the calendar dates within the range [start, end)
 */
DatetimeList HAYAKU_UTILS_API getDateRange(const Datetime &start,
                                           const Datetime &end);

///////////////////////////////////////////////////////////////////////////////
//
// The relational comparison functions are not defined in the class directly in
// order to support Null<>() == d, i.e. Null can be placed on the left
//
///////////////////////////////////////////////////////////////////////////////
...
template <typename Rep, typename Period>
inline Datetime operator+(std::chrono::duration<Rep, Period> duration,
                          const Datetime &date) {
...
inline Datetime Datetime::fromLocalTime(
    std::chrono::local_time<Duration> local_time) {
...
template <typename Clock, typename Duration>
inline Datetime Datetime::fromTimePointUTC(
    std::chrono::time_point<Clock, Duration> time_point) {
...
inline std::chrono::local_time<Duration> Datetime::to_local_time() const {
  ...
  auto target_duration = std::chrono::duration_cast<Duration>(
      std::chrono::microseconds(timestamp_us));
  return std::chrono::local_time<Duration>(target_duration);
}

inline int Datetime::dayOfWeek() const { return date().day_of_week(); }

inline int Datetime::dayOfYear() const { return date().day_of_year(); }
...
namespace std {
template <>
class hash<hayaku::Datetime> {
 public:
  size_t operator()(hayaku::Datetime const &d) const noexcept {
    return std::hash<uint64_t>()(d.ticks());
  }
};

inline string to_string(const hayaku::Datetime &date) { return date.str(); }

}  // namespace std

#if FMT_VERSION >= 90000
template <>
struct fmt::formatter<hayaku::Datetime> {
  FMT_CONSTEXPR auto parse(format_parse_context &ctx) -> decltype(ctx.begin()) {
    return ctx.end();
  }

  template <typename FormatContext>
  auto format(const hayaku::Datetime &d, FormatContext &ctx) const
      -> decltype(ctx.out()) {
