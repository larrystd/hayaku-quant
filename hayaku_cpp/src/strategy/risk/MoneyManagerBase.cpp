...
HAYAKU_API std::ostream& operator<<(std::ostream& os, const MoneyManagerBase& mm) {
    os << "MoneyManager(" << mm.name() << ", " << mm.getParameter() << ")";
    return os;
}

HAYAKU_API std::ostream& operator<<(std::ostream& os, const MoneyManagerPtr& mm) {
    if (mm) {
        os << *mm;
    } else {
...
double MoneyManagerBase::getSellNumber(const Datetime& datetime, const Stock& stock, price_t price,
    ...
    HAYAKU_ERROR_IF_RETURN(!m_account, 0.0,
    ...
    if (OrderOrigin::ENVIRONMENT == origin) {
        ...
        HAYAKU_IF_RETURN(!getParam<bool>("disable_ev_force_clean_position"), MAX_DOUBLE);
    }

    if (OrderOrigin::CONDITION == origin) {
        HAYAKU_IF_RETURN(!getParam<bool>("disable_cn_force_clean_position"), MAX_DOUBLE);
    }
...
double MoneyManagerBase::getBuyNumber(const Datetime& datetime, const Stock& stock, price_t price,
    ...
    HAYAKU_INFO_IF_RETURN(risk <= 0.0, 0.0,
    ...
    HAYAKU_TRACE_IF_RETURN(m_account->getStockNumber() >= getParam<int>("max-stock"), 0.0,
    ...
    HAYAKU_TRACE_IF_RETURN(n < min_trade, 0.0,
    ...
    HAYAKU_WARN_IF_RETURN(n > max_trade, max_trade,
...
double MoneyManagerBase::getSellShortNumber(const Datetime& datetime, const Stock& stock,
    ...
    HAYAKU_ERROR_IF_RETURN(!m_account, 0.0,
                        "m_account is null! Datetime({}) Stock({}) price({:<.3f}) risk({:<.2f})",
                        datetime, stock.market_code(), price, risk);
    HAYAKU_ERROR_IF_RETURN(risk >= 0.0, 0.0,
