 ...
 *
 *   - T2 a concurrent first access: 32 threads released by a barrier call the
 * getters in a mixed way and the build happens exactly once;
 *   - T3 a failure retry: the first _calculate throws and maliciously corrupts
 * the base derived state; it verifies that the original exception propagates,
 * the base half-finished product is cleaned and the second call succeeds
 * without dirty data;
 *   - T4 reset: the recalculation after the reset leaves no old result;
 *   - T6 nesting: the _calculate of one MF triggers the lazy calculation of
 * another MF without a deadlock;
 *   - T7 serialization: after load the state is not published and the
...
/** A simple spin barrier: it does not depend on the C++20 latch/barrier and all
...
//-----------------------------------------------------------------------------
// A counting MF: it counts and delays on entering _calculate, enlarging the
// concurrency window
//-----------------------------------------------------------------------------
...
IndicatorList TestCountingMF::_calculate(
  ...
  return global_parallel_for_index(0, stk_count, [&](size_t si) {
    ...
    for (size_t di = 0; di < days_total; di++) {
      sumByDate[di] = (countByDate[di] == 0) ? Null<price_t>()
...
/** Build and configure a counting MF (two factors take the _calculate branch,
 * not the single-factor pass-through) */
...
/** @par A concurrent first access: 32 threads released by a barrier call the
 * getters in a mixed way and the build happens exactly once */
...
//-----------------------------------------------------------------------------
// A FailOnce MF: the first call corrupts the base derived state and throws; the
// second is normal
//-----------------------------------------------------------------------------
...
IndicatorList TestFailOnceMF::_calculate(
    const vector<IndicatorList>& all_stk_inds) {
...
/** @par The failure retry: the first exception propagates up, the half-finished
 * product is cleaned and the second call succeeds */
TEST_CASE("test_MF_failed_first_call_clean_retry") {
  ...
  // The first call: the original exception propagates up and Ready is not
  // published
  ...
  // The dirty derived state left by the first call must have been cleared by
  // clearCalculatedData
  CHECK_UNARY_FALSE(
      TestFailOnceMF::s_saw_dirty.load(std::memory_order_relaxed));
...
/** @par The recalculation after reset: no old result is left and it matches the
 * first one */
...
/** @par The nested calculation: the lazy calculation of B is triggered inside
 * the _calculate of A and the concurrent triggering has no deadlock
 */
...
/** @par The clone independent state: after the original object is Ready the
 * clone does not share the calculation state Two threads access the original
 * object (already Ready, no recalculation) and the clone (the first access
 * triggers a recalculation) at the same time, without interfering with each
 * other and the results are equal.
 */
TEST_CASE("test_MF_clone_independent_state") {
  ...
  // A forced recalculation after clone: the original is not recalculated, only
  // the clone triggers
  auto mfB = mfA->clone();
...
/** @par After the serialization load the state is not published: the
 * recalculation is complete */
...
TEST_CASE("test_MF_serialization_load_recalculates") {
  ...
  // After load m_calculated must be false: an access recalculates instead of
