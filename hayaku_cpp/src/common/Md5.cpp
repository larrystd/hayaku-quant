 ...
 *      Author: fasiondog
 *
 *  Note: the md5 calculation of boost gives different results on the windows
 * and the linux platforms, so it was ported from dlib instead
 */
...
void scramble_block(uint32_t& a, uint32_t& b, uint32_t& c, uint32_t& d,
                    const uint32_t* x) {
