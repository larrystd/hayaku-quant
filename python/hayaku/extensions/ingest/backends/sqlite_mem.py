...
def dbcommit(func):
    def wrapfunc(*args, **kwargs):
        ...
        x = args[0]
        try:
            hayaku_check(x.cursor is None, "You cannot nest using database cursors!")
            x.cursor = x._get_cursor()
            result = func(*args, **kwargs)
            x._cnx.commit()
        except Exception as e:
            hayaku_error("{} [{}.{}]".format(e, func.__module__, func.__name__))
            result = None
...
class SqliteMemDriver:
    def __init__(self):
        self._cnx = sqlite3.connect("file:hayaku_mem?mode=memory&cache=shared")
        self.cursor = None
        self._create_table()
    ...
    def _create_table(self):
        filename = os.path.join(os.path.dirname(__file__), '..', 'schema', 'memory',
                                'createdb.sql')
