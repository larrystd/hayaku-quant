...
void registerClickHouseDrivers(const SessionOptions& options) {
    ...
    auto* plugin = getPlugin<DataDriverPluginInterface>(HAYAKU_PLUGIN_CLICKHOUSE_DRIVER);
    HAYAKU_CHECK(plugin, "{}: {}", htr("Can not find plugin"), HAYAKU_PLUGIN_CLICKHOUSE_DRIVER);

    if (baseInfo) {
        auto driver = plugin->getBaseInfoDriver();
        HAYAKU_CHECK(driver, "{}", htr("Can not get clickhouse driver! Check your license!"));
...
void prepareDataRuntimeAssembly(const SessionOptions& options) {
    ...
    plugins.configurePluginPath(options.hayakuParam().tryGet<string>("plugindir", ""));
    ...
    const bool useShmServer = options.hayakuParam().tryGet<bool>("use_shm_server", false);
    if (useShmServer) {
        setRealtimeDataSourceResolver([]() -> RealtimeDataSource* {
            if (isShmServerRole()) {
                return nullptr;
            }
            return getPlugin<ShmServerPluginInterface>(HAYAKU_PLUGIN_SHM_SERVER, false);
        });
    } else {
