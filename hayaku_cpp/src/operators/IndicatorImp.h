...
/**
 * Indicator implementation class; when defining a new indicator, this class
 * should be inherited
 * @ingroup Indicator
 */
class HAYAKU_API IndicatorImp : public enable_shared_from_this<IndicatorImp> {
 ...
 public:
  enum OPType : uint8_t {
    ...
    AND,    ///< And
    OR,     ///< Or
    WEAVE,  ///< Special, an indicator that requires two indicators as the
            ///< parameters
    OP_IF,  /// if operation
    INVALID
  };
 ...
 public:
  ...
  /** Get the given output set in the form of an Indicator, it contains the
   * discard information */
  IndicatorImpPtr getResult(size_t result_num);

  /**
   * After using the IndicatorImp(const Indicator&...) constructor, this
   * function is used for the calculation result; no out-of-range protection is
   * done
   */
  ...
  /**
   * Prepare the memory
   * @param len the length; std::invalid_argument is thrown if the length is
   * greater than MAX_RESULT_NUM
  ...
  /**
   * @brief Construction origin identifier (unique within the process, inherited
   * by clone/cloneNode, not serialized)
   *
   * It is used as the identity by the caching facilities (such as the
   * cross-section panel cache). One construction and its clone chain share the
   * same id; independent constructions necessarily have different ids. It does
   * not participate in any existing equality judgment or display such as
   * alike()/operator==/formula().
   *
   * Note: it is defined in IndicatorImp.cpp (not inline). The inline member of
   * a dllexport class is not guaranteed to be exported, whether an external
   * symbol reference is generated depends on the inline decision of the caller,
   * and a plugin dll will fail to link.
   *
   * Note (identity semantic limitation): origin_id only identifies the
   * "origin", it is not aware of the in-place mutation after the construction
   * (setParam/setIndParam/add). Any indicator object used as an identity by a
   * caching facility must be regarded as immutable after the construction;
   * construct a new object if different parameters are needed (a new object
   * always holds a new id).
   */
  ...
  void add_if(IndicatorImpPtr cond, IndicatorImpPtr left,
  ...
  /**
   * Whether this implementation can be recalculated repeatedly on a reusable
   * batch executor.
   *
   * It is allowed by default (a non-Python implementation without an explicit
   * `_support_batch_reuse=false` is regarded as reusable). Participating in the
   * reuse means that the calculation graph of the same node is repeatedly
   * rebounded to the context across the stocks and recalculated by
   * `CompiledFactorPlan`, so a custom C++ indicator must satisfy: apart from
   * the result buffer of IndicatorImp itself and `m_params/m_ind_params`, it
   * holds no member state that would remain across the stocks (such as cached
   * statistics, mutable buffers, intermediate results that depend on the last
   * input, and so on). Any implementation that keeps the above state outside
   * `_calculate`/`_dyn_calculate`, or cannot be reset cleanly through
   * `scrubTemplateNode`, should call `supportBatchReuse(false)` at construction
   * to exit the fast path actively and fall back to the old behavior.
   */
  bool supportBatchReuse() const;

  void supportBatchReuse(bool enable);

  /** Only used to swap the data of two indicators with the same number of
   * result sets and the same length, no other parameters are swapped. An
   * exception is thrown on failure */
  void swap(IndicatorImp* other);
  void swap(IndicatorImp* other, size_t other_result_idx,
   ...
   * @note If there is only one dynamic parameter and it represents the
   * calculation window period, _dyn_run_one_step can be overloaded directly
   * together with isSerial
   */
  virtual void _dyn_calculate(const Indicator&);

  /**
   * If there is only one dynamic parameter and it represents the calculation
   * window period, this macro can be used to define the dynamic cycle
   * calculation interface; otherwise you need to overload the _dyn_calculate
   * function yourself to implement the dynamic parameter calculation
   */
  virtual void _dyn_run_one_step(const Indicator& ind, size_t curPos,
                                 size_t step) {}
  ...
  virtual size_t min_increment_start() const { return 0; }
 ...
 public:
  ...
  /** Judge whether it is equivalent to another indicator, i.e. the calculation
   * effect is the same */
  ...
  // returning true means the two indicators are equivalent
  virtual bool selfAlike(const IndicatorImp& other) const noexcept {
    return false;
  }

  // Get the inner nodes of the special indicator that uses the input context,
  // so that it can be merged into the indicator tree optimization
  virtual void getSelfInnerNodesWithInputConext(
      vector<IndicatorImpPtr>& nodes) const {}

  // Overload of a leaf node with a separate ktype to get its inner child nodes
  virtual void getSeparateKTypeLeafSubNodes(
      vector<IndicatorImpPtr>& nodes) const {}
 ...
 private:
  ...
  void _printTree(int depth = 0, bool isLast = true,
                  bool show_long_name = false) const;
 ...
 protected:
   ...
   * that there is only one counter across the DLLs */
...
#define INDICATOR_IMP(classname)                           \
 public:                                                   \
  ...
  virtual IndicatorImpPtr _clone() override { return make_shared<classname>(); }

// If there is only one dynamic parameter and it represents the calculation
// window period, this macro can be used to define the dynamic cycle calculation
// interface; otherwise you need to overload the _dyn_calculate function
// yourself to implement the dynamic parameter calculation
...
template <>
struct fmt::formatter<hayaku::IndicatorImp> : ostream_formatter {};
#endif
