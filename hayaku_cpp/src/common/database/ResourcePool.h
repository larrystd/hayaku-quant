...
public:
    ...
    ResourcePtr get() {
        ...
        if (m_resourceList.empty()) {
            ...
            try {
                ...
                HAYAKU_THROW_EXCEPTION(CreateResourceException, "Failed create a new Resource! {}",
                                    e.what());
            } catch (...) {
    ...
    ResourcePtr getWaitFor(uint64_t ms_timeout) {  // NOSONAR
        ...
        if (m_resourceList.empty()) {
            if (m_maxPoolSize > 0 && m_count >= m_maxPoolSize) {
                // HAYAKU_TRACE("The maximum number of the resources is exceeded, waiting for an idle
                // resource");
                if (ms_timeout > 0) {
                    if (m_cond.wait_for(lock,
                                        std::chrono::duration<uint64_t, std::milli>(ms_timeout),
                                        [&] { return !m_resourceList.empty(); })) {
                        HAYAKU_CHECK_THROW(!m_resourceList.empty(), GetResourceTimeoutException,
                                        "Failed get resource!");
                    } else {
                        HAYAKU_THROW_EXCEPTION(GetResourceTimeoutException, "Failed get resource!");
                    }
                } else {
...
public:
    ...
    void setParam(const std::string &name, const ValueType &value) {
        ...
        HAYAKU_IF_RETURN(m_param.have(name) && value == m_param.get<ValueType>(name), void());
