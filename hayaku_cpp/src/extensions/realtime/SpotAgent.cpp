...
string SpotAgent::ms_pubUrl{"ipc:///tmp/hayaku_real.ipc"};  // The data sending service address
...
void SpotAgent::start() {
    HAYAKU_CHECK(!isInCallback(), "Cannot restart SpotAgent from its own callback");
    HAYAKU_CHECK(m_work_num > 0, "SpotAgent worker count must be greater than zero");
...
class ProcessTask {
public:
    ProcessTask(SpotAgent* agent, const std::function<void(const SpotRecord&)>& func,
...
unique_ptr<SpotRecord> SpotAgent::parseFlatSpot(const hayaku::flat::Spot* spot) {
...
void SpotAgent::parseSpotData(const void* buf, size_t buf_len,
    ...
    const auto* spot_list_buf = static_cast<const uint8_t*>(buf) + ms_spotTopicLength;
    ...
    for (size_t i = 0; i < total; i++) {
        ...
        if (spot_record) {
            for (const auto& process : m_processList) {
                tasks.emplace_back(m_tg->submit(ProcessTask(this, process, *spot_record)));
            }
        }
    }
...
void SpotAgent::work_thread() {
    ...
    while (!m_stop && rv != 0) {
        ...
        HAYAKU_WARN_IF(m_print && rv != 0 && (now - pretime) > Seconds(5),
                    "Faied connect quotation server {}, will retry after 5 seconds!", ms_pubUrl);
        pretime = now;
        std::this_thread::sleep_for(std::chrono::milliseconds(300));
    }

    HAYAKU_INFO_IF(!m_stop && m_print, "Ready to receive quotation from {} ...", ms_pubUrl);
            ...
            switch (m_status) {
                ...
                case RECEIVING:
                    if (length == ms_endTagLength &&
                        ...
                        m_receive_data_tg->submit(
                          [this, length, startReceiveTime, new_buf = std::move(data_buf)]() {
                              try {
