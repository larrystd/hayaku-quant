...
inline StockList get_stock_list_from_python(const py::object& stks) {
    ...
    if (py::isinstance<StockList>(stks)) {
        ...
        HKU_THROW("Failed get StockList! Input stks must be Block, DataEngine or sequence(Stock)!");
    }
    return ret;
}
...
inline Block get_block_from_python(const py::object& blk) {
    ...
    if (py::isinstance<StockList>(blk)) {
        ...
        HKU_THROW("Failed get Block! Input blk must be Block, DataEngine or sequence(Stock)!");
    }
    return ret;
}
