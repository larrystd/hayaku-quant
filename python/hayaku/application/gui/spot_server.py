...
import hayaku.application.gui.flat as fb

from hayaku._support import *
from hayaku.extensions.ingest.sources.stock.zh_stock_a_sina_qq import get_spot as qq_get_spot
from hayaku.extensions.ingest.sources.stock.zh_stock_a_qmt import get_spot as qmt_get_spot
from hayaku import open_session, Datetime, TimeDelta
from hayaku.core import constant
...
g_spot_topic = ':spot:'
g_ipc_addr = 'ipc:///tmp/hayaku_real.ipc'
g_tcp_addr = 'tcp://*:9200'
...
def release_nng_senders():
    ...
    if g_nng_tcp_sender is not None:
        ...
        g_nng_tcp_sender = None
    g_nng_sender_lock.release()
    hayaku_info("release pynng socket")
...
def collect(server, use_proxy, source, seconds, phase1, phase2, ignore_weekend):
    ...
    if phase1_delta is None or len(phase1_delta) != 2:
        hayaku_error("无效参数 phase1: {}".format(phase1))
        exit(1)
    if phase1_delta[0] > phase1_delta[1]:
        hayaku_error("无效参数 phase1: {}, 结束时间应大于等于起始时间".format(phase1))
        exit(1)

    phase2_delta = parse_phase(phase2)
    if phase2_delta is None or len(phase2_delta) != 2:
        hayaku_error("无效参数 phase2: {}".format(phase2))
        exit(1)
    if phase2_delta[0] > phase2_delta[1]:
        hayaku_error("无效参数 phase2: {}, 结束时间应大于等于起始时间".format(phase2))
        exit(1)
    if phase1_delta[1] > phase2_delta[0]:
        hayaku_error("无效参数 phase1: {}, phase2: {}, phase2 起始时间应大于等于 phase1 结束时间".format(phase1, phase2))
        exit(1)

    hayaku_logger.info("采集时间段1：{}".format(phase1))
    hayaku_logger.info("采集时间段2：{}".format(phase2))

    config_file = os.path.expanduser('~') + '/.hayaku/hayaku.ini'
    if not os.path.exists(config_file):
        print("未找到配置文件，请先运行 HayakuTDX 进行配置与数据导入")
        exit(1)
    ...
    hayaku_info("启动采集时间：{}".format(next_time))
    ...
    while True:
        try:
            ...
            hayaku_info("{}:{}:{} 采集数量: {}".format(start_time.hour, start_time.minute, start_time.second, len(records)))
            # pub_sock.send('{}{}'.format(spot_topic, '[end spot]').encode('utf-8'))
            end_send_spot()
            delta = next_delta(start_time, seconds, phase1_delta, phase2_delta, ignore_weekend)
            hayaku_info("sleep {}'s".format(delta.total_seconds()))
