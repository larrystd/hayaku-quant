...
HAYAKU_API std::ostream &operator<<(std::ostream &os, const IndicatorImpPtr &imp) {
    if (!imp) {
        os << "Indicator {}";
    } else {
        os << imp->str();
...
PriceList IndicatorImp::getResultAsPriceList(size_t result_num) {
    HAYAKU_IF_RETURN(result_num >= m_result_num || m_pBuffer[result_num] == NULL, PriceList());
...
IndicatorImpPtr IndicatorImp::getResult(size_t result_num) {
    HAYAKU_IF_RETURN(result_num >= m_result_num || m_pBuffer[result_num] == NULL, IndicatorImpPtr());
...
#if CHECK_ACCESS_BOUND
    // cppcheck-suppress [arrayIndexOutOfBoundsCond]
    HAYAKU_CHECK_THROW((num <= MAX_RESULT_NUM && m_pBuffer[num] && !m_pBuffer[num]->empty()),
...
DatetimeList IndicatorImp::getDatetimeList() const {
    HAYAKU_IF_RETURN(haveParam("align_date_list"), getParam<DatetimeList>("align_date_list"));
...
void IndicatorImp::add(OPType op, IndicatorImpPtr left, IndicatorImpPtr right) {
    HAYAKU_ERROR_IF_RETURN(op == LEAF || op >= INVALID || !right, void(), "Wrong used!");
    if (OP == op && !isLeaf()) {
        if (m_left) {
...
Indicator IndicatorImp::calculate() {
    ...
    try {
        ...
        // Do not make the core indicator path depend on the host application (Python or otherwise).
        // Python-derived implementations should be managed by shared_ptr through the binding.
...
void IndicatorImp::_dyn_calculate(const Indicator &ind) {
    ...
    HAYAKU_CHECK(ind_param->size() >= ind.size(), "ind_param->size()={}, ind.size()={}!",
...
bool IndicatorImp::alike(const IndicatorImp &other) const {
    ...
    HAYAKU_IF_RETURN(m_optype != other.m_optype || m_discard != other.m_discard ||
    ...
    HAYAKU_IF_RETURN(bool(m_three) != bool(other.m_three) || bool(m_left) != bool(other.m_left) ||
