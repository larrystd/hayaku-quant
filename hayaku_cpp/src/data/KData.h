...
class HAYAKU_API KData {
 public:
  ...
  // Move semantics brings no real benefit to KData, and it may leave KData with
  // a null imp. They are kept mainly because boost::any_cast needs them, but do
  // not std::move a KData.
  KData(KData&&);
  ...
  /**
   * Get a new KData from the current one, keeping the data type and price
  ...
  /**
   * Get the K-line data of another type over the same time range, e.g. the
   * minute data corresponding to the daily data
   * @param ktype
   * @return KData
   */
  ...
  /** Get the index position of the given date. Note: it is the position inside
   * this KData, not the position of the original K-line record in the Stock */
  ...
  /** Get the associated Stock; Null<Stock> is returned if there is no
   * association */
  ...
  /** Get the position of the last record in the original K-line record; 0 if it
   * is empty, otherwise endPos - 1 */
  size_t lastPos() const;

  /** Get the position of the next record after the range in the original K-line
   * record; 0 if it is empty, otherwise lastPos + 1 */
  size_t endPos() const;
  ...
  /**
   * Special purpose! Use with care! Get a query condition by date for another
   * K-line type over the given date range, based on the current K-line range
   * @note
   *  1. The given date range must fall inside the current K-line data range,
   * otherwise it is truncated to the K-line range
   *  2. The precision of start_datetime / end_datetime should match the current
   * KData
   *  3. If the original end condition is Null<Datetime>() and end_datetime is
   * not given, the returned query condition is Null<Datetime>()
   * @param start_datetime
   * @param end_datetime
   * @param ktype
   * @return KData
   */
  KQuery getOtherQueryByDate(const Datetime& start_datetime,
                             const Datetime& end_datetime,
                             const KQuery::KType& ktype) const;

 public:
  const KRecord* data() const noexcept;
  KRecord*
  data() noexcept;  // Use with care (intended for forcibly adjusting the data)
 ...
 * KData{
 *   size : 738501
 *   stock: Stock(SH, 000001, Shanghai Composite Index, Index, 1, 1990-Dec-19
 * 00:00:00, +infinity), query: KQuery(0, 99999999999, INDEX, MIN, NO_RECOVER)
 *  }
 * </pre>
 * @ingroup StockManage
 */
...
template <>
struct fmt::formatter<hayaku::KData> : ostream_formatter {};
#endif
