...
if sys.platform == 'win32':
    # config_file = os.path.join(head, "test_data/hayaku_win.ini")
    config_file = "test_data/hayaku_win.ini"
else:
    # config_file = os.path.join(head, "test_data/hayaku_linux.ini")
    config_file = "test_data/hayaku_linux.ini"
    # print(config_file)
...
def get_real_tdx_filepath(code, market='sh'):
    ...
    该函数按以下顺序确定文件路径:
    1. 检查环境变量 `HAYAKU_real_tdx_file_path` 是否被设置。
    ...
    """
    # 检查环境变量
    env_path = os.environ.get('HAYAKU_real_tdx_file_path')
