...
// The vectors of the simple types are no longer exported, only the complex
// structList is exported, avoiding the performance impact
// PYBIND11_MAKE_OPAQUE(StringList);
// PYBIND11_MAKE_OPAQUE(PriceList);
PYBIND11_MAKE_OPAQUE(DatetimeList);
PYBIND11_MAKE_OPAQUE(KRecordList);
// PYBIND11_MAKE_OPAQUE(StockList); // The StockList is not large, so letting
// pybind convert it from and to a list automatically is more convenient
...
inline Datetime pydatetime_to_Datetime(const pybind11::object& source) {
  ...
  if (PyDateTime_Check(src)) {
    ...
    throw std::invalid_argument(
        "Can't convert this python object to Datetime!");
  }

  // Datetime only goes down to January 1, 1400 and up to 0:00 on December 31,
  // 9999
  if (year < 1400) {
    value = Datetime::min();
 ...
 public:
  ...
  bool load(handle source, bool) {
    ...
    if (isinstance<Datetime>(obj)) {
      ...
      HAYAKU_CHECK(total > 0, "Can't support empty sequence!");
      ...
      } else if (PyFloat_Check(pyseq[0].ptr()) ||
                 PyLong_Check(pyseq[0].ptr())) {
   ...
   */
  static handle cast(boost::any x, return_value_policy /* policy */,
                     handle /* parent */) {
    if (x.type() == typeid(bool)) {
      ...
      if (k == Null<KData>()) {
        ...
        if (query.queryType() == KQuery::INDEX) {
          ...
          q_cmd << "Query(Datetime('" << query.startDatetime()
                << "'), Datetime('" << query.endDatetime() << "'), " << "Query."
                << KQuery::getKTypeName(query.kType()) << ", Query."
                << KQuery::getRecoverTypeName(query.recoverType()) << ")";
        }
    ...
    HAYAKU_THROW_EXCEPTION(std::runtime_error, "convert failed! Unkown type!!");
  }
};
}  // namespace detail
}  // namespace pybind11
