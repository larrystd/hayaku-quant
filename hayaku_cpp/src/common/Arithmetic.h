...
#if defined(_MSC_VER)
std::string HAYAKU_UTILS_API utf8_to_gb(const char *szinput);
std::string HAYAKU_UTILS_API utf8_to_gb(const std::string &szinput);
std::string HAYAKU_UTILS_API gb_to_utf8(const char *szinput);
std::string HAYAKU_UTILS_API gb_to_utf8(const std::string &szinput);
#else
std::string HAYAKU_UTILS_API utf8_to_gb(const std::string &szinput);
std::string HAYAKU_UTILS_API gb_to_utf8(const std::string &szinput);
#endif

#define UTF8ToGB hayaku::utf8_to_gb
#define GBToUTF8 hayaku::gb_to_utf8

/**
 * Under Windows it is used to convert a string from UTF8 to the GB2312 encoding
 * (for the printing only; use HAYAKU_PATH if it is needed for a path), Under
 * Linux it does nothing
 * @note Currently Visual Studio 2019 has fixed the UTF8 problem, so it is not
 * needed in most cases; HAYAKU_STR/HAYAKU_CSTR, but the paths and file names
 * under Windows are still GB and need the conversion processing.
 */
#if defined(_MSC_VER) && _MSC_VER < 1928  // 1928 (Visual Studio 2019)
// Convert a UTF8 encoded string to the GB2312 encoding
#define HAYAKU_STR(s) UTF8ToGB(s)
#define HAYAKU_CSTR(s) UTF8ToGB(s)
#else
#define HAYAKU_STR(s) (s)
#define HAYAKU_CSTR(s) (s)
#endif

/**
 * Used to process the path file names, compatible with the Chinese Windows
 * platform HAYAKU_PATH is for std::string HAYAKU_CPATH is for char *
 */
#if defined(_MSC_VER)
#define HAYAKU_PATH(s) UTF8ToGB(s)
#define HAYAKU_CPATH(s) UTF8ToGB(s)
#else
#define HAYAKU_PATH(s) (s)
#define HAYAKU_CPATH(s) (s)
#endif
...
ValueT roundEx(ValueT number, int ndigits = 0) {
  ...
  if (ndigits < 0)
    return number;  // An invalid number of the digits returns the original
                    // value directly

  const double factor = std::pow(10.0, ndigits);
  const double epsilon =
      1e-10 *
      factor;  // Adjust epsilon dynamically to avoid the precision error

  if (number >= 0)
    return static_cast<ValueT>(std::floor(number * factor + 0.5 + epsilon) /
                               factor);
  else
    return static_cast<ValueT>(std::ceil(number * factor - 0.5 - epsilon) /
                               factor);
}

extern template double HAYAKU_UTILS_API roundEx(double number, int ndigits);

template <>
float HAYAKU_UTILS_API roundEx(float number, int ndigits);
...
extern template double HAYAKU_UTILS_API roundUp(double number, int ndigits);
extern template float HAYAKU_UTILS_API roundUp(float number, int ndigits);
extern template double HAYAKU_UTILS_API roundDown(double number, int ndigits);
extern template float HAYAKU_UTILS_API roundDown(float number, int ndigits);
...
std::string HAYAKU_UTILS_API utf8_to_lower(const std::string &s) noexcept;

/** Convert a UTF-8 string to uppercase */
std::string HAYAKU_UTILS_API utf8_to_upper(const std::string &s) noexcept;

/** Case folding comparison of UTF-8 strings */
bool HAYAKU_UTILS_API utf8_fold_equal(const std::string &s1,
                                      const std::string &s2) noexcept;

/** Whether a UTF-8 string contains a substring */
bool HAYAKU_UTILS_API utf8_contains(const std::string &s,
                                    const std::string &sub) noexcept;
 ...
 * @note Note that the lifetime of the returned result should be shorter than
 * that of the input string!
 */
inline std::vector<std::string_view> split(const std::string_view &view,
                                           char c) {
  std::vector<std::string_view> result;
  size_t prepos = 0;
  size_t pos = view.find_first_of(c);
...
/**
 * Convert bytes to a hexadecimal string, e.g. "abcd" is converted to "0x61 0x62
 * 0x63 0x64"
 * @param bytes the input byte array
 * @param in_len the length of the byte array
 */
...
bool HAYAKU_UTILS_API isInteger(double num);

/** Judge whether a float is an integer */
bool HAYAKU_UTILS_API isInteger(float num);
...
extern template double HAYAKU_UTILS_API
get_quantile(const std::vector<double> &vec, double quantile);
extern template float HAYAKU_UTILS_API
get_quantile(const std::vector<float> &vec, double quantile);

HAYAKU_UTILS_API std::ostream &operator<<(std::ostream &os,
