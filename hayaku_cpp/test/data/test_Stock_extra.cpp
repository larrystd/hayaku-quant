...
#include "application/plugins/HayakuExtraPluginInterface.h"
...
/**
 * @defgroup test_hayaku_Stock_extra test_hayaku_Stock_extra
...
TEST_CASE("test_Stock_extra_ktype") {
  HAYAKU_IF_RETURN(!_pluginValid(), void());

  // Extra K types are now an explicit optional extension, not registered by
  // core Session startup.
  registerExtraKType(KQuery::DAY3, KQuery::DAY, 3);
  registerExtraKType(KQuery::MIN3, KQuery::MIN, 3,
                     [](const Datetime& datetime) {
                       ...
                       return Datetime(datetime.year(), datetime.month(),
                                       datetime.day(), datetime.hour(), minute);
                     });

  /** @arg Check the number of the minutes of the extended K-line type */
  CHECK_EQ(KQuery::getKTypeInMin(KQuery::DAY) * 3,
           KQuery::getKTypeInMin(KQuery::DAY3));
  ...
  CHECK_EQ(stk.getMarketValue(Datetime(199203050000), KQuery::DAY3),
           kday3[100].closePrice);
  CHECK_EQ(stk.getMarketValue(Datetime(199203040000), KQuery::DAY3),
           kday3[99].closePrice);

  size_t startix = 0, endix = 0;
  bool success = stk.getIndexRange(
      KQueryByDate(Datetime(199203050000), Null<Datetime>(), KQuery::DAY3),
      startix, endix);
  CHECK_UNARY(success);
  CHECK_EQ(startix, 100);
  CHECK_EQ(endix, 1707);
  success =
      stk.getIndexRange(KQueryByDate(Datetime(199203050000),
                                     Datetime(199204160000), KQuery::DAY3),
                        startix, endix);
  CHECK_UNARY(success);
  CHECK_EQ(startix, 100);
  CHECK_EQ(endix, 110);
  ...
  auto kday3_2 = stk.getKData(KQueryByDate(
      Datetime(199203050000), Datetime(199205270001), KQuery::DAY3));
  ...
  CHECK_EQ(stk.getMarketValue(Datetime(200001051033), KQuery::MIN3),
           kmin3[100].closePrice);
  CHECK_EQ(stk.getMarketValue(Datetime(200001051032), KQuery::MIN3),
           kmin3[99].closePrice);

  success = stk.getIndexRange(
      KQueryByDate(Datetime(200001051033), Null<Datetime>(), KQuery::MIN3),
      startix, endix);
  CHECK_UNARY(success);
  CHECK_EQ(startix, 100);
  CHECK_EQ(endix, 228099);
  success =
      stk.getIndexRange(KQueryByDate(Datetime(200001051033),
                                     Datetime(200001051103), KQuery::MIN3),
                        startix, endix);
  CHECK_UNARY(success);
  CHECK_EQ(startix, 100);
  CHECK_EQ(endix, 110);
  ...
  auto kmin3_2 = stk.getKData(KQueryByDate(
      Datetime(200001051033), Datetime(200001051131), KQuery::MIN3));
