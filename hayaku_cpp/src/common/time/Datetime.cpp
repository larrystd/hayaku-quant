...
HAYAKU_UTILS_API std::ostream &operator<<(std::ostream &out, const Datetime &d) {
    out << d.str();
    return out;
}
...
Datetime::Datetime(long year, long month, long day, long hh, long mm, long sec, long millisec,
                   long microsec) {
    HAYAKU_CHECK(millisec >= 0 && millisec <= 999, "Out of range! millisec: {}", millisec);
    HAYAKU_CHECK(microsec >= 0 && microsec <= 999, "Out of range! microsec: {}", microsec);
...
Datetime::Datetime(unsigned long long datetime) {
    ...
    if (datetime <= 99999999LL) {
        ...
        HAYAKU_CHECK_THROW(hh < 24, std::out_of_range, "Hour value is out of rang 0..23");
        HAYAKU_CHECK_THROW(mm < 60, std::out_of_range, "Minute value is out of range 0..59");
        bd::date d((unsigned short)year, (unsigned short)month, (unsigned short)day);
...
uint64_t Datetime::hex() const noexcept {
    try {
        HAYAKU_IF_RETURN(m_data.date() == bd::date(bd::pos_infin), Null<unsigned long long>());
...
DatetimeList HAYAKU_UTILS_API getDateRange(const Datetime &start, const Datetime &end) {
    DatetimeList result;
...
Datetime Datetime::nextDay() const {
    HAYAKU_IF_RETURN(*this == Null<Datetime>() || *this == Datetime::max(), *this);
...
Datetime Datetime::preDay() const {
    HAYAKU_IF_RETURN(*this == Null<Datetime>() || *this == Datetime::min(), *this);
    return Datetime(date() - bd::date_duration(1));
}
