...
HAYAKU_API std::ostream& operator<<(std::ostream& os, const KRecord& record) {
  ...
  os << std::fixed;
  (void)os.precision(4);
  os << "KRecord(Datetime(" << record.datetime.ymdhms() << ")" << strip
     << record.openPrice << strip << record.highPrice << strip
     << record.lowPrice << strip << record.closePrice << strip
     << record.transAmount << strip << record.transCount << ")";
