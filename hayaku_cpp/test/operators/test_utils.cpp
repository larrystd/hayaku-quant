...
TEST_CASE("test_multi_regression_empty") {
  ...
  // An invalid query
  auto result2 = multi_regression(
      stock, KQuery(Datetime(2099, 1, 1), Datetime(2099, 1, 2)),
      MA(CLOSE(), 5));
  CHECK(result2.empty());
}
...
TEST_CASE("test_multi_regression_insufficient_data") {
  ...
  auto result = multi_regression(stock, KQuery(-5), MA(CLOSE(), 5),
...
TEST_CASE("test_multi_regression_variadic_template") {
  ...
  if (!kdata.empty()) {
    ...
    // Test with 2 factors
    auto result2 =
        multi_regression(stock, KQuery(-252), MA(CLOSE(), 5), MA(CLOSE(), 10));
    ...
    // Test with 3 factors
    auto result3 = multi_regression(stock, KQuery(-252), MA(CLOSE(), 5),
                                    MA(CLOSE(), 10), MA(CLOSE(), 20));
...
TEST_CASE("test_multi_regression_full") {
  ...
  if (!kdata.empty()) {
    ...
    // The fixed value verification - set according to the actual run result
    CHECK(result[0] ==
          doctest::Approx(0.035479989606263).epsilon(1e-6));  // alpha
    CHECK(result[1] ==
          doctest::Approx(-0.000014864499944).epsilon(1e-6));  // beta

    size_t rss_pos = result.size() - 2;
    size_t r2_pos = result.size() - 1;
    CHECK(result[rss_pos] ==
          doctest::Approx(0.015468753348483).epsilon(1e-6));  // RSS
    CHECK(result[r2_pos] ==
          doctest::Approx(0.019762192080754).epsilon(1e-6));  // R²
...
TEST_CASE("test_multi_regression_benchmark") {
  ...
  {
    BENCHMARK_TIME_MSG(test_multi_regression_benchmark, cycle,
                       "multi_regression benchmark");
    SPEND_TIME_CONTROL(false);
    for (int i = 0; i < cycle; i++) {
      auto result = multi_regression(stock, KQuery(-252), MA(CLOSE(), 5),
                                     MA(CLOSE(), 10));
    }
  }
}
