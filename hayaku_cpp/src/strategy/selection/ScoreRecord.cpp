...
ScoreRecord::ScoreRecord(const Stock& stock_, value_t value_)
    : stock(stock_), value(value_) {}

ScoreRecord::ScoreRecord(const ScoreRecord& other)
    : stock(other.stock), value(other.value) {}

ScoreRecord::ScoreRecord(ScoreRecord&& rv)
    : stock(std::move(rv.stock)), value(rv.value) {}

ScoreRecord& ScoreRecord::operator=(const ScoreRecord& rv) {
  HAYAKU_IF_RETURN(this == &rv, *this);
  stock = rv.stock;
  value = rv.value;
  return *this;
}
