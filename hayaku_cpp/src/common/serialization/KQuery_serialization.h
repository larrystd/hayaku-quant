...
void load(Archive& ar, hayaku::KQuery& query, unsigned int version) {
  ...
  hayaku::KQuery::QueryType enum_query =
      hayaku::KQuery::getQueryTypeEnum(queryType);
  hayaku::KQuery::KType enmu_ktype = hayaku::KQuery::getKTypeEnum(kType);
  hayaku::KQuery::RecoverType enum_recover =
      hayaku::KQuery::getRecoverTypeEnum(recoverType);
  ...
  if (enum_query == hayaku::KQuery::INDEX) {
    ...
    ar& BOOST_SERIALIZATION_NVP(start);
    ar& BOOST_SERIALIZATION_NVP(end);
    query = hayaku::KQueryByDate(hayaku::Datetime(start), hayaku::Datetime(end),
