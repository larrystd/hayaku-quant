 ...
 * @ingroup DBConnect
 */
class HAYAKU_UTILS_API DBConnectBase : public std::enable_shared_from_this<DBConnectBase> {
...
NumberType DBConnectBase::queryNumber(const std::string &query, NumberType default_val) {
    ...
    if (!(st->moveNext() && st->getNumColumns() == 1)) {
        HAYAKU_CHECK(default_val != Null<NumberType>(), "query doesn't result in exactly 1 element");
        return default_val;
    }
    NumberType result = 0;
...
void DBConnectBase::batchSave(InputIterator first, InputIterator last, bool autotrans) {
    ...
    try {
        ...
        HAYAKU_THROW("failed batch save! sql: {}! {}", st->getSqlString(), e.what());
    } catch (...) {
        if (autotrans) {
            rollback();
        }
        HAYAKU_THROW("failed batch save! sql: {}! Unknown error!", st->getSqlString());
    }
}
...
void DBConnectBase::batchUpdate(InputIterator first, InputIterator last, bool autotrans) {
    ...
    try {
        ...
        HAYAKU_THROW("failed batch update! sql: {}! {}", st->getSqlString(), e.what());
    } catch (...) {
        if (autotrans) {
            rollback();
        }
