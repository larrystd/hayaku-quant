...
static void check_result(SignalPtr sg,
                         const std::map<Datetime, double>& expect) {
  for (auto iter = expect.begin(); iter != expect.end(); ++iter) {
    // HAYAKU_INFO("{}: {}, {}", iter->first, sg->getValue(iter->first),
    // iter->second);
...
TEST_CASE("test_SG_Add") {
  auto k = getKData("sz000001",
                    KQueryByDate(Datetime(20111108), Datetime(20111125)));
