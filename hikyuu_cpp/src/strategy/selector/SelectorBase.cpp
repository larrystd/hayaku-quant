...
void SelectorBase::calculate(const internal::StrategyRuntimeList& pf_realSysList, const KQuery& query) {
...
void SelectorBase::addSystem(const internal::StrategyRuntimePtr& sys) {
    ...
    HKU_CHECK(!sys->getParam<bool>("shared_account"),
              "A selector prototype cannot share an execution account");
    if (getParam<bool>("depend_on_proto_sys")) {
        HKU_CHECK(sys->getAccount(), "Scenarios that depend on prototype systems need to specify a TM!");
    }
...
void SelectorBase::addStock(const Stock& stock, const internal::StrategyRuntimePtr& protoSys) {
    ...
    HKU_CHECK(!protoSys->getParam<bool>("shared_account"),
              "A selector prototype cannot share an execution account");
    if (getParam<bool>("depend_on_proto_sys")) {
        HKU_CHECK(protoSys->getAccount(),
...
void SelectorBase::addStockList(const StockList& stkList, const internal::StrategyRuntimePtr& protoSys) {
