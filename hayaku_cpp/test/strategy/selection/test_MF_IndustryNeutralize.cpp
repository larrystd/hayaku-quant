 ...
 *
 * The white box unit test of the industry neutralization + the end-to-end
 * routing verification.
 *
 *   - The white box case tests the calculate_industry_residuals pure function
 * directly (exposed inline via industry_neutralize.h); a PriceList is built to
 * verify the math properties with no external data dependency.
 *   - The E2E case verifies the routing connectivity of _buildDummyIndex ->
 * calculate_industry_residuals, depending on test_data/block/hybk.ini (Chinese
 * block names, supportChineseSimple needed).
 */

#include <common/RuntimeInfo.h>
#include <data/DataRuntime.h>
#include <operators/SeriesOperators.h>
#include <operators/WindowOperators.h>
#include <strategy/selection/IndustryNeutralize.h>
...
/** @par Permutation invariance - the core counter example that pins down this
 * bug fix The result must not depend on the block numbering. The original
 * implementation did a unary regression on the integer numbering, so reordering
 * the block list changed the residuals; removing the group mean is unaffected.
 */
TEST_CASE("test_industry_residuals_permutation_invariance") {
  ...
  // Scenario B: the industry numbering is reversed - the residuals must match A
  // exactly
...
/** @par Boundary: an industry orphan (no membership) and a NaN label - the
 * residual is NaN and it is excluded from the group mean */
...
/** @par Boundary: k=1, the whole market in one industry - it degenerates to the
 * global mean */
...
/** @par Boundary: the industry contains invalid data - the NaN is skipped in
 * the group mean and the residual of an invalid point is NaN */
...
/** @par Boundary: an all NaN/Inf input - no crash, no floating point exception,
 * all NaN */
...
/** @par Boundary: a negative / oversized label - guard the UB of
 * static_cast<size_t>, no crash static_cast<size_t>(negative) is a C++ UB: it
 * may overflow to SIZE_MAX and cause an out of range segfault of sums[g]. The
 * implementation must intercept label<0 or label>=blk_count with a double
 * compare before the  cast.
 */
TEST_CASE("test_industry_residuals_negative_and_oversized_label") {
  PriceList y{10.0, 20.0, 30.0, 40.0};
  size_t blk_count = 2;
  // label[0]=-1 (a negative UB risk), [1]=999 (oversized), [2]=0 (valid), [3]=1
  // (valid)
  PriceList labels{-1.0, 999.0, 0.0, 1.0};
  auto res = calculate_industry_residuals(y, labels, blk_count);
  // The negative and the oversized labels: the residual is NaN (no crash, no
  // out of range)
  CHECK_UNARY(std::isnan(res[0]));
  CHECK_UNARY(std::isnan(res[1]));
  // The valid labels: group 0 has 30 only (mean 30, residual 0), group 1 has 40
  // only (mean 40)
...
//-----------------------------------------------------------------------------
// The E2E routing verification: _buildDummyIndex ->
// calculate_industry_residuals connectivity
//-----------------------------------------------------------------------------

/** @par The end-to-end routing: with the neutralization on, the cross-sectional
 * residual sum of the stocks in one industry is about 0 The dynamic discovery:
 * iterate over the blocks of the industry category and find the first one  that
 * contains at least 2 stocks with valid K-lines as the same-industry probe
 * pair. No stock code is hard coded, so a CI data change is adapted;  if the
 * whole category has no same-industry pair, a REQUIRE failure exposes the
 * insufficient data instead of passing  silently.
 *
 *  Note: the test environment uses QLBlockInfoDriver (reading an ini) whose
 * save throws  "Not support", so sm.addBlock cannot inject an in-memory Mock,
 * hence the dynamic discovery.
 */
TEST_CASE("test_mf_industry_neutralize_routing") {
  ...
  // Key: calculate() must be triggered first to fill m_ref_dates, otherwise
  // getAllSrcFactors() would skip the cross-sectional loop with an empty
  // m_ref_dates (days_total=0) and return the
  ...
  // A probe against a silent failure: the residual sum of the same industry is
  // about 0. If the name routing misses (an ALIGN rename makes
  // m_special_category miss), the raw MA values are output     // and the MA
  // sum of two stocks with different prices cannot be 0.
