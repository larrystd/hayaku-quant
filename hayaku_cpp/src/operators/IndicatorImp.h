...
vector<Indicator> HAYAKU_API combineCalculateIndicators(const vector<Indicator>& indicators,
...
class HAYAKU_API IndicatorImp : public enable_shared_from_this<IndicatorImp> {
    ...
    friend HAYAKU_API std::ostream& operator<<(std::ostream& os, const IndicatorImp& imp);
    friend class detail::CompiledFactorPlan;

    typedef vector<Indicator> IndicatorList;
    friend IndicatorList HAYAKU_API combineCalculateIndicators(const IndicatorList& indicators,
                                                            const KData& kdata, bool tovalue);
