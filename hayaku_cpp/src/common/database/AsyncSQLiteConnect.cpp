...
net::awaitable<bool> AsyncSQLiteConnect::ping() {
  if (!m_impl || !m_impl->m_db) {
    try {
      ...
      HAYAKU_ERROR("Failed connect to sqlite! {}", e.what());
      co_return false;
    }
  }
...
net::awaitable<int64_t> AsyncSQLiteConnect::exec(
...
net::awaitable<bool> AsyncSQLiteConnect::tableExist(
    const std::string &tablename) {
  bool result = false;
  try {
    auto st = co_await getStatement(fmt::format(
...
net::awaitable<void> AsyncSQLiteConnect::resetAutoIncrement(
    const std::string &tablename) {
  ...
  SQL_CHECK(count == 0, -1,
            ...
            tablename);
  co_await exec(fmt::format("UPDATE sqlite_sequence SET seq=0 WHERE name='{}'",
                            tablename));
}
          ...
          do {
            ...
            if (step_sleep > 0 &&
              ...
              std::this_thread::sleep_for(
                  std::chrono::milliseconds(step_sleep));
            }
