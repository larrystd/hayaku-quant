...
class HAYAKU_API PositionRecord {
 public:
  ...
  PositionRecord(const Stock& stock, const Datetime& takeDatetime,
                 ...
                 price_t goalPrice, double totalNumber, price_t buyMoney,
  ...
  /**
   * @brief Profit and loss = sell funds - accumulated total trade cost - buy
   * funds
   * @note It is valid for the closed records only, 0.0 is returned for the ones
   * not yet closed
   */
  ...
  price_t totalRisk{
      0.0};  ///< Accumulated trade risk = sum of (buy price - stop-loss) * buy
...
template <>
struct fmt::formatter<hayaku::PositionRecord> : ostream_formatter {};
#endif
