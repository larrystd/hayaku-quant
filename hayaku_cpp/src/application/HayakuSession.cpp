...
bool sameSessionOptions(const SessionOptions& lhs, const SessionOptions& rhs) {
    return lhs.baseInfoParam() == rhs.baseInfoParam() && lhs.blockParam() == rhs.blockParam() &&
           lhs.kdataParam() == rhs.kdataParam() && lhs.preloadParam() == rhs.preloadParam() &&
    ...
    try {
        ...
        if (firstSession) {
            ...
            runtime.init(options.baseInfoParam(), options.blockParam(), options.kdataParam(),
                         options.preloadParam(), options.hayakuParam(), options.context());
            initInnerTask();
            g_activeSessionOptions = options;
        } else {
            HAYAKU_CHECK(g_activeSessionOptions &&
                        sameSessionOptions(*g_activeSessionOptions, options),
                      "All concurrently open HayakuSession handles must use identical options");
        }
...
void HayakuSession::close() noexcept {
    ...
    if (detail::spotAgentCallbackActive()) {
        HAYAKU_ERROR("HayakuSession::close must be called outside a SpotAgent callback");
        return;
    }
...
bool HayakuSession::ready() const {
    return data().ready();
}

void HayakuSession::waitReady() const {
    data().waitReady();
}
...
StrategyEngine& HayakuSession::bindStrategy(const StrategyDefinition& definition) {
    HAYAKU_CHECK(isOpen(), "HayakuSession is closed");
    HAYAKU_CHECK(m_execution,
