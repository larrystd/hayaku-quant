...
StrategyEngine::StrategyEngine(StrategyDefinition definition,
    ...
    : m_definition(std::move(definition)),
      ...
      m_runtime(std::make_unique<internal::StrategyRuntime>(m_definition,
                                                            m_account)) {}
...
BacktestResult StrategyEngine::run(const BacktestRequest& request) {
  _ensureActive();
  {
    ...
    _ensureActive();
    HAYAKU_CHECK(!m_running.load(std::memory_order_relaxed),
...
void StrategyEngine::_stopAndWait() noexcept {
  ...
  if (m_running.load(std::memory_order_relaxed)) {
    m_stopRequested.store(true, std::memory_order_release);
    m_stateChanged.wait(
        lock, [this] { return !m_running.load(std::memory_order_acquire); });
  }
}
