...
扩展协议位于 ``hayaku.spi``；数据服务控制位于 ``hayaku.advanced``，历史数据导入使用可选的
``hayaku.ingest``。指标、工厂、绘图和
DataFrame 等宽研究接口改为显式导入 ``hayaku.interactive``，不再由包导入隐式加载。
...
``hayaku.core.KDataToHdf5Importer`` 等名称和 ``hayaku.advanced`` 中的导入别名已删除；
请从 ``hayaku.ingest`` 显式导入。MySQL 与 ClickHouse 是存储后端，不是额外的产品能力面。
...
需要使用时，从 ``hayaku.advanced`` 显式导入 ``start_spot_agent``、
...
``hayaku.trade_manage`` 记录类型 ``hayaku.execution`` 记录类型
券商辅助函数                     ``hayaku.execution`` 券商辅助函数
``hayaku.trade_sys`` 组件工厂    ``hayaku.strategy`` 组件工厂
记录的 ``to_np`` / ``to_df``     ``hayaku.execution`` 中的显式转换函数
===============================  ================================================
