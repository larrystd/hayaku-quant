...
static Indicator getExtIndicator(const string& name, const Parameter& params) {
  ...
  HAYAKU_ERROR_IF_RETURN(
      !plugin, ret,
      htr("Can't find {} plugin!", HAYAKU_PLUGIN_EXTEND_INDICATOR));
...
Indicator HAYAKU_API RANK(const Block& block, int mode, bool fill_null,
                          const string& market) {
  Parameter params;
...
Indicator HAYAKU_API RANK(const Block& block, const Indicator& ref_ind,
                          int mode, bool fill_null, const string& market) {
...
Indicator HAYAKU_API AGG_SAMPLE(const Indicator& ind, const string& time,
...
Indicator HAYAKU_API AGG_STD(const Indicator& ind, const KQuery::KType& ktype,
...
Indicator HAYAKU_API AGG_VAR(const Indicator& ind, const KQuery::KType& ktype,
                             bool fill_null, int unit, int ddof) {
  Parameter params;
  params.set<string>("ktype", ktype);
  params.set<bool>("fill_null", fill_null);
...
Indicator HAYAKU_API AGG_VWAP(const KQuery::KType& ktype, bool fill_null,
                              int unit) {
  Parameter params;
  params.set<string>("ktype", ktype);
  params.set<bool>("fill_null", fill_null);
