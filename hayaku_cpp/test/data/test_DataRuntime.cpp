...
/**
 * @defgroup test_hayaku_DataRuntime test_hayaku_DataRuntime
 * @ingroup test_hayaku_base_suite
 * @{
 */

/** @par Test points */
TEST_CASE("test_DataRuntime_size") {
...
/** @par Test points */
TEST_CASE("test_DataRuntime_getStock") {
  DataRuntime& sm = getDataRuntime();
  Stock stock;
  ...
  /** @arg Query by "market code" while the market exists but the security code
   * does not */
...
/** @par Test points */
TEST_CASE("test_DataRuntime_getMarketInfo") {
  DataRuntime& sm = getDataRuntime();
...
/** @par Test points */
TEST_CASE("test_DataRuntime_getStockTypeInfo") {
  DataRuntime& sm = getDataRuntime();
...
/** @par Test points */
TEST_CASE("test_DataRuntime_getAllMarket") {
  DataRuntime& sm = getDataRuntime();
...
/** @par Test points */
TEST_CASE("test_DataRuntime_getBlock") {
...
TEST_CASE("test_DataRuntime_TempCsvStock") {
  ...
  /** @arg Read KRecord[10] (a record in the middle) of the temporarily added
   * Stock */
...
/** @par Test points */
TEST_CASE("test_DataRuntime_isHoliday") {
...
/** @par Test points */
TEST_CASE("test_DataRuntime_isTradingHours") {
  auto& sm = getDataRuntime();
...
/** @par Test points */
TEST_CASE("test_DataRuntime_getZhBond10") {
  auto& sm = getDataRuntime();
...
/** @par Test points */
TEST_CASE("test_DataRuntime_releaseShmServerBaseInfoCache") {
  HAYAKU_IF_RETURN(!pluginValid(), void());

  auto& sm = getDataRuntime();
  const bool org_eviction = sm.isBaseInfoCacheEvictionEnabled();
  ...
  /** @arg The call is a no-op in a non-server role (an ordinary standalone /
   * client process): the cache is unaffected */
  sm.setBaseInfoCacheEvictionEnabled(false);
  sm.releaseShmServerBaseInfoCache();
  CHECK_EQ(stk.getWeight().size(), weight_cnt);
  CHECK_EQ(stk.getHistoryFinance().size(), finance_cnt);

  /** @arg The server role releases the ex-rights/ex-dividend cache: switching
   * back to the standalone mode (where getWeight has no lazy loading fallback)
   * and querying again returns empty, proving that the cache has really been
   * cleared and returned */
  sm.setBaseInfoCacheEvictionEnabled(true);
  sm.releaseShmServerBaseInfoCache();
  sm.setBaseInfoCacheEvictionEnabled(false);
  CHECK_EQ(stk.getWeight().size(), 0);

  /** @arg Accessing a released security again in the server role: the on-demand
   * lazy reload heals it and the result matches the one before the release */
  sm.setBaseInfoCacheEvictionEnabled(true);
  CHECK_EQ(stk.getWeight().size(), weight_cnt);
  CHECK_EQ(stk.getHistoryFinance().size(), finance_cnt);

  // Restore the state: Stock::getWeight has no lazy loading fallback in the
  // standalone / client mode, so the ex-rights/ex-dividend cache must be
  // refilled before the role is restored, avoiding an emptied state affecting
  // the later cases (the historical finance has a fallback)
  for (const auto& stock : sm.getStockList(nullptr)) {
    stock.getWeight();
  }
  sm.setBaseInfoCacheEvictionEnabled(org_eviction);
}
