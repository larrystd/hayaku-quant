...
class HAYAKU_API BlockInfoDriver {
 ...
 public:
  ...
  /**
   * If a subclass needs a cache, it can implement this method to load the data
   * into its own cache
   */
  virtual void load() {}
   ...
   * @note A block with the same name is overwritten; if the block category or
   * name is modified, the original block needs to be deleted manually before
   * the modification
   * @param block
   */
