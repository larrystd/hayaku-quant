...
class StockTable {
 public:
  StockTable()
      : stockid(0), marketid(0), type(0), valid(0), startDate(0), endDate(0) {}

 public:
  static const char* getInsertSQL() {
    return "insert into `stock` "
           "(`marketid`, `code`, `name`, `type`, `valid`, `startDate`, "
           "`endDate`) "
           "values (?,?,?,?,?,?,?)";
  }
  ...
  static const char* getUpdateSQL() {
    return "update `stock` set `marketid`=?, `code`=?, `name`=?, `type`=?, "
           "`valid`=?, `startDate`=?, `endDate`=?, `lastDate`=? where "
           "`stockid`=?";
  }
  ...
  void load(const SQLStatementPtr& st) {
    st->getColumn(0, stockid, marketid, code, name, type, valid, startDate,
                  endDate);
  }
