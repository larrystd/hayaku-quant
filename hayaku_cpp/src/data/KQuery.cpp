...
int32_t KQuery::getKTypeInMin(const KType& ktype) {
    ...
    HAYAKU_WARN_IF(sec <= 0, "Can't get KType in minutes: {}, will return 0", nktype);
    return sec;
}
...
Datetime KQuery::startDatetime() const {
    HAYAKU_IF_RETURN(m_queryType != DATE || (uint64_t)m_start == Null<uint64_t>(), Null<Datetime>());
    return Datetime(m_start);
}

Datetime KQuery::endDatetime() const {
    HAYAKU_IF_RETURN(m_queryType != DATE || (uint64_t)m_end == Null<uint64_t>(), Null<Datetime>());
