...
void startDataServer(const std::string& addr, size_t work_num, bool save_tick,
    ...
    HAYAKU_ERROR_IF_RETURN(!plugin, void(), htr("Can't find {} plugin!", HAYAKU_PLUGIN_DATASERVER));
    plugin->start(addr, work_num, save_tick, buf_tick, parquet_path);
}

void stopDataServer() {
    auto* plugin = getPlugin<DataServerPluginInterface>(HAYAKU_PLUGIN_DATASERVER);
    HAYAKU_ERROR_IF_RETURN(!plugin, void(), "Can't find {} plugin!", HAYAKU_PLUGIN_DATASERVER);
    plugin->stop();
}
...
void pullFromBufferServerLocal(const std::string& addr, const StockList& stklist,
                                       const KQuery::KType& ktype) {
