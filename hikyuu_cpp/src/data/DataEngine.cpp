...
#include "internal/DataRuntime.h"

namespace hku {

DataEngine::DataEngine(std::shared_ptr<std::atomic_bool> active) : m_active(std::move(active)) {}
...
DataRuntime& DataEngine::_backend() const {
    HKU_CHECK(m_active && m_active->load(std::memory_order_acquire),
              "DataEngine belongs to a closed HikyuuSession");
    HKU_CHECK(m_backend, "DataEngine is not attached to a data runtime");
    return *m_backend;
}

bool DataEngine::ready() const {
    return _backend().dataReady();
}

bool DataEngine::initializing() const {
    return _backend().initializing();
}

void DataEngine::waitReady() const {
    _backend().waitDataReady();
}

size_t DataEngine::size() const {
...
StockList DataEngine::getStockList() const {
    return _backend().getStockList();
}

KData DataEngine::getKData(const string& marketCode, const KQuery& query) const {
...
BlockList DataEngine::getStockBelongs(const Stock& stock, const string& category) const {
    return _backend().getStockBelongs(stock, category);
}

DatetimeList DataEngine::getTradingCalendar(const KQuery& query, const string& market) const {
    return _backend().getTradingCalendar(query, market);
}

DatetimeList DataEngine::getTradingCalendar(const StockList& stocks, const KQuery& query) const {
...
bool DataEngine::isTradingHours(const Datetime& datetime, const string& market) const {
...
StockWeightList DataEngine::getStockWeightList(const Stock& stock, Datetime start,
                                               Datetime end) const {
...
vector<std::pair<size_t, string>> DataEngine::getHistoryFinanceAllFields() const {
    return _backend().getHistoryFinanceAllFields();
}

vector<HistoryFinanceInfo> DataEngine::getHistoryFinance(const Stock& stock, Datetime start,
                                                         Datetime end) const {
