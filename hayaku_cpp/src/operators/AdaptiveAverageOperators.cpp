...
void IAma::_dyn_calculate(const Indicator& ind) {
    ...
    HAYAKU_CHECK(fast_n.size() == ind.size(), "ind_param(fast_n).size()={}, ind.size()={}!",
              fast_n.size(), ind.size());
    HAYAKU_CHECK(slow_n.size() == ind.size(), "ind_param(slow_n).size()={}, ind.size()={}!",
...
Indicator HAYAKU_API AMA(const IndParam& n, const IndParam& fast_n, int slow_n) {
    IndicatorImpPtr p = make_shared<IAma>();
    p->setIndParam("n", n);
    p->setIndParam("fast_n", fast_n);
    p->setParam<int>("slow_n", slow_n);
...
Indicator HAYAKU_API AMA(const IndParam& n, const IndParam& fast_n, const IndParam& slow_n) {
    IndicatorImpPtr p = make_shared<IAma>();
    p->setIndParam("n", n);
    p->setIndParam("fast_n", fast_n);
    p->setIndParam("slow_n", slow_n);
...
Indicator HAYAKU_API DMA(const Indicator& x, const Indicator& a, bool fill_null) {
