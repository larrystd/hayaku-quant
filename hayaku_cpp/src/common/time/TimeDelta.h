...
public:
    ...
    /**
     * Divide the duration by a coefficient, the result is rounded. hayaku::exception is thrown if it
     * is divided by zero.
     * Use floorDiv if the rounding is not desired
     */
    TimeDelta operator/(double p) const;

    /** Divide two durations to get their ratio. hayaku::exception is thrown when it is divided by a
     *  zero duration. */
    ...
    /** The remainder of the division of two durations. hayaku::exception is thrown when it is divided
     *  by a zero duration. */
    TimeDelta operator%(TimeDelta td) const;
...
inline string to_string(const hayaku::TimeDelta &delta) {
    return delta.str();
}
...
struct fmt::formatter<hayaku::TimeDelta> {
    ...
    auto format(const hayaku::TimeDelta &d, FormatContext &ctx) const -> decltype(ctx.out()) {
