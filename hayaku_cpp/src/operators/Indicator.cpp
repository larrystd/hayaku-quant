...
HAYAKU_API std::ostream& operator<<(std::ostream& os, const Indicator& indicator) {
    os << indicator.str();
    return os;
}
...
void Indicator::extend() {
    if (m_imp) {
        ...
        if (query.queryType() == KQuery::INDEX) {
            ...
            HAYAKU_WARN("query type ({}) error!", static_cast<int>(query.queryType()));
        }
        m_imp->setContext(k);
    }
}
...
Indicator HAYAKU_API IF(const Indicator& ind1, const Indicator& ind2, const Indicator& ind3) {
    HAYAKU_ERROR_IF_RETURN(!ind1.getImp() || !ind2.getImp() || !ind3.getImp(), Indicator(),
