...
Indicator HAYAKU_API REPLACE(double old_value, double new_value, bool ignore_discard) {
...
void ISlice::_calculate(const Indicator& data) {
    ...
    if (isLeaf()) {
        ...
        HAYAKU_ERROR_IF_RETURN(startix < 0 || size_t(startix) >= total, void(), "start {}, total {}",
                            startix, total);
        ...
        HAYAKU_IF_RETURN(endix < 0 || size_t(endix) > total || startix == endix, void());
    ...
    HAYAKU_ERROR_IF_RETURN(result_index >= 0 && result_index >= data.getResultNumber(), void(),
