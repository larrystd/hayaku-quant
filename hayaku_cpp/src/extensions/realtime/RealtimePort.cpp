...
bool registerRealtimePort(const RealtimePort* port) noexcept {
  if (!port || !port->stopSpot || !port->startStrategySpot ||
      !port->reloadAround || !port->shutdown || !port->quiescent) {
    return false;
  }
  auto& state = portState();
  std::lock_guard<std::mutex> lock(state.mutex);
...
void startRealtimeForStrategy(const RealtimeSpotProcess& process,
  ...
  HAYAKU_CHECK(withRealtimePort([&](const RealtimePort& port) {
                 port.startStrategySpot(process, postProcess, workerNum,
                                        address);
               }),
               "Realtime extension is not installed; Strategy::start(true) "
...
void reloadWithRealtimePaused(void (*reload)()) {
  ...
  if (!withRealtimePort(
          [&](const RealtimePort& port) { port.reloadAround(reload); })) {
    reload();
  }
}
...
void detail::enterRealtimeCallback() noexcept { ++g_realtimeCallbackDepth; }

void detail::leaveRealtimeCallback() noexcept { --g_realtimeCallbackDepth; }
