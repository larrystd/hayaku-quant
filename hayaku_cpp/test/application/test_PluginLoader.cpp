...
class FixtureHandle {
public:
    explicit FixtureHandle(const std::string& filename) : m_handle(openFixture(filename)) {}
...
TEST_CASE("test_PluginLoader_getFileName") {
    ...
    CHECK_EQ(loader.getFileName("absent"), "/missing/hayaku-plugins/libabsent.so");
#elif HAYAKU_OS_OSX
    /** @arg macOS plugins use the lib*.dylib filename convention. */
    CHECK_EQ(loader.getFileName("absent"), "/missing/hayaku-plugins/libabsent.dylib");
#endif
}
...
TEST_CASE("test_PluginLoader_load_v1_and_destroy") {
    ...
    CHECK_EQ(fixtureCount(fixture.get(), "hayakuTestCreateCount"), createCount + 1);
    ...
    REQUIRE_UNARY(loader.load("hayaku_abi_valid", false));
    CHECK_EQ(fixtureCount(fixture.get(), "hayakuTestCreateCount"), createCount + 2);
    CHECK_EQ(fixtureCount(fixture.get(), "hayakuTestDestroyCount"), destroyCount + 1);

    /** @arg A subsequent failed load unloads the current instance. */
    CHECK_UNARY(!loader.load("absent", false));
    CHECK_EQ(fixtureCount(fixture.get(), "hayakuTestDestroyCount"), destroyCount + 2);
...
TEST_CASE("test_PluginLoader_load_legacy") {
    ...
    /** @arg Existing createPlugin-only binaries still load and use the legacy delete path. */
...
TEST_CASE("test_PluginManager_cached_interface_version") {
    ...
    /** @arg A cached object must not satisfy a request for a different interface version,
     * even when its C++ dynamic type is convertible to that interface. */
    CHECK_EQ(manager.getPlugin<VersionTwoPluginInterface>("hayaku_abi_valid", false), nullptr);
}

TEST_CASE("test_PluginLoader_reject_incompatible_abi") {
    PluginLoader loader(getCurrentDir());
    for (const char* name : {"hayaku_abi_wrong_version", "hayaku_abi_wrong_id", "hayaku_abi_no_destroy",
        ...
        /** @arg Invalid ABI, ID, interface version, or missing destroy is rejected before construction. */
