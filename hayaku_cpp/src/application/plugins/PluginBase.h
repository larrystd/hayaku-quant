...
// The C entry points below are optional for binaries built before ABI v1. Do
// not add virtual methods to PluginBase: doing so would change the vtable of
// existing out-of-tree plugins.
...
class PluginBase {
 public:
   ...
   * @details
   * The plugin information is in the json format, containing the name (string),
   * version (int), description (string) and author (string) fields e.g.:
   * {"name":"unknown","version": 1.0,"description":"","author":"unknown"}
   * @return std::string
   */
...
// Opt-in ABI v1. The host checks the version and stable ID before calling
// createPlugin, then calls destroyPlugin before unloading the library. Keep
// HAYAKU_PLUGIN_DEFINE legacy-compatible because external plugins may provide
// their own metadata/destroy symbols alongside that macro.
#define HAYAKU_PLUGIN_DEFINE_V1(plugin, pluginId, interfaceVersion)       \
  ...
  HAYAKU_PLUGIN_EXPORT void destroyPlugin(                                \
      hayaku::PluginBase* instance) noexcept {                            \
    delete instance;                                                      \
  }
