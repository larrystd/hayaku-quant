...
public:
    ...
    void pluginPath(const std::string& plugin_path) noexcept {
        HAYAKU_TRACE_IF_RETURN(!m_plugins.empty(), void(),
                            "Existing loaded plugins, Ignore set plugin path: {}, ", plugin_path);
        m_plugin_path = plugin_path;
    }

    // Invalidates every raw interface pointer returned by getPlugin(). Callers must first stop
    // all plugin services and release every interface pointer and plugin-created object; the lock
    // below protects the map only, not a caller using a pointer after getPlugin() returns.
    void clear() noexcept {
        std::unique_lock<std::shared_mutex> write_lock(m_mutex);
        m_plugins.clear();
    }

    // The returned pointer is borrowed. It is valid only while this manager, the corresponding
    // plugin library and its loaded instance remain alive and clear() is not called.
        ...
        try {
            {
                ...
                if (it != m_plugins.end()) {
                    if (!it->second->supportsInterfaceVersion(
                        ...
                        HAYAKU_WARN_IF(print, "Plugin {} does not match the requested interface version",
                                    pluginname);
                        return nullptr;
                    }
            ...
            if (!loader->load(pluginname, print, pluginInterfaceVersion<PluginInterfaceT>())) {
                HAYAKU_DEBUG("Load plugin {} failed: {}", pluginname, loader->getFileName(pluginname));
                return ret;
            }
            ret = loader->instance<PluginInterfaceT>();
            if (!ret) {
                HAYAKU_WARN_IF(print, "Plugin {} does not implement the requested interface", pluginname);
                return nullptr;
            }
            ...
            HAYAKU_ERROR_IF(print, "Load plugin {} failed: unknown exception", pluginname);
            ret = nullptr;
        }
        return ret;
    }
