...
class HAYAKU_API Factor final {
 ...
 public:
  ...
  /**
   * Only the factor name (+ K-line type) is given, it will try to load the
   * factor from the database automatically
  ...
  /**
   * Construct a new factor (factor name + K-line type is the unique identifier
   * of the factor)
   * @param name factor name
   * @param formula calculation formula indicator, it cannot be changed once
   * created
   * @param ktype K-line type
   * @param brief brief description
   * @param details detailed description
   * @param start_date start date of the factor data
   * @param save_value whether the factor value data needs to be saved
   * persistently
   * @param block block information, a security set; all the securities if it is
   * empty
   * @param recover_type adjustment (recover) type
   */
  Factor(const string& name, const Indicator& formula,
         const KQuery::KType& ktype = KQuery::DAY, const string& brief = "",
         const string& details = "", bool save_value = false,
   ...
   * @param align whether to align the dates (according to the given align_dates
   * or the default trading calendar), false by default
   ...
   * @param check whether to check that the stock belongs to the block specified
   * by itself, false by default
   * @param align_dates the align date list, empty by default
   * @return the calculation result indicator
   */
  Indicator getValue(const Stock& stock, const KQuery& query,
                     bool align = false, bool fill_null = false,
                     bool tovalue = false, bool check = false,
                     const DatetimeList& align_dates = {}) const;

  Indicator getValue(const KData& kdata, bool align = false,
                     bool fill_null = false, bool tovalue = false,
                     bool check = false,
                     const DatetimeList& align_dates = {}) const {
    return getValue(kdata.getStock(), kdata.getQuery(), align, fill_null,
                    tovalue, check, align_dates);
  }

  /**
   * Get the calculation results of the given query condition on the given stock
   * list
   ...
   * @param fill_null whether to fill the null values, false by default
   * @param tovalue whether to convert to values, false by default
   * @param check whether to check that the stock list belongs to the block
   * specified by itself, false by default
   * @param align_dates the align date list, empty by default
   ...
   */
  IndicatorList getAllValues(const KQuery& query, bool align = false,
                             bool fill_null = false, bool tovalue = false,
  ...
  /**
   * Save the factor and all its calculation results to the database; the
   * existing factor is updated, otherwise a new record is inserted
   * @note The factor name is case insensitive, name + ktype is used as the
   * unique identifier
   * @param update_before whether to check and update the existing factor before
   * saving, true by default. Note: it usually must be true, otherwise the data
   * will be wrong, unless you are sure that all the factor values have been
   * updated
   */
  void save_to_db(bool update_before = true);

  /**
   * Save the values of a special factor to the database; its values are not
   * obtained through the indicator calculation, such as PRICELIST, so they need
   * to be set manually
   */
  ...
  /**
   * Delete the factor and its data from the database; note: to prevent
   * misoperation, the values of a special factor are not deleted, they need to
   * be deleted manually
   */
  void remove_from_db();

  /**
   * Load the factor from the database, name + ktype is used as the unique
   * identifier; the current object is not modified if it does not exist
   */
  void load_from_db();
 ...
 private:
  struct Data {
    ...
    bool need_save_value{false};  ///< Whether the factor value data needs to be
                                  ///< saved persistently
    KQuery::RecoverType recover_type{KQuery::RecoverType::NO_RECOVER};

    Data() = default;
    Data(const string& name, const Indicator& formula,
         const KQuery::KType& ktype, const string& brief, const string& details,
         bool need_save_value, const Datetime& start_date, const Block& block,
...
namespace std {
template <>
class hash<hayaku::Factor> {
 public:
  size_t operator()(hayaku::Factor const& factor) const noexcept {
    return factor.hash();
  }
};
...
template <>
struct fmt::formatter<hayaku::Factor> : ostream_formatter {};
#endif
