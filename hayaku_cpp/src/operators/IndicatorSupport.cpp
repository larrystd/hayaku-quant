...
std::vector<double> multi_regression(const Stock& stk, const KQuery& query,
  ...
  HAYAKU_IF_RETURN(kdata.empty(), {});

  size_t factor_count = inds.size();
  HAYAKU_IF_RETURN(factor_count == 0, {});
  ...
  HAYAKU_IF_RETURN(n <= factor_count, {});

  std::vector<size_t> valid_indices;
  valid_indices.reserve(n);
  for (size_t i = 0; i < n; ++i) {
    if (std::isnan(returns[i])) continue;
    bool valid = true;
  ...
  HAYAKU_IF_RETURN(valid_count <= factor_count, {});
 ...
 * @param query the K-line query condition
 * @param inds the indicator list (the independent variables)
 * @return the regression result vector [alpha, beta1, ..., betan, e1, ..., en,
 * RSS, R2]
 */
