...
HAYAKU_API std::ostream& operator<<(std::ostream& out, const MultiFactorBase& mf) {
...
HAYAKU_API std::ostream& operator<<(std::ostream& out, const MultiFactorPtr& mf) {
    if (mf) {
        out << *mf;
    } else {
...
void MultiFactorBase::_checkData() {
    ...
    HAYAKU_CHECK(m_ref_dates.size() >= 2, "The dates len is insufficient! current len: {}",
              m_ref_dates.size());

    HAYAKU_CHECK(m_stks.size() >= 2, "The number of stock is insufficient! current stock number: {}",
              m_stks.size());
}
...
void MultiFactorBase::setRefStock(const Stock& stk) {
    ...
    HAYAKU_CHECK(ref_dates.size() >= 2, "The dates len is insufficient! current len: {}",
              ref_dates.size());
    m_ref_stk = tmp_stk;
    m_calculated.store(false, std::memory_order_relaxed);
}
...
void MultiFactorBase::setRefFactorSet(const FactorSet& factorset) {
    HAYAKU_CHECK(!factorset.isNull() && !factorset.empty(), "Input factor set is null or empty!");
...
unordered_map<string, std::pair<PriceList, size_t>> MultiFactorBase::_buildDummyIndex() {
    ...
    for (const auto& [ind_name, catefory] : m_special_category) {
        ...
        if (blks.empty()) {
            HAYAKU_WARN("Block list ({}) is empty, please check your block category!", catefory);
            continue;
        }
