...
TEST_CASE("test_Signal_clone_cycle_bounds") {
  ...
  /** @arg After clone the cycle boundary must be copied completely, end must
   * not become start */
  SignalPtr c = p->clone();
  CHECK_NE(p, c);
...
TEST_CASE("test_Signal_clone_operator_behavior") {
  ...
  // The parent signal: cycle=true and startCycle already called (an Operator
  // parent holds it)
  ...
  // Before the fix: startCycle(t0, t0) triggers HAYAKU_CHECK(start < close)
  // After the fix: startCycle(t0, t1) passes legally and produces a buy signal
  auto child = SG_Cycle();
  child->setTO(k);
  REQUIRE_NOTHROW(
      child->startCycle(cloned->getCycleStart(), cloned->getCycleEnd()));
