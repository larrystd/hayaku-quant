...
void OptimalSelectorBase::calculate(const internal::StrategyRuntimeList& pf_realSysList, const KQuery& query) {
    ...
    auto dates = getDataRuntime().getTradingCalendar(query, getParam<string>("market"));
      ...
      0, train_ranges.size(), [this, &train_ranges, &dates, query = m_query, trace](size_t i) {
          ...
          for (const auto& sys : m_pro_sys_list) {
              try {
                  ...
                  nsys->reset();
                  if (!std::isnan(value)) {
                      selected_sys_list->emplace_back(StrategyWeight(nsys->clone(), value));
                  }
          ...
          if (!selected_sys_list->empty()) {
              ...
              std::stable_sort(
                ...
                [](const StrategyWeight& a, const StrategyWeight& b) { return a.weight > b.weight; });
          }
