 ...
 * @details
 * A typical ini file format is as follows, where ";" is the line comment
 * symbol: \n [section1] \n ;The first section \n key1 = value1 \n key2 = value2
 * \n
 *     \n
 *     [section2] \n
 ...
 *
 * @note The same section can be defined in different positions, but if the
 * sections at different positions contain an option (key) with the same name,
 *       the value of that option (key) is the last read value. This may cause a
 * potential error. It is recommended not to define the same section in
 * different positions. \n When the configuration information is scattered in
 * multiple files, all of them can be read in by calling the read member method
 * multiple times and then processed uniformly. \n This class does not support
 * the copy operation at present, there is no such requirement for now \n
 *
 * @author fasiondog
 * @date 20100519
 * @ingroup Utilities
...
class HAYAKU_UTILS_API IniParser {
 public:
  ...
  // The reason why the following default value type uses string is that the
  // int/float/double/bool types have no empty object
