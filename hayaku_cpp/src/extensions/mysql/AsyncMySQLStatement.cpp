...
struct AsyncMySQLStatement::Impl {
  ...
  boost::mysql::results
      results;  // Used by async_execute (the case with parameters)
  boost::mysql::execution_state
      exec_state;  // The state used for the streaming reading
  std::vector<boost::mysql::row>
      current_batch;  // The data of the current batch (owned)
  ...
  boost::mysql::field_view getField(int idx) const {
    if (is_streaming) {
      ...
      SQL_CHECK(current_row > 0 && current_row <= rows.size(), -1,
                "Invalid row index!");
...
net::awaitable<void> AsyncMySQLStatement::sub_exec() {
  ...
  try {
    ...
    HAYAKU_ERROR(
        "Execute failed! Error code: {}, Server message: {}, Client message: "
        "{}",
...
void AsyncMySQLStatement::sub_getColumnAsDatetime(int idx, Datetime& item) {
  ...
  try {
    ...
    item =
        Datetime(static_cast<long>(dt.year()), static_cast<long>(dt.month()),
