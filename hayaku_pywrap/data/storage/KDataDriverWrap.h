...
class PyKDataDriver : public KDataDriver {
 ...
 public:
  ...
  bool _init() override { PYBIND11_OVERLOAD(bool, KDataDriver, _init, ); }

  bool isIndexFirst() override {
    PYBIND11_OVERLOAD_PURE(bool, KDataDriver, isIndexFirst, );
  }

  bool canParallelLoad() override { return false; }
  ...
  bool getIndexRangeByDate(const string& market, const string& code,
    ...
    if (!py_func) {
      return KDataDriver::getIndexRangeByDate(market, code, query, out_start,
                                              out_end);
    }
    ...
    if (len(t) != 2) {
      PyErr_SetObject(
          PyExc_ValueError,
          py::str(
              "expected 2-item tuple in call to _getIndexRangeByDate; got {}")
              .format(t)
              .ptr());
    ...
    if (py::isinstance<py::int_>(t[0]) && py::isinstance<py::int_>(t[1])) {
      ...
      throw std::logic_error(
          "expected 2-int tuple in call to _getIndexRangeByDate");
    }
