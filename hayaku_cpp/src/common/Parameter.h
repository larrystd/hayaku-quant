 ...
 * @details In the definition of a class that needs the named parameter
 * settings, add the PARAMETER_SUPPORT macro, e.g.:
 * @code
 * #// C++ example:
 * class Test {
 *     PARAMETER_SUPPORT
 ...
 * @note
 * <pre>
 * Because of the limitation of Python, only the int, bool and double types are
 * supported at present; to add the support of a new type, the following places
 * need to be modified; for an unsupported type an exception is thrown directly
 * at add, please do not catch this exception, so that the program termination
 * caused in this way can complete the types to be supported as soon as
 * possible: 1、Parameter::support 2、std::ostream& operator <<(std::ostream
 * &os, const Parameter& param)
 * 3. AnyToPython, Parameter::add<object> and Parameter::set<object> in
 * _Parameter.cpp 4、getNameValueList
 * 5. The serialization support of Parameter <br/>
 * In Python, to add and modify the parameters a Parameter object instance needs
 * to be created or got first, and through the Parameter instance it is done,
 * afterwards the Parameter instance is set and modified directly, e.g.:
...
class HAYAKU_API Parameter {
 ...
 public:
   ...
   * @note An existing parameter has its value modified, a non-existing one is
   * added
  ...
  /**
   * Get the given parameter value; an exception is thrown when the parameter
  ...
  /**
   * Try to get the given parameter value; the default value is returned when
   * the parameter does not exist or the type does not match
   * @param name parameter name
...
/**
 * It supports the parameter checking and the change notification of a custom
 * class The subclass needs to implement (overload) the following virtual
 * function interfaces: virtual void _checkParam(const string& name) const The
 * base class needs to implement the following interfaces: void
 * baseCheckParam(const string& name) const void paramChanged() Note:
 * paramChanged/checkParam/_checkParam generally do not need to be exposed in
 * python, the python subclass can do the checking itself during the
 * initialization
 */
...
#if HAYAKU_SUPPORT_SERIALIZATION
BOOST_CLASS_VERSION(::hayaku::ParamItemRecord, 1)
#endif
