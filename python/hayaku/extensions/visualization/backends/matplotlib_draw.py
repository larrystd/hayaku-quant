...
from hayaku.core import constant, isnan, Indicator, KData, IF, ALIGN
from hayaku._support.notebook import in_interactive_session
...
def kplot(kdata, new=True, axes=None, colorup='r', colordown='g'):
    ...
    text = '{} Open:{:.2f} High:{:.2f} Low:{:.2f} Close:{:.2f} Change:{:.2f}%'.format(
...
def mkplot(kdata, new=True, axes=None, colorup='r', colordown='g', ticksize=3):
    ...
    last_record = kdata[-1]
    color = 'r' if last_record.close > kdata[-2].close else 'g'
    text = '{} Open:{:.2f} High:{:.2f} Low:{:.2f} Close:{:.2f}'.format(
        last_record.datetime.number / 10000, last_record.open, last_record.high, last_record.low, last_record.close)
    axes.text(
        0.99, 0.97, text, horizontalalignment='right', verticalalignment='top', transform=axes.transAxes, color=color
    )
...
def iheatmap(ind, axes=None):
    ...
    if len(ind) == 0:
        hayaku_error("The indicator length is 0; the indicator should have been calculated (i.e. it has values)")
        return

    dates = ind.get_datetime_list()
    if len(dates) == 0:
        hayaku_error("Failed to get the date list! The indicator should be a time series")
        return
    ...
    if len(monthly) < 2:
        hayaku_warn("Insufficient monthly data!")
        return
...
def tm_performance(tm, query: Query, ref_stk: Stock = None, ext: bool = True, log: bool = False):
    ...
    t1 = 'Total Invested Assets: {:<.2f}    Current Total Assets: {:<.2f}    Current Profit: {:<.2f}'.format(
        invest_total, cur_fund, cur_fund - invest_total)
    t2 = ('Current Strategy Return: {:<.2f}%    Annualized Return: {:<.2f}%    Max Drawdown: {:<.2f}%'
          '    Current Drawdown from Peak: {:<.2f}%').format(
            funds_return[-1] * 100 - 100, per["Account Avg Annual Return %"], max_pullback, mdd_current)
    t3 = 'Win Rate: {:<.2f}%    Avg Win/Avg Loss: 1 : {:<.2f}    Sharpe Ratio: {:<.2f}'.format(
    ...
    ref_return.plot(axes=ax1,
                    legend_on=True,
                    label='{} Return Curve'.format(f'{ref_stk.name}({ref_stk.market_code})'))
    funds_return.plot(axes=ax1,
                      legend_on=True,
                      label='{} Cumulative Return {:<.2f}%'.format(tm.name, funds_return[-1] * 100.))
    ax1.set_title('Account({}) Cumulative Return'.format(tm.name))
    label = t1 + '\n\n' + t2 + '\n\n' + t3
    ax2.text(0,
             1,
             text,
...
def tm_heatmap(tm, start_date, end_date=None, axes=None, show_high_low=False):
    ...
    if len(dates) == 0:
        hayaku_error("No data, please check the date range! start_date={}, end_date={}", start_date, end_date)
        return

    funds = tm.get_funds_curve(dates)
    if len(funds) == 0:
        hayaku_error(
            "Failed to get the tm return curve, please check the tm initial date!"
    ...
    # Set the title and the axis labels
    axes.set_title('{} Year-Month Return (%) Heatmap'.format(tm.name))
    axes.set_xlabel('Month')
    axes.set_ylabel('Year')
...
def tm_year_profit(tm, start_date, end_date=None, axes=None, show_high_low=True):
    ...
    if len(yearly) < 1:
        hayaku_warn("Insufficient yearly data!")
        return
        ...
        if show_high_low:
            if max_ret > ret:
                axes.bar(x_pos[i], max_ret - ret, bottom=ret, width=bar_width,
                         color=max_color, alpha=0.4, label='Highest Return Range' if i == 0 else "")
            if min_ret < ret:
                axes.bar(x_pos[i], ret - min_ret, bottom=min_ret, width=bar_width,
                         color=min_color, alpha=0.7, label='Lowest Return Range' if i == 0 else "")
    ...
    axes.set_title('{} Annual Return (%) Bar Chart'.format(tm.name))
...
def sys_heatmap(sys, axes=None):
    ...
    """
    hayaku_check(sys.tm is not None, "The system has no trade manager initialized")
    query = sys.query
...
def RGB(r: int, g: int, b: int):
    hayaku_check(0 <= r <= 255 and 0 <= g <= 255 and 0 <= b <= 255, "r,g,b must in [0,255]!")
...
def STICKLINE(cond: Indicator, price1: Indicator, price2: Indicator, width: float = 2.0,
    ...
    """
    hayaku_check(cond is not None and price1 is not None and price2 is not None, "cond, price1, price2 cannot be None")

    if kdata is not None:
        cond = cond(kdata)
        price1 = price1(kdata)
        price2 = price2(kdata)
    hayaku_check(len(cond) == len(price1) == len(price2), "cond, price1, price2 length not match")
    hayaku_warn_if(len(cond) <= 0, "cond, price1, price2 length <=0")
...
def DRAWBAND(val1: Indicator, color1='m', val2: Indicator = None, color2='b', kdata=None, alpha=0.2, new=False, axes=None, linestyle='-'):
    ...
    """
    hayaku_check(val1 is not None, "val1 cannot be None")
    ...
    hayaku_check(len(val1) == len(val2), "val1, val2 length not match")
    hayaku_warn_if(len(val1) <= 0, "val1, val2 length <=0")
...
def PLOYLINE(cond: Indicator, price: Indicator, kdata: KData = None, color: str = 'm', linewidth=1.0, new=False, axes=None, *args, **kwargs):
    ...
    """
    hayaku_check(cond is not None and price is not None, "cond, price cannot be None")

    ind = IF(cond, price, constant.null_price)
    if kdata is not None:
        ind = ind(kdata)
        price = price(kdata)
    hayaku_check(len(ind) == len(price), "cond, price length not match!")
    hayaku_warn_if(len(ind) <= 0, "cond length <=0")
...
def DRAWLINE(cond1: Indicator, price1: Indicator, cond2: Indicator, price2: Indicator, expand: int = 0, kdata: KData = None, color: str = 'm', new=False, axes=None, *args, **kwargs):
    ...
    right.
        ...
        axes (_type_, optional): the specified axes. Defaults to None.
    """
    hayaku_check(cond1 is not None and cond2 is not None and price1 is not None and price2 is not None,
              "cond1, cond2, price1, price2 cannot be None")
    hayaku_check(expand in (0, 1, 10, 11), "expand must be 0, 1, 10 or 11")
    ...
    if kdata is not None:
        ...
        cond2 = cond2(kdata)
        price2 = price2(kdata)
    hayaku_check(len(cond1) == len(cond2) == len(price1) == len(price2), "cond1, cond2, price1, price2 length not match")
    hayaku_warn_if(len(cond1) <= 0, "cond1, cond2, price1, price2 length <=0")
...
def DRAWTEXT(cond: Indicator, price: Indicator, text: str, kdata: KData = None, color: str = 'm', new=False, axes=None, *args, **kwargs):
    ...
    if kdata is not None:
        cond = cond(kdata)
        price = price(kdata)
    hayaku_check(len(cond) == len(price), "cond, price length not match")
    hayaku_warn_if(len(cond) <= 0, "cond length <=0")
...
def DRAWTEXT_FIX(cond: Indicator, x: float, y: float,  type: int, text: str, kdata: KData = None, color: str = 'm', new=False, axes=None, *args, **kwargs):
    ...
    last trading day is greater than 8%.
        ...
        axes (_type_, optional): the specified axes. Defaults to None.
    """
    hayaku_check(cond is not None, "cond cannot be None")
    if kdata is not None:
        cond = cond(kdata)
    hayaku_warn_if(len(cond) <= 0, "cond length <=0")
...
def DRAWNUMBER(cond: Indicator, price: Indicator, number: Indicator, kdata: KData = None, color: str = 'm', new=False, axes=None, *args, **kwargs):
    ...
    if kdata is not None:
        ...
        price = price(kdata)
        number = number(kdata)
    hayaku_check(len(cond) == len(price), "cond, price, number length not match")
    hayaku_warn_if(len(cond) <= 0, "cond length <=0")
...
def DRAWSL(cond: Indicator, price: Indicator, slope: Union[Indicator, float, int], length: Union[Indicator, float, int], direct: int, kdata: KData = None, color: str = 'm', new=False, axes=None, *args, **kwargs):
    ...
    """
    hayaku_check(cond is not None and price is not None, "cond, price cannot be None")
    hayaku_check(direct in (0, 1, 2), "direct must be 0,1,2")
...
def DRAWICON(cond: Indicator, price: Indicator, type: int, kdata: KData = None, new=False, axes=None, *args, **kwargs):
    ...
    SHOWICONS() can be used to display all the built-in icons.
        ...
        axes (_type_, optional): draw in the specified axes. Defaults to None.
    """
    DRAWIMG(cond, price, f'{ICON_PATH}/{type}.png', kdata, new, axes, *args, **kwargs)
