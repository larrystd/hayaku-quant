...
#include "operators/IndicatorImpBuffer.h"
...
TEST_CASE("test_IndicatorImpBuffer_modifiers") {
  ...
  CHECK_EQ(
      buf[3],
      2.0);  // Fixed: the original 4th element (2.5) was erased, so it is 2.0
  CHECK_EQ(*it,
           2.0);  // Fixed: the returned iterator points to the next element

  // Test erasing a range
  it = buf.erase(buf.begin() + 1, buf.begin() + 3);
  CHECK_EQ(buf.size(), 4);
  CHECK_EQ(buf[1],
           2.0);  // Fixed: after erasing [1.5, 2.5] the 2nd element is 2.0
  CHECK_EQ(
      *it,
      2.0);  // Fixed: the iterator points to the first element after the range
...
TEST_CASE("test_IndicatorImpBuffer_missing_coverage") {
  ...
  // Test the boundary case of an empty container (note: front/back on an empty
  // container is undefined behavior)
  IndicatorImpBuffer empty_buf;
  // empty_buf.front() and empty_buf.back() are not tested, because that is
  // undefined behavior
