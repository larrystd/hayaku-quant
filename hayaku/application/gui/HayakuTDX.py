...
from PySide6.QtGui import QIcon, QTextCursor, QFont, QPalette
...
from hayaku.application.gui.generated.MainWindow import *
from hayaku.application.gui.tasks.EscapetimeThread import EscapetimeThread
from hayaku.application.gui.tasks.UseTdxImportToH5Thread import UseTdxImportToH5Thread
from hayaku.application.gui.tasks.ImportTdxToH5Task import ImportTdxToH5Task
from hayaku.application.gui.tasks.UsePytdxImportToH5Thread import UsePytdxImportToH5Thread
from hayaku.application.gui.tasks.UseQmtImportToH5Thread import UseQmtImportToH5Thread
# from hayaku.application.gui.tasks.CollectToMySQLThread import CollectToMySQLThread
# from hayaku.application.gui.tasks.CollectToMemThread import CollectToMemThread
from hayaku.application.gui.tasks.CollectSpotThread import CollectSpotThread
from hayaku.application.gui.tasks.SchedImportThread import SchedImportThread
from hayaku.application.gui.spot_server import release_nng_senders

from hayaku import Datetime, TimeDelta
from hayaku.core import (can_upgrade, get_latest_version_info, is_valid_license,
                         get_expire_date)
from hayaku.application.config import generator as hayaku_config_template
...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def getUserConfigDir(self):
        return os.path.expanduser('~') + '/.hayaku'

    def getHayakuConfigFileName(self):
        return self.getUserConfigDir() + '/hayaku.ini'
    ...
    def saveConfig(self):
        ...
        filename = self.getHayakuConfigFileName()
        old_config = ConfigParser()
        if os.path.exists(filename):
            old_config.read(filename, encoding='utf-8')
        old_reload_time = "00:00"
        if old_config.has_section('hayaku'):
            old_reload_time = old_config.get('hayaku', 'reload_time', fallback="00:00")
        ...
        if current_config.getboolean('hdf5', 'enable', fallback=True):
            ...
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(
                    hayaku_config_template.hdf5_template.format(
                        ...
                        quotation_server=current_config.get(
                            'collect', 'quotation_server', fallback='ipc:///tmp/hayaku_real.ipc'),
            ...
            with open(filename, 'w', encoding="utf-8") as f:
                f.write(
                    hayaku_config_template.mysql_template.format(
                        dir=data_dir,
            ...
            with open(filename, 'w', encoding="utf-8") as f:
                f.write(
                    hayaku_config_template.clickhouse_template.format(
                        dir=data_dir,
                        reload_time=old_reload_time,
                        lazy_preload=current_config.getboolean('lazy_preload', 'enable', fallback=False),
                        quotation_server=current_config.get(
    ...
    def initUI(self):
        ...
        if self._capture_output:
            ...
            if self._stream is not None:
                ...
                # python构建时丢失stderr通道，导致安装后的hayakutdx执行时，
        ...
        icon = QIcon(f"{current_dir}/resources/images/hayaku_small.png")
        ...
        self.quotation_server = import_config.get(
            'collect', 'quotation_server', fallback='ipc:///tmp/hayaku_real.ipc')
        hayaku_warn_if(len(self.quotation_server) < 3, "Invalid quotation server addr!")
        if self.quotation_server[:3] == "ipc":
            self.quotation_server = 'ipc:///tmp/hayaku_real.ipc'
        ...
        if msg_name == 'ESCAPE_TIME':
            ...
            if msg_task_name == 'INFO':
                ...
                if can_upgrade():
                    ...
                    self.import_detail_textEdit.append(
                        "Hayaku 新版本 ({}) 已发布，建议更新".format(release_info['version']))
                    self.import_detail_textEdit.append("更新命令: pip instal hayaku --upgrade")
    ...
    def on_start_import_pushButton_clicked(self):
        ...
        now = hayaku.Datetime.now()
        today = hayaku.Datetime.today()
        if now.day_of_week() not in (0, 6) and hayaku.TimeDelta(0, 8, 30) < now - today < hayaku.TimeDelta(0, 15, 45):
