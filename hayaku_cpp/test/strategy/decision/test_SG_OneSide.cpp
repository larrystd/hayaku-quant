...
TEST_CASE("test_SG_OneSide") {
  ...
  sg->setTO(k);
  auto sg_expect =
      SG_Bool(CLOSE() > REF(CLOSE(), 1), CLOSE() < REF(CLOSE(), 1));
  sg_expect->setTO(k);
  for (size_t i = 0; i < k.size(); i++) {
    CHECK_EQ(sg->shouldBuy(k[i].datetime), sg_expect->shouldBuy(k[i].datetime));
    CHECK_EQ(sg->shouldSell(k[i].datetime),
             sg_expect->shouldSell(k[i].datetime));
  }

  sg->setParam<bool>("alternate", false);
  sg_expect =
      SG_Bool(CLOSE() > REF(CLOSE(), 1), CLOSE() < REF(CLOSE(), 1), false);
  sg->setTO(k);
  sg_expect->setTO(k);
  for (size_t i = 0; i < k.size(); i++) {
    CHECK_EQ(sg->shouldBuy(k[i].datetime), sg_expect->shouldBuy(k[i].datetime));
