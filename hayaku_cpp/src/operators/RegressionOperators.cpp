...
void IIc::_calculate(const Indicator& inputInd) {
    ...
    HAYAKU_ERROR_IF_RETURN(stk_count < 2, void(),
                        "The number(>=2) of stock is insufficient! current stock number: {}",
                        stk_count);
    for (size_t i = 0; i < stk_count; i++) {
        HAYAKU_ERROR_IF_RETURN(m_stks[i].isNull(), void(), "The [{}] stock is null!", i);
    }
    ...
    Indicator (*spearman)(const Indicator&, const Indicator&, int, bool) = hayaku::SPEARMAN;
...
Indicator HAYAKU_API IC(const StockList& stks, int n, bool spearman, bool strict) {
...
Indicator HAYAKU_API IC(IndicatorList inds, IndicatorList returns, int n, bool use_spearman,
                     bool strict) {
    HAYAKU_CHECK(n >= 1, "The n({}) must be greater than 1!", n);
    HAYAKU_CHECK(inds.size() == returns.size(),
