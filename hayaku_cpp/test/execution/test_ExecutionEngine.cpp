...
TEST_CASE("test_ExecutionEngine_constructor") {
  /** @arg account construction is explicit and independent of the removed
   * legacy facade. */
  ExecutionEngine engine(
      AccountConfig(Datetime(199901010000), 100000.0, TC_Zero(), "EXECUTION"));
...
TEST_CASE("test_ExecutionEngine_owns_runtime") {
  /** @arg the new construction path owns a real account runtime without a
   * compatibility facade */
  const AccountId requestedId(42);
  ExecutionEngine engine(AccountConfig(Datetime(199901010000), 100000.0,
                                       TC_Zero(), "OWNED", 2, false, false,
                                       requestedId));
  ...
  ExecutionReport report = engine.submit(
      OrderRequest(OrderSide::BUY, Datetime(199911170000), stock, 27.18, 100,
                   20.0, 35.0, 27.0, OrderOrigin::SIGNAL, "runtime-owned buy"));
...
TEST_CASE("test_ExecutionEngine_submit_long_order") {
  ...
  OrderRequest buyRequest(OrderSide::BUY, Datetime(199911170000), stock, 27.18,
                          100, 20.0, 35.0, 27.0, OrderOrigin::SIGNAL,
                          "engine buy");
  ...
  OrderRequest sellRequest(OrderSide::SELL, Datetime(199911180000), stock,
                           27.20, MAX_DOUBLE, 0.0, 0.0, 27.20,
                           OrderOrigin::STOP_LOSS, "engine sell");
...
TEST_CASE("test_ExecutionEngine_submit_rejected_order") {
  ...
  /** @arg normal business rejection is represented by status, not a facade
   * exception */
  ExecutionReport report = engine.submit(
      OrderRequest(OrderSide::BUY, Datetime(199911170000), stock, 27.18, 100));
...
TEST_CASE("test_ExecutionEngine_submit_short_order") {
  Stock stock = getDataRuntime().getStock("sh600000");
  ExecutionEngine engine(AccountConfig(Datetime(199901010000), 100000.0,
                                       TC_Zero(), "EXECUTION", 2, false, true));

  /** @arg enabled securities lending is handled inside the owned runtime */
  ExecutionReport sellReport = engine.submit(OrderRequest(
      OrderSide::SELL_SHORT, Datetime(199911170000), stock, 10.0, 100));
  CHECK_UNARY(sellReport.filled());
  ...
  ExecutionReport buyReport = engine.submit(OrderRequest(
      OrderSide::BUY_SHORT, Datetime(199911180000), stock, 10.0, MAX_DOUBLE));
  CHECK_UNARY(buyReport.filled());
...
TEST_CASE("test_ExecutionBrokerPort_fetchAssetInfoFromBroker") {
  auto account = internal::makeExecutionAccount(
      AccountConfig(Datetime(199901010000), 0.0));
  auto brokerPort =
  ...
  /** @arg a live broker snapshot replaces cash and positions in the owned
   * ledger */
