...
from hayaku._support.singleton import SingletonType
from hayaku._support.checks import checkif
...
class HubManager(metaclass=SingletonType):
    ...
    def __init__(self):
        ...
        hayaku_dir = '{}/.hayaku'.format(usr_dir)
        if not os.path.lexists(hayaku_dir):
            os.mkdir(hayaku_dir)

        # Create the repository database
        engine = create_engine("sqlite:///{}/.hayaku/hub.db".format(usr_dir))
    ...
    @dbsession
    def setup_hub(self):
        """Initialize the default hayaku strategy repository"""
        usr_dir = os.path.expanduser('~')
        ...
        if self.remote_cache_dir is None:
            self.remote_cache_dir = "{}/.hayaku/hub_cache".format(usr_dir)
        ...
        # Check and download the default hayaku strategy repository; hayaku_hub avoids the module
        # name conflicting with hayaku when importing. Use gitee for Chinese locales and GitHub for
        # the others.
        hayaku_hub_path = self._session.query(HubModel.local).filter(HubModel.name == 'default').first()
        if hayaku_hub_path is None:
    ...
    def download_remote_hub(self, local_dir, url, branch):
        print('Downloading the hayaku strategy repository to: "{}"'.format(local_dir))
...
try:
    ...
    HubManager().logger.warning("Unable to initialize the hayaku strategy repository! {}".format(e))
...
if __name__ == "__main__":
    ...
    remove_hub('dev')
    add_local_hub('dev', r'D:\workspace\hayaku_hub')
    # update_hub('test1')
    # update_hub('default')
