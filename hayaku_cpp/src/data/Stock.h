...
class Stock {
 ...
 public:
  ...
  /**
   * Get the internal id, usually used as a map key. The id is actually the
   * memory address of m_data.
  ...
  void atom(double num) { return minTradeNumber(num); }
   ...
   *       - Client mode (IPC): at startup, the full weight set is materialized
   * according to the load_stock_weight config (the source is the shared-memory
   * snapshot published by the main process: IpcBaseInfoDriver shm first,
   * falling back to IPC/local when not covered), so getWeight hits the local
   * cache directly. Only when the config is disabled (load_stock_weight=false),
   * or for securities that were not materialized (added by addStock, newly
   * constructed, ...), the whole weight list of the security is read through
   * the driver on first access and then cached (an empty result is cached as
   *         well, to avoid repeated empty queries).
   *       - shm server role: after the base info snapshot containing finance
   * data is published, DataRuntime::releaseShmServerBaseInfoCache() releases
   * the local weight cache of each security to reclaim memory (clients read
   * through shared memory, so the server does not need to keep a copy). Later
   * access to a released security (IPC fallback reply, or the in-process API)
   * lazily reloads and caches it again, so the result stays correct.
   *       - Plain standalone main process: only the local cache materialized at
   * startup is read, with no lazy-loading fallback.
   */
  ...
  /** Get the market value at the given datetime, i.e. the close price of the
   * last record whose date is not later than the given datetime; 0 if the
   * security is no longer valid */
  price_t getMarketValue(const Datetime&, KQuery::KType) const;

  /**
   * Get the K-line index range that matches the condition specified by the
   * query
   ...
   */
  bool getIndexRange(const KQuery& query, size_t& out_start,
                     size_t& out_end) const;

  /** Get the K-line record at the given index; returns KRecord::NullRecord if
   * pos is invalid */
  KRecord getKRecord(size_t pos,
                     const KQuery::KType& dataType = KQuery::DAY) const;

  /** Get the KRecord of the given date according to the data type (daily /
   * weekly / ...) */
  KRecord getKRecord(const Datetime&,
                     const KQuery::KType& ktype = KQuery::DAY) const;

  /** Get the K-line data */
  KData getKData(const KQuery&) const;

  /**
   * Get the KRecordList that matches the query condition; not recommended for
   * direct use in a client
   ...
   * @param category the sector category; if empty, the sectors of all
   ...
   * @note Returns a copy of the historical financial records. The main process
   * (not in client mode) returns a copy of its materialized internal cache;
   * after that cache is released by
   * DataRuntime::releaseShmServerBaseInfoCache() (invoked by the shm server
   * role once the snapshot containing finance data has been published), it is
   * lazily reloaded on demand. In client mode (IPC) it is not materialized
   * locally; instead it is read on demand through the base info driver from the
   * shared-memory snapshot published by the main process (IpcBaseInfoDriver shm
   * first, falling back to IPC/local when not covered), so that it does not
   * duplicate the snapshot in the client's memory.
   */
  vector<HistoryFinanceInfo> getHistoryFinance() const;

  /**
   * Get the trading calendar of its own market (not the trading dates of the
   * security itself)
   * @param query
   * @return DatetimeList
   */
  ...
  /**
   * Whether the given time falls inside the trading session; the date part is
   * ignored and only the hour/minute/second is checked
  ...
  /** Get a full copy of the cached K-line data of the given type; empty if not
   * cached */
   ...
   * @note Call with care; usually used by an external data source to set the
   * data directly
   */
  void setKRecordList(const KRecordList& ks,
                      const KQuery::KType& ktype = KQuery::DAY);
  void setKRecordList(KRecordList&& ks,
                      const KQuery::KType& ktype = KQuery::DAY);
 ...
 private:
  bool _getIndexRangeByIndex(const KQuery&, size_t& out_start,
                             size_t& out_end) const;

  // The following functions are basic operations guarded by a read lock
  const KRecord& _getKRecordFromBuffer(size_t pos,
  ...
  bool _getIndexRangeFromBuffer(const KQuery& query, size_t& out_start,
                                size_t& out_end) const;
  ...
  // Only called by DataRuntime during initialization
  void setPreload(const vector<KQuery::KType>& preload_ktypes);

  void loadKDataToBufferFromKRecordList(const KQuery::KType& ktype,
...
struct Stock::Data {
  ...
  // enabled; set on the
  //   first lazy load for securities that were not materialized, e.g. added by
  //   addStock or newly constructed, when the config is disabled;
  // - shm server role: set to false by releaseShmServerBaseInfoCache() after
  // the snapshot
  //   containing finance data is published (the cache has been released and the
  //   memory returned), then set again after the next access lazily reloads it
  //   through the driver;
  ...
  Data(const string& market, const string& code, const string& name,
       uint32_t type, bool valid, const Datetime& startDate,
       const Datetime& lastDate, price_t tick, price_t tickValue, int precision,
...
/**
 * Print the Stock information, e.g. Stock(market, code, name, type, valid,
...
/**
 * Get a Stock instance through the active data runtime.
 * @param querystr in the form of "market abbreviation + security code", e.g.
 * "sh000001"
 * @return the corresponding security instance; Null<Stock>() if it does not
 ...
 * the copy constructor, which is inefficient */
...
namespace std {
template <>
class hash<hayaku::Stock> {
 public:
  size_t operator()(hayaku::Stock const& stk) const noexcept {
    return stk.id();
  }
};
}  // namespace std
...
template <>
struct fmt::formatter<hayaku::Stock> : ostream_formatter {};
#endif
