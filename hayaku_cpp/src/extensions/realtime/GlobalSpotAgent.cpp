...
void releaseGlobalSpotAgent() {
    ...
    if (g_spot_agent) {
        if (g_spot_agent->isInCallback()) {
            ...
            g_spot_agent->stop();
            HAYAKU_ERROR("releaseGlobalSpotAgent called from a spot callback; final release deferred");
            return;
        }
...
void startSpotAgent(bool print, size_t worker_num, const string& addr) {
    ...
    if (!g_spot_agent_handlers_installed.exchange(true, std::memory_order_acq_rel)) {
...
const RealtimePort g_realtimePort{stopSpotAgent, startStrategySpot, reloadAroundSpot,
                                  shutdownSpotRuntime, realtimePortQuiescent};

// The extension is linked for process lifetime. Explicit dynamic unload must first
// stop all services and successfully unregister this port before dlclose.
[[maybe_unused]] const bool g_realtimePortRegistered = registerRealtimePort(&g_realtimePort);
