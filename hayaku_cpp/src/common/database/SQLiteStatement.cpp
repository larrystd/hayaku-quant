...
SQLiteStatement::SQLiteStatement(DBConnectBase *driver,
    ...
    : SQLStatementBase(driver, sql_statement),
      ...
      m_stmt(NULL) {
  int status = sqlite3_prepare_v2(m_db, m_sql_string.c_str(),
                                  int(m_sql_string.size() + 1), &m_stmt, NULL);
  if (status != SQLITE_OK) {
    sqlite3_finalize(m_stmt);
    SQL_THROW(status, "Failed prepare sql statement: {}! error msg: {}",
              m_sql_string, sqlite3_errmsg(m_db));
  }

  HAYAKU_CHECK(m_stmt != 0, "Invalid SQL statement: {}", m_sql_string);
}
...
void SQLiteStatement::sub_bindText(int idx, const std::string &item) {
  _reset();
  int status = sqlite3_bind_text(m_stmt, idx + 1, item.c_str(),
...
void SQLiteStatement::sub_bindText(int idx, const char *item, size_t len) {
  _reset();
  int status =
      sqlite3_bind_text(m_stmt, idx + 1, item, (int)len, SQLITE_TRANSIENT);
  SQL_CHECK(status == SQLITE_OK, status, "{}", sqlite3_errmsg(m_db));
}
...
void SQLiteStatement::sub_bindBlob(int idx, const std::string &item) {
  _reset();
  int status = sqlite3_bind_blob(m_stmt, idx + 1, item.data(), (int)item.size(),
                                 SQLITE_TRANSIENT);
  SQL_CHECK(status == SQLITE_OK, status, "{}", sqlite3_errmsg(m_db));
}
...
void SQLiteStatement::sub_getColumnAsText(int idx, std::string &item) {
  const char *data =
      reinterpret_cast<const char *>(sqlite3_column_text(m_stmt, idx));
  item = (data != 0) ? std::string(data) : std::string();
}

void SQLiteStatement::sub_getColumnAsBlob(int idx, std::string &item) {
  const char *data =
      static_cast<const char *>(sqlite3_column_blob(m_stmt, idx));
  if (data == NULL) {
