...
// The HttpConnection class definition - a reusable connection for the
// connection pool
struct HttpConnection {
  ...
  SocketType& lowest_layer() {
    ...
    HAYAKU_THROW("Socket not initialized");
  }
#else
  explicit HttpConnection(const Parameter& params)
      : last_used_time(std::chrono::steady_clock::now()) {}
  ...
  bool is_open() const { return socket && socket->is_open(); }
...
std::string AsioHttpClient::_buildURI(const std::string& path,
  ...
  if (!path.empty()) {
    ...
    // URL encode the path segment by segment (keeping / as a path separator,
    // not encoded) For example: "ipquery/index" -> encoded as "ipquery/index"
    ...
    while (pos < path_view.size()) {
      ...
      if (slash_pos == std::string_view::npos) {
        ...
        if (slash_pos > pos) {
          std::string segment =
              std::string(path_view.substr(pos, slash_pos - pos));
          uri_stream << url_escape(segment.c_str());
        }
...
net::awaitable<std::vector<tcp::endpoint>> AsioHttpClient::_resolveDNS() {
...
#if HAYAKU_OS_OSX || HAYAKU_OS_IOS
  // macOS uses the native getaddrinfo way (the beast resolution has a known
  // issue and would hang)
  ...
  for (struct addrinfo* ai = res; ai != nullptr; ai = ai->ai_next) {
    if (ai->ai_family == AF_INET) {
      ...
      dns_endpoints.push_back(tcp::endpoint(net::ip::make_address_v4(v4_bytes),
                                            ntohs(sin->sin_port)));
      ...
      dns_endpoints.push_back(tcp::endpoint(net::ip::make_address_v6(v6_bytes),
  ...
  struct ResolveOp {
    std::shared_ptr<tcp::resolver>
        resolver;  // Changed to a shared_ptr to extend the lifetime
    std::string host, port;
    tcp::resolver::results_type endpoints;
    net::error_code ec;
    bool done = false;

    ResolveOp(std::shared_ptr<tcp::resolver> r, const std::string& h,
              const std::string& p)
        : resolver(r), host(h), port(p) {}

    net::awaitable<net::error_code> run() {
      auto [e, eps] = co_await resolver->async_resolve(
          host, port, net::as_tuple(net::use_awaitable));
      ec = e;
  ...
  // Check whether it was cancelled due to a timeout (operation_aborted means it
  // was cancelled by cancel())
  if (resolve_result == boost::asio::error::operation_aborted) {
    HAYAKU_THROW_EXCEPTION(HttpTimeoutException, "DNS resolve timeout");
  }

  if (resolve_result) {
    HAYAKU_THROW("DNS resolve failed: {}", resolve_result.message());
  }
  ...
  if (dns_endpoints.empty()) {
    HAYAKU_THROW("No valid endpoints from DNS resolve");
  }
...
AsioHttpClient::_getConnection() {
  ...
  HAYAKU_CHECK(conn_ptr != nullptr, "Failed to get connection from pool");
  ...
  if (!conn_ptr->is_open()) {
    ...
    if (is_https_) {
      ...
      for (const auto& endpoint : conn_ptr->endpoints) {
        ...
        struct ConnectOp {
          ...
          net::awaitable<net::error_code> run() {
            ...
            captured_ec = ec;
            co_return ec ? ec
                         : (socket->is_open() ? net::error_code()
                                              : net::error::make_error_code(
                                                    net::error::not_connected));
          }
        };

        // Start the timer and the connection operation
        timer.async_wait([&connect_completed,
                          &conn_ptr](const net::error_code& ec) {
      ...
      // The SSL handshake (with a timeout)
      {
        ...
        timer.async_wait([&handshake_completed,
                          &conn_ptr](const net::error_code& ec) {
        ...
        SslHandshakeOp handshake_op{&conn_ptr->ssl_socket.value(),
                                    handshake_completed, captured_ec};
        ...
        // Check whether it was cancelled due to a timeout
        if (captured_ec == boost::asio::error::operation_aborted) {
          HAYAKU_THROW_EXCEPTION(HttpTimeoutException, "SSL handshake timeout");
        }

        if (captured_ec) {
          HAYAKU_THROW("SSL handshake failed: {}", captured_ec.message());
        }
      }
#endif
    } else {
      ...
      for (const auto& endpoint : conn_ptr->endpoints) {
        ...
        // Start the timer and the connection operation
        timer.async_wait(
            [&connect_completed, &conn_ptr](const net::error_code& ec) {
        ...
        // The connection failed without a timeout, continue trying the next
        // endpoint
  ...
  // The existing connection is reused with no extra operation; the connection
  // pool manages its lifetime automatically It is enough to update
  // last_used_time after every use
  conn_ptr->last_used_time = std::chrono::steady_clock::now();

  // A reused connection does not need the socket options to be set here again,
  // because they were already set in _connect or when the connection was
  // established before. When a connection is reused its socket state is
  // preserved.

  co_return std::make_pair(conn_ptr, is_new_connection);
}

// Create the socket (a variant is used to store an ordinary socket or an SSL
// socket)
...
net::awaitable<void> AsioHttpClient::_connect(
  ...
  for (const auto& endpoint : dns_endpoints) {
...

    {
      ...
      // Start the timer and the connection operation
      timer.async_wait(
          [&connect_completed, &socket_variant](const net::error_code& ec) {
  ...
  if (is_https_) {
    ...
    // Start the timer and the handshake operation
    timer.async_wait(
        [&handshake_completed, &socket_variant](const net::error_code& ec) {
...
net::awaitable<AsioHttpResponse> AsioHttpClient::async_request(
  ...
  try {
    // Get a connection from the connection pool (the DNS cache and the
    // connection reuse are handled automatically)
    auto [conn, is_new] = co_await _getConnection();
    HAYAKU_CHECK(conn != nullptr, "Failed to get connection from pool");
    ...
    // Send the request (with a timeout)
    {
      ...
      if (conn->ssl_socket) {
        ...
        // Check whether it was cancelled due to a timeout (operation_aborted
        // means it was cancelled by cancel())
        if (write_ec == boost::asio::error::operation_aborted) {
          HAYAKU_THROW_EXCEPTION(HttpTimeoutException, "HTTP write timeout");
        }

        if (write_ec) {
          HAYAKU_THROW("HTTP write failed: {}", write_ec.message());
        }
      } else {
#endif
        struct WriteOp {
          ...
          net::awaitable<std::pair<net::error_code, std::size_t>> run() {
            auto [ec, bytes] = co_await http::async_write(
                sock, req, net::as_tuple(net::use_awaitable));
            completed_flag = true;
            co_return std::make_pair(ec, bytes);
          }
        };
...

    {
      ...
      if (conn->ssl_socket) {
        ...
        auto read_op =
            ReadOp{*conn->ssl_socket, buffer, res, read_completed, captured_ec};
        ...
        // means it was cancelled by cancel())
        if (captured_ec == boost::asio::error::operation_aborted) {
          HAYAKU_THROW_EXCEPTION(HttpTimeoutException, "HTTP read timeout");
        }

        if (captured_ec) {
          HAYAKU_THROW("HTTP read failed: {}", captured_ec.message());
        }
      } else {
#endif
        struct ReadOp {
        ...
        auto read_op = ReadOp{conn->socket.value(), buffer, res, read_completed,
                              captured_ec};
        co_await read_op.run();
    ...
    // HAYAKU_DEBUG("HTTP request system error! {}", e.what());
    throw;
  } catch (const std::exception&) {
    // HAYAKU_DEBUG("HTTP request failed! {}", e.what());
    throw;
  }
  ...
  try {
    ...
    // Send the request - use the event driven way
    {
      ...
      if (conn->ssl_socket) {
        ...
        // Check whether it was cancelled due to a timeout
        if (!write_completed &&
            write_ec == boost::asio::error::operation_aborted) {
          HAYAKU_THROW_EXCEPTION(HttpTimeoutException, "HTTP write timeout");
        }
      ...
      // Read the response header - use the event driven way
      {
        ...
        if (conn->ssl_socket) {
          ...
          if (read_ec && read_ec != http::error::end_of_stream) {
            HAYAKU_THROW("HTTP read header failed: {}", read_ec.message());
          }
        } else {
#endif
          struct ReadHeaderOp {
          ...
          auto read_header_op =
              ReadHeaderOp{conn->socket.value(), buffer, parser};
          auto [read_ec, bytes_transferred] = co_await read_header_op.run();
      ...
      while (!parser.is_done()) {
        ...
        if (conn->ssl_socket) {
          ...
          if (read_ec && read_ec != http::error::need_buffer &&
            ...
            HAYAKU_THROW("HTTP read body failed: {}", read_ec.message());
          }
        } else {
#endif
          struct ReadOp {
    ...
    // Do not close the connection, let the connection pool manage it (it is
    // returned to the pool)

  } catch (const net::system_error&) {
    // HAYAKU_DEBUG("HTTP stream request system error! {}", e.what());
    throw;
  } catch (const std::exception&) {
    // HAYAKU_DEBUG("HTTP stream request failed! {}", e.what());
    throw;
  }
...
// ============================================================================
// The synchronous method implementation - it blocks until the asynchronous
// operation completes
// ============================================================================

AsioHttpResponse AsioHttpClient::request(const std::string& method,
