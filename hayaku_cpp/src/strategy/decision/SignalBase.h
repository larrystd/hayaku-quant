...
class HAYAKU_API SignalBase : public enable_shared_from_this<SignalBase> {
 ...
 public:
  ...
  /**
   * Get the buy signal value of the given moment; a value less than or equal to
   * 0 means there is no buy signal
   * @param datetime
   * @return double
   */
  ...
  /**
   * Get the sell signal value of the given moment; a value greater than or
   * equal to 0 means there is no sell signal
   * @param datetime
   * @return double
   */
  ...
  /**
   * Whether a buy is possible at the next moment, equivalent to whether the
   * last moment indicates a buy
   */
  bool nextTimeShouldBuy() const;

  /**
   * Whether a sell is possible at the next moment, equivalent to whether the
   * last moment indicates a sell
   */
   ...
   * @param value signal value, 1.0 by default; it must be greater than 0,
   ...
   * @param value signal value, -1.0 by default; it must be less than 0,
   * otherwise an exception is thrown
   */
...
/**
 * Client programs should all use this pointer type to operate the signal
 * generator
 * @ingroup Signal
 */
typedef shared_ptr<SignalBase> SignalPtr;
...
inline void SignalBase::_addBuySignal(const Datetime& datetime, double value) {
  HAYAKU_IF_RETURN(std::isnan(value), void());
  HAYAKU_CHECK(value > 0.0, "buy value muse be > 0", value);
  _addSignal(datetime, value);
}

inline void SignalBase::_addSellSignal(const Datetime& datetime, double value) {
  HAYAKU_IF_RETURN(std::isnan(value), void());
  HAYAKU_CHECK(value < 0.0, "sell value muse be > 0", value);
  _addSignal(datetime, value);
}
...
template <>
struct fmt::formatter<hayaku::SignalBase> : ostream_formatter {};

template <>
struct fmt::formatter<hayaku::SignalPtr> : ostream_formatter {};
#endif
