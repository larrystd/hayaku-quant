...
void ComponentContext::prepare() {
    HAYAKU_CHECK(m_runtime.m_account, "Strategy has no execution account: {}", m_runtime.m_name);
    HAYAKU_CHECK(m_runtime.m_mm, "Strategy has no MoneyManager: {}", m_runtime.m_name);
...
void ComponentContext::bind(const KData& kdata) {
    ...
    HAYAKU_TRACE_IF_RETURN(m_runtime.m_calculated, void(), "No need to calculate!");
