...
/**
 * Indicator class, concretely implemented by IndicatorImp; when implementing a
 * new indicator, IndicatorImp should be inherited
 * @details When implementing a new indicator, the IndicatorImp class should be
 * inherited and the related indicator creation function should be defined,
 * e.g.: <pre> class Ma: public IndicatorImp { public: MA(size_t); MA(const
 * Indicator&, size_t);
 *
...
class HAYAKU_API Indicator {
 ...
 public:
  ...
  /** Calculate a new value with the existing parameters, returning a brand new
   * Indicator */
  Indicator operator()(const Indicator& ind);

  /** Create a new clone and use the parameter k as the context of the new
   * instance */
  ...
  /** When there is a context, the context is automatically extended to the
   * current latest data and calculated */
  void extend();
  ...
  /** Set the number to discard; it has no effect if it is less than the
   * original discard */
  void setDiscard(size_t discard) noexcept;

  /** Update the discard number according to its own values; force=true forces
   * the update, otherwise it updates starting from its current discard() */
  ...
  /** Get the output at the corresponding position of the first result set only,
   * the same as get(pos, 0) */
  value_t operator[](size_t pos) const;

  /** Get the output at the corresponding position of the first result set only,
   * the same as getByDate(date, 0) */
   ...
   * For an indicator that no longer needs to be calculated, the occupied memory
   * can be released through this method.
   ...
   * @note The operator== overload creates a new Indicator; this function is
   * used to compare the values of two ind
   */
 ...
 public:
  class Iterator {
   ...
   public:
    ...
    value_t operator*() const { return container.get(index); }
 ...
 * = WEAVE(ind1, ind2) the ind contains multiple results at this time, stored in
 * the order of ind1 and ind2
...
inline Indicator WEAVE(const Indicator& ind1, const Indicator& ind2,
                       const Args&... others) {
  HAYAKU_CHECK(sizeof...(others) <= 4, "WEAVE() only support 6 Indicator!");
 ...
 * For example: IF(CLOSE>OPEN,HIGH,LOW) returns the high price if the period
 * closes up, otherwise it returns the low price
 * </pre>
 ...
 * @details
 * Combine multiple indicators for a unified calculation, the dependency between
 * the indicators and the context settings are handled automatically. This
 * function gets all the child nodes of all the input indicators and removes the
 * duplicates, then sets the given K-line data context for every indicator, and
 * finally performs the calculation and returns the result.
 *
 * <pre>
 * Usage example:
 ...
 * </pre>
 *
 * @param indicators indicator list, the indicator set to be calculated in
 * combination
 * @param kdata K-line data context, used to set the environment of the
 * indicator calculation
 * @param tovalue whether to return the first result column only, false by
 * default (all the result columns are returned)
...
namespace std {
template <>
class hash<hayaku::Indicator> {
 public:
  size_t operator()(hayaku::Indicator const& ind) const noexcept {
    auto imp = ind.getImp();
    return imp.get() ? std::hash<hayaku::IndicatorImpPtr>()(imp) : 0;
  }
};

inline string to_string(const hayaku::Indicator& ind) { return ind.str(); }
}  // namespace std

#if FMT_VERSION >= 90000
template <>
struct fmt::formatter<hayaku::Indicator> : ostream_formatter {};
#endif
