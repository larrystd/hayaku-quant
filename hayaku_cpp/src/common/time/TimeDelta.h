 ...
 * @ingroup DataType
 */
class HAYAKU_UTILS_API TimeDelta {
 public:
  /**
   * Constructor
   * @note
   * <pre>
   * 1. Total duration range: TimeDetla(-99999999) - TimeDelta(99999999, 23, 59,
   * 59, 999, 999).
   * 2. It differs from boost::posix_time::time_duration: as long as one
   * parameter of time_duration is negative it is regarded as a negative
   * duration, while TimeDelta is the sum of the ticks represented by every
   * parameter.
   * 3. When it is a negative duration only days() is negative, all the other
   * parts (hours() ...) are positive. This is consistent with the behavior of
  ...
  explicit TimeDelta(int64_t days = 0, int64_t hours = 0, int64_t minutes = 0,
                     int64_t seconds = 0, int64_t milliseconds = 0,
  ...
  double total_hours() const { return double(ticks()) / 3600000000.0; }

  /** Return the total minutes with a fraction */
  double total_minutes() const { return double(ticks()) / 60000000.0; }

  /** Return the total seconds with a fraction */
  double total_seconds() const { return double(ticks()) / 1000000.0; }
  ...
  std::string str() const {
    ...
    return fmt::format(
        "{} days, {:0>2d}:{:0>2d}:{:<2.6f}", days(), hours(), minutes(),
        seconds() + double(milliseconds() * 1000 + microseconds()) * 0.000001);
#else
    return fmt::format(
        "{} days, {:>02d}:{:>02d}:{:<2.6f}", days(), hours(), minutes(),
        seconds() + double(milliseconds() * 1000 + microseconds()) * 0.000001);
#endif
  }

  /** Convert to a string, the format:
   * TimeDelta(days,hours,mins,secs,millisecs,microsecs) */
  std::string repr() const {
    return fmt::format("TimeDelta({}, {}, {}, {}, {}, {})", days(), hours(),
  ...
  /** The - operator, it gives the negative value, equivalent to TimeDelta(0)
   * minus itself */
  TimeDelta operator-() const { return TimeDelta::fromTicks(-ticks()); }

  /** Multiply the duration by a coefficient, the result is rounded */
  TimeDelta operator*(double p) const;

  /**
   * Divide the duration by a coefficient, the result is rounded.
   * hayaku::exception is thrown if it is divided by zero. Use floorDiv if the
   * rounding is not desired
   */
  TimeDelta operator/(double p) const;

  /** Divide two durations to get their ratio. hayaku::exception is thrown when
   * it is divided by a zero duration. */
  double operator/(TimeDelta td) const;

  /** Floor division, the decimal part is truncated directly */
  TimeDelta floorDiv(double p) const;

  /** The remainder of the division of two durations. hayaku::exception is
  ...
  static TimeDelta min() { return TimeDelta(-99999999); }

  /** Get the maximum expressible value TimeDelta(99999999, 23, 59, 59, 999,
   * 999) */
 ...
 */
template <typename Rep, typename Period>
inline TimeDelta operator+(std::chrono::duration<Rep, Period> duration,
                           TimeDelta td) {
  return td + duration;
}
 ...
 */
template <typename Rep, typename Period>
inline bool operator==(std::chrono::duration<Rep, Period> duration,
                       TimeDelta td) {
  return td == duration;
}
...
inline TimeDelta Days(int64_t days) { return TimeDelta(days); }
 ...
 * @ingroup DataType
 */
TimeDelta HAYAKU_UTILS_API Hours(int64_t hours);
 ...
 * @ingroup DataType
 */
TimeDelta HAYAKU_UTILS_API Minutes(int64_t mins);
 ...
 * @ingroup DataType
 */
TimeDelta HAYAKU_UTILS_API Seconds(int64_t secs);
 ...
 * @ingroup DataType
 */
TimeDelta HAYAKU_UTILS_API Milliseconds(int64_t milliseconds);
 ...
 * @return TimeDelta
 */
TimeDelta HAYAKU_UTILS_API UTCOffset();
...
template <>
class hash<hayaku::TimeDelta> {
 public:
  size_t operator()(hayaku::TimeDelta const &d) const noexcept {
    return std::hash<int64_t>()(d.ticks());
  }
};

inline string to_string(const hayaku::TimeDelta &delta) { return delta.str(); }

}  // namespace std

#if FMT_VERSION >= 90000
template <>
struct fmt::formatter<hayaku::TimeDelta> {
  FMT_CONSTEXPR auto parse(format_parse_context &ctx) -> decltype(ctx.begin()) {
    return ctx.end();
  }

  template <typename FormatContext>
  auto format(const hayaku::TimeDelta &d, FormatContext &ctx) const
      -> decltype(ctx.out()) {
