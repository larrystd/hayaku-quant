...
TEST_CASE("test_SE_PerformanceOptimal") {
  ...
  CHECK_THROWS(se->addSystemList(
      internal::StrategyRuntimeList{sys, internal::StrategyRuntimePtr()}));
...
TEST_CASE("test_SE_PerformanceOptimal_export") {
  ...
  // Currently a reset is needed after the calculation to serialize and reload
  // normally
  se1->reset();
  ...
  for (const auto& date : dates) {
    if (se1->getSelected(date).empty()) {
      ...
      CHECK_EQ(se1->getSelected(date)[0].strategy->name(),
               se2->getSelected(date)[0].strategy->name());
    }
  }
}
