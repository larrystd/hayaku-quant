...
from hayaku.core import constant, isnan, Indicator, KData, IF, ALIGN, htr
...
def iheatmap(ind, axes=None):
    ...
    if len(ind) == 0:
        hayaku_error(htr("The indicator length is 0; the indicator should have been calculated (i.e. it has values)"))
        return

    dates = ind.get_datetime_list()
    if len(dates) == 0:
        hayaku_error(htr("Failed to get the date list! The indicator should be a time series"))
        return
    ...
    if len(monthly) < 2:
        hayaku_warn(htr("Insufficient monthly data!"))
        return
...
def tm_heatmap(tm, start_date, end_date=None, axes=None, show_high_low=False):
    ...
    if len(dates) == 0:
        hayaku_error(htr("No data, please check the date range! start_date={}, end_date={}"), start_date, end_date)
        return
...
def tm_year_profit(tm, start_date, end_date=None, axes=None, show_high_low=True):
    ...
    if len(yearly) < 1:
        hayaku_warn(htr("Insufficient yearly data!"))
        return
...
def sys_heatmap(sys, axes=None):
    ...
    """
    hayaku_check(sys.tm is not None, htr("The system has no trade manager initialized"))
    query = sys.query
