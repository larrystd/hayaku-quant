...
Hayaku是研究系统交易的Python量化框架，可以在其基础上构建自己的客户端程序。而Hayaku库本身自带一个交互式工具，可在 Python Shell 中使用 ``from hayaku.application.interactive import *`` 引入。该工具主要提供基于 matplotlib 的图形绘制函数，便于在交互式环境下绘制K线、指标、信号等。
...
`<https://nbviewer.jupyter.org/github/larrystd/hayaku-quant/blob/poc/examples/python/notebook/zh/000-Index.ipynb?flush_cache=True>`_
