...
#pragma once
#ifndef HIKYUU_TRADE_ACCOUNTCONFIG_H
#define HIKYUU_TRADE_ACCOUNTCONFIG_H
...
#include "cost/TradeCostBase.h"
#include "AccountId.h"

namespace hku {

/** Immutable construction parameters for an execution account. */
class HKU_API AccountConfig {
public:
    explicit AccountConfig(Datetime initDatetime = Datetime(199001010000LL),
                           price_t initialCash = 100000.0,
                           TradeCostPtr costPolicy = TC_Zero(), string name = "SYS",
                           int precision = 2, bool supportBorrowCash = false,
                           bool supportBorrowStock = false, AccountId accountId = {},
    ...
    : m_initDatetime(initDatetime),
      ...
      m_brokers(std::move(brokers)) {
        HKU_CHECK(m_precision > 0, "Account precision must be greater than zero");
    }
    ...
    [[nodiscard]] price_t initialCash() const noexcept {
        return m_initialCash;
    }
...
#endif /* HIKYUU_TRADE_ACCOUNTCONFIG_H */
