...
namespace {
std::unique_ptr<HikyuuSession> g_testSession;
}
...
void init_hikyuu_test() {
    ...
    g_testSession = std::make_unique<HikyuuSession>(HikyuuSession::open(config_file));
...
void init_hikyuu_test() {
    ...
    setDataRuntimePluginPath(plugin_path);

    fmt::print("configure file: {}\n", config_file);
    g_testSession = std::make_unique<HikyuuSession>(HikyuuSession::open(config_file));
    fmt::print("current plugin path: {}\n", getDataRuntimePluginPath());
