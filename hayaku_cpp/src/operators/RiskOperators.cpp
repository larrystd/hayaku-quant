...
void ISaftyLoss::_dyn_calculate(const Indicator& ind) {
    ...
    HAYAKU_CHECK(n1.size() == ind.size(), "ind_param(n1).size()={}, ind.size()={}!", n1.size(),
              ind.size());
    HAYAKU_CHECK(n2.size() == ind.size(), "ind_param(n2).size()={}, ind.size()={}!", n2.size(),
              ind.size());
    HAYAKU_CHECK(p.size() == ind.size(), "ind_param(p).size()={}, ind.size()={}!", p.size(),
              ind.size());
...
Indicator HAYAKU_API SAFTYLOSS(const IndParam& n1, const IndParam& n2, double p) {
    IndicatorImpPtr result = make_shared<ISaftyLoss>();
