...
static Indicator getExtIndicator(const string& name, const Parameter& params) {
  ...
  HAYAKU_ERROR_IF_RETURN(
      !plugin, ret,
      fmt::format("Can't find {} plugin!", HAYAKU_PLUGIN_EXTEND_INDICATOR));
...
Indicator FIXED_START_INDEX(int start_index) {
  Parameter params;
  params.set<int>("index", start_index);
  return getExtIndicator("FIXED_START_INDEX", params);
}

Indicator FIXED_START_INDEX(const Indicator& ind, int start_index) {
  return FIXED_START_INDEX(start_index)(ind);
}

Indicator FIXED_START_DATE(const Datetime& start_date) {
  Parameter params;
  params.set<Datetime>("start_date", start_date);
  return getExtIndicator("FIXED_START_DATE", params);
}

Indicator FIXED_START_DATE(const Indicator& ind, const Datetime& start_date) {
  return FIXED_START_DATE(start_date)(ind);
}

Indicator WITHKTYPE(const KQuery::KType& ktype, bool fill_null) {
  Parameter params;
...
Indicator RANK(const Block& block, int mode, bool fill_null,
               const string& market) {
  Parameter params;
...
Indicator RANK(const Block& block, const Indicator& ref_ind, int mode,
...
Indicator AGG_SAMPLE(const Indicator& ind, const string& time,
                     const KQuery::KType& ktype, bool fill_null, int unit) {
  Parameter params;
...
Indicator AGG_SAMPLE_MAX(const Indicator& ind, const string& start_time,
...
Indicator AGG_SAMPLE_MIN(const Indicator& ind, const string& start_time,
                         const string& last_time, const KQuery::KType& ktype,
                         bool fill_null, int unit) {
  Parameter params;
  params.set<string>("start_time", start_time);
...
Indicator AGG_SAMPLE_MEAN(const Indicator& ind, const string& start_time,
                          const string& last_time, const KQuery::KType& ktype,
                          bool fill_null, int unit) {
  Parameter params;
  params.set<string>("start_time", start_time);
...
Indicator AGG_STD(const Indicator& ind, const KQuery::KType& ktype,
                  bool fill_null, int unit, int ddof) {
  Parameter params;
  params.set<string>("ktype", ktype);
  params.set<bool>("fill_null", fill_null);
  params.set<int>("unit", unit);
...
Indicator AGG_VAR(const Indicator& ind, const KQuery::KType& ktype,
                  bool fill_null, int unit, int ddof) {
  Parameter params;
  params.set<string>("ktype", ktype);
  params.set<bool>("fill_null", fill_null);
...
Indicator AGG_QUANTILE(const Indicator& ind, const KQuery::KType& ktype,
                       bool fill_null, int unit, double quantile) {
  Parameter params;
  params.set<string>("ktype", ktype);
  params.set<bool>("fill_null", fill_null);
  params.set<int>("unit", unit);
...
Indicator AGG_VWAP(const KQuery::KType& ktype, bool fill_null, int unit) {
  Parameter params;
  params.set<string>("ktype", ktype);
  params.set<bool>("fill_null", fill_null);
  params.set<int>("unit", unit);
...
Indicator AGG_FUNC(const Indicator& ind, agg_func_t agg_func,
  ...
  auto* plugin = getPlugin<ExtendIndicatorsPluginInterface>(
      HAYAKU_PLUGIN_EXTEND_INDICATOR);
  HAYAKU_ERROR_IF_RETURN(!plugin, ret, "Can't find {} plugin!",
                         HAYAKU_PLUGIN_EXTEND_INDICATOR);
