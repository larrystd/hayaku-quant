...
StrategyRuntime::StrategyRuntime(const StrategyDefinition& definition,
  ...
  HAYAKU_CHECK(m_account, "StrategyRuntime requires an execution account");
...
void StrategyRuntime::run(const BacktestRequest& request) {
  ...
  HAYAKU_DEBUG_IF_RETURN(m_calculated && m_kdata == request.kdata(), void(),
                         "Not need calculate.");
  prepare();
  bind(request.kdata());
  ...
  HAYAKU_ASSERT(m_kdata.size() == m_rawKData.size());
  ...
  for (size_t i = 0; i < total; ++i) {
    ...
    if (trace) {
      ...
      const PositionRecord position =
          m_execution.position(records[i].datetime, m_stock);
      const FundsRecord funds =
          m_execution.funds(records[i].datetime, m_kdata.getQuery().kType());
...
void StrategyRuntime::run(const KQuery& query, bool resetState, bool resetAll) {
  HAYAKU_CHECK(!m_stock.isNull(), "Strategy stock is null");
...
TradeRecord StrategyRuntime::runMomentOnOpen(const Datetime& datetime) {
  const size_t pos = m_kdata.getPos(datetime);
  HAYAKU_IF_RETURN(pos == Null<size_t>(), TradeRecord());
  return runMomentOnOpenNative(m_kdata.getKRecord(pos),
                               m_rawKData.getKRecord(pos));
}

TradeRecord StrategyRuntime::runMomentOnClose(const Datetime& datetime) {
  const size_t pos = m_kdata.getPos(datetime);
  HAYAKU_IF_RETURN(pos == Null<size_t>(), TradeRecord());
  return runMomentOnCloseNative(m_kdata.getKRecord(pos),
                                m_rawKData.getKRecord(pos));
}
...
void StrategyRuntime::clearPendingBuy() { m_pendingOrders.buy().clear(); }
...
const string& StrategyRuntime::name() const noexcept { return m_name; }

void StrategyRuntime::name(string value) { m_name = std::move(value); }

Stock StrategyRuntime::getStock() const { return m_stock; }
...
KData StrategyRuntime::getTO() const { return m_kdata; }

MoneyManagerPtr StrategyRuntime::getMM() const { return m_mm; }

SignalPtr StrategyRuntime::getSG() const { return m_sg; }

SlippagePtr StrategyRuntime::getSP() const { return m_sp; }
...
double StrategyRuntime::getBuyNumber(const Datetime& datetime, price_t price,
  ...
  return m_mm ? m_mm->getBuyNumber(datetime, m_stock, price, risk, origin)
              : 0.0;
}

double StrategyRuntime::getSellNumber(const Datetime& datetime, price_t price,
                                      price_t risk, OrderOrigin origin) {
  return m_mm ? m_mm->getSellNumber(datetime, m_stock, price, risk, origin)
              : 0.0;
}
...
double StrategyRuntime::getSellShortNumber(const Datetime& datetime,
                                           ...
                                           OrderOrigin origin) {
  return m_mm ? m_mm->getSellShortNumber(datetime, m_stock, price, risk, origin)
              : 0.0;
}
...
double StrategyRuntime::getBuyShortNumber(const Datetime& datetime,
                                          price_t price, price_t risk,
                                          OrderOrigin origin) {
  return m_mm ? m_mm->getBuyShortNumber(datetime, m_stock, price, risk, origin)
              : 0.0;
}
...
TradeRecord StrategyRuntime::runMomentOnOpenNative(const KRecord& today,
  ...
  if (trace) {
    HAYAKU_INFO("{} ------------------------------------------------------",
                today.datetime);
    HAYAKU_INFO(htr("[{}] cal today {}", m_name, today));
    HAYAKU_INFO_IF(m_kdata.getQuery().recoverType() != KQuery::NO_RECOVER,
...
TradeRecord StrategyRuntime::runMomentOnCloseNative(const KRecord& today,
  ...
  const PositionRecord position =
      m_account->getPosition(today.datetime, m_stock);
  HAYAKU_INFO_IF(trace,
                 htr("[{}] current position: {}", m_name, position.number));
  if (position.number == 0.0) {
    return {};
  }
...
TradeRecord StrategyRuntime::buy(const KRecord& today, const KRecord& src_today,
  ...
  if (getParam<bool>("buy_delay")) {
    ...
    HAYAKU_INFO_IF(trace, htr("[{}] will be delay to buy", m_name));
    return result;
  }
  ...
  if (today.highPrice == today.lowPrice) {
    if (getParam<bool>("can_trade_when_high_eq_low")) {
      HAYAKU_WARN_IF(trace, htr("[{}] buy one-price board", m_name));
    ...
    if (pos == 0 || pos == Null<size_t>()) {
      HAYAKU_INFO_IF(trace, htr("[{}] delay to buy, one-price board", m_name));
      submitBuyRequest(today, src_today, from);
      return result;
    }
    ...
    if (today.closePrice > pre_day.closePrice) {
      HAYAKU_INFO_IF(
          trace, htr("[{}] delay to buy, one-price up-limit board", m_name));
      submitBuyRequest(today, src_today, from);
      return result;
    }
  }
  ...
  if (iszero(today.transAmount) || iszero(today.transCount)) {
    HAYAKU_INFO_IF(
        trace,
        htr("[{}] delay to buy, current amount == 0 or count == 0", m_name));
    submitBuyRequest(today, src_today, from);
    return result;
  }
...
TradeRecord StrategyRuntime::buyNow(const KRecord& today,
  ...
  if (iszero(number) || number > m_stock.maxTradeNumber()) {
    HAYAKU_INFO_IF(
        trace, "[{}] {}, number: {} == 0 or > maxTradeNumber: {}, {}", m_name,
  ...
  TradeRecord record = submitStrategyOrder(
      m_execution,
      OrderRequest(OrderSide::BUY, today.datetime, m_stock, realPrice, number,
                   stoploss, goalPrice, planPrice, from));
  if (BUSINESS_BUY != record.business) {
    HAYAKU_INFO_IF(trace, htr("[{}] buy failed, {}", m_name, record));
    return result;
  }
...
TradeRecord StrategyRuntime::buyDelay(const KRecord& today,
  ...
  price_t realPrice = getRealBuyPrice(today.datetime, planPrice);
  TradeRecord record = submitStrategyOrder(
      m_execution, OrderRequest(OrderSide::BUY, today.datetime, m_stock,
                                realPrice, number, stoploss, goalPrice,
                                planPrice, m_pendingOrders.buy().origin));
  if (BUSINESS_BUY != record.business) {
    m_pendingOrders.buy().clear();
    return result;
  }
...
TradeRecord StrategyRuntime::sellForce(const Datetime& date, double num,
  ...
  bool trace = getParam<bool>("trace");
  HAYAKU_INFO_IF(trace, "[{}] {} {} by {}", m_name, htr("force sell"), num,
  ...
  record = submitStrategyOrder(
      m_execution,
      OrderRequest(OrderSide::SELL, date, m_stock, realPrice, realsell_num,
  ...
  HAYAKU_WARN_IF_RETURN(record == Null<TradeRecord>(), record,
                        "[{}] {}: {} by {}", m_name, htr("Failed force sell"),
...
TradeRecord StrategyRuntime::sell(const KRecord& today,
  ...
  if (getParam<bool>("sell_delay")) {
    ...
    HAYAKU_INFO_IF(trace, htr("[{}] will be delay to sell", m_name));
    return result;
  }
  ...
  if (today.highPrice == today.lowPrice) {
    if (getParam<bool>("can_trade_when_high_eq_low")) {
      HAYAKU_WARN_IF(trace, htr("[{}] sell one-price board", m_name));
    ...
    size_t pos = m_kdata.getPos(today.datetime);
    if (pos == 0 || pos == Null<size_t>()) {
      HAYAKU_INFO_IF(trace, htr("[{}] delay to sell, one-price board", m_name));
      submitSellRequest(today, src_today, from);
      return result;
    }
    ...
    if (today.closePrice < preday.closePrice) {
      HAYAKU_INFO_IF(trace, htr("[{}] sell delayed: limit-down lock", m_name));
      submitSellRequest(today, src_today, from);
      return result;
    }
  }
  ...
  if (iszero(today.transAmount) || iszero(today.transCount)) {
    HAYAKU_INFO_IF(
        trace,
        htr("[{}] delay to sell, current amount == 0 or count == 0", m_name));
    submitSellRequest(today, src_today, from);
    return result;
  }

  result = sellNow(today, src_today, from);
  HAYAKU_INFO_IF(trace, htr("[{}] sell now: {}", m_name, result));
  return result;
}
...
TradeRecord StrategyRuntime::sellNow(const KRecord& today,
  ...
  TradeRecord record = submitStrategyOrder(
      m_execution,
      OrderRequest(OrderSide::SELL, today.datetime, m_stock, realPrice, number,
                   stoploss, goalPrice, planPrice, from));
...
TradeRecord StrategyRuntime::sellDelay(const KRecord& today,
  ...
  price_t realPrice = getRealSellPrice(today.datetime, planPrice);
  TradeRecord record = submitStrategyOrder(
      m_execution, OrderRequest(OrderSide::SELL, today.datetime, m_stock,
                                realPrice, number, stoploss, goalPrice,
                                planPrice, m_pendingOrders.sell().origin));
  if (BUSINESS_SELL != record.business) {
    m_pendingOrders.sell().clear();
    return result;  // The sell operation failed
  }
...
TradeRecord StrategyRuntime::buyShort(const KRecord& today,
  ...
  if (getParam<bool>("buy_delay")) {
    ...
    HAYAKU_INFO_IF(trace, htr("[{}] will buy short next bar open", m_name));
    return result;
  }
  ...
  if (today.highPrice == today.lowPrice) {
    if (getParam<bool>("can_trade_when_high_eq_low")) {
      HAYAKU_WARN_IF(trace, htr("[{}] buy short one-price board", m_name));
    ...
    if (pos == 0 || pos == Null<size_t>()) {
      HAYAKU_INFO_IF(trace,
                     htr("[{}] delay to buy short, one-price board", m_name));
      submitBuyShortRequest(today, src_today, from);
      return result;
    }
    ...
    if (today.closePrice > preday.closePrice) {
      HAYAKU_INFO_IF(trace,
                     htr("[{}] short covering delayed: limit-up lock", m_name));
      submitBuyShortRequest(today, src_today, from);
      return result;
    }
  }
...
TradeRecord StrategyRuntime::buyShortNow(const KRecord& today,
  ...
  TradeRecord record = submitStrategyOrder(
      m_execution, OrderRequest(OrderSide::BUY_SHORT, today.datetime, m_stock,
                                realPrice, number, stoploss, goalPrice,
  ...
  if (today.highPrice == today.lowPrice &&
    ...
    if (pos == 0 || pos == Null<size_t>()) {
      ...
      submitBuyShortRequest(today, src_today,
                            m_pendingOrders.buyShort().origin);
      return result;
    }
...
TradeRecord StrategyRuntime::sellShort(const KRecord& today,
  ...
  if (today.highPrice == today.lowPrice) {
    ...
    if (pos == 0 || pos == Null<size_t>()) {
      HAYAKU_INFO_IF(trace,
                     htr("[{}] delay to sell short, one-price board", m_name));
      submitSellShortRequest(today, src_today, from);
      return result;
    }
    ...
    if (today.closePrice < preday.closePrice) {
      HAYAKU_INFO_IF(
          trace, htr("[{}] short selling delayed: limit-down lock", m_name));
      submitSellShortRequest(today, src_today, from);
      return result;
    }
  }
...
TradeRecord StrategyRuntime::sellShortNow(const KRecord& today,
  ...
  price_t realPrice = getRealSellPrice(today.datetime, planPrice);
  TradeRecord record = submitStrategyOrder(
      m_execution, OrderRequest(OrderSide::SELL_SHORT, today.datetime, m_stock,
                                realPrice, number, stoploss, goalPrice,
                                planPrice, OrderOrigin::SIGNAL));
...
TradeRecord StrategyRuntime::sellShortDelay(const KRecord& today,
  ...
  TradeRecord record = submitStrategyOrder(
      m_execution, OrderRequest(OrderSide::SELL_SHORT, today.datetime, m_stock,
                                realPrice, number, stoploss, goalPrice,
                                planPrice, m_pendingOrders.sellShort().origin));
  if (BUSINESS_SELL_SHORT != record.business) {
    m_pendingOrders.sellShort().clear();
    return result;  // The sell operation failed
  }
...
void StrategyRuntime::submitSellShortRequest(const KRecord& today,
  ...
  m_pendingOrders.sellShort().stoploss =
      getStoplossPrice(today, src_today, today.closePrice);
  m_pendingOrders.sellShort().goal =
      getGoalPrice(today.datetime, src_today.closePrice);
...
TradeRecord StrategyRuntime::processRequest(const KRecord& today,
                                            const KRecord& src_today) {
  HAYAKU_IF_RETURN(m_pendingOrders.buy().valid, buyDelay(today, src_today));
  HAYAKU_IF_RETURN(m_pendingOrders.sell().valid, sellDelay(today, src_today));
  HAYAKU_IF_RETURN(m_pendingOrders.sellShort().valid,
                   sellShortDelay(today, src_today));
  HAYAKU_IF_RETURN(m_pendingOrders.buyShort().valid,
