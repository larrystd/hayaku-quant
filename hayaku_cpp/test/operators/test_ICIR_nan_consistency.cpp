 ...
 *
 * Verify the semantic consistency of the MA(ic,n) / STDEV(ic,n) combination
 * with scattered NaN. ICIR = MA / STDEV: the numerator and the denominator must
 * be based on exactly the same valid sample set, and when either of them is
 * NaN, ICIR must silently output NaN.
 */
...
/** @par Test point: with count=1 std=NaN and ICIR is silently NaN (no division
 * by 0 crash) */
