 ...
 * server side)
 * @details The facade first sets the app-side server role (to prevent self-connection, see design
 *          §5.5), then loads the optional realtime plugin through PluginRuntime and calls start().
 *          The plugin requires a valid license; false is returned when it is not licensed or not
 *          installed.
 * @param datadir data directory; the current data directory of the data runtime is used when it is
 * empty
 ...
 * @note It must be called after opening a HayakuSession; importing hayaku alone does not initialize
 *       market data.
 * @ingroup DataDriver
 */
bool HAYAKU_REALTIME_API startShmServer(const std::string& datadir = "", bool publish_shm = true,
 ...
 * destroyed internal structures and crash. It is called when the final HayakuSession closes.
 * @ingroup DataDriver
 */
