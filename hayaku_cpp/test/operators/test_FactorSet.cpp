...
#include <operators/CompiledFactorPlan.h>
 ...
 * @ingroup test_hayaku_factor_suite
 * @{
 */
...
TEST_CASE("test_FactorSet_ktype_check") {
  ...
  Factor month_factor("MONTH_MA", month_ma, KQuery::MONTH,
                      "monthly bar factor");
...
TEST_CASE("test_FactorSet_duplicate_name") {
  Indicator ma5 = MA(CLOSE(), 5);
  Indicator ma5_new =
      MA(CLOSE(), 5);  // The same name but a different indicator

  Factor factor1("MA5", ma5, KQuery::DAY, "5-day MA factor",
                 "original version");
  Factor factor2("MA5", ma5_new, KQuery::DAY, "5-day MA factor",
                 "updated version");
  ...
  fs.add(factor2);
  CHECK_EQ(fs.size(),
           1);  // The size stays because it is an overwrite, not a new entry

  // Verify the overwritten factor
  Factor retrieved2 = fs.get("MA5");
  CHECK_EQ(retrieved2.brief(), "5-day MA factor");
  CHECK_EQ(retrieved2.details(),
           "updated version");  // it should be the new value
...
TEST_CASE("test_FactorSet_shallow_copy") {
  ...
  // Verify that the original object was modified too (the shallow copy
  // property)
  CHECK_EQ(fs1.size(), 1);  // The size of the original object was modified too
  CHECK_FALSE(
      fs1.have("MA5"));  // The factor of the original object was removed
  ...
  // Test the move semantics - with a shallow copy the move constructor is a
  // copy constructor
  FactorSet fs4(std::move(fs2));
  // fs4 should reflect the latest state, because all the objects share the same
  // data
...
TEST_CASE("test_FactorSet_iterator") {
  ...
  // Test the range based for loop - the iterator returns a Factor reference
  // directly
  size_t count = 0;
  ...
  // Test the explicit iterator - the dereference returns a Factor reference
  // directly
  auto it = fs.begin();
  auto end = fs.end();
  count = 0;
  while (it != end) {
    const Factor& factor =
        *it;  // A direct dereference gives the Factor reference
    CHECK_UNARY_FALSE(factor.isNull());
    CHECK_UNARY(factor.name() == "MA5" || factor.name() == "MA10");
    count++;
    ++it;
  ...
  for (auto it = fs.begin(); it != fs.end(); ++it) {
    const Factor* factor_ptr =
        it.operator->();  // The arrow operator returns a Factor pointer
...
TEST_CASE("test_FactorSet_block") {
  ...
  // Modifying the Block of the copy should affect the original object (a
  // shallow copy)
...
TEST_CASE("test_FactorSet_block_check") {
  ...
  Indicator ma5 = MA(CLOSE(), 5);
  Factor factor1("MA5_B1", ma5, KQuery::DAY, "MA5 factor", "block1 test", false,
                 Datetime::min(), block1);
  Factor factor2("MA5_B2", ma5, KQuery::DAY, "MA5 factor", "block2 test", false,
                 Datetime::min(), block2);
  Factor factor3("MA5_B3", ma5, KQuery::DAY, "MA5 factor", "block3 test", false,
                 Datetime::min(), block3);
  ...
  SUBCASE("FactorSet with default empty block") {
    ...
    Block empty_block;
    Factor factor_empty("EMPTY", ma5, KQuery::DAY, "empty Block factor", "",
                        false, Datetime::min(), empty_block);
...
TEST_CASE("test_FactorSet_order_preservation") {
  ...
  SUBCASE("Remove middle factor") {
    ...
    // Verify the order: the 1st and the 3rd factors stay and the 2nd was
    // removed
...
TEST_CASE("test_FactorSet_getValues") {
  ...
  StockList stocks;
  // Since DataRuntime is not initialized, an empty list is used here for the
  // test In the real usage it should contain valid stock objects
  ...
  SUBCASE("Basic getValues functionality") {
    ...
    // The result of every stock should contain the calculation result of every
    // factor
...
/** @par Test point: test the full parameter combination of the FactorSet
 * getValues method */
TEST_CASE("test_FactorSet_getValues_complete_params") {
  ...
  SUBCASE("Fill null=true") {
    vector<IndicatorList> result =
        factorset.getValues(stocks, query, false, true);
    CHECK_FALSE(result.empty());
    CHECK_EQ(result.size(), stocks.size());
    for (const auto& stock_result : result) {
      CHECK_EQ(stock_result.size(), factorset.size());
  ...
  SUBCASE("To value=true") {
    vector<IndicatorList> result =
        factorset.getValues(stocks, query, false, false, true);
    CHECK_FALSE(result.empty());
    CHECK_EQ(result.size(), stocks.size());
    for (const auto& stock_result : result) {
      CHECK_EQ(stock_result.size(), factorset.size());
  ...
  SUBCASE("All parameters true") {
    vector<IndicatorList> result =
        factorset.getValues(stocks, query, true, true, true, true);
    CHECK_FALSE(result.empty());
    CHECK_EQ(result.size(), stocks.size());
    for (const auto& stock_result : result) {
      CHECK_EQ(stock_result.size(), factorset.size());
...
TEST_CASE("test_FactorSet_getValues_check") {
  ...
  SUBCASE("FactorSet without block") {
    ...
    CHECK_NOTHROW(
        factorset.getValues(test_stocks, query, false, false, false, false));

    // It should also work with check=true (there is no Block restriction)
    CHECK_NOTHROW(
        factorset.getValues(test_stocks, query, false, false, false, true));
  }
  ...
  SUBCASE("FactorSet with block") {
    ...
    Indicator ma5 = MA(CLOSE(), 5);
    Factor factor("TEST_FACTOR", ma5, KQuery::DAY, "test factor", "", false,
    ...
    CHECK_NOTHROW(
        factorset.getValues(block_stocks, query, false, false, false, true));
...
/** @par Test point: test the boundary conditions of the FactorSet getValues
 * method */
TEST_CASE("test_FactorSet_getValues_edge_cases") {
  ...
  SUBCASE("Invalid query range") {
    KQuery invalid_query(1000000, 1000010,
                         KQuery::DAY);  // Beyond the actual data range
    ...
    CHECK_EQ(result.size(),
             stocks.size());  // But the result of every stock is empty
    for (const auto& stock_result : result) {
      CHECK_EQ(stock_result.size(), factorset.size());
  ...
  SUBCASE("Different KType") {
    ...
    vector<IndicatorList> month_result =
        factorset.getValues(stocks, month_query);
...
/** @par Test point: test the result correctness of the FactorSet getValues
 * method */
TEST_CASE("test_FactorSet_getValues_result_correctness") {
  ...
  SUBCASE("Basic functionality and result validation") {
    ...
    for (size_t stock_idx = 0; stock_idx < result.size(); ++stock_idx) {
      ...
      CHECK_EQ(stock_result.size(),
               2);  // Every stock has the results of the two factors

      // Verify the result of every factor
      for (size_t factor_idx = 0; factor_idx < stock_result.size();
           ++factor_idx) {
  ...
  SUBCASE("Edge case handling validation") {
    ...
    StockList empty_stocks;
    vector<IndicatorList> empty_result =
        factorset.getValues(empty_stocks, query);
    CHECK_UNARY(empty_result.empty());

    // Test an invalid query range
    KQuery invalid_query(1000000, 1000010, KQuery::DAY);
    vector<IndicatorList> invalid_result =
        factorset.getValues(stocks, invalid_query);
  ...
  SUBCASE("Align parameter validation") {
    ...
    vector<IndicatorList> result_false =
        factorset.getValues(stocks, query, false);
    vector<IndicatorList> result_true =
        factorset.getValues(stocks, query, true);
    ...
    for (size_t stock_idx = 0; stock_idx < result_true.size(); ++stock_idx) {
      for (size_t factor_idx = 0; factor_idx < result_true[stock_idx].size();
           ++factor_idx) {
...
TEST_CASE("test_CompiledFactorPlan_executor_reuse") {
  ...
  Indicator low = LOW();
  formulas.emplace_back(CORR(MA(VOL(), 20), low, 5) + (HIGH() + low) / 2.0 -
                        CLOSE());
  ...
  // Exercise buffer growth, shrinkage, and returning to the original stock on
  // one executor.
  check_one(k1);
...
TEST_CASE("test_CompiledFactorPlan_fallback_eligibility") {
  detail::CompiledFactorPlan context_plan({hayaku::CONTEXT(CLOSE())});
...
TEST_CASE("test_FactorSet_compiled_values_match_legacy") {
  ...
  auto aligned =
      factorset.getValues(stocks, query, true, false, true, false, dates);
  ...
  for (size_t si = 0; si < stocks.size(); ++si) {
    ...
    for (size_t fi = 0; fi < factorset.size(); ++fi) {
      ...
      CAPTURE(factorset[fi].name());
      Indicator expected =
          ALIGN(factorset[fi].formula(), dates, false)(kdata).getResult(0);
...
TEST_CASE("test_FactorSet_formula_results_keep_independent_graphs") {
  ...
  auto result = factorset.getValues({stock}, KQuery(0, 30, KQuery::DAY), false,
                                    false, false);
...
TEST_CASE("test_FactorSet_compiled_path_nested_invocation") {
  Stock stock1 = getDataRuntime().getStock("sh000001");
  Stock stock2 = getDataRuntime().getStock("sz000001");
  REQUIRE_FALSE(stock1.isNull());
  REQUIRE_FALSE(stock2.isNull());
  StockList stocks{stock1, stock2};
  KQuery query(0, 30, KQuery::DAY);
...
/** @par Test point: test the boundary conditions and the exception handling of
 * FactorSet */
...
/** @par Test point: test the memory management and the resource release of
 * FactorSet */
...
TEST_CASE("test_FactorSet_integration") {
  // Test the integration with DataRuntime
  SUBCASE("DataRuntime integration") {
    ...
    // Test getAllValues (it calls DataRuntime)
    KQuery query(0, 10, KQuery::DAY);
    CHECK_NOTHROW(fs.getAllValues(query));
    // Note: the real result depends on the state of DataRuntime; only no
...
TEST_CASE("test_FactorSet_serialization") {
  ...
  Factor factor2("MA10", ma10, KQuery::DAY, "10-day MA factor",
                 "test 10-day MA", false, Datetime::min(), test_block);
  ...
  SUBCASE("Modified FactorSet serialization") {
    ...
    // Verify that the deserialized object has the state at the save time,
    // unaffected by the later modification
