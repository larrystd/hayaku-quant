...
from .hayakucmd import HAYAKUShell
from hayaku._support.checks import hayaku_catch

g_server_url = "http://127.0.0.1:520/hayaku"
g_api_version = "v1"
g_token = None
...
def login():
    data = json.dumps({'user': 'hayaku'})
    url = get_url('login', 'login')
    r = requests.post(url, data)
    global g_token
    g_token = r.json()['hayaku_token']
...
def get_headers():
    headers = {"hayaku_token": get_token(), "User-Agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36"}
    return headers


@hayaku_catch()
def start_server():
    if HAYAKUShell.server is not None:
        server_status()
        return
    target = 'hayakuserver.exe' if sys.platform == 'win32' else 'hayakuserver'
    if os.path.exists(target):
        HAYAKUShell.server = subprocess.Popen([target])
        return
    ...
    try:
        ...
        target = "{}/cpp/{}".format(hayaku.__path__[0], target)
        if os.path.exists(target):
            HAYAKUShell.server = subprocess.Popen([target])
    except:
...
def stop_server():
    if HAYAKUShell.server:
        HAYAKUShell.server.terminate()
        HAYAKUShell.server.wait()
        HAYAKUShell.server = None
        print("server stopped.")


def server_status():
    if HAYAKUShell.server is None:
        print("server stopped.")
        return
