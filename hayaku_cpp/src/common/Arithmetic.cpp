...
template double HAYAKU_UTILS_API roundEx(double number, int ndigits);

template <>
float roundEx(float number, int ndigits) {
  // The traditional round-half-up is generally used in China
  if (ndigits < 0)
    return number;  // An invalid digit count returns the original value
                    // directly
...
template double HAYAKU_UTILS_API roundUp(double number, int ndigits);
template float HAYAKU_UTILS_API roundUp(float number, int ndigits);
template double HAYAKU_UTILS_API roundDown(double number, int ndigits);
template float HAYAKU_UTILS_API roundDown(float number, int ndigits);
...
bool HAYAKU_UTILS_API isInteger(double num) {
  ...
  double fractionalPart =
      num - std::floor(num);  // Same as num % 1.0, but safer for negatives

  // A tiny error is allowed (due to the floating point precision,
  // e.g. 5.0000000001 should be regarded as 5)
  const double epsilon = 1e-9;
  return std::fabs(fractionalPart) < epsilon ||
         std::fabs(fractionalPart - 1.0) < epsilon;
}

bool HAYAKU_UTILS_API isInteger(float num) {
  // Handle the special values: NaN (not a number) or infinity (the float
  ...
  // Adjust the precision error for float (the float precision is lower than
  // double, so a larger epsilon is needed)
  const float epsilon =
      1e-5f;  // The error threshold within the typical float precision range
  // Judge whether the fractional part is close to 0 or 1.0 (handling the cases
  // like 5.9999997)
  return std::fabs(fractionalPart) < epsilon ||
         std::fabs(fractionalPart - 1.0f) < epsilon;
}

template double HAYAKU_UTILS_API get_quantile(const std::vector<double> &vec,
                                              double quantile);
template float HAYAKU_UTILS_API get_quantile(const std::vector<float> &vec,
                                             double quantile);
