...
class FundsRecord {
 public:
  ...
  price_t borrow_asset{
      0.0}; /**< Value of the currently borrowed security assets */

  // Current total assets = cash + long market value + short quantity × (borrow
  // price - current price)
  ...
  // Currently invested principal value assets = invested principal + invested
  // asset value
  //                = base_cash + base_asset
  // Current profit = current net assets - currently invested principal value
  // assets
  //         = cash + market_value - short_market_value - borrow_cash -
  //         base_cash - base_asset
  ...
  // Total liabilities
  price_t total_borrow() const { return borrow_cash + borrow_asset; }

  // Currently invested principal value assets
  price_t total_base() const { return base_cash + base_asset; }

  // Current profit
  price_t profit() const {
    return cash + market_value - short_market_value - borrow_cash - base_cash -
           base_asset;
  }
...
template <>
struct fmt::formatter<hayaku::FundsRecord> : ostream_formatter {};
#endif
