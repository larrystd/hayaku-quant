 ...
 * @ingroup DBConnect
 *
 * It provides an asynchronous SQL statement execution interface based on the
 * boost::asio coroutine. All the I/O operations (such as exec, moveNext and
 * getColumn) return an awaitable.
 *
 * @note It keeps the same bind and getColumn interface design as
 * SQLStatementBase
 * @note Only the I/O intensive operations are made asynchronous; the local
 * operations such as the data binding stay synchronous
 */
class HAYAKU_UTILS_API AsyncSQLStatementBase {
 public:
  /**
   * Constructor
   ...
   */
  AsyncSQLStatementBase(AsyncDBConnectBase *driver,
  ...
  //-------------------------------------------------------------------------
  // Synchronous methods - the bind, getColumn and getLastRowid operations are
  // local memory operations, they stay synchronous
  //-------------------------------------------------------------------------

  /** Get the rowid of the last record inserted by the INSERT execution, it is
   * not thread safe */
  ...
  /** Get the data given by idx into item */
  template <typename T>
  typename std::enable_if<std::numeric_limits<T>::is_integer>::type getColumn(
      int idx, T &item);
  ...
  /** Get the given data into item1, item2 and item3 sequentially starting from
   * the given idx */
  ...
  /** Bind the value of item to the SQL parameter given by idx */
  template <typename T>
  typename std::enable_if<std::numeric_limits<T>::is_integer>::type bind(
  ...
  sub_exec() = 0;  ///< Subclass interface @see exec
  virtual net::awaitable<bool>
  sub_moveNext() = 0;  ///< Subclass interface @see moveNext

  //-------------------------------------------------------------------------
  // Subclass interface - the synchronous methods (the bind, getColumn and
  // getLastRowid operations are usually local, so they stay synchronous)
  //-------------------------------------------------------------------------
  virtual uint64_t
  sub_getLastRowid() = 0;  ///< Subclass interface @see getLastRowid
  virtual void sub_bindNull(int idx) = 0;  ///< Subclass interface @see bind
  virtual void sub_bindInt(
      int idx, int64_t value) = 0;  ///< Subclass interface @see bind
  virtual void sub_bindDouble(
      int idx, double item) = 0;  ///< Subclass interface @see bind
  virtual void sub_bindDatetime(
      int idx, const Datetime &item) = 0;  ///< Subclass interface
                                           ///< @see bind
  virtual void sub_bindText(
      int idx, const std::string &item) = 0;  ///< Subclass interface
                                              ///< @see bind
  virtual void sub_bindText(int idx, const char *item,
                            size_t len) = 0;  ///< Subclass interface
                                              ///< @see bind
  virtual void sub_bindBlob(
      int idx, const std::string &item) = 0;  ///< Subclass interface
                                              ///< @see bind
  virtual void sub_bindBlob(
      int idx, const std::vector<char> &item) = 0;  ///< Subclass interface
                                                    ///< @see bind

  virtual int sub_getNumColumns() const = 0;  ///< Subclass interface
                                              ///< @see getNumColumns
  virtual void sub_getColumnAsInt64(
      int idx,
      int64_t &) = 0;  ///< Subclass interface @see getColumn
  virtual void sub_getColumnAsDouble(
      int idx,
      double &) = 0;  ///< Subclass interface @see getColumn
  virtual void sub_getColumnAsDatetime(
      int idx,
      Datetime &) = 0;  ///< Subclass interface @see getColumn
  virtual void sub_getColumnAsText(
      int idx,
...
//-------------------------------------------------------------------------
// Implementation of the synchronous methods (the bind and getColumn operations
// are local memory operations, they stay synchronous)
//-------------------------------------------------------------------------
...
//-------------------------------------------------------------------------
// Implementation of the synchronous methods (the bind operations stay
// synchronous)
//-------------------------------------------------------------------------
...
inline void AsyncSQLStatementBase::bind(int idx) { sub_bindNull(idx); }
...
inline void AsyncSQLStatementBase::bindBlob(int idx,
...
template <typename T>
typename std::enable_if<std::numeric_limits<T>::is_integer>::type
AsyncSQLStatementBase::bind(int idx, const T &item) {
