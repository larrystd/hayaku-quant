...
KDataImpPtr KDataShmBufferImp::create(const Stock& stock, const KQuery& query) {
    ...
    if (KQuery::isBaseKType(query.kType())) {
        if (query.queryType() == KQuery::INDEX) {
            ...
            HAYAKU_IF_RETURN(!conn->getIndexRangeByDate(market, code, query, start, end), nullptr);
        } else {
            return nullptr;
        }
    } else {
    ...
    // locally)
    HAYAKU_IF_RETURN(!conn->tryGetKRecordView(market, code, query.kType(), start, end, view), nullptr);
