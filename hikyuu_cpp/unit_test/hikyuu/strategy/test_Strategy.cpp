 ...
 *
 * Strategy public-order routing baselines. These tests intentionally avoid starting the live
 * strategy runtime; they verify the stable command boundary only.
 */
...
public:
    ...
    TradeRecord buy(const Stock&, price_t price, double num, double stoploss, double goalPrice,
                    SystemPart part, const string& remark) override {
        ++buyCount;
        lastPrice = price;
        lastNum = num;
    ...
    TradeRecord sell(const Stock&, price_t price, double num, price_t stoploss, price_t goalPrice,
                     SystemPart part, const string& remark) override {
        ++sellCount;
        lastPrice = price;
        lastNum = num;
    ...
    SystemPart lastPart{PART_INVALID};
    string lastRemark;
};

Stock makeTestStock() {
    return Stock("TS", "000001", "Strategy order test", 0, true, Datetime(20000101),
                 Null<Datetime>(), 0.01, 0.01, 2, 100, 1000);
}
...
TEST_CASE("test_Strategy_order_routes_buy") {
    ...
    CHECK_EQ(strategy.lastPart, PART_SIGNAL);
...
TEST_CASE("test_Strategy_order_captures_current_negative_quantity_behavior") {
    ...
    /**
     * @arg Record the current behavior: even an exact negative lot is converted to the sell-all
     * sentinel. This is a known defect candidate, not the desired future contract.
     */
