...
#include "app/runtime/schedule/inner_tasks.h"
#include "data/driver/kdata/cvs/KDataTempCsvDriver.h"
#include "app/plugin/interface/plugins.h"
#include "app/plugin/device.h"
#include "app/plugin/hkuextra.h"
...
DataRuntimeState& dataRuntimeState() {
    // The legacy process runtime is intentionally not destroyed by static-object ordering. Normal
    // shutdown remains owned by GlobalInitializer; explicit sessions call releaseDataRuntime().
...
DataRuntime& getDataRuntime() {
    ...
    if (!state.runtime) {
        ...
        if (!state.plugin_path.empty()) {
            state.runtime->setPluginPath(state.plugin_path);
        }
...
void setDataRuntimePluginPath(const std::string& path) {
    auto& state = dataRuntimeState();
    std::lock_guard<std::mutex> lock(state.mutex);
    state.plugin_path = path;
    if (state.runtime) {
        state.runtime->setPluginPath(path);
    }
}

const std::string& getDataRuntimePluginPath() noexcept {
    auto& state = dataRuntimeState();
    auto* runtime = state.active.load(std::memory_order_acquire);
    return runtime ? runtime->getPluginPath() : state.plugin_path;
}
