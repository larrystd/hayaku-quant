...
string Portfolio::str() const {
    ...
    os << "Portfolio{\n"
       ...
       << (getAccount() ? fmt::format("Account({})", getAccount()->accountId().value())
                        : "Account(NULL)")
       << strip << "}";
    return os.str();
}
...
Portfolio::Portfolio(const string& name, const internal::PortfolioAccountPortPtr& account,
...
PortfolioPtr Portfolio::clone() {
    ...
    if (m_af)
        p->m_af = m_af->clone();
    if (m_account)
        p->m_account = m_account->cloneAccount();
    if (m_cashAccount)
        p->m_cashAccount = m_cashAccount->cloneAccount();
    return p;
}
...
void Portfolio::runMoment(const Datetime& date, const Datetime& nextCycle, bool adjust) {
    ...
    if (trace) {
        ...
        HAYAKU_INFO("{}: {}", htr("[PF] current running system size"), m_running_sys_set.size());
    }
...
void Portfolio::run(const KQuery& query, bool force) {
    ...
    to_lower(mode);
    if (mode != "query") {
        HAYAKU_CHECK(query.kType() == KQuery::DAY, "{} {}", query.kType(),
...
void Portfolio::traceMomentTMAfterRunAtOpen(const Datetime& date) {
    HAYAKU_IF_RETURN(!getParam<bool>("trace") || m_running_sys_set.empty(), void());
    ...
    for (const auto& sys : m_running_sys_set) {
        ...
        HAYAKU_INFO("| {:<11}| {:<11}| {:<11}| {:<13.2f}| {:<12.2f}|", stk.market_code(), stk_name,
...
void Portfolio::traceMomentTMAfterRunAtClose(const Datetime& date) {
    ...
    for (const auto& sys : m_running_sys_set) {
        ...
        KRecord krecord = stk.getKRecord(date, m_query.kType());
        auto stk_name = stk.name();
        HAYAKU_INFO("| {:<11}| {:<11}| {:<11}| {:<13.2f}| {:<13.2f}| {:<12.2f}| {:<12.2f}|",
