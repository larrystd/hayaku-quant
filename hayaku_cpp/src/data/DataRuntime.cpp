...
DataRuntimeState& dataRuntimeState() {
    // Keep the state container process-lived to avoid static-destruction ordering; Session-owned
    // DataRuntime instances are still explicitly released.
...
DataRuntime& getDataRuntime() {
    ...
    HAYAKU_CHECK(runtime, "DataRuntime is not active; open a HayakuSession before accessing data");
    return *runtime;
}
...
void DataRuntime::init(const Parameter& baseInfoParam, const Parameter& blockParam,
                       const Parameter& kdataParam, const Parameter& preloadParam,
                       const Parameter& hayakuParam, const StrategyContext& context) {
    std::lock_guard<std::mutex> lock(m_init_mutex);
    HAYAKU_WARN_IF_RETURN(m_initializing, void(),
    ...
    HAYAKU_CHECK(!context.empty(), "No stock code list is included in the context!");
...
void DataRuntime::_negotiateShmServer() {
    ...
    HAYAKU_IF_RETURN(!m_hayakuParam.tryGet<bool>("use_shm_server", false), void());
    // The app-side resolver returns no source when this process is a service host. The data
    // runtime only asks for an explicitly assembled client source; it does not decide the service
    // role or discover a plugin itself. A missing source degrades to the configured local driver.
    ...
    auto wait_timeout = m_hayakuParam.tryGet<int64_t>("shm_server_wait_timeout", 600);
    HAYAKU_WARN_IF_RETURN(!source->connect(m_datadir, wait_timeout < 0 ? 0 : (uint64_t)wait_timeout),
                       void(), "Failed connect to hayaku shm server, fallback to standalone mode!");
    ...
    m_ipc_kdata_pool =
      std::make_shared<KDataDriverConnectPool>(source->createKDataDriver(local_pool));
    ...
    HAYAKU_INFO("Connected to hayaku shm server: {}, running in client mode.", source->serverAddr());
}

bool DataRuntime::isIpcClientMode() const {
    return m_ipc_client_mode;
}

// ── LoadEvent event bus (the core library only dispatches and the plugin subscribes; see
// design §5.2)
namespace {
...
// destruction chain would call std::terminate (the same as constraint 2 of the old §4.7).
// Therefore the container itself is also placed on the heap and never released.
...
void DataRuntime::loadAllKData() {
    ...
    HAYAKU_INFO_IF(lazy_preload && canLazyLoad(KQuery::MIN), htr("Use lazy preload!"));
...
void DataRuntime::_loadAllKDataSerial(vector<KQuery::KType> ktypes, vector<string> low_ktypes) {
    ...
    if (!m_cancel_load && m_hayakuParam.tryGet<bool>("load_history_finance", true)) {
        ThreadPool tg;
        std::shared_lock<std::shared_mutex> lock(*m_stockDict_mutex);
        for (auto iter = m_stockDict.begin(); iter != m_stockDict.end(); ++iter) {
            if (m_cancel_load) {
...
void DataRuntime::_loadAllKDataParallel(vector<KQuery::KType> ktypes, vector<string> low_ktypes) {
...
std::unordered_set<string> DataRuntime::tryLoadAllKDataFromColumnFirst(
  const vector<KQuery::KType>& ktypes) {
    ...
    auto driver = _getKDataDriverPool();
    HAYAKU_IF_RETURN(!driver || !driver->getPrototype()->isColumnFirst(), loaded_codes);
...
const string& DataRuntime::tmpdir() const {
    return m_tmpdir;
}

const string& DataRuntime::datadir() const {
    return m_datadir;
}
...
StockList DataRuntime::getStockList(std::function<bool(const Stock&)>&& filter) const {
    StockList ret;
    std::shared_lock<std::shared_mutex> lock(*m_stockDict_mutex);
...
BlockList DataRuntime::getStockBelongs(const Stock& stk, const string& category) {
    BlockList result;
...
DatetimeList DataRuntime::getTradingCalendar(const KQuery& query, const string& market) {
    auto marketinfo = getMarketInfo(market);
    return getStock(fmt::format("{}{}", marketinfo.market(), marketinfo.code()))
      .getDatetimeList(query);
}

DatetimeList DataRuntime::getTradingCalendar(const StockList& stk_list, const KQuery& query) {
...
const ZhBond10List& DataRuntime::getZhBond10() const {
    return m_zh_bond10;
}
...
bool DataRuntime::isTradingHours(const Datetime& d, const string& market) const {
    HAYAKU_IF_RETURN(isHoliday(d), false);
    auto hour = d - d.startOfDay();
    MarketInfo marketinfo = getMarketInfo(market);
    HAYAKU_CHECK(marketinfo != Null<MarketInfo>(), "{}: {}!", htr("Not found market info"), market);
    HAYAKU_IF_RETURN((hour >= marketinfo.openTime1() && hour <= marketinfo.closeTime1()) ||
                    (hour >= marketinfo.openTime2() && hour <= marketinfo.closeTime2()),
                  true);
    return false;
}

Stock DataRuntime::addTempCsvStock(const string& code, const string& day_filename,
...
bool DataRuntime::addStock(const Stock& stock) {
    ...
    HAYAKU_ERROR_IF_RETURN(m_stockDict.find(market_code) != m_stockDict.end(), false,
...
void DataRuntime::loadAllStockWeights() {
    HAYAKU_IF_RETURN(!m_hayakuParam.tryGet<bool>("load_stock_weight", true), void());
...
void DataRuntime::releaseShmServerBaseInfoCache() {
    // The app assembly enables this only for the process that publishes a complete base-info
    // snapshot. Ordinary standalone and client runtimes keep their materialized cache.
    HAYAKU_IF_RETURN(!isBaseInfoCacheEvictionEnabled() || isIpcClientMode(), void());
    HAYAKU_DEBUG(htr("Release stock weight/finance cache after shm base info published"));
    std::shared_lock<std::shared_mutex> lock1(*m_stockDict_mutex);
    for (auto iter = m_stockDict.begin(); iter != m_stockDict.end(); ++iter) {
        Stock& stock = iter->second;
        {
...
vector<std::pair<size_t, string>> DataRuntime::getHistoryFinanceAllFields() const {
...
void DataRuntime::waitDataReady() const {
    HAYAKU_INFO_IF(!dataReady(), htr("Waiting for preload data loading to complete..."));
    while (!dataReady()) {
