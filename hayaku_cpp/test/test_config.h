...
// Cross-compiler: force the compiler not to optimize and not to delete the
// given variables Used for the benchmark, preventing the compiler from deleting
// the "useless" intermediate calculations
#if defined(_MSC_VER)
...
inline void print_indicator(const Indicator& result) {
  HAYAKU_INFO("Indicator: {}", result);
  for (size_t i = 0, total = result.size(); i < total; ++i) {
    HAYAKU_INFO("{}: {:<.6f}", i, result[i]);
  }
}
inline void print_check_indicator(const Indicator& result,
                                  const Indicator& expect) {
  for (size_t i = 0, total = result.size(); i < total; ++i) {
    HAYAKU_INFO("{}: {:<.6f}(result), {:<.6f}(expect)", i, result[i],
                expect[i]);
  }
}
