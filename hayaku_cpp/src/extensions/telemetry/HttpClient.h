...
#include "extensions/telemetry/ResourceAsioPool.h"
 ...
 *
 * It is used for the streaming response processing and is called every time a
 * data chunk is received. It is suitable for the scenarios such as the large
 * file download and the realtime data stream, avoiding loading everything into
 * the memory at once.
 *
 * @param data the data chunk pointer
 * @param size the data chunk size (the number of the bytes)
 *
 * @note The callback function should stay non-blocking, avoiding the
 * time-consuming operations
 * @note An exception should not be thrown in the callback, otherwise the
 * request is terminated
 */
using HttpChunkCallback = std::function<void(const char* data, size_t size)>;

class HAYAKU_UTILS_API AsioHttpClient;
 ...
 *
 * It encapsulates the complete HTTP response information, including the status
 * code, the response headers and the response body. It is suitable for the
 * traditional scenario of loading the whole response body at once.
 *
 * @note For a large response body (>100MB) or a memory limited scenario,
 * AsioHttpStreamResponse is recommended
 */
class HAYAKU_UTILS_API AsioHttpResponse final {
  friend class HAYAKU_UTILS_API AsioHttpClient;
 ...
 public:
   ...
   * @throws nlohmann::json::exception when the response body is not a valid
   * JSON
   */
  hayaku::json json() const;
   ...
   * @return the response header value; an empty string is returned when it does
   * not exist
   */
   ...
   * @return the value of Content-Length; 0 is returned when it is not set or
   * the parsing fails
   */
 ...
 *
 * It is used for the streaming download of a large response body, avoiding
 * loading everything into the memory at once. Both the Content-Length and the
 * Transfer-Encoding: chunked modes are supported.
 *
 ...
 * - The scenario where the complete data is needed for the processing (such as
 * a JSON parsing)
 *
 ...
 */
class HAYAKU_UTILS_API AsioHttpStreamResponse final {
  friend class HAYAKU_UTILS_API AsioHttpClient;
 ...
 public:
   ...
   * @return true means Transfer-Encoding: chunked is used and false means
 ...
 *
 * It supports the HTTP/1.1 and HTTPS protocols and provides the synchronous and
 * asynchronous request modes. It uses the connection pool technology to improve
 * the performance and supports the streaming response for the large file
 * download.
 *
 * ## Basic features
 * - Underlying network library: Boost.Beast (HTTP/1.1) + Boost.Asio (the
 * asynchronous I/O)
 * - Coroutine support: the C++20 coroutines, the return type is
 * boost::asio::awaitable<T>
 * - Connection pool: it manages the HTTP persistent connections automatically
 * and supports the version detection
 * - SSL/TLS: the optional HTTPS support (OpenSSL is required)
 * - Timeout control: the timeout of every stage of the DNS resolution, the
 * connection, the sending and the receiving
 * - Streaming: Content-Length and Transfer-Encoding: chunked are supported
 *
 * ## Running modes
 * - **The internal io_context mode**: the default constructor and the
 * constructor with a URL create an independent io_context and start the
 * background threads to run the event loop, the user does not need to manage it
 *   manually
 * - **The external io_context mode**: it is injected through the constructor
 * with an external io_context parameter, the lifetime is fully controlled from
 * outside and multiple clients can share the same io_context
 *
 * ## Asynchronous operation modes
 * - **The wait-for-completion mode** (async_*): it is suitable for the
 * scenarios needing a return value, an exception handling or a guaranteed order
 * - **The fire-and-forget mode**: it is suitable for the scenarios not caring
 * about the result, such as the background logging and the monitoring reporting
 * (to be implemented)
 *
 * ## Resource management
 * - std::unique_ptr<net::io_context> is used to manage the internal io_context
 * - An executor_work_guard is created to prevent the io_context from exiting
 * when there is no task
 * - At the destruction the work_guard is released first, then it waits for the
 * worker threads to be finished, and finally the io_context is stopped
 ...
 * @endcode
 *
 * @note This class cannot be moved or copied, because it manages the lifetimes
 * of the background threads and the io_context
 * @note When an external io_context is used, its lifetime must be longer than
 * the client
...
class HAYAKU_UTILS_API AsioHttpClient {
 public:
   ...
   *
   * It creates the internal io_context and starts the background threads to run
   * the event loop.
   *
   * @param thread_count the number of the worker threads, 1 by default
   * @param max_concurrency the maximum number of the concurrent connections of
   * the connection pool, 0 means unlimited
   */
   ...
   *
   * It creates the internal io_context, starts the background threads and sets
   * the target URL.
   *
   ...
   * It is suitable for the scenario where multiple clients share the same event
   * loop.
   *
   * @param ctx the external io_context reference, its lifetime is managed by
   * the caller
   * @param url the target URL (e.g. "https://api.example.com:8080/v1")
   * @param timeout the timeout (ms); MAX_TIMEOUT_MS is used when it is <= 0
   * @param max_concurrency the maximum number of the concurrent connections of
   ...
   * 1. Release the executor_work_guard, allowing the io_context to exit
   * naturally
   * 2. Wait for all the worker threads to be finished
   * 3. Explicitly stop the io_context
   *
   * @note It ensures that all the asynchronous operations are finished safely,
   * without a forced interruption
   */
  ...
  // The move operations are disabled, because managing the lifetimes of the
  // background threads and the io_context is unsafe
   ...
   *
   * It parses the URL and extracts the information such as the protocol, the
   * host, the port and the path. The connection pool parameters are updated
   * automatically.
   *
   ...
   *
   * It controls the timeout of every stage of the DNS resolution, the
   * connection, the sending and the receiving.
   *
   * @param ms the timeout (ms)
   *
   * @note When ms <= 0, MAX_TIMEOUT_MS (60000 ms) is used automatically as the
   * default value
   ...
   *
   * It is used to start a custom coroutine or an asynchronous operation on the
   * io_context managed by AsioHttpClient. It follows the executor exposure
   * convention and does not expose the internal implementation details.
   *
   ...
   * @param headers the request header map, it is moved into the internal
   * storage
   */
   ...
   * Configuration priority: the custom CA certificate > the system default
   * certificate store.
   *
   * @param filename the CA certificate file path (the PEM format)
   *
   * @note It must be set before the HTTPS connection is established
   * @note An exception is thrown at the connection when the file does not exist
   * or its format is wrong
   * @see HttpAsyncClient the asynchronous communication, resource management
   * and timeout control specification, item 7
   */
  void setCaFile(const std::string& filename);

  /**
   * @brief Set the default request headers (the copy semantics)
   * @param headers the request header map, it is copied into the internal
   * storage
   */
  ...
  // They return net::awaitable and need to be called with co_await in a
  // coroutine

  /**
   * @brief General asynchronous HTTP request
   *
   * The underlying asynchronous request implementation, it supports all the
   * HTTP methods and the custom parameters. It handles the whole process of the
   * DNS resolution, the connection establishment, the request sending and the
   * response receiving automatically.
   *
   * @param method the HTTP method (GET, POST, PUT, DELETE, etc.)
   * @param path the request path (e.g. "/api/users")
   * @param params the URL query parameters (a GET request) or the form
   * parameters (a POST form)
   ...
   * @param content_type the content type (e.g. "app/json", "text/plain")
   ...
   *
   * @note A boost::system::system_error exception is thrown on a timeout or a
   * network error
   * @note The concrete error cause can be determined by catching system_error
   * and checking its error_code
   * @note operation_aborted means it was cancelled due to a timeout
   */
  net::awaitable<AsioHttpResponse> async_request(
   ...
   * @return AsioHttpResponse the complete HTTP response
   */
  net::awaitable<AsioHttpResponse> async_post(const std::string& path,
                                              const HttpParams& params,
                                              const HttpHeaders& headers,
                                              const char* body, size_t len,
                                              const std::string& content_type) {
    co_return co_await async_request("POST", path, params, headers, body, len,
                                     content_type);
  }
  ...
  net::awaitable<AsioHttpResponse> async_post(const std::string& path,
                                              ...
                                              const char* body, size_t len,
                                              const std::string& content_type) {
    co_return co_await async_request("POST", path, {}, headers, body, len,
                                     content_type);
  }
   ...
   * @return AsioHttpResponse the complete HTTP response
   */
  net::awaitable<AsioHttpResponse> async_post(
      const std::string& path, const HttpParams& params,
      const HttpHeaders& headers, const std::string& content,
      const std::string& content_type = "text/plain") {
    co_return co_await async_post(path, params, headers, content.data(),
   ...
   * @return AsioHttpResponse the complete HTTP response
   *
   * @note Content-Type is set to "app/json" automatically
   */
  net::awaitable<AsioHttpResponse> async_post(const std::string& path,
                                              const HttpParams& params,
                                              const HttpHeaders& headers,
                                              const json& body) {
    co_return co_await async_post(path, params, headers, body.dump(),
                                  "app/json");
  }
  ...
  /**
   * @brief Streaming asynchronous HTTP request (it supports the large file
   * download)
   *
   * It uses a callback function to process the response data chunks, avoiding
   * loading everything into the memory at once. Both the Content-Length and the
   * Transfer-Encoding: chunked modes are supported automatically.
   *
   ...
   * @param body_len the request body length
   * @param content_type the content type
   * @param chunk_callback the data chunk callback function, it is called every
  ...
  net::awaitable<AsioHttpStreamResponse> async_getStream(
      const std::string& path, const HttpParams& params,
      const HttpHeaders& headers, const HttpChunkCallback& chunk_callback) {
    co_return co_await async_requestStream("GET", path, params, headers,
                                           nullptr, 0, "", chunk_callback);
  }
  ...
  /**
   * @brief Streaming asynchronous POST request (with the parameters and the
   * request body)
   *
   * @param path request path
   * @param params the URL query parameters or the form parameters
  ...
  net::awaitable<AsioHttpStreamResponse> async_postStream(
      ...
      const std::string& content_type,
      const HttpChunkCallback& chunk_callback) {
    co_return co_await async_requestStream("POST", path, params, headers, body,
   ...
   */
  net::awaitable<AsioHttpStreamResponse> async_postStream(
      const std::string& path, const HttpHeaders& headers, const char* body,
      size_t body_len, const std::string& content_type,
      const HttpChunkCallback& chunk_callback) {
    co_return co_await async_requestStream("POST", path, {}, headers, body,
                                           body_len, content_type,
                                           chunk_callback);
  }

  // ==================== Synchronous request methods ====================
  // They block until the request is finished and use the asynchronous
  // implementation internally

  /**
   * @brief General synchronous HTTP request
   *
   * A blocking request, it uses the asynchronous implementation internally and
   * waits for its completion. It is suitable for a non-coroutine environment or
   * the scenario needing a synchronous call.
   *
   * @param method the HTTP method (GET, POST, PUT, DELETE, etc.)
   * @param path request path
   * @param params the URL query parameters or the form parameters
   ...
   * @return AsioHttpResponse the complete HTTP response
   */
  AsioHttpResponse get(const std::string& path,
                       const HttpHeaders& headers = {}) {
  ...
  AsioHttpResponse post(const std::string& path, const HttpParams& params,
                        const HttpHeaders& headers, const std::string& content,
                        const std::string& content_type = "text/plain") {
    return post(path, params, headers, content.data(), content.size(),
                content_type);
  }
  ...
  AsioHttpResponse post(const std::string& path, const HttpParams& params,
    ...
    return post(path, params, headers, body.dump(), "app/json");
  }
  ...
  /**
   * @brief Synchronous streaming HTTP request (it supports the large file
   * download)
   *
   * A blocking streaming request, it uses a callback function to process the
   * response data chunks. Both the Content-Length and the Transfer-Encoding:
   * chunked modes are supported automatically.
   *
   * @param method the HTTP method (GET, POST, etc.)
   * @param path request path
   * @param params the URL query parameters
   ...
   * @note Pay attention to the thread safety of the shared data
   */
  AsioHttpStreamResponse requestStream(const std::string& method,
  ...
  AsioHttpStreamResponse getStream(const std::string& path,
                                   ...
                                   const HttpHeaders& headers,
                                   const HttpChunkCallback& chunk_callback) {
    return requestStream("GET", path, params, headers, nullptr, 0, "",
                         chunk_callback);
  }
  ...
  AsioHttpStreamResponse getStream(const std::string& path,
                                   const HttpHeaders& headers,
                                   const HttpChunkCallback& chunk_callback) {
    return requestStream("GET", path, {}, headers, nullptr, 0, "",
                         chunk_callback);
  }

  /**
   * @brief Synchronous streaming POST request (with the parameters and the
   * request body)
   *
   * @param path request path
   * @param params the URL query parameters or the form parameters
  ...
  AsioHttpStreamResponse postStream(const std::string& path,
                                    ...
                                    const std::string& content_type,
                                    const HttpChunkCallback& chunk_callback) {
    return requestStream("POST", path, params, headers, body, body_len,
  ...
  AsioHttpStreamResponse postStream(const std::string& path,
                                    ...
                                    const std::string& content_type,
                                    const HttpChunkCallback& chunk_callback) {
    return requestStream("POST", path, {}, headers, body, body_len,
                         content_type, chunk_callback);
  }
 ...
 private:
   ...
   * @brief Parse the URL
   *
   * It extracts the information such as the protocol, the host, the port and
   * the path. It is responsible for the parsing logic only and does not modify
   * the external state.
   */
   ...
   *
   * @return pair<the connection object, whether it is newly created>; true
   * means it is a newly created connection
   */
   ...
   *
   * It is responsible for combining the base path, the resource path and the
   * query parameters into a legal URI. It handles the logic such as the URL
   * encoding of the path segments and the deduplication of the slashes
   * automatically.
   *
