...
// Used to record the format error information during the file parsing (the line
 ...
 * @details Multiple different ini files may be read and then the information is
 * processed together
 * @throw std::invalid_argument this exception is thrown when the given file
 * cannot be opened
 * @throw std::logic_error this exception is thrown when the file format is
 * wrong
 * @param filename the given file name
 */
void IniParser::read(const std::string& filename) {
  std::ifstream inifile(HAYAKU_PATH(filename), std::ifstream::in);
  if (!inifile) {
    throw(std::invalid_argument("Can't read file(" + filename + ")!"));
  }

  size_t line_no =
      0;  // The current line number, used to record the error information
  ...
  while (std::getline(inifile, line_str)) {
    ...
    // Check the first comment marker and clear it together with the following
    // characters
    ...
    if (line_str.at(0) == '[') {
      ...
      if (section.empty()) {
        ...
        break;  // The section definition is missing, no further processing is
                // needed
      }
 ...
 * @throw std::invalid_argument this exception is thrown when the given option
 * has no value and no default value is given
 * @param section the given section
 * @param option the given option
 * @param default_str
 * the default value, it is returned when there is no corresponding option
 * value. When it is empty there is no default value, and if there is no
 * corresponding option value a std::invalid_argument exception is thrown. It is
 * empty by default.
 */
 ...
 * @throw std::invalid_argument this exception is thrown when the given option
 * has no value and no default value is given; it is also thrown when the given
 * default value (non-empty) cannot be converted into int,
 * @throw std::domain_error this exception is thrown when the given option value
 * cannot be converted into int
 * @param section the given section
 * @param option the given option
 * @param default_str the default value, returned when there is no corresponding
 * option value; an empty value means none It is empty by default, i.e. no
 ...
 * @throw std::invalid_argument this exception is thrown when the given option
 * has no value and no default value is given; it is also thrown when the given
 * default value (non-empty) cannot be converted into float,
 * @throw std::domain_error this exception is thrown when the given option value
 ...
 * @throw std::invalid_argument this exception is thrown when the given option
 * has no value and no default value is given; it is also thrown when the given
 * default value (non-empty) cannot be converted into double,
 * @throw std::domain_error this exception is thrown when the given option value
 ...
 * @details when the option means true, the acceptable values are: 1|true|yes|on
 * (case insensitive)
 * \n when the option means false, the acceptable values are: 0|false|no|off
 * (case insensitive)
 * @throw std::invalid_argument this exception is thrown when the given option
 * has no value and no default value is given; it is also thrown when the given
 * default value (non-empty) cannot be converted into bool,
 * @throw std::domain_error this exception is thrown when the given option value
...
bool IniParser::getBool(const std::string& section, const std::string& option,
  ...
  if (!default_str.empty()) {
    if (new_default_str != "1" && new_default_str != "0") {
      ...
      if (new_default_str == "TRUE" || new_default_str == "YES" ||
          new_default_str == "ON") {
