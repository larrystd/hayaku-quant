...
TEST_CASE("test_Factor_with_block") {
  ...
  Indicator ma5 = MA(CLOSE(), 5);
  Factor factor("BLOCK_FACTOR", ma5, KQuery::DAY, "factor with sector",
                "test sector feature", false, Datetime::min(), test_block);
...
TEST_CASE("test_Factor_getValues_check") {
  ...
  SUBCASE("Factor with block") {
    Block test_block("industry", "test sector");
    Factor factor("BLOCK_FACTOR", ma5, KQuery::DAY, "test", "description",
...
/** @par Test point: test the full parameter combination of the Factor getValues
 * method */
TEST_CASE("test_Factor_getValues_complete_params") {
  ...
  SUBCASE("All parameters true") {
    IndicatorList result =
        factor.getValues(stocks, query, true, true, true, false);
    CHECK_FALSE(result.empty());
    CHECK_EQ(result.size(), stocks.size());
  }
  ...
  SUBCASE("Check=true with valid stocks") {
    IndicatorList result =
        factor.getValues(stocks, query, false, false, false, true);
    CHECK_FALSE(result.empty());
    CHECK_EQ(result.size(), stocks.size());
  }
}
...
TEST_CASE("test_Factor_getValues_result_correctness") {
  ...
  StockList stocks = {stock1,
                      stock2};       // Two securities of two different markets
  ...
  SUBCASE("Basic functionality and result validation") {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      ...
      CHECK_EQ(ind.getResultNumber(),
               1);  // Verify the number of the result sets

      // Verify the basic attributes of the Indicator with the existing
      // check_indicator method
  ...
  SUBCASE("Parameter combinations validation") {
    ...
    IndicatorList result1 =
        ma5_factor.getValues(stocks, query, false, false, false);
    IndicatorList result2 =
        ma5_factor.getValues(stocks, query, true, false, false);
    IndicatorList result3 =
        ma5_factor.getValues(stocks, query, false, true, false);
    IndicatorList result4 =
        ma5_factor.getValues(stocks, query, false, false, true);
  ...
  SUBCASE("Multi-stock processing validation") {
    // Test the multi-stock handling (it is already multi-stock, this is an
    // extra verification)
...
TEST_CASE("test_Factor_getAllValues") {
  ...
  SUBCASE("Factor with block") {
    Block test_block("industry", "test sector");
    Indicator ma5 = MA(CLOSE(), 5);
    Factor factor("ALL_BLOCK_FACTOR", ma5, KQuery::DAY, "test", "description",
                  false, Datetime::min(), test_block);
...
TEST_CASE("test_Factor_copy_semantics") {
  ...
  Indicator ma5 = MA(CLOSE(), 5);
  Block test_block("industry", "test sector");
  Factor original("COPY_TEST", ma5, KQuery::DAY, "copy test",
                  "detailed description", true, Datetime(202001010000LL),
                  test_block);
...
TEST_CASE("test_Factor_getValues_align") {
  ...
  StockList stocks = {stock1,
                      stock2};  // Two securities of two different markets
  KQuery query(0, 30,
               KQuery::DAY);  // Get 30 days of data for the alignment test
  ...
  SUBCASE("Align=true trading calendar alignment") {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      ...
      // Verify that the indicator size matches the trading calendar (the core
      // alignment check)
  ...
  SUBCASE("Align with fill_null parameter") {
    ...
    IndicatorList result1 =
        factor.getValues(stocks, query, true, false);  // fill_null=false
    IndicatorList result2 =
        factor.getValues(stocks, query, true, true);  // fill_null=true
  ...
  SUBCASE("Align with different query ranges") {
    ...
    for (size_t stock_idx = 0; stock_idx < 2; ++stock_idx) {
      ...
      // Verify the size relation (a longer query range should have more trading
      // days)
...
TEST_CASE("test_Factor_basic_serialize") {
  ...
  // Create the test Factor
  Indicator ma5 = MA(CLOSE(), 5);
  Factor factor1("SERIALIZE_TEST", ma5, KQuery::DAY,
                 "serialization test factor",
...
TEST_CASE("test_Factor_with_block_serialize") {
  ...
  // Create a Factor with a Block
  Indicator ma10 = MA(CLOSE(), 10);
  Factor factor1("BLOCK_SERIALIZE_TEST", ma10, KQuery::WEEK,
                 "serialization test with sector",
                 "test the serialization of a Factor containing a Block", false,
...
TEST_CASE("test_Factor_list_serialize") {
  ...
  factors1.emplace_back("FACTOR_1", ma5, KQuery::DAY, "factor 1",
                        "first test factor");
  factors1.emplace_back("FACTOR_2", ma10, KQuery::WEEK, "factor 2",
                        "second test factor");
  factors1.emplace_back("FACTOR_3", ma20, KQuery::MONTH, "factor 3",
                        "third test factor");
