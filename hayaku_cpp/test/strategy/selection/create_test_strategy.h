...
inline internal::PortfolioAccountPortPtr create_test_account(
  ...
  HAYAKU_CHECK(portfolio_account,
               "Test execution account must implement PortfolioAccountPort");
  return portfolio_account;
}

inline internal::StrategyRuntimePtr create_test_strategy(
    MoneyManagerPtr money_manager, SignalPtr signal,
    string name = "test_strategy",
    const AccountConfig& account_config = AccountConfig()) {
  auto definition = StrategyDefinition(std::move(money_manager),
                                       std::move(signal), std::move(name));
  return std::make_shared<internal::StrategyRuntime>(
      definition, create_test_account(account_config));
}

inline internal::StrategyRuntimePtr create_test_strategy(int fast_n,
                                                         int slow_n) {
