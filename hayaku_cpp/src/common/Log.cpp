...
std::shared_ptr<spdlog::logger> getHayakuLogger() {
  ...
  // SPDLOG_DISABLE_DEFAULT_LOGGER leaves the default logger null until a
  // session opens. Logging before that point must remain safe without creating
  // a file or process-wide logger.
  static auto bootstrapLogger = std::make_shared<spdlog::logger>(
      "hayaku-bootstrap", std::make_shared<spdlog::sinks::null_sink_mt>());
  return bootstrapLogger;
}

void HAYAKU_UTILS_API initLogger(bool not_use_color,
  ...
  logger = std::make_shared<spdlog::async_logger>(
      logname, sinks.begin(), sinks.end(), spdlog::thread_pool(),
