...
Indicator Indicator::operator()(const Indicator& ind) {
    ...
    // parameter semantics of hayaku (alike has verified that m_params are the same, setParam
...
void IKData::_checkParam(const string& name) const {
    if ("kpart" == name) {
        ...
        HAYAKU_ASSERT("KDATA" == part || "OPEN" == part || "HIGH" == part || "LOW" == part ||
...
void IPriceList::_calculate(const Indicator& data) {
    HAYAKU_WARN_IF(!isLeaf() && !data.empty(), "The PRICELIST indicator is not leaf node");
...
Indicator HAYAKU_API PRICELIST(const PriceList& data, const DatetimeList& ds, int discard) {
...
Indicator HAYAKU_API PRICELIST(PriceList&& data, const DatetimeList&& ds, int discard) {
    HAYAKU_CHECK(data.size() == ds.size(),
              "The data length must be the same as the length of the reference date list");
...
Indicator HAYAKU_API PRICELIST(const DatetimeList& dates, double value, int discard) {
...
Indicator HAYAKU_API PRICELIST(DatetimeList&& dates, double value, int discard) {
    IndicatorImpPtr ptr = make_shared<IPriceList>(dates.size(), value, discard);
...
void IContext::_calculate(const Indicator& ind) {
    ...
    HAYAKU_IF_RETURN((self_k == in_k || in_k == null_k) && this->size() != 0, void());

    auto self_dates = m_ref_ind.getDatetimeList();
    // HAYAKU_WARN_IF((self_k == null_k && m_ref_ind.empty() && self_dates.empty()),
...
Indicator HAYAKU_API CONTEXT(bool fill_null, bool use_self_ktype, bool use_self_recover_type) {
    auto p = make_shared<IContext>();
...
Indicator HAYAKU_API CONTEXT(const Indicator& ind, bool fill_null, bool use_self_ktype,
...
Indicator HAYAKU_API CONTEXT(const Indicator& ind, const Stock& stk, bool fill_null) {
...
void IResult::_calculate(const Indicator& ind) {
    int result_ix = getParam<int>("result_ix");
    HAYAKU_IF_RETURN(ind.empty(), void());  // For a formula the ind may not be calculated yet
