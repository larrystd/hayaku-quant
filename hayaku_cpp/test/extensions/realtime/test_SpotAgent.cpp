...
TEST_CASE("test_SpotAgent_stop_from_callback") {
  ...
  for (unsigned attempt = 0; attempt < 128 && listenResult == NNG_EADDRINUSE;
       ++attempt) {
  ...
  address =
      "ipc:///tmp/hayaku-spotagent-stop-" + std::to_string(unique) + ".ipc";
  const int listenResult =
      nng_listen(publisher.socket, address.c_str(), &listener, 0);
#endif
  if (listenResult == NNG_EPERM) {
    WARN_MESSAGE(
        false,
        "NNG listener is not permitted in this sandbox; callback test skipped");
    return;
  }
  ...
  SessionOptions options(
      runtime.getBaseInfoDriverParameter(), runtime.getBlockDriverParameter(),
  ...
  hayaku::flat::FinishSpotListBuffer(
      builder, hayaku::flat::CreateSpotListDirect(builder, &spots));
  ...
  for (int i = 0; i < 20 && !callbackReturned; ++i) {
    ...
    CHECK_EQ(nng_send(publisher.socket, const_cast<char*>(start),
                      sizeof(start) - 1, 0),
             0);
  ...
  /** @arg Stopping from a handler returns without joining its own worker
   * thread. */
