 ...
 * Parameters: n: the period window for calculating the average, it must be an
 * integer greater than 0 Discard number = 0
 */
...
IEma::IEma() : IndicatorImp("EMA", 1) { setParam<int>("n", 22); }
...
IMa::IMa() : IndicatorImp("MA", 1) { setParam<int>("n", 22); }
...
void IMa::_calculate(const Indicator& indicator) {
  ...
  // Welford rolling mean: the state is (valid_count, mean) and the enqueue /
  // dequeue is O(1). It shares the same valid_count increment / decrement logic
  // with IStdev, guaranteeing that MA/STDEV are based on exactly the same
  // sample set.
...
void IMa::_increment_calculate(const Indicator& indicator, size_t startPos) {
  ...
  // Rebuild the Welford mean state with a single pass from the window start
  // startPos+1-n, then roll to total
...
void IMa::_dyn_run_one_step(const Indicator& ind, size_t curPos, size_t step) {
  ...
  // A single pass Welford mean (the left boundary of the dynamic window jumps,
  // so remove is not used)
  size_t valid_count = 0;
  price_t mean = 0.0;
...
void ISma::_dyn_one_circle(const Indicator& ind, size_t curPos, int n,
                           double m) {
...
void ISma::_dyn_calculate(const Indicator& ind) {
  ...
  global_parallel_for_index_void(
      ind.discard(), total,
      [&](size_t i) { _dyn_one_circle(ind, i, n[i], m[i]); }, 400);
...
IWma::IWma() : IndicatorImp("WMA", 1) { setParam<int>("n", 22); }
...
bool IWma::supportIncrementCalculate() const { return getParam<int>("n") > 1; }

size_t IWma::min_increment_start() const { return getParam<int>("n") - 1; }
...
void IWma::_increment_calculate(const Indicator& ind, size_t start_pos) {
  ...
  value_t subsum = 0.0, sum = 0.0;
  for (size_t i = start_pos - n, end = start_pos, count = 1; i < end;
       i++, count++) {
    subsum += src[i];
    sum += src[i] * count;
  }
