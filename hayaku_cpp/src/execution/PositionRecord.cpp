...
PositionRecord::PositionRecord(const Stock& stock, const Datetime& takeDatetime,
                               const Datetime& cleanDatetime, double number,
                               price_t stoploss, price_t goalPrice,
                               double totalNumber, price_t totalMoney,
                               price_t totalCost, price_t totalRisk,
                               price_t sellMoney)
    : stock(stock),
...
string PositionRecord::str() const {
  ...
  os << "Position(" << market << strip << code << strip << name << strip
     << takeDatetime << strip << cleanDatetime << strip << number << strip
     << costPrice << strip << stoploss << strip << goalPrice << strip
     << totalNumber << strip << buyMoney << strip << totalCost << strip
     << totalRisk << strip << sellMoney << strip << buyCount << strip
     << sellCount << ")";
...
price_t PositionRecord::totalProfit() const {
  if (cleanDatetime == Null<Datetime>()) {
    HAYAKU_ERROR("The profit cannot be calculated without cleaned records!");
    return 0.0;
  }
