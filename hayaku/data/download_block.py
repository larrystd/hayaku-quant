...
from hayaku.data.common import modifiy_code
from hayaku.util import *
from hayaku.fetcher.stock.zh_block_em import *
import os
...
_BLOCK_SAVE_PATH = os.path.expanduser('~') + '/.hayaku/downloads/block'
...
def save_block(stkcodes: list,  filename: str):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        for code in stkcodes:
            f.write(f"{code}\n")
    # hayaku_info(f"已保存至 {filename}")


@hayaku_catch(trace=False)
@timeout(60*10)
    ...
    for i, blk in enumerate(blk_list):
        ...
        if is_file_can_download(filename, 5 * 24 * 60 * 60):
            try:
                ...
                hayaku_info(f"{i+1}|{total} 获取行业板块{blk[1]}成分: {len(stk_codes)}")
            except (ConnectionError, ConnectionAbortedError, ConnectionResetError, ConnectionRefusedError) as e:
                hayaku_error(f"ip 封禁! 请至东财官网首页拖动划窗解锁后重试!")
                raise e
            except Exception as e:
                hayaku_warn(f"Failed! {type(e).__name__}: {str(e)}")
                continue
...
def down_em_all_gnbk_info():
    ...
    for i, blk_name in enumerate(blk_names):
        ...
        if is_file_can_download(filename, 30 * 24 * 60 * 60):
            ...
            stk_codes = [modifiy_code(code) for code in stk_codes]
            stk_codes = [code for code in stk_codes if code is not None]
            hayaku_info(f"{i+1}|{total} 获取概念板块{blk_name}成分: {len(stk_codes)}")
            save_block(stk_codes, filename)
            time.sleep(random.uniform(1, 3))
...
def down_em_all_dybk_info():
    ...
    for i, v in enumerate(blk_list):
        ...
        stk_codes = [code for code in stk_codes if code is not None]
        save_block(stk_codes, filename)
        hayaku_info(f'{i+1}|{total} 获取地域板块{blk_name}成分: {len(stk_codes)}')
...
def download_all_zsbk_info():
    ...
    for i in range(total):
        ...
        try:
            ...
            stk_codes = [code for code in stk_codes if code is not None]
            save_block(stk_codes, filename)
            hayaku_info("{}|{} 获取指数板块 {}|{} 成分: {}", i, total, blk_code, blk_name, len(stk_codes))
            blk_set[blk_name] = 1
        except KeyboardInterrupt:
            break
        except (ConnectionError, ConnectionAbortedError, ConnectionResetError, ConnectionRefusedError):
            if blk_code[:3] == "399":
                hayaku_warn("ConnectionError! Sina closed!")
                failed_sina = 100
            else:
                hayaku_warn("ConnectionError! CSIndex closed!")
                failed_csindex = 100
        except Exception as e:
