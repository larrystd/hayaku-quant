...
TEST_CASE("test_spearmanLevel") {
  ...
  IndicatorImp::value_t null_value = Null<IndicatorImp::value_t>();
  std::vector<IndicatorImp::value_t> c{null_value, null_value, 5.,        10.,
                                       null_value, null_value, 8.,        10.,
  ...
  std::vector<IndicatorImp::value_t> expectc = {
      null_value, 1., 4.5, null_value, 3., 4.5, 2., null_value};
...
TEST_CASE("test_SPEARMAN_n0_full_window") {
  // The regression of the n=0 default parameter: _calculate normalizes n to
  // total and delegates to _increment_calculate, which must normalize it again,
  // otherwise the window length is 0 and everything is NaN.
  ...
  CHECK_EQ(result.size(), a.size());
  CHECK_EQ(result.discard(), 4);
  // The hard coded expectation pins down the pre-fix "all NaN" fallback and
  // compares it with an explicit n=total
  ...
  /** @arg An empty sequence with n=0: an empty indicator should be returned
   * safely */
  result = SPEARMAN(Indicator(), Indicator(), 0);
  CHECK_UNARY(result.empty());

  /** @arg A very short sequence total=1 with n=0: n is normalized to 1 and
   * m_discard=0+1-1=0; act_count=1<2 so no value is written and result[0] is
   * NaN (the discard is still 0) */
  PriceList short_a{1.};
  PriceList short_b{2.};
  ...
  /** @arg The lower bound trigger total=2 with n=0: n becomes 2 and
   * act_count==2 exactly */
  ...
  /** @arg The input has a discard: it is total when the valid data cannot fill
   * the whole window
   */
...
TEST_CASE("test_SPEARMAN_with_ties") {
  // The tie-handling regression: when the input has equal values (the average
  // rank) the simplified formula 1 - 6*sum_d2/(n^3-n) is no longer exact, the
  // Pearson-on-ranks calculation must be used. The following expected values
  // are derived independently with the tie corrected formula:
  //   rho = [n(n^2-1)/6 - T_x - T_y - sum_d2]
  //         / [2*sqrt((n(n^2-1)/12 - T_x)(n(n^2-1)/12 - T_y))]

  /** @arg A two-way tie: both a and b contain duplicate values (T_x = T_y
   * = 1.0) a={3,3,5,7,7} -> ranks {1.5,1.5,3,4.5,4.5} b={10,8,8,6,10} -> ranks
   * {4.5,2.5,2.5,1,4.5} the buggy simplified formula gives -0.125 while the
  ...
  /** @arg The zero variance degeneration: a is all equal (all the ranks are 3),
   * so 0.0 is returned the buggy simplified formula gives 0.5 (mistaking "no
   * discrimination" for a correlation) */
  ...
  /** @arg A two-way tie + the NaN filter (act_count < n): verify that, after
   * the act_count==n branch is removed, the calculation on the valid pairs only
   * is still correct. The window n=7, 4 valid pairs: (3,10),(5,8),(7,6),(7,10)
   * -> ranks a{1,2,3.5,3.5} b{3.5,2,1,3.5} the correct value is -0.388889 */
  price_t null_value = Null<price_t>();
...
TEST_CASE("test_SPEARMAN_benchmark") {
  ...
  {
    BENCHMARK_TIME_MSG(test_SPEARMAN_benchmark, cycle,
