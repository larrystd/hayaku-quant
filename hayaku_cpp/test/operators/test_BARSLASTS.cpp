...
TEST_CASE("test_BARSLASTS") {
  ...
  // Position 4: the condition holds for the 2nd time, the distance from the 1st
  // (position 0) is 4
  CHECK_EQ(result[4], 4);
  // Positions 5-6: the condition has held twice, the distances from the 1st
  // (position 0) are 5 and 6
  CHECK_EQ(result[5], 5);
  CHECK_EQ(result[6], 6);
  // Position 7: the condition has held twice, the distance from the 1st
  // (position 0) is 7
  CHECK_EQ(result[7], 7);
...
TEST_CASE("test_BARSLASTS_dyn") {
  /** @arg The comparison between the dynamic parameter n and the static
   * parameter n=2 */
  PriceList a;
  for (int i = 0; i < 10; ++i) {
  ...
  /** @arg The comparison between the dynamic parameter n and the IndParam
   * version */
  ...
  // Build a varying n indicator: n=1 in the first half and n=2 in the second
  // half
  PriceList n_values;
  for (int i = 0; i < 10; ++i) {
  ...
  // Verify that the first 5 positions use the n=1 logic (they should match
  // BARSLAST)
