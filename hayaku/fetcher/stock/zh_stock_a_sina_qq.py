...
from hayaku.fetcher.proxy import request_with_proxy, request_with_local


@hayaku_catch(callback=lambda resultstr: hayaku_warn(resultstr))
def parse_one_result_sina(resultstr):
    result = {}
    hayaku_check_ignore(resultstr, "Invalid input param!")
    hayaku_check_ignore(len(resultstr) > 3 and resultstr[:3] == 'var', "Invalid input param! {}", resultstr)
...
def parse_one_result_qq(resultstr):
    result = {}
    hayaku_check_ignore(resultstr, "Invalid input param!")
    # hayaku_check_ignore(len(resultstr) > 3 and resultstr[:2] == 'v_', "Invalid input param! {}", resultstr)
