...
Indicator HAYAKU_API CYCLE(int adjust_cycle, const string& adjust_mode, bool delay_to_trading_day) {
...
Indicator HAYAKU_API CYCLE(const KData& k, int adjust_cycle, const string& adjust_mode,
...
void IFinance::_increment_calculate(const Indicator& data, size_t start_pos) {
    ...
    if (field_ix < 0 && !field_name.empty()) {
        field_ix = static_cast<int>(
          getDataRuntime().getHistoryFinanceFieldIndex(getParam<string>("field_name")));
    }
