...
#include "extensions/mysql/KRecordTable.h"

namespace hayaku {

inline bool isBaseKType(const KQuery::KType& ktype) {
  return (ktype == KQuery::DAY || ktype == KQuery::MIN ||
          ktype == KQuery::MIN5);
}

inline KQuery::KType getBaseKType(const KQuery::KType& ktype) {
  KQuery::KType base_ktype;
  if (ktype == KQuery::WEEK || ktype == KQuery::MONTH ||
      ktype == KQuery::QUARTER || ktype == KQuery::HALFYEAR ||
      ktype == KQuery::YEAR) {
    base_ktype = KQuery::DAY;
  } else if (ktype == KQuery::MIN15 || ktype == KQuery::MIN30 ||
             ktype == KQuery::MIN60 || ktype == KQuery::HOUR2 ||
             ktype == KQuery::HOUR4 || ktype == KQuery::HOUR6 ||
             ktype == KQuery::HOUR12) {
    base_ktype = KQuery::MIN5;
  } else if (ktype == KQuery::MIN3) {
    base_ktype = KQuery::MIN;
  } else {
    HAYAKU_ERROR("Unable to convert ktype {} to a base ktype", ktype);
  }
  return base_ktype;
}
...
bool SQLiteKDataDriver::_init() {
  ...
  for (auto iter = keys.begin(); iter != keys.end(); ++iter) {
    ...
    try {
      ...
      HAYAKU_ERROR("Can't open sqlite file: {}", db_filename);
    }
  }
...
string SQLiteKDataDriver::_getTableName(const string&, const string& code,
                                        KQuery::KType) {
...
KRecordList SQLiteKDataDriver::getKRecordList(const string& market,
                                              const string& code,
                                              const KQuery& query) {
  KRecordList result;
...
KRecordList SQLiteKDataDriver::_getKRecordList(const string& market,
                                               const string& code,
  ...
  try {
    ...
    SQLStatementPtr st = connection->getStatement(
        fmt::format("{} order by date limit {}, {}", r.getSelectSQLNoDB(),
                    start_ix, end_ix - start_ix));
    ...
    HAYAKU_ERROR("Failed to get record by index: {}", key);
  }
  return result;
}
KRecordList SQLiteKDataDriver::_getKRecordList(const string& market,
  ...
  try {
    ...
    SQLStatementPtr st = connection->getStatement(fmt::format(
        "{} where date >= {} and date < {} order by date", r.getSelectSQLNoDB(),
    ...
    // The table may not exist
    HAYAKU_ERROR("Failed to get record by date: {}", key);
  }
  return result;
}
...
size_t SQLiteKDataDriver::getCount(const string& market, const string& code,
  ...
  size_t result = 0;
  result = connection->queryInt(fmt::format("select count(1) from {}",
...
bool SQLiteKDataDriver::getIndexRangeByDate(const string& market,
                                            const string& code,
                                            const KQuery& query,
                                            size_t& out_start,
                                            size_t& out_end) {
  ...
  try {
    out_start = connection->queryInt(
        fmt::format("select count(1) from {} where date<{}", tablename,
                    query.startDatetime().number()),
        0);
    out_end = connection->queryInt(
        fmt::format("select count(1) from {} where date<{}", tablename,
...
KRecordList SQLiteKDataDriver::convertToNewInterval(
    const KRecordList& candles, const KQuery::KType& from_ktype,
  ...
  for (size_t x = 0, total = candles.size(); x < total; ++x) {
    if (candles[x].openPrice != 0 && candles[x].highPrice != 0 &&
        candles[x].lowPrice != 0 && candles[x].closePrice != 0) {
      ...
      if ((result[target].lowPrice == 0) ||
          (candles[x].lowPrice < result[target].lowPrice))
    ...
    if ((x + 1) % old_intervals_per_new_candle == 0) {
      if ((total - x) < old_intervals_per_new_candle + 1) break;
      target++;
    }
  }
