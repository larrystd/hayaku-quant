...
std::ostream& operator<<(std::ostream& os, const TransRecord& record) {
  ...
  os << std::fixed;
  (void)os.precision(4);
  os << "TransRecord(Datetime(\"" << record.datetime.ymdhms() << "\")" << strip
     << record.price << strip << record.vol << strip << record.direct << ")";
...
TransRecord::TransRecord()
    : datetime(Datetime()), price(0.0), vol(0.0), direct(0) {}

TransRecord::TransRecord(const Datetime& datetime, price_t price, price_t vol,
                         int direct)
