...
TEST_CASE("test_MRR") {
  ...
  /** @arg A counter example: a high is made first and then a pit is dug,
   * pinning the look-ahead bias bug (symmetric to IMdd) The original
   * incremental algorithm used the global min of the window as the rise base;
   * when the lowest point of the window appears after the highest point, it
   * overestimated the rise rate. The data [1.0, 2.0, 1.5, 0.5, 1.2], n=4: i=4,
   * the window [2.0, 1.5, 0.5, 1.2] has its lowest 0.5 after the highest 2.0,
   *     the standard MRR=140 (1.2 against its preceding accumulated min 0.5)
   * while the buggy version=300 (2.0 against the global min 0.5) The full
   * calculation with n=4<total=5 goes through the branch B (a longer first
   * segment) + the delegated _increment_calculate, after the fix both segments
   * use the standard run_min base and match the standard MRR.
   */
...
TEST_CASE("test_MRR_with_nan") {
  ...
  PriceList data{1.0,  2.0, 1.5, std::numeric_limits<double>::quiet_NaN(),
                 -5.0, 0.5, 1.2};
  ...
  // i=6, the window [4,6]=[-5,0.5,1.2]; skipping -5 the valid part is
  // [0.5,1.2], run_min=0.5, rr=1.2/0.5-1=140
  CHECK_GE(mrr[6], 0.0);
}
