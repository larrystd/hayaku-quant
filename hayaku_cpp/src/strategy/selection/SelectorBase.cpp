...
void SelectorBase::calculate_proto(const KQuery& query) {
    if (m_proto_query != query && !m_proto_calculated) {
        HAYAKU_WARN_IF_RETURN(m_pro_sys_list.empty(), void(), "m_pro_sys_list is empty!");
...
void SelectorBase::addSystem(const internal::StrategyRuntimePtr& sys) {
    ...
    if (getParam<bool>("depend_on_proto_sys")) {
        HAYAKU_CHECK(sys->getAccount(), "Scenarios that depend on prototype systems need to specify a TM!");
    }
...
void SelectorBase::addStock(const Stock& stock, const internal::StrategyRuntimePtr& protoSys) {
    ...
    HAYAKU_CHECK(!protoSys->getParam<bool>("shared_account"),
              "A selector prototype cannot share an execution account");
    if (getParam<bool>("depend_on_proto_sys")) {
        HAYAKU_CHECK(protoSys->getAccount(),
