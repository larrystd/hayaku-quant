...
void AllocateFundsBase::initParam() {
  // Whether to adjust the positions of the strategies already held; when not,
  // only the current remaining funds of the total account are allocated,
  // otherwise the total market value is allocated Note: whether or not the held
  // strategies are adjusted, the weight ratios are relative to the total assets
  // rather than to the remaining cash balance
  //       adjusting against the remaining cash ratio only is meaningless, and
  //       the allocation may not even complete a real trade due to the trade
  //       cost
  //  adjust_running_sys: True - actively increase or reduce the positions of
  //  the held strategies according to the asset allocation adjust_running_sys:
  //  False - the held strategies are not forcibly increased or reduced
  //  according to the current allocation weights
  setParam<bool>("adjust_running_sys", true);

  // Automatically adjust the weights; in this case the passed weights are
  // regarded as the mutual ratios of the securities (see the ignore_zero_weight
  // description) otherwise the passed weights are taken as the given weights
  // without adjustment (in this case every passed weight must be less than 1)
  setParam<bool>("auto_adjust_weight", true);

  // This parameter takes effect when auto_adjust_weight is used
  // Whether to filter out the 0 values (including the negative ones) and the
  // nan values in the ratio weight list returned by the subclass For example,
  // when the subclass returns the weight ratio list [6, 2, 0, 0, 0]:
  //   with the 0 values filtered out, the actually adjusted weights are Xi /
  //   sum(Xi): [6/8, 2/8] without filtering, m is the number of the non-zero
  //   elements and n is the number of all the elements, (Xi / Sum(Xi)) * (m /
  //   n):
  //           [(6/8)*(2/5), (2/8)*(2/5), 0, 0, 0]
  //          i.e. the total is divided into 5 parts and the relative ratio is
  //          kept within the 2 parts only
  setParam<bool>("ignore_zero_weight", false);

  // Ignore the systems whose score is null in the selected system list,
  // Note: some SEs (such as SE_MultiFactor) may have a similar control
  // themselves
  setParam<bool>("ignore_se_score_is_null", false);

  // Ignore the systems whose score is not greater than 0 in the selected system
  // list
  setParam<bool>("ignore_se_score_lt_zero", false);

  setParam<double>("reserve_percent",
                   0.0);  // Ratio of the assets reserved from the reallocation
...
void AllocateFundsBase::baseCheckParam(const string& name) const {
  if ("reserve_percent" == name) {
    ...
    HAYAKU_ASSERT(reserve_percent >= 0.0 && reserve_percent < 1.0);
  }
}

void AllocateFundsBase::paramChanged() {}

void AllocateFundsBase::reset() { _reset(); }
...
AFPtr AllocateFundsBase::clone() {
  ...
  if (!p || p.get() == this) {
    HAYAKU_WARN("Failed clone! Will use self-ptr!");
...
StrategyWeightList AllocateFundsBase::adjustFunds(
 ...
 * auto_adjust - whether to adjust automatically by ratio, making the total
 * weight 1
 */
void AllocateFundsBase::adjustWeight(StrategyWeightList& sw_list,
  ...
  if (auto_adjust) {
    ...
    //          i.e. the total is divided into 5 parts and the relative ratio is
    //          kept within the 2 parts only
    double per_weight = ignore_zero ? 1.0 / sum * can_allocate_weight
                                    : (new_list.size() * can_allocate_weight) /
                                          (sum * sw_list.size());
...
// All the weight allocations are relative to the total assets rather than to
// the remaining cash The held strategies are not forcibly increased or reduced
// according to the current weights
void AllocateFundsBase::_adjust_without_running(
    const Datetime& date, const StrategyWeightList& se_list,
    const std::unordered_set<internal::StrategyRuntimePtr>& running_set) {
  bool trace = getParam<bool>("trace");
  HAYAKU_INFO_IF(trace, "[AF] {} _adjust_without_running", date);
  ...
  HAYAKU_IF_RETURN(sw_list.size() == 0, void());
  ...
  HAYAKU_INFO_IF(trace,
                 "can_allocate_weight: {:<.4f}, can_allocate_cash: {:<.2f}, "
                 "current cash: {:<.2f}, "
                 "total funds: {:<.2f}, "
                 "reserved funds: {:<.2f}",
                 can_allocate_weight, can_allocate_cash, funds.cash,
                 total_funds, reserve_funds);
  HAYAKU_IF_RETURN(can_allocate_cash <= 1.0, void());

  // Adjust the weights (accumulate and normalize them), sort them in the
  // descending order and filter out the 0 and NaN values
  adjustWeight(sw_list, can_allocate_weight,
               getParam<bool>("auto_adjust_weight"),
               getParam<bool>("ignore_zero_weight"));

  // Traverse the selected subsystem list and transfer the remaining cash into
  // the sub accounts by the weight ratios
  double sum_weight = 0.0;  // The running systems are not adjusted, so their
                            // actual ratio may differ
  for (auto iter = sw_list.begin(), end_iter = sw_list.end(); iter != end_iter;
       ++iter) {
    ...
    // For a running system the calculated weight is not used; the accumulated
    // occupied weight is updated with the actual assets of the subsystem Note:
    // subAccount must be used rather than the total account m_account
    // (otherwise every running system would raise sum_weight to about 1) kType
    // uses the unified m_query of AF, guaranteeing the same valuation context
    // as total_funds
    ...
    price_t need_cash =
        will_cash <= can_allocate_cash ? will_cash : can_allocate_cash;

    // Skip it when the needed funds cannot buy even one lot
    KRecord krecord = iter->strategy->getStock().getKRecord(
        date, iter->strategy->getTO().getQuery().kType());
    if (krecord.isValid() &&
        need_cash < (krecord.closePrice *
                     iter->strategy->getStock().minTradeNumber())) {
      continue;
    }
    ...
    if (cash_account_->checkout(date, need_cash)) {
      ...
      HAYAKU_INFO_IF(trace, "[AF] ({}, {}, weight: {:<.4f}) fetched cash: {}",
                     iter->strategy->name(),
                     iter->strategy->getStock().market_code(), current_weight,
                     need_cash);
...
StrategyWeightList AllocateFundsBase::_adjust_with_running(
  ...
  bool trace = getParam<bool>("trace");
  HAYAKU_INFO_IF(trace, "[AF] {} _adjust_with_running", date);

  // When the selected list is empty everything must be liquidated, so it must
  // not return here HAYAKU_IF_RETURN(se_list.size() == 0, delay_list);
  ...
  StrategyWeightList sw_list = _allocateWeight(date, se_list);
  HAYAKU_IF_RETURN(sw_list.size() == 0, delay_list);
  ...
  //-----------------------------------------------------------------
  // Force a liquidation of the running systems no longer in sw_list first,
  // recalling the allocatable funds The delayed buy systems need no
  // distinction; whatever the type, the liquidation is done immediately with
  // the close price
  //-----------------------------------------------------------------
  ...
  for (const auto& sys : running_set) {
    if (running_in_sw_set.find(sys) == running_in_sw_set.cend()) {
      PositionRecord position =
          sys->getAccount()->getPosition(date, sys->getStock());
      if (position.takeDatetime >= date) {
        // When the buy date of the position is today, it is delayed to the open
        // of the next day
        delay_list.emplace_back(sys, position.number);
      } else {
        auto tr = sys->sellForceOnClose(date, position.number,
        ...
        if (!tr.isNull()) {
          ...
          // A failed liquidation sell is also added into the delayed sell list
          // so that it can be executed on the next trading day
          if (position.number > 0.0) {
            delay_list.emplace_back(sys, position.number);
            HAYAKU_INFO_IF(trace, "[AF] Clean delay {}", sys->name());
          }
        }
      }
    }
  ...
  //-----------------------------------------------------------------
  // For the running systems still in the selected system, reduce their
  // positions according to their weights and recall the allocatable funds
  //-----------------------------------------------------------------
  ...
  std::unordered_set<internal::StrategyRuntimePtr>
      reduced_running_set;  // Cache the running systems already reduced
  for (auto iter = sw_list.begin(), end_iter = sw_list.end(); iter != end_iter;
       ++iter) {
    if (!iter->strategy) {
      continue;
    ...
    if (running_set.find(iter->strategy) != running_set.cend()) {
      ...
      if (sub_total_funds > sub_will_funds) {
        ...
        if (last_close_price <= 0.0) {
          // The security is invalid and cannot be processed, all the assets are
          // lost
          HAYAKU_WARN_IF(trace, "{} has been delisted!",
                         iter->strategy->name());
          continue;
        }
        ...
        double need_back_num =
            static_cast<int64_t>(need_back_funds / last_close_price / min_num) *
            min_num;
        ...
        if (position.takeDatetime >= date) {
          ...
          HAYAKU_INFO_IF(trace,
                         "[AF] Delay deduce position {}, need sell num: {}",
  ...
  //-----------------------------------------------------------------
  // Traverse the currently selected systems and allocate the funds by the given
  // weights
  //-----------------------------------------------------------------
  ...
  HAYAKU_IF_RETURN(can_allocate_cash < 1.0, delay_list);
  ...
  for (auto iter = sw_list.begin(), end_iter = sw_list.end(); iter != end_iter;
    ...
    if (running_set.find(iter->strategy) != running_set.cend()) {
      ...
      price_t sub_total_funds = sub_funds.cash + sub_funds.market_value +
                                sub_funds.borrow_asset -
                                sub_funds.short_market_value;

      // If the position of this system has already been reduced
      if (reduced_running_set.find(iter->strategy) !=
          reduced_running_set.cend()) {
        // The remaining allocatable funds stay unchanged and the occupied
        // weight is accumulated with the actual weight
        sum_weight += sub_total_funds / total_funds;

      } else {
        // A system whose position has not been reduced needs the corresponding
        // funds allocation
        if (sub_total_funds >= will_funds) {
          ...
          // Skip it when the expected funds cannot buy even one lot (including
          // a delisting)
          auto last_price =
              iter->strategy->getStock().getMarketValue(date, query.kType());
          if (need_cash <
              last_price * iter->strategy->getStock().minTradeNumber()) {
            continue;
          }
          ...
          if (cash_account_->checkout(date, need_cash)) {
            ...
            can_allocate_cash =
                roundDown(can_allocate_cash - need_cash, precision);
      ...
      // Calculate the funds the sub account can actually get
      price_t need_cash =
          will_funds <= can_allocate_cash ? will_funds : can_allocate_cash;

      // Try to withdraw the funds from the cash account and deposit them into
      // the sub account
      internal::PortfolioAccountPortPtr subAccount =
          iter->strategy->getAccount();
      if (cash_account_->checkout(date, need_cash)) {
