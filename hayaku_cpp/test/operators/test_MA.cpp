...
TEST_CASE("test_MA") {
  ...
  /** @arg n = 0 and the data contains an interior NaN (the valid_count
   * denominator) */
  {
    PriceList d_nan;
    d_nan.push_back(1.0);
...
TEST_CASE("test_MA_benchmark") {
  ...
  {
    BENCHMARK_TIME_MSG(test_MA_benchmark, cycle,
...
//----------------------------------------------------------------------------
// The NaN semantics test (the Welford rolling mean, sharing the valid_count
// logic with IStdev)
//----------------------------------------------------------------------------
...
TEST_CASE("test_MA_dyn_nan_equivalence") {
  ...
  CHECK_EQ(expect.size(), result.size());
  // The _dyn path discard matches the static one (both have the window-not-full
  // guard)
  CHECK_EQ(expect.discard(), result.discard());
