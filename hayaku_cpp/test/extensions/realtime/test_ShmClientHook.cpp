...
using namespace hayaku::ipc;

namespace {

/** A forwarding recorder: it captures the arguments and the returns of the
 * registered callbacks, verifying that the core library keeps plain callbacks
 * only, depending on no shm type */
...
/** Build the callback group registered to the thin forwarding layer (in the
 * real scenario it is built and registered by the shmserver plugin after a
 * successful connect) */
ShmClientForwarders makeForwarders(ForwardRecorder& rec) {
  ShmClientForwarders fwd;
  fwd.realtimeUpdate = [&rec](const std::string& market_code,
  ...
  fwd.getLastUpdateTime = [&rec](const std::string& market_code,
                                 const KQuery::KType& ktype) {
    rec.last_market_code = market_code;
...
TEST_CASE("test_ShmClientHook_forward") {
  ...
  /** @arg Without a registered client (the standalone mode / no plugin): a safe
   * no-op forwarding
   */
  CHECK_UNARY(!shmClient());
  CHECK_UNARY(!forwardRealtimeUpdate("SH600000", KQuery::DAY, KRecord()));
  CHECK_UNARY(
      !forwardPullFromBufferServer("ipc://buf", {"SH600000"}, KQuery::MIN));
  CHECK_UNARY(forwardGetLastUpdateTime("SH600000", KQuery::DAY) ==
              Datetime::min());
  CHECK_EQ(rec.rt_count, 0);

  /** @arg After the registration the three forwarding paths pass the arguments
   * and the returns through as they are */
  ...
  CHECK_UNARY(forwardGetLastUpdateTime("SZ000001", KQuery::DAY) ==
              rec.lu_result);
  CHECK_EQ(rec.last_market_code, "SZ000001");
  CHECK_EQ(rec.last_lu_ktype, std::string(KQuery::DAY));
  CHECK_EQ(rec.lu_count, 1);

  CHECK_UNARY(forwardPullFromBufferServer("ipc://buf", {"SH600000", "SZ000001"},
                                          KQuery::MIN));
  ...
  /** @arg When the implementer fails the thin layer passes it on faithfully,
   * neither swallowing nor rewriting it */
  rec.rt_result = false;
  rec.pull_result = false;
  CHECK_UNARY(!forwardRealtimeUpdate("SH600000", KQuery::DAY, KRecord()));
  CHECK_UNARY(!forwardPullFromBufferServer("ipc://buf", {}, KQuery::DAY));

  /** @arg After the unregistration (an empty callback group) it becomes a no-op
   * again and the unregistration is repeatable */
...
TEST_CASE("test_ShmClientHook_interrupt") {
  ...
  /** @arg After the registration the check result is reflected faithfully (for
   * the plugin side waiting loop callback) */
  setInterruptChecker([]() { return false; });
  CHECK_UNARY(!checkInterrupted());
  setInterruptChecker([]() { return true; });
  CHECK_UNARY(checkInterrupted());

  /** @arg Restore it to an empty checker, avoiding polluting the other cases in
   * the same process
   */
  setInterruptChecker(nullptr);
  CHECK_UNARY(!checkInterrupted());
