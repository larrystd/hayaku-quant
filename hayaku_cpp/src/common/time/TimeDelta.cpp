...
TimeDelta::TimeDelta(int64_t days, int64_t hours, int64_t minutes,
                     int64_t seconds, int64_t milliseconds,
                     int64_t microseconds) {
  // Limits are added to the parameters to prevent a total overflow
  HAYAKU_CHECK(days <= 99999999 && days >= -99999999,
               "Out of range! Input days: {}", days);
  HAYAKU_CHECK(hours >= -100000 && hours <= 100000,
               "Out of range! Input hours: {}", hours);
  HAYAKU_CHECK(minutes >= -100000 && minutes <= 100000,
               "Out of range! Input minutes: {}", minutes);
  HAYAKU_CHECK(seconds >= -8639900 && seconds <= 8639900,
               "Out of range! Input seconds: {}", seconds);
  HAYAKU_CHECK(milliseconds >= -86399000000 && milliseconds <= 86399000000,
               "Out of range! Input milliseconds: {}", milliseconds);
  HAYAKU_CHECK(microseconds >= -86399000000 && microseconds <= 86399000000,
...
TimeDelta::TimeDelta(const std::string& delta) {
  ...
  to_lower(val);
  auto vals = split(val, ' ');
  HAYAKU_CHECK(vals.size() == 3, "{}", errmsg);
...
TimeDelta HAYAKU_UTILS_API Hours(int64_t hours) {
  HAYAKU_CHECK(hours >= TimeDelta::minTicks() / 3600000000LL &&
...
TimeDelta HAYAKU_UTILS_API Minutes(int64_t mins) {
  HAYAKU_CHECK(mins >= TimeDelta::minTicks() / 60000000LL &&
                   mins <= TimeDelta::maxTicks() / 60000000LL,
               "Out of total range!");
  return TimeDelta::fromTicks(mins * 60000000LL);
}

TimeDelta HAYAKU_UTILS_API Seconds(int64_t secs) {
  HAYAKU_CHECK(secs >= TimeDelta::minTicks() / 1000000LL &&
                   secs <= TimeDelta::maxTicks() / 1000000LL,
               "Out of total range!");
  return TimeDelta::fromTicks(secs * 1000000LL);
}

TimeDelta HAYAKU_UTILS_API Milliseconds(int64_t milliseconds) {
  HAYAKU_CHECK(milliseconds >= TimeDelta::minTicks() / 1000LL &&
...
TimeDelta TimeDelta::operator*(double p) const {
  return TimeDelta::fromTicks(
      static_cast<int64_t>(roundEx<double>(double(ticks()) * p, 0)));
}

TimeDelta TimeDelta::operator/(double p) const {
  HAYAKU_CHECK(p != 0, "Attempt to divide by 0!");
  return TimeDelta::fromTicks(
      static_cast<int64_t>(roundEx<double>(double(ticks()) / p, 0)));
}
...
double TimeDelta::operator/(TimeDelta td) const {
  HAYAKU_CHECK(td.ticks() != 0, "Attemp to divide by zero TimeDelta!");
  return double(ticks()) / double(td.ticks());
}

TimeDelta TimeDelta::operator%(TimeDelta td) const {
  HAYAKU_CHECK(td.ticks() != 0, "Attemp to divide(mod) by zero TimeDelta!");
...
TimeDelta HAYAKU_UTILS_API UTCOffset() {
  ...
  std::call_once(g_tz_set, []() {
    ...
    HAYAKU_CHECK(err == 0, "gmtime_s error!");
  ...
  std::call_once(g_tz_set, []() {
    ...
    // timezone is the number of the seconds of "UTC - the local time" (for
    // UTC+8, timezone = -28800)
    g_timezone = -timezone;
  });
#endif
