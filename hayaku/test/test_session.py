...
"""DataEngine and HayakuSession boundary tests."""
...
class SessionTest(unittest.TestCase):
    ...
    def test_open_close(self):
        session = hayaku.open_session(config_file)
        ...
        session.close()
        with self.assertRaises(hayaku.HAYAKUException):
            _ = session.data

    def test_context_manager_and_queries(self):
        with hayaku.open_session(config_file) as session:
            ...
            self.assertEqual(session.data.get_kdata("sh000001", hayaku.Query(-10)),
                             stock.get_kdata(hayaku.Query(-10)))
    ...
    def test_native_execution_lifecycle(self):
        ...
        session = hayaku.open_session(config_file, account_config=account)
