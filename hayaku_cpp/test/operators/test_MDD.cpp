...
TEST_CASE("test_MDD") {
  ...
  CHECK_EQ(mdd.size(), kdata.size());
  CHECK_EQ(mdd.discard(), 0);
  expects = {0.,      0.,      0.72282, 0.72282, 0.72282,
             3.27389, 3.27389, 3.27389, 3.28155, 2.55377};
  check_indicator(mdd, PRICELIST(expects));
  ...
  expects = {0.0,      0.0,      2.85714,  2.85714,
             12.03704, 16.66667, 16.66667, 28.70370};
  ...
  /** @arg A counter example: a pit is dug first and then a new high is made,
   * pinning the look-ahead bias bug The original incremental algorithm used the
   * global max of the window as the drawdown base; when the highest point of
   * the window appears after the lowest point, it overestimated the drawdown.
   * The data [1.2,1.1,1.0,0.5,1.5,2.0,1.2,1.0], n=5: i=6, the window
   * [1.0,0.5,1.5,2.0,1.2] has its highest 2.0 after the lowest 0.5, the
   * standard MDD=50 (0.5 against its preceding accumulated max 1.0) while the
   * buggy version=75 (0.5 against the global max 2.0) i=7, the window
   * [0.5,1.5,2.0,1.2,1.0] likewise, the standard=50 and the buggy version=75
   * The full calculation with n=5<total=8 goes through the branch B (a longer
   * first segment) + the delegated _increment_calculate, after the fix both
   * segments use the standard run_max base and match the standard MDD per bar.
   */
...
/** @par Test point: the full calculation equals the incremental one (setContext
 * is called repeatedly on the same instance to trigger _increment_calculate) */
...
/** @par Test point: the NaN / non-positive data does not crash (no segfault or
 * division by 0) */
TEST_CASE("test_MDD_with_nan") {
  // The data covers the inner and outer defenses of the scheme 4: i=3 (NaN)
  // hits the outer isnan of the incremental path and i=4 (-5) hits the outer
  // <=0, i=5 has a window containing j=3 (NaN) hitting the inner isnan and i=6
  // has a window containing j=4 (-5) hitting the inner <=0 n=3<total=7: [0,3)
  // goes through the branch B full first segment and [3,7) delegates to the
  // scheme 4 incremental
  PriceList data{100.0, 105.0, 90.0, std::numeric_limits<double>::quiet_NaN(),
                 -5.0,  90.0,  110.0};
  Indicator mdd = MDD(PRICELIST(data), 3);
  CHECK_EQ(mdd.size(), 7);
  // i=3, the incremental current point is NaN; the scheme 4 continues without
  // writing, keeping the buffer NaN
  CHECK(std::isnan(mdd[3]));
  // i=6, the window [5,6]=[90,110] (skipping the -5 at i=4), run_max=90->110,
  // dd=(90-90)/90=0 The actual window [4,6]=[-5,90,110]; after skipping -5 the
  // valid part is [90,110], max=110, dd=0
  CHECK_GE(mdd[6], 0.0);
}
...
TEST_CASE("test_MDD_export") {
  ...
  // Use check_indicator to verify the consistency before and after the
  // serialization
