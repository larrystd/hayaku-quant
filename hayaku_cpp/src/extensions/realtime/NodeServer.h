 ...
 private:
  ...
  static void _processRequest(Work* work) {
    ...
    try {
      ...
      HAYAKU_CHECK(rv == 0, "Failed nng_aio_result!");

      msg = nng_aio_get_msg(work->aio);
      json req = decodeMsg(msg);
      NODE_CHECK(req.contains("cmd"), NodeErrorCode::MISSING_CMD,
                 "Missing command!");
      ...
      // For a tcp connection it tries to get the client address and port into
      // req
      req["remote_host"] = "";
      req["remote_port"] = 0;
      ...
      if (nng_pipe_id(p) > 0) {
        ...
        if (rv == 0) {
          if (ra.s_family == NNG_AF_INET) {
            ...
            inet_ntop(AF_INET, (void*)&ra.s_in.sa_addr, ipAddr,
                      INET_ADDRSTRLEN);
            ...
            inet_ntop(AF_INET6, (void*)&ra.s_in.sa_addr, ipAddr,
                      INET6_ADDRSTRLEN);
