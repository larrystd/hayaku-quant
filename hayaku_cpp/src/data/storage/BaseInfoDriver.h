...
struct StockInfo {
  ...
  static const char* getSelectSQL() {
    return "select c.market, a.code, a.name, a.type, a.valid, a.startDate, "
           "a.endDate, b.tick, "
           "b.tickValue, b.precision, b.minTradeNumber, b.maxTradeNumber from "
           "stock a, "
           "stocktypeinfo b, market c where a.type = b.id and a.marketid = "
           "c.marketid";
  }

  void load(const SQLStatementPtr& st) {
    st->getColumn(0, market, code, name, type, valid, startDate, endDate, tick,
...
class HAYAKU_API BaseInfoDriver {
 ...
 public:
  ...
  /**
   * Driver initialization; when it is implemented concretely, attention should
   * be paid to closing the related resources opened before.
   * @return
   */
   ...
   * @return vector<float> [[financial report release date (ymd), field1,
   * field2, ...], ...]
   */
   ...
   * @return the corresponding security type information; Null<StockTypeInf>()
  ...
  /**
   * Get all the historical financial information, used for the data
   * initialization in column storage
   */
  virtual unordered_map<string, vector<HistoryFinanceInfo>>
  getAllHistoryFinance(const std::atomic_bool& cancel_flag) {
    HAYAKU_ERROR("Not support getAllHistoryFinance()!");
