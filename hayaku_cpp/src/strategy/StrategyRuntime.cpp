...
void StrategyRuntime::run(const BacktestRequest& request) {
    ...
    HAYAKU_DEBUG_IF_RETURN(m_calculated && m_kdata == request.kdata(), void(), "Not need calculate.");
    prepare();
    bind(request.kdata());
    ...
    for (size_t i = 0; i < total; ++i) {
        ...
        if (trace) {
            ...
            if (position.number > 0.0) {
                HAYAKU_INFO("total: {:.2f}, cash: {:.2f}, position: {:.2f}, close: {:.2f}",
...
TradeRecord StrategyRuntime::runMomentOnOpenNative(const KRecord& today,
    ...
    ++m_buyDays;
    ++m_sellShortDays;
    HAYAKU_DEBUG_IF_RETURN((today.closePrice > today.highPrice || today.closePrice < today.lowPrice ||
...
TradeRecord StrategyRuntime::runMomentOnCloseNative(const KRecord& today,
    ...
    HAYAKU_INFO_IF(trace, htr("[{}] current position: {}", m_name, position.number));
    if (position.number == 0.0) {
        return {};
    }
...
TradeRecord StrategyRuntime::buy(const KRecord& today, const KRecord& src_today, OrderOrigin from) {
    ...
    if (today.highPrice == today.lowPrice) {
        ...
        if (today.closePrice > pre_day.closePrice) {
            HAYAKU_INFO_IF(trace, htr("[{}] delay to buy, one-price up-limit board", m_name));
            submitBuyRequest(today, src_today, from);
            return result;
        }
    }
    ...
    if (iszero(today.transAmount) || iszero(today.transCount)) {
        HAYAKU_INFO_IF(trace, htr("[{}] delay to buy, current amount == 0 or count == 0", m_name));
        submitBuyRequest(today, src_today, from);
        return result;
    }
...
TradeRecord StrategyRuntime::buyNow(const KRecord& today, const KRecord& src_today, OrderOrigin from) {
    ...
    if (planPrice <= stoploss) {
        HAYAKU_INFO_IF(trace, htr("[{}] buy failed, planPrice: {} <= stoploss: {}", m_name, planPrice,
                               stoploss));
        return result;
    }
    ...
    if (iszero(number) || number > m_stock.maxTradeNumber()) {
        HAYAKU_INFO_IF(trace, "[{}] {}, number: {} == 0 or > maxTradeNumber: {}, {}", m_name,
...
TradeRecord StrategyRuntime::sellForce(const Datetime& date, double num, OrderOrigin from, bool on_open) {
    bool trace = getParam<bool>("trace");
    HAYAKU_INFO_IF(trace, "[{}] {} {} by {}", m_name, htr("force sell"), num, getOrderOriginName(from));
    ...
    HAYAKU_WARN_IF_RETURN(record == Null<TradeRecord>(), record, "[{}] {}: {} by {}", m_name,
...
TradeRecord StrategyRuntime::sell(const KRecord& today, const KRecord& src_today, OrderOrigin from) {
    ...
    if (iszero(today.transAmount) || iszero(today.transCount)) {
        HAYAKU_INFO_IF(trace, htr("[{}] delay to sell, current amount == 0 or count == 0", m_name));
        submitSellRequest(today, src_today, from);
        return result;
    }
...
TradeRecord StrategyRuntime::buyShort(const KRecord& today, const KRecord& src_today, OrderOrigin from) {
    ...
    if (today.highPrice == today.lowPrice) {
        ...
        size_t pos = m_kdata.getPos(today.datetime);
        if (pos == 0 || pos == Null<size_t>()) {
            HAYAKU_INFO_IF(trace, htr("[{}] delay to buy short, one-price board", m_name));
            submitBuyShortRequest(today, src_today, from);
            return result;
        }
        ...
        const auto& preday = m_kdata.getKRecord(pos - 1);
        if (today.closePrice > preday.closePrice) {
            HAYAKU_INFO_IF(trace, htr("[{}] short covering delayed: limit-up lock", m_name));
            submitBuyShortRequest(today, src_today, from);
            return result;
        }
    }
...
TradeRecord StrategyRuntime::sellShort(const KRecord& today, const KRecord& src_today, OrderOrigin from) {
    ...
    bool trace = getParam<bool>("trace");
    if (getParam<bool>("support_borrow_stock") == false) {
        // HAYAKU_WARN("set system param support_borrow_stock to true to short sell");
        return result;
    }
    ...
    if (today.highPrice == today.lowPrice) {
        ...
        size_t pos = m_kdata.getPos(today.datetime);
        if (pos == 0 || pos == Null<size_t>()) {
            HAYAKU_INFO_IF(trace, htr("[{}] delay to sell short, one-price board", m_name));
            submitSellShortRequest(today, src_today, from);
            return result;
        }
        ...
        const auto& preday = m_kdata.getKRecord(pos - 1);
        if (today.closePrice < preday.closePrice) {
            HAYAKU_INFO_IF(trace, htr("[{}] short selling delayed: limit-down lock", m_name));
            submitSellShortRequest(today, src_today, from);
            return result;
        }
    }
...
TradeRecord StrategyRuntime::processRequest(const KRecord& today, const KRecord& src_today) {
    ...
    HAYAKU_IF_RETURN(m_pendingOrders.sellShort().valid, sellShortDelay(today, src_today));
    HAYAKU_IF_RETURN(m_pendingOrders.buyShort().valid, buyShortDelay(today, src_today));
    return TradeRecord();
}
