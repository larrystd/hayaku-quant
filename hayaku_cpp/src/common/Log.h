 ...
 * @param filename the log file name; it is "./hayaku.log" in the current
 * directory by default when it is empty; you need to guarantee that the
...
#define HAYAKU_TRACE(...) \
  SPDLOG_LOGGER_TRACE(::hayaku::getHayakuLogger(), __VA_ARGS__)
#define HAYAKU_DEBUG(...) \
  SPDLOG_LOGGER_DEBUG(::hayaku::getHayakuLogger(), __VA_ARGS__)
#define HAYAKU_INFO(...) \
  SPDLOG_LOGGER_INFO(::hayaku::getHayakuLogger(), __VA_ARGS__)
#define HAYAKU_WARN(...) \
  SPDLOG_LOGGER_WARN(::hayaku::getHayakuLogger(), __VA_ARGS__)
#define HAYAKU_ERROR(...) \
  SPDLOG_LOGGER_ERROR(::hayaku::getHayakuLogger(), __VA_ARGS__)
#define HAYAKU_FATAL(...) \
  SPDLOG_LOGGER_CRITICAL(::hayaku::getHayakuLogger(), __VA_ARGS__)

///////////////////////////////////////////////////////////////////////////////
//
// Under clang/gcc __PRETTY_FUNCTION__ contains the function parameters, it can
// be specified at compile time #define HAYAKU_FUNCTION __PRETTY_FUNCTION__
//
///////////////////////////////////////////////////////////////////////////////
...
/**
 * If the expression is false an hayaku::exception is thrown with the passed
 * information
...
#define HAYAKU_CHECK(expr, ...)                                                \
  do {                                                                         \
    if (!(expr)) [[unlikely]] {                                                \
      throw ::hayaku::exception(fmt::format(                                   \
          "HAYAKU_CHECK({}) {} [{}] ({}:{})", #expr, fmt::format(__VA_ARGS__), \
          HAYAKU_FUNCTION, __FILE__, __LINE__));                               \
    }                                                                          \
  } while (0)

/**
 * If the expression is false the given exception is thrown with the passed
 * information
 * @note Used for checking the external input parameters and the results
 */
  ...
  do {                                                                    \
    if (!(expr)) [[unlikely]] {                                           \
      throw except(fmt::format("HAYAKU_CHECK({}) {} [{}] ({}:{})", #expr, \
                               fmt::format(__VA_ARGS__), HAYAKU_FUNCTION, \
                               __FILE__, __LINE__));                      \
    }                                                                     \
  } while (0)
#else
  ...
  do {                                                                   \
    if (!(expr)) {                                                       \
      ...
      throw ::hayaku::exception(                                         \
          fmt::format("HAYAKU_CHECK({}) {} [{}] ({}:{})", #expr, errmsg, \
                      HAYAKU_FUNCTION, __FILE__, __LINE__));             \
    }                                                                    \
  } while (0)
  ...
  do {                                                                        \
    if (!(expr)) {                                                            \
      ...
      errmsg = fmt::format("{}\n {}", errmsg,                                 \
                           to_string(boost::stacktrace::stacktrace()));       \
      throw except(fmt::format("HAYAKU_CHECK({}) {} [{}] ({}:{})", #expr,     \
                               errmsg, HAYAKU_FUNCTION, __FILE__, __LINE__)); \
    }                                                                         \
  } while (0)
 ...
 * @note Used for checking the internal input parameters only; it can be
 * disabled at compile time with the HAYAKU_DISABLE_ASSERT macro
 */
#define HAYAKU_ASSERT(expr)                                                  \
  do {                                                                       \
    if (!(expr)) {                                                           \
      std::string err_msg(                                                   \
          fmt::format("HAYAKU_ASSERT({})\n{}", #expr,                        \
                      to_string(boost::stacktrace::stacktrace())));          \
      throw ::hayaku::exception(fmt::format(                                 \
          "{} [{}] ({}:{})", err_msg, HAYAKU_FUNCTION, __FILE__, __LINE__)); \
    }                                                                        \
  } while (0)
...
#define HAYAKU_THROW_EXCEPTION(except, ...)                             \
  do {                                                                  \
    throw except(fmt::format("EXCEPTION: {} [{}] ({}:{})",              \
                             fmt::format(__VA_ARGS__), HAYAKU_FUNCTION, \
                             __FILE__, __LINE__));                      \
  } while (0)
  ...
  do {                                                                       \
    std::string errmsg(                                                      \
        fmt::format("{}\n {}", fmt::format(__VA_ARGS__),                     \
                    to_string(boost::stacktrace::stacktrace())));            \
    throw ::hayaku::exception(fmt::format("EXCEPTION: {} [{}] ({}:{})",      \
                                          errmsg, HAYAKU_FUNCTION, __FILE__, \
                                          __LINE__));                        \
  } while (0)
...
#define HAYAKU_THROW_EXCEPTION(except, ...)                         \
  do {                                                              \
    std::string errmsg(                                             \
        fmt::format("{}\n {}", fmt::format(__VA_ARGS__),            \
                    to_string(boost::stacktrace::stacktrace())));   \
    throw except(fmt::format("EXCEPTION: {} [{}] ({}:{})", errmsg,  \
                             HAYAKU_FUNCTION, __FILE__, __LINE__)); \
  } while (0)
#endif  // #if !HAYAKU_ENABLE_STACK_TRACE
...
/**
 * Print the TRACE information and return the given value when the given
 * condition is satisfied
 * @param expr the given condition
 * @param ret return value
 */
...
/** Used for the printing in catch (...), it reduces the size of the compiled
 * code */
...
#define CLS_TRACE(...) \
  HAYAKU_TRACE(fmt::format("[{}] {}", ms_logger, fmt::format(__VA_ARGS__)))
#define CLS_DEBUG(...) \
  HAYAKU_DEBUG(fmt::format("[{}] {}", ms_logger, fmt::format(__VA_ARGS__)))
#define CLS_INFO(...) \
  HAYAKU_INFO(fmt::format("[{}] {}", ms_logger, fmt::format(__VA_ARGS__)))
#define CLS_WARN(...) \
  HAYAKU_WARN(fmt::format("[{}] {}", ms_logger, fmt::format(__VA_ARGS__)))
#define CLS_ERROR(...) \
  HAYAKU_ERROR(fmt::format("[{}] {}", ms_logger, fmt::format(__VA_ARGS__)))
#define CLS_FATAL(...) \
  HAYAKU_FATAL(fmt::format("[{}] {}", ms_logger, fmt::format(__VA_ARGS__)))

#define CLS_TRACE_IF(expr, ...) \
  HAYAKU_TRACE_IF(expr,         \
                  fmt::format("[{}] {}", ms_logger, fmt::format(__VA_ARGS__)))
#define CLS_DEBUG_IF(expr, ...) \
  HAYAKU_DEBUG_IF(expr,         \
                  fmt::format("[{}] {}", ms_logger, fmt::format(__VA_ARGS__)))
...
#define CLS_CHECK_THROW(expr, except, ...)                                 \
  do {                                                                     \
    if (!(expr)) [[unlikely]] {                                            \
      throw except(fmt::format("[{}] CLS_CHECK({}) {} [{}] ({}:{})",       \
                               ms_logger, #expr, fmt::format(__VA_ARGS__), \
                               HAYAKU_FUNCTION, __FILE__, __LINE__));      \
    }                                                                      \
  } while (0)
#else
