...
#pragma once
#ifndef HIKYUU_TRADE_INTERNAL_EXECUTIONACCOUNTPORT_H
#define HIKYUU_TRADE_INTERNAL_EXECUTIONACCOUNTPORT_H
 ...
 *
 * Portfolio capital transfer, child-account creation and broker synchronization intentionally do
 * not belong here. Those orchestration capabilities use a separate portfolio port.
 */
...
public:
    ...
    [[nodiscard]] virtual PositionRecord getPosition(const Datetime& datetime,
                                                     const Stock& stock) = 0;
    [[nodiscard]] virtual PositionRecord getShortPosition(const Stock& stock) const = 0;
    [[nodiscard]] virtual bool have(const Stock& stock) const = 0;
    [[nodiscard]] virtual bool haveShort(const Stock& stock) const = 0;
    [[nodiscard]] virtual double getHoldNumber(const Datetime& datetime, const Stock& stock) = 0;
    ...
    [[nodiscard]] virtual CostRecord getBuyCost(const Datetime& datetime, const Stock& stock,
                                                price_t price, double number) const = 0;
...
#endif /* HIKYUU_TRADE_INTERNAL_EXECUTIONACCOUNTPORT_H */
