...
TimeDelta::TimeDelta(int64_t days, int64_t hours, int64_t minutes, int64_t seconds,
    ...
    HAYAKU_CHECK(days <= 99999999 && days >= -99999999, "Out of range! Input days: {}", days);
    HAYAKU_CHECK(hours >= -100000 && hours <= 100000, "Out of range! Input hours: {}", hours);
    HAYAKU_CHECK(minutes >= -100000 && minutes <= 100000, "Out of range! Input minutes: {}", minutes);
    HAYAKU_CHECK(seconds >= -8639900 && seconds <= 8639900, "Out of range! Input seconds: {}",
              seconds);
    ...
    int64_t total =
      ...
      microseconds;
    HAYAKU_CHECK(total >= m_min_micro_seconds && total <= m_max_micro_seconds, "Out of total range!");
...
TimeDelta TimeDelta::fromTicks(int64_t ticks) {
    HAYAKU_CHECK(ticks >= m_min_micro_seconds && ticks <= m_max_micro_seconds, "Out of total range!");
