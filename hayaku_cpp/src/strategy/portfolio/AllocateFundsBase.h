 ...
 */
class HAYAKU_API AllocateFundsBase : public enable_shared_from_this<AllocateFundsBase> {
...
public:
     ...
     */
    StrategyWeightList adjustFunds(const Datetime& date, const StrategyWeightList& se_list,
     ...
     */
    virtual StrategyWeightList _allocateWeight(const Datetime& date,
                                             const StrategyWeightList& se_list) = 0;
...
public:
     ...
     * settings
     */
    static void adjustWeight(StrategyWeightList& sw_list, double can_allocate_weight,
...
private:
     ...
     * positions) */
    StrategyWeightList _adjust_with_running(const Datetime& date, const StrategyWeightList& se_list,
                                          const std::unordered_set<internal::StrategyRuntimePtr>& running_list);

    /* It does not adjust the sub-systems already running */
    void _adjust_without_running(const Datetime& date, const StrategyWeightList& se_list,
                                 const std::unordered_set<internal::StrategyRuntimePtr>& running_list);
...
private:
    ...
    internal::PortfolioAccountPortPtr m_account;       // Set by PF at runtime, the actual account of PF
    internal::PortfolioAccountPortPtr m_cashAccount;  // Set by PF at runtime, the shadow account of tm, used to coordinate the fund
                      // allocation
...
public:                                       \
    ...
    virtual StrategyWeightList _allocateWeight(const Datetime&, const StrategyWeightList&) override;
...
inline const internal::PortfolioAccountPortPtr& AllocateFundsBase::getAccount() const {
    return m_account;
}

inline void AllocateFundsBase::setAccount(const internal::PortfolioAccountPortPtr& account) {
    m_account = account;
}
...
inline const internal::PortfolioAccountPortPtr& AllocateFundsBase::getCashAccount() const {
