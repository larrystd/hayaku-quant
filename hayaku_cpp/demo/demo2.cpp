...
static void changed(Strategy* stg, const Stock& stk, const SpotRecord& spot) {
    HAYAKU_INFO("{} {} 当前收盘价: {}", stk.market_code(), stk.name(), spot.close);
}
...
static void my_process2(Strategy* stg) {
    HAYAKU_INFO("run at time: {} {}", Datetime::now(), getStock("sh000001").name());
}
...
int main(int argc, char* argv[]) {
    ...
    auto session = HayakuSession::open(fmt::format("{}/.hayaku/hayaku.ini", getUserDir()), true,
                                       context);
