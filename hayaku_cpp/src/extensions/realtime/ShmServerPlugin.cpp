...
// stopShmServer() is called on final Session close; going through getPlugin would repeatedly try
// dlopen on an ordinary standalone process without an installed / started service and pollute the
// close path.
std::atomic<ShmServerPluginInterface*> g_shm_server_plugin{nullptr};
}  // namespace

bool startShmServer(const std::string& datadir, bool publish_shm, bool recv_spot) noexcept {
    // Mark the app-side server role so future data reloads cannot connect this process back to its
    // own service.
    ...
    HAYAKU_INFO_IF_RETURN(g_shm_server_plugin.load(std::memory_order_acquire) != nullptr, true,
    ...
    if (!plugin) {
        ...
        HAYAKU_ERROR("Can't find {} plugin! It is an optional realtime plugin; check that it is "
