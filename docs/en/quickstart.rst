...
After installing Hayaku with pip, run the ``hayakutdx`` command in a terminal to start the data
...
.. note::
    ...
    If the ``hayakutdx`` command cannot be found in the terminal, look for ``HayakuTDX.exe`` in the
    ...
    ``hayaku`` package under ``python/Lib/site-packages``; its ``gui`` directory contains
    ``HayakuTdx.py``, which you can run directly with ``python HayakuTdx.py`` from the terminal to see
    the error message.
...
.. note::
    ...
    The ``importdata`` command uses the configuration file generated by HayakuTDX. On Windows it is
    recommended to run HayakuTDX at least once so that the configuration exists (you do not need to
    ...
    files in the ``.hayaku`` directory under your user directory.
...
Hayaku and the ideas behind systematic trading step by step. While working with Hayaku you can look
...
You can run Hayaku from any Python client. The figure below shows Hayaku running in a Python shell
...
To use the Hayaku interactive tool, import it first:
        
::

    # import the interactive tool before using hayaku interactively
    from hayaku.interactive import *
    
.. note::

    Hayaku itself is an ordinary Python package, while ``hayaku.interactive`` is the interactive tool
    shipped with it. If you want to build your own application on top of the ``hayaku`` package
    instead of using it interactively, see ``hayaku/interactive/interactive.py`` for how Hayaku is
...
6. For convenience you can create a batch file on the desktop. For example, if you want the working
   directory to be "d:\\workspace\\hayaku\\examples", save the following content as a ".bat" file
...
Hayaku plotting supports CJK characters out of the box. If they are still garbled, try the steps
below.
...
2. Run ``pybind11-stubgen hayaku``; hints then work as expected.

.. note::

    Since version 2.3.1, the pyi files are generated by default when hayaku is packaged.
