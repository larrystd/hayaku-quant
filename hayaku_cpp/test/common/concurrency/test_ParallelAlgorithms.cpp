...
/**
 * @defgroup test_hayaku_algorithm test_hayaku_algorithm
...
TEST_CASE("test_parallelIndexRange2") {
  ...
  if (cpu_num == 32) {
    ...
    expect = {{0, 3},   {3, 6},   {6, 9},   {9, 12},  {12, 15}, {15, 18},
              {18, 21}, {21, 24}, {24, 27}, {27, 30}, {30, 33}, {33, 36},
              {36, 39}, {39, 42}, {42, 45}, {45, 48}, {48, 51}, {51, 54},
              {54, 57}, {57, 60}, {60, 63}, {63, 66}, {66, 69}, {69, 72},
              {72, 75}, {75, 78}, {78, 81}, {81, 84}, {84, 87}, {87, 90},
              {90, 93}, {93, 96}, {96, 97}, {97, 98}, {98, 99}, {99, 100}};
    CHECK_EQ(result.size(), expect.size());
    for (size_t i = 0, len = expect.size(); i < len; i++) {
      CHECK_EQ(result[i].first, expect[i].first);
      CHECK_EQ(result[i].second, expect[i].second);
...
TEST_CASE("test_parallel_for_index") {
  ...
  auto result =
      parallel_for_index(0, values.size(), [](size_t i) { return i + 1; });
...
TEST_CASE("test_parallel_for_index_single") {
  ...
  auto result = parallel_for_index_single(0, values.size(),
                                          [](size_t i) { return i + 1; });
  // auto result = parallel_for_index_single<std::function<size_t(size_t)>,
  // MQStealThreadPool>(
...
/** @par The global thread pool drains on an exception: when a task throws, the
 * other unfinished tasks must finish first. It verifies that
 * wait_for_all_non_blocking does not return early: when the caller starts get,
 * all the tasks of the same batch have finished, leaving no background task
 * that keeps accessing the caller stack / container.
 */
TEST_CASE("test_global_wait_drains_all_futures_on_exception") {
  ...
  // Two tasks: fast throws immediately, slow waits for the signal, finishes
  // delayed, sets the flag
  ...
  // Start together, avoiding that fast finishes before slow is enqueued and the
  // test passes falsely
  ...
  CHECK_UNARY(caught);
  // When the exception returns the slow task must have finished: the drain
  // semantics hold
