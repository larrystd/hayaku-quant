...
bool FileLock::lockFile() noexcept {
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        HAYAKU_ERROR("Failed to open lock file: {} ({})", m_filename, ::GetLastError());
        return false;
    }
...
bool FileLock::lockFile() noexcept {
    ...
    if (fd < 0) {
        HAYAKU_ERROR("Failed to open lock file: {} ({})", m_filename, std::strerror(errno));
        return false;
    }
    ...
    if (ret == -1) {
        ...
        if (errno != EACCES && errno != EAGAIN) {
            HAYAKU_ERROR("Failed to lock file: {} ({})", m_filename, std::strerror(errno));
        }
        ::close(fd);
        return false;
    }
...
void FileLock::unlockFile() noexcept {
    ...
    if (::fcntl(m_handle, F_SETLK, &fl) == -1) {
        HAYAKU_WARN("Failed to unlock file: {} ({})", m_filename, std::strerror(errno));
    }
    ::close(m_handle);
    m_handle = -1;
}
