 ...
 * @ingroup Indicator
 */
inline Indicator MDD(const Indicator& ind, int n = 0) { return MDD(n)(ind); }
 ...
 * point; it is a positive value as per the industry convention
 * @note the values less than or equal to 0 and the nan values are not handled,
...
/**
 * Original RSRS (underlying β) indicator, based on the rolling N-day OLS
 regression

 * Formula: High = α + β · Low

 *
 * Every K-line contributes a coordinate point (Low[i], High[i]), and the N
 points within the rolling
 * window are used for the OLS regression.

 * β is the most original RSRS slope, representing the strength of the support
 and resistance.

 * Defect: the β center fluctuates greatly in different market ranges, so it
 cannot be compared
 ...
 * number of the downward crossings to get the average noise (i.e. within the
 * lookback period, the length by which every low price is lower than the
 * previous day's low price divided by the number of times), and subtract (the
 ...
 * @ingroup Indicator
 */
inline Indicator SAFTYLOSS(const Indicator& data, int n1 = 10, int n2 = 3,
                           double p = 2.0) {
  return SAFTYLOSS(n1, n2, p)(data);
}

inline Indicator SAFTYLOSS(const Indicator& data, const IndParam& n1,
                           const IndParam& n2, double p = 2.0) {
  return SAFTYLOSS(n1, n2, p)(data);
}

inline Indicator SAFTYLOSS(const Indicator& data, const IndParam& n1,
                           const IndParam& n2, const IndParam& p) {
  return SAFTYLOSS(n1, n2, p)(data);
}

inline Indicator SAFTYLOSS(const Indicator& data, const Indicator& n1,
                           const Indicator& n2, double p = 2.0) {
  return SAFTYLOSS(IndParam(n1), IndParam(n2), p)(data);
}

inline Indicator SAFTYLOSS(const Indicator& data, const Indicator& n1,
                           const Indicator& n2, const Indicator& p) {
