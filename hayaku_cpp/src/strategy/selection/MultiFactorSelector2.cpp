...
void MultiFactorSelector2::_checkParam(const string& name) const {
    if ("ic_n" == name) {
        ...
        HAYAKU_ASSERT("MF_ICIRWeight" == mode || "MF_ICWeight" == mode || "MF_EqualWeight" == mode ||
                   "CUSTOM" == mode);
...
SelectorPtr HAYAKU_API SE_MultiFactor2(const MFPtr& mf, const ScoresFilterPtr& filter) {
    auto p = make_shared<MultiFactorSelector2>(mf);
    p->setScoresFilter(filter);
    return p;
}

SelectorPtr HAYAKU_API SE_MultiFactor2(const FactorSet& factorset, int ic_n, int ic_rolling_n,
