...
bool H5KDataDriver::_init() {
  ...
  for (auto iter = keys.begin(); iter != keys.end(); ++iter) {
    ...
    if (pos == string::npos || pos == 0 || pos == iter->size() - 1) continue;
    ...
    if (!existFile(filename)) {
      // HAYAKU_ERROR("Can't open h5file: {}", filename);
      continue;
    }
...
void H5KDataDriver::H5ReadRecords(H5::DataSet& dataset, hsize_t start,
...
void H5KDataDriver::H5ReadIndexRecords(H5::DataSet& dataset, hsize_t start,
                                       hsize_t nrecords, void* data) {
  H5::DataSpace dataspace = dataset.getSpace();
  hsize_t offset[1];
  hsize_t count[1];
...
void H5KDataDriver::H5ReadTimeLineRecords(H5::DataSet& dataset, hsize_t start,
                                          hsize_t nrecords, void* data) {
  H5::DataSpace dataspace = dataset.getSpace();
  hsize_t offset[1];
  hsize_t count[1];
...
void H5KDataDriver::H5ReadTransRecords(H5::DataSet& dataset, hsize_t start,
                                       hsize_t nrecords, void* data) {
  H5::DataSpace dataspace = dataset.getSpace();
  hsize_t offset[1];
  hsize_t count[1];
...
bool H5KDataDriver::_getH5FileAndGroup(const string& market, const string& code,
  ...
  try {
    ...
    // HAYAKU_ERROR("Exception of some HDF5 operator!");
    return false;
  }
...
size_t H5KDataDriver::getCount(const string& market, const string& code,
  ...
  H5FilePtr h5file;
  H5::Group group;
  HAYAKU_IF_RETURN(!_getH5FileAndGroup(market, code, kType, h5file, group), 0);
  ...
  try {
    ...
    // HAYAKU_WARN("Exception of some HDF5 operator! stock: {}{} {}", market,
    // code,
...
bool H5KDataDriver::getIndexRangeByDate(const string& market,
...
bool H5KDataDriver::_getBaseIndexRangeByDate(const string& market,
                                             const string& code,
                                             const KQuery& query,
                                             size_t& out_start,
                                             size_t& out_end) {
  ...
  HAYAKU_IF_RETURN(!success, false);
  ...
  try {
    ...
    HAYAKU_WARN("Invalid datetime!");
    dataspace.close();
    dataset.close();
    return false;

  } catch (...) {
    HAYAKU_INFO("error in {}{}", market, code);
    dataspace.close();
    dataset.close();
  }
...
bool H5KDataDriver::_getOtherIndexRangeByDate(const string& market,
                                              const string& code,
                                              const KQuery& query,
                                              size_t& out_start,
                                              size_t& out_end) {
  ...
  assert(KQuery::WEEK == query.kType() || KQuery::MONTH == query.kType() ||
         ...
         KQuery::HALFYEAR == query.kType() || KQuery::YEAR == query.kType() ||
         KQuery::MIN15 == query.kType() || KQuery::MIN30 == query.kType() ||
         KQuery::MIN60 == query.kType() || KQuery::HOUR2 == query.kType());
  out_start = 0;
  out_end = 0;
  HAYAKU_IF_RETURN(query.startDatetime() >= query.endDatetime(), false);

  H5FilePtr h5file;
  H5::Group group;
  HAYAKU_IF_RETURN(
      !_getH5FileAndGroup(market, code, query.kType(), h5file, group), false);
...
KRecordList H5KDataDriver::getKRecordList(const string& market,
                                          const string& code,
                                          const KQuery& query) {
  KRecordList result;
  ...
  if (kType == KQuery::TIMELINE) {
    ...
    // Query by index
    HAYAKU_IF_RETURN(query.start() >= query.end(), result);
    if (KQuery::DAY == kType || KQuery::MIN5 == kType || KQuery::MIN == kType) {
      result =
          _getBaseKRecordList(market, code, kType, query.start(), query.end());
    } else {
      result =
          _getIndexKRecordList(market, code, kType, query.start(), query.end());
    }
...
KRecordList H5KDataDriver::_getBaseKRecordList(const string& market,
                                               const string& code,
                                               const KQuery::KType& kType,
  ...
  H5FilePtr h5file;
  H5::Group group;
  HAYAKU_IF_RETURN(!_getH5FileAndGroup(market, code, kType, h5file, group),
                   result);
  ...
  try {
    ...
    size_t total =
        end_ix > all_total ? all_total - start_ix : end_ix - start_ix;
    ...
    HAYAKU_WARN("Invalid date! market_code({}{}) {}", market, code, e.what());
...
KRecordList H5KDataDriver::_getIndexKRecordList(const string& market,
                                                const string& code,
                                                const KQuery::KType& kType,
                                                size_t start_ix,
                                                size_t end_ix) {
  ...
  HAYAKU_IF_RETURN(
      !_getH5FileAndGroup(market, code, kType, h5file, index_group), result);
  ...
  try {
    ...
    size_t index_len =
        end_ix > index_total ? index_total - start_ix : end_ix - start_ix;
    ...
    if (end_ix >= index_total) {
      ...
      H5ReadIndexRecords(index_dataset, start_ix, index_len + 1,
                         p_index_buf.get());
    }

    size_t base_len = p_index_buf[index_len].start - p_index_buf[0].start;
    std::unique_ptr<H5Record[]> p_base_buf =
        std::make_unique<H5Record[]>(base_len);
    H5ReadRecords(base_dataset, p_index_buf[0].start, base_len,
                  p_base_buf.get());
    ...
    HAYAKU_WARN("Invalid date! {}", e.what());
...
TimeLineList H5KDataDriver::getTimeLineList(const string& market,
                                            const string& code,
                                            const KQuery& query) {
  return query.queryType() == KQuery::INDEX
             ? _getTimeLine(market, code, query.start(), query.end())
             : _getTimeLine(market, code, query.startDatetime(),
                            query.endDatetime());
}

TimeLineList H5KDataDriver::_getTimeLine(const string& market,
                                         const string& code, int64_t start_ix,
                                         int64_t end_ix) {
  TimeLineList result;
  H5FilePtr h5file;
  H5::Group group;
  HAYAKU_IF_RETURN(
      !_getH5FileAndGroup(market, code, KQuery::TIMELINE, h5file, group),
      result);
  ...
  try {
    ...
    std::unique_ptr<H5TimeLineRecord[]> pBuf =
        std::make_unique<H5TimeLineRecord[]>(total);
...
TimeLineList H5KDataDriver::_getTimeLine(const string& market,
                                         ...
                                         const Datetime& end) {
  TimeLineList result;
  HAYAKU_IF_RETURN(start >= end || start > Datetime::max(), result);

  H5FilePtr h5file;
  H5::Group group;
  bool success;
  success = _getH5FileAndGroup(market, code, KQuery::TIMELINE, h5file, group);
  HAYAKU_IF_RETURN(!success, result);
...
TransList H5KDataDriver::getTransList(const string& market, const string& code,
  ...
  return query.queryType() == KQuery::INDEX
             ...
             : _getTransList(market, code, query.startDatetime(),
                             query.endDatetime());
}
...
TransList H5KDataDriver::_getTransList(const string& market, const string& code,
  ...
  H5::Group group;
  HAYAKU_IF_RETURN(
      !_getH5FileAndGroup(market, code, KQuery::TRANS, h5file, group), result);
  ...
  try {
    ...
    size_t total = endpos - startpos;
    std::unique_ptr<H5TransRecord[]> pBuf =
        std::make_unique<H5TransRecord[]>(total);
    ...
    for (hsize_t i = 0; i < total; i++) {
      ...
      Datetime d(number);
      record.datetime = Datetime(d.year(), d.month(), d.day(), d.hour(),
                                 d.minute(), (long)second);
      record.price = price_t(pBuf[i].price) * 0.001;
      record.vol = price_t(pBuf[i].vol);
    ...
    HAYAKU_WARN("You may be use 32bit system, but data is to larger!");

  } catch (std::exception& e) {
    HAYAKU_ERROR(e.what());

  } catch (...) {
    HAYAKU_ERROR("Unkown error!");
  }
