...
/**
 * @defgroup test_hayaku_LruCache test_hayaku_LruCache
...
TEST_CASE("test_LruCache_move_semantics") {
  LruCache<int, std::string> cache(
      3, 0);  // Overflow 0, keeping the original behavior
  ...
  CHECK_EQ(cache.get(1),
           original_value);  // Make sure the value is stored correctly
  ...
  cache.insert(
      1,
      std::move(new_value));  // Update the existing key with the move semantics
  CHECK_EQ(cache.get(1),
           original_new_value);  // Make sure the value was updated correctly
  ...
  cache.insert(
      4,
      std::move(
          value4));  // This should trigger the LRU elimination, key 1 removed
  CHECK_EQ(cache.get(4), original_value4);

  // Verify which keys still exist
  // Due to the LRU mechanism the least recently accessed key (probably 1) is
  // removed
...
TEST_CASE("test_LruCache_tryGet") {
  ...
  CHECK_EQ(value,
           std::string{});  // The value should be the default constructed one
  ...
  cache.insert(4, "four");  // The earliest accessed element should be
                            // eliminated; 2 was accessed
  std::string value4;
  bool found4 = cache.tryGet(1, value4);
  CHECK_UNARY(!found4);  // 1 should have been eliminated
  CHECK_EQ(cache.get(2),
           "two");  // 2 should still be there, it was accessed just now
...
TEST_CASE("test_LruCache_edge_cases") {
  LruCache<int, int> cache(
      0, 0);  // A cache with the capacity 0 and the overflow capacity 0

  /** @arg Test the case of the capacity 0 (an unlimited capacity) */
  cache.insert(1, 100);
  CHECK_EQ(
      cache.get(1),
      100);  // The stored value is returned, the capacity 0 means unlimited
  CHECK_UNARY(
      !cache
           .empty());  // The cache should not be empty, because it has elements
...
TEST_CASE("test_LruCache_contains_remove") {
  ...
  CHECK_UNARY(
      cache.contains("key2"));  // key2 should still be there, key1 was removed
}
...
TEST_CASE("test_LruCache_thread_safety") {
  ...
  auto cache = std::make_shared<LruCache<int, int, std::shared_mutex>>(
      1000,
      0);  // A larger capacity to avoid the LRU elimination in the concurrent
           // test, overflow 0
  ...
  // The concurrent read / write test - a separate scope makes sure these
  // threads finish
  {
    std::atomic<bool> stop_flag(false);
    std::vector<std::thread> rw_threads;

    // For the concurrent read / write test a larger capacity avoids the LRU
    // influence
          ...
          [r, cache_ptr = cache,
           &stop_flag]() {  // Every thread holds a shared_ptr of the cache
            ...
            while (!stop_flag.load()) {
              ...
              // A specific value is not verified any more, because the
              // concurrent writes may change it
              key++;
              std::this_thread::sleep_for(std::chrono::microseconds(100));
            }
          });
...
#pragma warning(pop)
  ...
  // The cache leaves the scope only after all the threads have finished,
  // ensuring the safety
}
...
TEST_CASE("test_LruCache_comprehensive") {
  ...
  CHECK_UNARY(
      !cache.contains(5));  // The early element should have been removed
  CHECK_UNARY(
      !cache.contains(6));  // The previous element should have been removed
...
TEST_CASE("test_LruCache_overflow") {
  LruCache<int, std::string> cache(
      3, 2);  // The capacity is 3 and the overflow capacity is 2
  ...
  cache.insert(3, "three");
  cache.insert(4,
               "four");  // Above the capacity but not above capacity + overflow
  ...
  cache.insert(
      6, "six");  // Beyond capacity + overflow, the elimination should start

  CHECK_EQ(cache.size(), 3);  // It should still equal the capacity

  // Due to the LRU mechanism the least recently accessed element (1) should be
  // removed
  CHECK_UNARY(!cache.contains(1));
  CHECK_UNARY(!cache.contains(2));
  CHECK_UNARY(!cache.contains(3));
  ...
  // The size is 3 while capacity + overflow becomes 4, which should trigger a
  // cleanup
  CHECK_EQ(cache.size(), 3);  // It should shrink to the capacity

  /** @arg Test the effect of resize on the overflow capacity */
  LruCache<int, std::string> cache2(
      2, 1);  // The capacity is 2 and the overflow capacity is 1
  ...
  cache2.insert(5, "five");  // Beyond the new capacity + overflow, a cleanup
                             // should be triggered
...
TEST_CASE("test_LruCache_overflow_functions") {
  ...
  CHECK_EQ(
      cache2.size(),
      2);  // It shrinks to the capacity, capacity + overflow (2+0) exceeded
  CHECK_UNARY(!cache2.contains(1));  // The earliest should have been eliminated
  CHECK_UNARY(
      cache2.contains(5));  // 5 should still exist (a more recent access)
