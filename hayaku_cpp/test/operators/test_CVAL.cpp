...
TEST_CASE("test_CVAL_nested_size_propagation") {
  // Before the fix: one=CVAL(10) inside CVAL(one, 10) -> goes through
  // Indicator::operator()
  //   alike (a new empty shell ICval and the calculated CVAL10) is true ->
  //   short-circuits and returns a clone of the empty shell m_imp (m_imp was
  //   not calculated, size==0) -> violating the CVAL.h contract "its length
  //   equals that of the input ind"
  // After the fix: the calculated ind is reused and the size is propagated
  // correctly
  ...
  // A nested call with alike==true (the same value) -> triggers the fixed
  // return ind
  Indicator two = CVAL(one, 10);
  CHECK_EQ(two.size(), one.size());  // 0 before the fix and 1 after
  CHECK_EQ(two.discard(), one.discard());
  CHECK_EQ(two[0], 10);
  // A white box assertion: verify that the return ind path (a pointer reuse) is
  // taken instead of a cloned empty shell
  CHECK_EQ(two.getImp().get(), one.getImp().get());

  // A nested call with alike==false (different values 5!=10) -> goes through
  // the normal branch B (clone + calculate)
  Indicator three = CVAL(two, 5);
  CHECK_EQ(three.size(), 1);
  CHECK_EQ(three.discard(), 0);
  CHECK_EQ(three[0], 5);
  // A white box assertion: verify that the short-circuit branch (an independent
  // clone) is not taken
...
TEST_CASE("test_CVAL_nested_state_sharing") {
  // The fix returns ind (sharing the underlying node) when alike==true instead
  // of an independent clone of m_imp. This is a semantic change introduced by
  // the fix: the returned value shares the same IndicatorImp with ind. Under
  // the immutable parameter semantics of hayaku, alike has verified that
  // m_params are equal, and setParam triggers an in-place recalculation
  // (without changing the buffer immediately), so the sharing is safe. This
  // case fixes that sharing behavior as an expected invariant.

  Indicator base = CVAL(100);
  CHECK_EQ(base.size(), 1);
  CHECK_EQ(base[0], 100);

  // A self nesting: base.operator()(base) -> alike (this==other) true -> return
  // ind (i.e. base)
  Indicator result = base(base);
  CHECK_EQ(result.size(), base.size());
  // The pointers are exactly the same: the AST trimming reuses the argument
  // directly
  CHECK_EQ(result.getImp().get(), base.getImp().get());

  // The state sharing case: modifying the parameters of result also affects
  // base (the same imp)
