...
module_text = (project_root / 'MODULE.bazel').read_text(encoding='utf-8')
...
if any(command in sys.argv for command in ('bdist_wheel', 'install')):
    for native_name in ('core310.so', 'libhayaku.so'):
        if not (root / 'hayaku/cpp' / native_name).is_file():
            raise RuntimeError('Run ./op.sh python-build to stage Bazel native libraries before packaging')
...
setup(
    ...
    package_data={
        '': [
            '*.png', '*.md', '*.ipynb', '*.ini', '*.sql', '*.ui', '*.properties', '*.xml',
            'LICENSE.txt', '*.h', '*.qm',
        ],
