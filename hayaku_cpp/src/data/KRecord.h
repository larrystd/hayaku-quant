...
class KRecord {
 public:
  ...
  price_t transCount;  ///< Trading volume (in lots); below the daily line it is
                       ///< the share count
  ...
  KRecord(const Datetime& date, price_t openPrice, price_t highPrice,
          price_t lowPrice, price_t closePrice, price_t transAmount,
          price_t transCount)
      : datetime(date),
        openPrice(openPrice),
        highPrice(highPrice),
...
/**
 * Output the KRecord information, e.g. KRecord(datetime, open, high, low,
...
template <>
struct fmt::formatter<hayaku::KRecord> : ostream_formatter {};
#endif
