 ...
 */
void HAYAKU_UTILS_API DBUpgrade(const DBConnectPtr &driver,
                                const char *module_name,
                                const std::vector<std::string> &upgrade_scripts,
                                int start_version, const char *create_script) {
  HAYAKU_TRACE("check {} database version ...", module_name);
  ...
  if (!driver->tableExist("module_version")) {
    ...
    if (need_create && isSQLite(driver.get())) {
      driver->exec(
          "CREATE TABLE `module_version` (`id` INTEGER PRIMARY KEY "
  ...
  // Return directly when there is neither an upgrade script nor a creation
  // script
  ...
  try {
    version =
        driver->queryInt(fmt::format("select `version` from `module_version` "
                                     "where module=\"{}\" limit 1",
                                     module_name),
                         0);
  } catch (...) {
    // Do noting
  ...
  // When the module database version is 0 (the module database does not exist),
  // create the database if a creation script is given, otherwise return
  // directly
  if (0 == version) {
    if (!create_script) {
      return;
    ...
    driver->exec(fmt::format(
        "INSERT INTO `module_version` (module, version) VALUES (\"{}\", 1);",
        module_name));
    version = 1;
  }

  // When the current version is smaller than the start version of the upgrade
  // script by one, an intermediate version upgrade script is missing and the
  // upgrade cannot proceed
  if (version < start_version - 1) {
    HAYAKU_ERROR("THe {} database is too old, can't upgrade!", module_name);
    return;
  }
  ...
  HAYAKU_TRACE("current {} database version: {}", module_name, version);

  // The current version is already not lower than the target version, no
  // upgrade is needed, return directly
  if (version >= to_version) {
    HAYAKU_TRACE(
        "current version({}) greater the upgrade version({}), ignored!",
        version, to_version);
    return;
  }

  // If the current version is smaller than the start version of the script, the
  // script index starts from 0; otherwise the execution starts from the upgrade
  // script whose index is (the current version - the start version of script 0)
  // + 1
  int start_index = version < start_version ? 0 : version - start_version + 1;
  HAYAKU_TRACE("update {} database ..., update script index: {}", module_name,
               start_index);
  ...
  driver->exec(fmt::format(
      "UPDATE module_version SET `version`={} where `module`=\"{}\"",
 ...
 */
net::awaitable<void> HAYAKU_UTILS_API
DBUpgrade(const AsyncDBConnectPtr &driver, const char *module_name,
  ...
  try {
    version = co_await driver->queryInt(
        fmt::format("select `version` from `module_version` where "
                    "module=\"{}\" limit 1",
                    module_name),
        0);
  } catch (...) {
