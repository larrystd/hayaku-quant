...
// Helper function: print the diagnostics information (internal use, not
// exposed)
...
struct AsyncMySQLConnect::Impl {
  ...
  // Get the io_context from the current coroutine environment (it is done at
  // the first call only)
  ...
  net::awaitable<std::shared_ptr<boost::mysql::statement>> get_statement(
    ...
    try {
      boost::mysql::statement stmt = co_await conn->async_prepare_statement(
          sql, prep_diag, boost::asio::use_awaitable);
...
net::awaitable<bool> AsyncMySQLConnect::ping() {
  ...
  if (need_reconnect) {
    ...
    if (!reconnected) {
      printAsyncMySQLDiag(boost::mysql::error_code(), diag,
                          "MySQL ping failed!");
      co_return false;
    }
    co_return true;
...
net::awaitable<bool> AsyncMySQLConnect::tableExist(
    const std::string& tablename) {
  bool result = false;
  try {
    auto st = co_await getStatement(
        fmt::format("SELECT 1 FROM {} LIMIT 1;", tablename));
    co_await st->exec();
    result = true;
  } catch (...) {
    result = false;
...
net::awaitable<void> AsyncMySQLConnect::resetAutoIncrement(
    const std::string& tablename) {
  int64_t count = co_await queryNumber<int64_t>(
      fmt::format("select count(1) from {}", tablename));
  SQL_CHECK(count == 0, -1,
...
net::awaitable<void> AsyncMySQLConnect::commit() { co_await exec("COMMIT"); }
