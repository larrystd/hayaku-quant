 ...
 * @details In client mode a NO_RECOVER query directly reads the shared memory
 * snapshot published by the main process through a raw pointer view, without
 * copying the records (all the clients share the same physical page), which
 * aligns with the zero-copy semantics of KDataSharedBufferImp in the main
 * process. The view pointer is pinned by m_pin (which holds a type-erased
 * KDataShmReader), so the old mapping stays alive after the epoch is replaced.
 * It is constructed successfully through create() only when the driver supports
 * the view (a client IPC proxy driver with the snapshot covered and no reverse
 * scaling needed), otherwise a null pointer is returned and the upper layer
 * falls back to the KDataPrivatedBufferImp copy path.
 * @note Read-only mapping: the writable overload of data() returns a const_cast
 * pointer only for interface compatibility, a write will crash, which is the
 * same kind of risk as the "use with caution (forced data adjustment)" contract
...
class KDataShmBufferImp : public KDataImp {
 public:
   ...
   * @param query the query condition (should be NO_RECOVER, guaranteed by the
   * caller)
   * @return the view imp on success; nullptr when it does not apply (not a
   * client / not covered / reverse scaling needed / invalid range)
   */
