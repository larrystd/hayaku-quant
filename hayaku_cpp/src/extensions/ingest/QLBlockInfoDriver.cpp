/*
 * QLBlockInfoDriver.cpp
 *
 *  Created on: 2015-2-10
 *      Author: fasiondog
 */

#include "QLBlockInfoDriver.h"

#include <fstream>

#include "common/Arithmetic.h"

namespace hayaku {

QLBlockInfoDriver::~QLBlockInfoDriver() {}

bool QLBlockInfoDriver::_init() { return true; }

Block QLBlockInfoDriver ::getBlock(const string& category, const string& name) {
  Block result(category, name);
  HAYAKU_ERROR_IF_RETURN(!haveParam("dir"), result, "Missing 'dir' param!");
  HAYAKU_INFO_IF_RETURN(!haveParam(category), result, "No such category ({})!",
                        category);

  string filename;
  try {
    filename = getParam<string>("dir") + "/" + getParam<string>(category);
  } catch (...) {
    HAYAKU_ERROR("Maybe parameters errors!");
    return result;
  }

  std::ifstream inifile(filename.c_str(), std::ifstream::in);
  HAYAKU_ERROR_IF_RETURN(!inifile, result, "Can't open file({})!", filename);

  string line_str;
  string section, market, code;
  bool is_find = false;
  // string gb_name = utf8_to_gb(name);
  while (std::getline(inifile, line_str)) {
    trim(line_str);

    // Skip the empty or the comment lines
    if (line_str.empty() || line_str.at(0) == ';') {
      continue;
    }

    // Check the first comment marker and clear it together with the following
    // characters
    size_t pos = line_str.find(';');
    if (pos != std::string::npos) {
      line_str.assign(line_str, 0, pos);
      trim(line_str);
    }

    // A section line
    if (line_str.at(0) == '[') {
      if (is_find) {
        break;  // Break the loop
      }
      size_t len = line_str.size();
      if (line_str[len - 1] != ']') {
        continue;
      }

      section.assign(line_str, 1, len - 2);
      trim(section);
      if (section.empty()) {
        continue;
      }

      if (section == name) {
        is_find = true;
      }
    }

    if (is_find) {
      pos = line_str.find(',');
      if (pos != std::string::npos) {
        market.assign(line_str, 0, pos);
        code.assign(line_str, pos + 1, line_str.size());
        trim(market);
        trim(code);
        if (market == "0") {
          result.add("SH" + code);
        } else {
          result.add("SZ" + code);
        }
      }
    }
  }

  inifile.close();
  return result;
}

StringList QLBlockInfoDriver::getAllCategory() {
  HAYAKU_WARN("Not support getAllCategory!");
  return StringList();
}

BlockList QLBlockInfoDriver::getBlockList(const string& category) {
  BlockList result;
  HAYAKU_ERROR_IF_RETURN(!haveParam("dir"), result, "Missing 'dir' param!");
  HAYAKU_INFO_IF_RETURN(!haveParam(category), result, "No such category ({})!",
                        category);

  string filename;
  try {
    filename = getParam<string>("dir") + "/" + getParam<string>(category);
  } catch (...) {
    HAYAKU_ERROR("Maybe parameters errors!");
    return result;
  }

  std::ifstream inifile(filename.c_str(), std::ifstream::in);
  HAYAKU_ERROR_IF_RETURN(!inifile, result, "Can't open file({})!", filename);

  std::string section;
  std::string key;
  std::string value;
  std::string line_str;
  Block block;
  while (std::getline(inifile, line_str)) {
    trim(line_str);

    // Skip the empty or the comment lines
    if (line_str.empty() || line_str.at(0) == ';') continue;

    // Check the first comment marker and clear it together with the following
    // characters
    size_t pos = line_str.find(';');
    if (pos != std::string::npos) {
      line_str.assign(line_str, 0, pos);
      trim(line_str);
    }

    // A section line
    if (line_str.at(0) == '[') {
      size_t len = line_str.size();
      if (line_str[len - 1] != ']') continue;

      section.assign(line_str, 1, len - 2);
      trim(section);
      if (section.empty()) continue;

      block = Block(category, section);
      result.push_back(block);

    } else {
      if (section.empty())
        break;  // The section definition is missing, no further processing is
                // needed

      pos = line_str.find(',');
      if (pos == std::string::npos || pos == line_str.size() - 1) continue;

      key.assign(line_str, 0, pos);
      trim(key);
      if (key.empty()) continue;

      value.assign(line_str, pos + 1, std::string::npos);
      trim(value);
      if (value.empty()) continue;

      if (key == "0") {
        block.add("SH" + value);
      } else {
        block.add("SZ" + value);
      }
    }
  }

  inifile.close();
  return result;
}

BlockList QLBlockInfoDriver::getBlockList() {
  BlockList result;
  HAYAKU_ERROR_IF_RETURN(!haveParam("dir"), result, "Missing 'dir' param!");
  StringList category_list = params_.getNameList();
  for (auto iter = category_list.begin(); iter != category_list.end(); ++iter) {
    if (*iter == "dir" || *iter == "type") continue;

    BlockList tmp_blk_list = getBlockList(*iter);
    for (auto b_it = tmp_blk_list.begin(); b_it != tmp_blk_list.end(); ++b_it) {
      result.push_back(*b_it);
    }
  }

  return result;
}

void QLBlockInfoDriver::save(const Block& block) {
  HAYAKU_THROW("Not support save block info! You can use ini file to do it!");
}

void QLBlockInfoDriver::remove(const string& category, const string& name) {
  HAYAKU_THROW("Not support save block info! You can use ini file to do it!");
}

} /* namespace hayaku */
