...
public:
    ...
    int addFunc(Datetime start_date, Datetime end_date, TimeDelta start_time, TimeDelta end_time,
        ...
        HAYAKU_CHECK(end >= start, "end_date({}) need > start_date({})!", end, start);
        HAYAKU_CHECK(start_time >= TimeDelta(0) && start_time <= TimeDelta(0, 23, 59, 59, 999, 999),
                  "Invalid start_time: {}", start_time.repr());
        HAYAKU_CHECK(end_time >= TimeDelta(0) && end_time <= TimeDelta(0, 23, 59, 59, 999, 999),
                  "Invalid end_time: {}", end_time.repr());
        HAYAKU_CHECK(end_time >= start_time, "end_time({}) need >= start_time({})!", end_time,
                  start_time);
        HAYAKU_CHECK(repeat_num > 0, "Invalid repeat_num: {}", repeat_num);
        HAYAKU_CHECK(duration > TimeDelta(0), "Invalid duration: {}", duration.repr());
    ...
    int addDurationFunc(int repeat_num, TimeDelta duration, F&& f, Args&&... args) {
        HAYAKU_CHECK(repeat_num > 0, "Invalid repeat_num: {}, must > 0", repeat_num);
        HAYAKU_CHECK(duration > TimeDelta(), "Invalid duration: {}, must > TimeDelta(0)!",
                  duration.repr());
    ...
    int addDelayFunc(TimeDelta delay, F&& f, Args&&... args) {
        HAYAKU_CHECK(delay > TimeDelta(), "Invalid delay: {}, must > TimeDelta(0)!", delay);
    ...
    int addFuncAtTime(Datetime time_point, F&& f, Args&&... args) {
        Datetime now = Datetime::now();
        HAYAKU_CHECK(time_point > now, "You want run at {}, but now is {}", time_point, now);
    ...
    int addFuncAtTimeEveryDay(Datetime start_date, Datetime end_date, TimeDelta time, F&& f,
        ...
        HAYAKU_CHECK(time >= TimeDelta() && time <= TimeDelta(0, 23, 59, 59, 999, 999),
...
private:
    ...
    int getNewTimerId() {
        ...
        HAYAKU_WARN_IF_RETURN(m_timers.size() >= size_t(max_int), -1, "Timer queue is full!");
...
private:
    ...
    int _addFunc(Datetime start_date, Datetime end_date, TimeDelta start_time, TimeDelta end_time,
        ...
        Datetime now = Datetime::now();
        Datetime today = now.startOfDay();
        HAYAKU_CHECK(end_date >= today, "Invalid end_date {}, because today is {}", end_date, today);
        ...
        if (start_time < TimeDelta()) {
            ...
            if (first_start_time >= now) {
                ...
                HAYAKU_CHECK(repeat_num > 1, "The time has expired! expect time {}, but now is {}",
