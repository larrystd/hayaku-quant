...
TEST_CASE("test_IC") {
  ...
  /** @arg The passed stks contains a null stock, so its actual length is less
   * than 2 */
  result = IC(MA(CLOSE()), {sm["sh600004"], Stock()},
              1)(ref_stk.getKData(KQuery(-2)));
  CHECK_EQ(result.name(), "IC");
  CHECK_UNARY(!result.empty());
  ...
  result = IC(CLOSE(), {sm["sh600004"], sm["sh600005"]},
              1)(ref_stk.getKData(KQuery(-2)));
  CHECK_EQ(result.name(), "IC");
  CHECK_UNARY(!result.empty());
  CHECK_EQ(result.size(), 2);
  ...
  // The strict mode
  result = IC(CLOSE(), {sm["sh600004"], sm["sh600005"]}, 1, true,
...
TEST_CASE("test_IC_benchmark") {
  ...
  {
    BENCHMARK_TIME_MSG(test_IC_benchmark, cycle,
