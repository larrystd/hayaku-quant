...
internal::ExecutionAccountPortPtr makeBrokerAccount(
  const OrderBrokerPtr& broker, const TradeCostPtr& costfunc, const string& name,
  const std::vector<OrderBrokerPtr>& other_brokers) {
    ...
    auto account = internal::makeExecutionAccount(
      AccountConfig(Datetime::now(), 0.0, costfunc, name, 2, false, false, {},
                    std::move(brokers)));
    auto broker_port = std::dynamic_pointer_cast<internal::ExecutionBrokerPort>(account);
    HAYAKU_CHECK(broker_port, "Execution account does not support live broker synchronization");
...
void Strategy::start(bool autoRecieveSpot) {
    ...
    HAYAKU_WARN_IF(!m_on_recieved_spot && !m_on_change && m_run_daily_at_list.empty() &&
    ...
    if (autoRecieveSpot) {
        RealtimeSpotProcess process = [this](const SpotRecord& spot) { _receivedSpot(spot); };
        ...
        startRealtimeForStrategy(process, postProcess, getParam<int>("spot_worker_num"),
...
TradeRecord Strategy::order(const Stock& stk, double num, const string& remark) {
    TradeRecord ret;
    HAYAKU_WARN_IF_RETURN(num == 0.0, ret, "{} {} order num is zero!", stk.market_code(), stk.name());
...
TradeRecord Strategy::orderValue(const Stock& stk, price_t value, const string& remark) {
    TradeRecord ret;
    HAYAKU_WARN_IF_RETURN(value == 0.0, ret, "{} {} order value is zero!", stk.market_code(),
                       stk.name());
    ...
    if (value > 0.0) {
        ...
        if (n == 0.0) {
            HAYAKU_WARN("{} {} can buy number is zero!", stk.market_code(), stk.name());
        } else {
...
TradeRecord Strategy::buy(const Stock& stk, price_t price, double num, double stoploss,
                          double goal_price, OrderOrigin origin, const string& remark) {
    HAYAKU_ASSERT(m_account);
    return m_account
      ->submit(OrderRequest(OrderSide::BUY, Datetime::now(), stk, price, num, stoploss, goal_price,
                            price, origin, remark))
      .trade();
}

TradeRecord Strategy::sell(const Stock& stk, price_t price, double num, price_t stoploss,
                           price_t goal_price, OrderOrigin origin, const string& remark) {
    HAYAKU_ASSERT(m_account);
    return m_account
      ->submit(OrderRequest(OrderSide::SELL, Datetime::now(), stk, price, num, stoploss, goal_price,
                            price, origin, remark))
      .trade();
}

void HAYAKU_API runInStrategy(const internal::StrategyRuntimePtr& strategy, const Stock& stk,
                           const KQuery& query,
    ...
    auto account = makeBrokerAccount(broker, costfunc, strategy->name(), other_brokers);
    auto portfolio_account = std::dynamic_pointer_cast<internal::PortfolioAccountPort>(account);
    HAYAKU_CHECK(portfolio_account, "Execution account does not support strategy capital management");
    strategy->setAccount(std::move(portfolio_account));
    strategy->setSP(SlippagePtr());
    strategy->run(BacktestRequest(stk.getKData(query)));
}

void HAYAKU_API runInStrategy(const PFPtr& pf, const KQuery& query, const OrderBrokerPtr& broker,
    ...
    auto portfolio_account = std::dynamic_pointer_cast<internal::PortfolioAccountPort>(account);
    HAYAKU_CHECK(portfolio_account, "Execution account does not support portfolio capital management");
