...
public:
    ...
    [[nodiscard]] virtual KRecordList getExtraKRecordList(const Stock& stock,
                                                          const KQuery& query) = 0;
    [[nodiscard]] virtual size_t getStockExtraCount(const Stock& stock,
                                                    const string& ktype) = 0;
    [[nodiscard]] virtual bool getStockExtraIndexRange(const Stock& stock, const KQuery& query,
                                                       size_t& outStart, size_t& outEnd) = 0;
...
/** Install/remove the application-provided extension resolver. No plugin is loaded here. */
...
[[nodiscard]] KRecordList getExtraKRecordList(const Stock& stock, const KQuery& query);
[[nodiscard]] size_t getStockExtraCount(const Stock& stock, const string& ktype);
[[nodiscard]] bool getStockExtraIndexRange(const Stock& stock, const KQuery& query,
